...
// service-worker.js'deki CACHE_VERSION ile eşleşmeli).
console.log('%cAYGÜN AVM — app.js build: 20260729rev83', 'color:#1C1C1E;font-weight:bold;');
...
async function _checkAndRegisterSession(email, rol) {
  ...
  try {
    ...
    window._sessionHeartbeat = setInterval(()=>{
      ...
      setDoc(sessionRef,{lastSeen:new Date().toISOString()},{merge:true}).catch(()=>{});
    },60000);
