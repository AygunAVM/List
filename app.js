...
logs.forEach(l => {
  ...
  if (h>=0) { 
    ...
    // Huni ham değerleri — Math.min UYGULANMAZ, gerçek veri korunur
    // UI gösterimde kırpma yapılabilir ama loglarda ve hesaplamalarda ham değer kalır
    const _rawSepet   = logs.filter(l => (l.derinlik || 0) > 0 || (l.urunler||[]).length > 0).length;
