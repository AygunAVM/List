...
function buildAbakusKartlar() {
  ...
  const kartlar = [];
  allRates.forEach(r => {
    // Tüketici kredisi (TOMBank, Zip) abaküs kartlarına dahil edilmez
  ...
  const ks = document.getElementById('ab-kart');
  if (!ks) return;
  // Kredi kurumlarını ayrı optgroup olarak ekle
  const krediKartlar = [];
  allRates.forEach(r => {
    if (r.Kart && !krediKartlar.includes(r.Kart) && (r.Tip||'').toLowerCase() === 'kredi') krediKartlar.push(r.Kart);
  });
  let html = kartlar.map(k => `<option value="${k}">${k}</option>`).join('');
  if (krediKartlar.length) {
    html += `<optgroup label="── Tüketici Kredisi ──">` +
            krediKartlar.map(k => `<option value="${k}" data-tip="kredi">${k}</option>`).join('') +
            `</optgroup>`;
  }
  ks.innerHTML = html;
}
...
function openQuickFinance(urunIdx, nakitFiyat) {
  ...
  // Banner: proje fiyatı veya kampanya
  let kampanyaBanner = '';
  if (aciklama.includes('\u2756')) {
    kampanyaBanner = '<div class="qf-camp-banner" style="background:linear-gradient(135deg,#f3e8ff,#ede9fe);border-color:#7c3aed;color:#4c1d95">' +
      ...
      '<span style="font-size:.62rem">Sepete eklendiğinde Ödeme Seçenekleri\u2019nden proje fiyatını seçebilirsiniz.</span>' +
      '<button onclick="closeQuickFinance();addToBasket(' + _qfUrunIdx + ');toggleCart();" style="margin-top:7px;width:100%;padding:7px;background:#7c3aed;color:#fff;border:none;border-radius:8px;font-size:.70rem;font-weight:700;cursor:pointer;">\uD83D\uDED2 Sepete Git \u2192</button>' +
      '</div>';
    ...
    if (kampanyalar.length > 0) {
      ...
      kampanyaBanner = '<div class="qf-camp-banner">' +
        ...
        '<button onclick="closeQuickFinance();addToBasket(' + _qfUrunIdx + ');toggleCart();" style="margin-top:7px;width:100%;padding:7px;background:#d97706;color:#fff;border:none;border-radius:8px;font-size:.70rem;font-weight:700;cursor:pointer;">\uD83D\uDED2 Sepete Git \u2192</button>' +
        '</div>';
    }
  }
...
function _qfSelectPlan(el, kart, taksit, tahsilat, zincir, tip) {
  taksit   = parseInt(taksit)   || taksit;   // string → int
...
function qfAddToBasket() {
  ...
  // 2. Plan bilgisini item'a yaz (item bazlı — global state kirletilmez)
  if (_planSnapshot && basket.length > 0) {
    const item = basket[0];
    item._qfPlanLabel = _planSnapshot.label;
    item._qfAylik     = _planSnapshot.aylik;
    item._qfTaksit    = parseInt(_planSnapshot.taksit) || _planSnapshot.taksit;
    item._qfKart      = _planSnapshot.kart;
    item._qfZincir    = _planSnapshot.zincir;
  }

  // 3. QF modal kapat (body overflow temizlenir)
  closeQuickFinance();

  // 4. Kısa bekleme sonrası abaküsü aç — _fromQF flag ile
...
// ══════════════════════════════════════════════════════════════
// Tüketici Kredisi Abaküs Render
// ══════════════════════════════════════════════════════════════
function _calcAbakusKredi(resEl, zRows, nakit, kurum) {
  const fmt = n => Number(n).toLocaleString('tr-TR');
  const VADE_LIST = [3, 6, 9, 12, 15, 18, 24];

  let html = '<div class="ab-nakit-row"><span>Tüketici Kredisi — ' + kurum + '</span>' +
    '<strong>' + fmt(nakit) + ' TL</strong></div>';

  html += '<table style="width:100%;border-collapse:collapse;margin-top:8px;font-size:.75rem">' +
    '<thead><tr style="background:#f1f5f9">' +
    '<th style="padding:6px 8px;text-align:left">Vade</th>' +
    '<th style="padding:6px 8px;text-align:right">Aylık</th>' +
    '<th style="padding:6px 8px;text-align:right">Toplam</th>' +
    '<th style="padding:6px 8px;text-align:center">Seç</th>' +
    '</tr></thead><tbody>';
  ...
  VADE_LIST.forEach(ay => {
    zRows.forEach(r => {
      ...
      html += '<tr class="ab-row-sel" data-abrow="' + rowData + '" ' +
        'onclick="selectAbakusRow(this)" ' +
        'style="cursor:pointer;border-bottom:1px solid #e2e8f0">' +
        '<td style="padding:7px 8px">' + ay + ' Ay <span style="font-size:.65rem;color:#94a3b8">%' + vadeFarki + '</span></td>' +
        '<td style="padding:7px 8px;text-align:right;font-weight:600">' + fmt(aylik) + ' TL</td>' +
        '<td style="padding:7px 8px;text-align:right;color:#dc2626">' + fmt(toplam) + ' TL</td>' +
        '<td style="padding:7px 8px;text-align:center"><span style="color:#2563eb;font-size:1rem">○</span></td>' +
        '</tr>';
    });
  });

  if (!hasRow) {
    html += '<tr><td colspan="4" style="padding:12px;text-align:center;color:#94a3b8">Vade bilgisi bulunamadı</td></tr>';
  }
...
function calcAbakus() {
  ...
  // Tüketici Kredisi ise farklı render: vade farkı bazlı
  const isKredi = (zRows[0]?.Tip || '').toLowerCase() === 'kredi';
  if (isKredi) { _calcAbakusKredi(resEl, zRows, nakit, secKart); return; }
