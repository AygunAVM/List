...
function fmt(val) {
  ...
  // Güvenli yol: manuel binlik ayırıcı + sabit '+_tlSym()+' sembolü
...
function renderCampaignPills(item, idx) {
  ...
  const summaryRow = secilenSayi > 0
    ? `<div style="display:flex;align-items:center;gap:5px;margin-top:3px">`
      + (projeAktifPill
        ? (() => {
            // Satır özeti: "❖ EA+SFS · 159.999'+_tlSym()+'" — kimlik + fiyat birlikte
  ...
  container.innerHTML = `
    ...
    <div class="ms-stat-row">
      ...
      <div class="ms-stat-card ms-stat-blue">
        ...
        <div class="ms-stat-lbl">Ciro '+_tlSym()+'</div>
      </div>
    </div>
