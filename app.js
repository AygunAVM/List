...
async function openAbakus() {
  ...
  abakusSelection = null;
  // Pazarlık indirimini sıfırla (sepet gir/çık yenileme)
  const _ekInpReset = document.getElementById('ab-ek-indirim');
  if (_ekInpReset) _ekInpReset.value = '';
  const _pazNotReset = document.getElementById('ab-pazarlik-notu');
  if (_pazNotReset) _pazNotReset.value = '';
...
function calcAbakus() {
  ...
  // ── Pazarlık değerlerini innerHTML yazmadan ÖNCE oku ──────────────
  ...
  // indirimliNakit: satır indirimleri + alt indirim uygulanmış net nakit fiyat
  const _altIndirimCalc = discountType === 'TRY'
    ? discountAmount
    : (t.nakit - totalItemDisc) * discountAmount / 100;
  let nakit = t.nakit - totalItemDisc - _altIndirimCalc;
...
if (!enKarliMap[td.n] || oran < enKarliMap[td.n].oran) {
  ...
  let html = '';
  // Baz nakit satırı — indirim % hesapla
  const _t2 = basketTotals();
  const _bruteTotal = _t2.nakit;
  const _totalItemDisc2 = basket.reduce((s, i) => s + (i.itemDisc || 0), 0);
  const _altIndirim2 = discountType === 'TRY' ? discountAmount : (_bruteTotal - _totalItemDisc2) * discountAmount / 100;
  const _toplamInd2 = _totalItemDisc2 + _altIndirim2 + ekIndirim;
  const _indPct = _bruteTotal > 0 ? ((_toplamInd2 / _bruteTotal) * 100).toFixed(1) : 0;
  const _indBadge = _toplamInd2 > 0
    ? `<span style="font-size:.60rem;background:#dcfce7;color:#15803d;border-radius:5px;padding:1px 6px;font-weight:700;margin-left:4px">%${_indPct} İndirim</span>`
    : '';
  html += `<div class="ab-nakit-row"><span>Baz Nakit</span><strong>${fmt(nakit)}</strong>${_indBadge}<span class="ab-kart-badge">${secKart} · max ${maxT}T</span></div>`;

  // ── Ek İndirim Alanı (Pazarlık) — Opsiyonel gizli panel ──────────
  const _pazarlikPanelAcik = ekIndirim > 0 || _savedNot !== '';
  const _indirimBadge = ekIndirim > 0 ? ' · -' + fmt(ekIndirim) : '';
  const _pnBorder = _pazarlikPanelAcik ? '#f59e0b' : 'var(--border)';
  const _pnColor  = _pazarlikPanelAcik ? '#b45309' : 'var(--text-2)';
  const _pnDisplay = _pazarlikPanelAcik ? 'block' : 'none';
  html += '<div style="margin:4px 0 6px">'
    + '<button onclick="(function(){'
    +   'var p=document.getElementById(\'ab-pazarlik-panel\');'
    +   'var b=document.getElementById(\'ab-pazarlik-togbtn\');'
    +   'var open=p.style.display!==\'none\';'
    +   'p.style.display=open?\'none\':\'block\';'
    +   'if(open){document.getElementById(\'ab-ek-indirim\').value=\'\';calcAbakus();}'
    +   'b.style.borderColor=open?\'var(--border)\':\'#f59e0b\';'
    +   'b.style.color=open?\'var(--text-2)\':\'#b45309\';'
    + '})()"'
    + ' id="ab-pazarlik-togbtn"'
    + ' style="background:none;border:1px dashed ' + _pnBorder + ';border-radius:7px;'
    +        'padding:4px 10px;font-size:.68rem;font-weight:700;cursor:pointer;'
    +        'color:' + _pnColor + ';font-family:inherit;transition:all .2s">'
    + '💬 Pazarlık İndirimi' + _indirimBadge
    + '</button></div>'
    + '<div id="ab-pazarlik-panel" style="display:' + _pnDisplay + ';background:#fffbeb;border:1px solid #fde68a;border-radius:9px;padding:10px 12px;margin-bottom:8px">'
    ...
    + ' style="width:100px;padding:5px 8px;border:1.5px solid #fcd34d;border-radius:7px;font-size:.82rem;font-family:inherit;text-align:right;background:#fff;color:#78350f;font-weight:700">'
    ...
    + ' style="width:100%;box-sizing:border-box;padding:5px 8px;border:1px solid #fcd34d;border-radius:7px;font-size:.72rem;font-family:inherit;background:#fff;color:#78350f">'
    + '</div>';
  ...
  const _newEkInp = document.getElementById('ab-ek-indirim');
  const _newNot   = document.getElementById('ab-pazarlik-notu');
  if (_newEkInp && _savedIndirim !== '') { _newEkInp.value = _savedIndirim; }
  ...
  if (actDiv) {
    ...
    if (infoDiv) {
      if (abakusSelection === null) {
        ...
        const _nakitAbrow = parsed;
        const _nakitStr = fmt(_nakitAbrow.nakit || nakit);
        const _ekStr = (_nakitAbrow.ekIndirim > 0) ? ` <span style="color:#16a34a;font-size:.8em">(-${fmt(_nakitAbrow.ekIndirim)})</span>` : '';
        infoDiv.innerHTML = `<span class="ab-sel-chip ab-sel-nakit">💵 Nakit — ${_nakitStr}${_ekStr}</span>`;
      } else {
        const _ekIndirimChip = abakusSelection.ekIndirim > 0
          ? `<span class="ab-sel-chip" style="color:#16a34a">-${fmt(abakusSelection.ekIndirim)} pazarlık</span>`
          : '';
        infoDiv.innerHTML = `<span class="ab-sel-chip">${abakusSelection.label}</span><span class="ab-sel-chip">${abakusSelection.zincir} POS</span><span class="ab-sel-chip ab-sel-tahsilat">${fmt(abakusSelection.tahsilat)}</span><span class="ab-sel-chip ab-sel-aylik">Aylık ${fmt(abakusSelection.aylik)}</span>${_ekIndirimChip}`;
      }
    }
  }
...
async function finalizeAksiyon() {
  ...
  const altIndirim = discountType === 'TRY'
    ? discountAmount
    : (nakitFiyat - totalItemDisc) * discountAmount / 100;
  const toplamIndirim  = totalItemDisc + altIndirim;
  const indirimliNakit = nakitFiyat - toplamIndirim;                       // nakit net fiyat

  // Pazarlık indirimi — SADECE abaküs selection'dan gel (tabloda hesaplanmış tahsilat kullan)
  const _ekIndirimF    = abakusSelection ? (abakusSelection.ekIndirim || 0) : (parseFloat(document.getElementById('ab-ek-indirim')?.value) || 0);
  const _pazarlikNotuF = (document.getElementById('ab-pazarlik-notu')?.value || '').trim();

  let od = '', odText = '', tahsilat = indirimliNakit - _ekIndirimF;

  if (abakusSelection) {
    // Abaküs tablosundan gelen tahsilat: zaten doğru hesaplı (indirimliNakit bazlı + pazarlık)
    tahsilat = abakusSelection.tahsilat;
    const taksitSayisi = abakusSelection.taksit;
    const _taksTaban = taksitSayisi === 1 ? tahsilat : Math.floor(tahsilat / taksitSayisi);
    const _taksKalan = taksitSayisi === 1 ? 0 : (tahsilat - _taksTaban * taksitSayisi);
    const aylikTutar = _taksTaban + _taksKalan;
    ...
    tahsilat = Math.max(0, indirimliNakit - _ekIndirimF);
    od     = 'Nakit: ' + fmt(tahsilat) + (toplamIndirim + _ekIndirimF > 0 ? ' (İndirim: -' + fmt(toplamIndirim + _ekIndirimF) + ')' : '');
    odText = 'Nakit — ' + fmt(tahsilat);
  }
  ...
  if (_aksiyonMode === 'wa') {
    ...
    if (_waToplamInd > 0) {
      const _pazarlikPart = _ekIndirimF > 0 ? ` + Pazarlık -${fmt(_ekIndirimF)}` : '';
      indirimMetni = `\n_İndirimler -${fmt(toplamIndirim)}${_pazarlikPart}_\n\n`;
    } else {
      indirimMetni = `\n\n`;
    }
    waMsg += indirimMetni;
    ...
    if (abakusSelection === null) {
      ...
      const kartAdi = abakusSelection.kart || abakusSelection.label || '';
      const taksitSayisi = abakusSelection.taksit;
      const _waTaban = Math.floor(tahsilat / taksitSayisi);
      const _waKalan = tahsilat - _waTaban * taksitSayisi;
      const aylikTutar = _waTaban + _waKalan;
      waMsg += `* ${kartAdi}\n`;
...
function buildPremiumPDF(docType, data) {
  ...
  const _pdfToplamIndirim = Number(data.toplamIndirim || 0);  // satır+alt indirim (pazarlık hariç)
  let gosterilecekIndirim = isNakit ? _pdfToplamIndirim : 0;

  // Nakit: ham tutar = net + satır indirimleri + alt indirim (pazarlık ayrı gösterilir)
  // Kart : ham tutar = tahsilat (vade farkı dahil)
  ...
  if (isNakit) {
    // NAKİT: Ham Tutar → -İndirimler → -Pazarlık → Toplam
    odemeRows = `
      <tr><td class="ol">Liste Fiyatı<\/td><td class="or">${fmt(targetBrutTotal)}<\/td><\/tr>
    `;
    if (gosterilecekIndirim > 0) {
      odemeRows += `
        <tr><td class="ol" style="color:#16a34a;">İndirimler<\/td><td class="or" style="color:#16a34a;font-weight:700;">-${fmt(gosterilecekIndirim)}<\/td><\/tr>
      `;
    }
    if (_pdfEkInd > 0) {
      odemeRows += `
        <tr><td class="ol" style="color:#b45309;">💬 Pazarlık İndirimi<\/td><td class="or" style="color:#b45309;font-weight:700;">-${fmt(_pdfEkInd)}<\/td><\/tr>
      `;
      if (data.pazarlikNotu) {
        odemeRows += `
          <tr><td class="ol" style="color:#92400e;font-size:.85em;">Not<\/td><td class="or" style="color:#92400e;font-size:.85em;">${data.pazarlikNotu}<\/td><\/tr>
        `;
      }
    }
    odemeRows += `
    ...
    odemeRows = `
      ...
      ${Number(data.ekIndirim||0)>0?`<tr><td class="ol" style="color:#b45309;">💬 Pazarlık İndirimi<\/td><td class="or" style="color:#b45309;font-weight:700;">-${fmt(data.ekIndirim)}<\/td><\/tr>`:''}
      ${data.pazarlikNotu&&Number(data.ekIndirim||0)>0?`<tr><td class="ol" style="color:#92400e;font-size:.85em;">Not<\/td><td class="or" style="color:#92400e;font-size:.85em;">${data.pazarlikNotu}<\/td><\/tr>`:''}
      <tr class="grand-tr"><td class="ol">Toplam Ödenecek<\/td><td class="or total-cell">${fmt(gosterilecekToplam)}<\/td><\/tr>
    `;
...
function _doPrintTeklif(p) {
  ...
  // İndirim hesapları — indirimTip bilgisiyle tam doğru hesap
  ...
  const _pdfIndirimTip = p.indirimTip || 'TRY';
  const _pdfIndirimMiktar = Number(p.indirim || 0);
  const toplamAltIndirim = _pdfIndirimTip === 'TRY'
    ? _pdfIndirimMiktar
    : (toplamNakit - toplamItemDisc) * _pdfIndirimMiktar / 100;
  const toplamIndirim    = toplamItemDisc + toplamAltIndirim;
  const ekIndirimPdf     = Number(p.ekIndirim || 0);
  const nakitNet         = toplamNakit - toplamIndirim - ekIndirimPdf;  // pazarlık dahil

  // Ödeme tipi belirle
  let odemeTipi = 'nakit', kartAdi = '', taksitSayisi = 0, aylikTaksit = 0, toplamOdeme = nakitNet;
  if(ab) {
    toplamOdeme  = Number(ab.tahsilat) || nakitNet;
    kartAdi      = ab.kart || ab.label || '';
    taksitSayisi = ab.taksit || 1;
    const _tTaban = taksitSayisi <= 1 ? toplamOdeme : Math.floor(toplamOdeme / taksitSayisi);
    const _tKalan = taksitSayisi <= 1 ? 0 : (toplamOdeme - _tTaban * taksitSayisi);
    aylikTaksit  = ab.aylik || (_tTaban + _tKalan);
    odemeTipi    = taksitSayisi <= 1 ? 'tek_cekim' : 'taksit';
  }
  ...
  const data = {
    ...
    toplamIndirim: toplamItemDisc + toplamAltIndirim, // satır+alt indirim (pazarlık hariç, ayrı gösterilir)
    ekIndirim:     ekIndirimPdf,        // pazarlık indirimi
...
async function logSessionResult(sonuc, neden) {
  if (!currentUser || !_db) return;
  // Sepet boşsa: sadece 'kacti' loglanabilir (blur-only oturum), 'satis' ve 'teklif' loglanmaz
...
logs.forEach(l => {
  ...
  if (h >= 0 && h <= 23) { 
    ...
    const l3KayipCiro = logs
      ...
      .reduce((s, l) => {
        // toplamTutar varsa kullan, yoksa bakilanFiyatlar'dan tahmin et
      ...
      .map(([,s]) => {
        ...
        return `<div style="background:var(--surface);border:1.5px solid var(--border);border-radius:16px;padding:14px;position:relative">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:.62rem">
            ...
            <div style="background:#f8fafc;border-radius:8px;padding:7px;text-align:center;border:1px solid ${l3KapCol}44">
              <b style="font-size:.92rem;color:${l3KapCol}">${l3KapanisOran}</b>
              <div style="color:${l3KapCol};font-size:.58rem;margin-top:2px;font-weight:700">Kapanış Oranı</div>
              <div style="color:#94a3b8;font-size:.54rem">${s.l3Giris} abaküs · ${s.l3Satis} satış</div>
            </div>
          </div>
    ...
    // Tutarsızlık tespiti: blur < sepet gibi durumlar konsola yazdır
    if (_rawSepet > funnelBlur)
      console.warn('⚠️ Huni tutarsızlık: Sepet(' + _rawSepet + ') > Blur(' + funnelBlur + ') — veri kontrol et');
    if (_rawL3 > _rawSepet)
      console.warn('⚠️ Huni tutarsızlık: L3(' + _rawL3 + ') > Sepet(' + _rawSepet + ')');
...
<div style="display:flex;gap:6px;margin-bottom:12px;flex-wrap:wrap">
  ${['saha','destek','admin','hepsi'].map(f=>`
    <button class="funnel-filter-btn" data-filter="${f}"
      ...
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:12px;margin-bottom:12px">
        <div style="font-size:.68rem;font-weight:700;color:var(--text-3);margin-bottom:4px;text-transform:uppercase;letter-spacing:.06em">🔍 Saatlik Aktivite Isı Haritası</div>
        <div style="font-size:.58rem;color:var(--text-3);margin-bottom:8px">En yoğun 3 saat ve trafik dağılımı — fare ile üzerine gelin</div>
        ${(() => {
      ...
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:14px;margin-bottom:12px">
        ...
        <div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap">
          ...
          ${l3KayipCiro > 0 ? `<div style="flex:1;min-width:120px;background:#fff7ed;border-radius:8px;padding:8px;font-size:.64rem;color:#c2410c">
            💸 Kaçırılan Potansiyel Ciro: <b>${fmt(l3KayipCiro)}</b>
          </div>` : ''}
        </div>` : ''}
      </div>
