...
async function loadFunnelAnaliz() {
  ...
  try {
    ...
    // ── PERSONEL KARTLARI ────────────────────────────────────
    // (devam edecek - loadFunnelAnaliz'in kalan kısmı)// ─── SEPET LOGLAMA (Firebase sepet_loglari) ─────────────────────
// Firebase ücretsiz plan uyumlu: günde ~500 yazma, koleksiyon hafif tutulur
async function logSepet(islem, tutar, urunAdi) {
  if (!currentUser || !_db) return;
  try {
...
// ─── TÜM CANLI SEPETLERİ TEMİZLE ────────────────────────────────
async function clearAllLiveBaskets() {
  if (!confirm("Tüm personelin aktif sepet verileri silinecek. Emin misiniz?")) return;
  try {
    const q = query(collection(_db, 'live_baskets'));
    const snap = await getDocs(q);
    const sils = [];
    snap.forEach(d => sils.push(deleteDoc(doc(_db, 'live_baskets', d.id))));
    await Promise.all(sils);
    alert("Tüm canlı sepetler temizlendi.");
  } catch (e) { 
    console.error("Sepet temizleme hatası:", e); 
  }
}

// ─── SATIŞ HUNİSİ ANALİZ (GÜNCELLENMİŞ & HATASIZ) ───────────────
async function loadFunnelAnaliz(gunAralik = 90) {
  const cont = document.getElementById('funnel-analiz-konteynir');
  if (!cont) return;
  cont.innerHTML = '<div class="admin-empty" style="padding:24px">⏳ Firebase\'den çekiliyor…</div>';

  try {
    // 90 GÜN SINIRI (Madde 2 Uygulaması)
    const limitDate = new Date();
    limitDate.setDate(limitDate.getDate() - gunAralik);
    const limitTarih = limitDate.toISOString().split('T')[0]; // YYYY-MM-DD
    ...
    // --- Mevcut Render Kodlarınız Buraya Gelecek ---
    // (Daha önce paylaştığınız personel kartları, momentum vs. kodları)
    cont.innerHTML = `Analiz başarıyla yüklendi: ${allLogs.length} kayıt.`; 

  } catch (err) {
    console.error("Analiz yükleme hatası:", err);
    cont.innerHTML = `<div class="admin-empty" style="color:#dc2626">⚠️ Veri çekilemedi: ${err.message}</div>`;
  }
}
