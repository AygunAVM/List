...
function showApp() {
  ...
  // Çıkış: satış kullanıcıları header'dan çıkış yapar
...
function showPendingChanges(logKey) {
  const log = JSON.parse(localStorage.getItem(logKey)||'[]');
  // Sadece "görülmemiş" olanlar — popup kapatılınca temizlenecek
  const pending = log.filter(entry => !entry.shown);
  if(!pending.length) return;

  // Tüm değişimleri düzleştir, versiyon etiketiyle grupla
  const allChanges = [];
  pending.forEach(entry => {
    allChanges.push({ type:'versiyon', from:entry.fromV, to:entry.toV, ts:entry.ts });
    entry.changes.forEach(c => allChanges.push(c));
    entry.shown = true;
  });
  localStorage.setItem(logKey, JSON.stringify(log));

  showChangePopup(allChanges);
}
...
function showChangePopup(changes) {
  ...
  list.innerHTML=changes.map(c => {
    ...
    if(c.type==='aciklama') {
      return `<div class="change-item"><span class="change-icon">📝</span><div style="flex:1"><span class="change-product">${c.urun}</span><span class="change-detail">Açıklama → <em>${c.new||'(boş)'}</em></span></div><span class="change-badge badge-desc">Açıklama</span></div>`;
    }
    if(c.type==='urunadi') {
      return `<div class="change-item"><span class="change-icon">🏷️</span><div style="flex:1"><span class="change-product">${c.new}</span><span class="change-detail">Ürün adı güncellendi</span></div><span class="change-badge badge-desc">Ad</span></div>`;
    }
    if(c.type==='versiyon') {
      const d = new Date(c.ts);
      const saat = d.toLocaleTimeString('tr-TR',{hour:'2-digit',minute:'2-digit'});
      return `<div class="change-ver-header"><span class="change-ver-pill">${c.from} → ${c.to}</span><span class="change-ver-time">${saat}</span></div>`;
    }
    return '';
  }).join('');
...
Object.assign(window, {
  ...
  clearAllPendingProps, logoutUser,
