...
// ─── VERİ YOLU — index.html ile aynı klasörde Data/ beklenir ───
// app.js src'sinden base path'i al (script yüklenirken currentScript geçerlidir)
const _APP_BASE = (function() {
  ...
  // Fallback: window.location'dan al
  return window.location.href.replace(/[^/]*$/, '');
})();

function dataUrl(file) {
  return _APP_BASE + 'Data/' + file;
}
...
async function loadData() {
  try {
    const json = safeJSON(await (await fetch(dataUrl('urunler.json')+'?v='+Date.now())).text());
  ...
  try {
    allRates = safeJSON(await (await fetch(dataUrl('oranlar.json')+'?v='+Date.now())).text());
...
// ─── WA / TEKLİF ────────────────────────────────────────────────
// WA butonu — sadece abaküsten taksit/nakit seçilince açılır
function openWaFromAbakus() {
  haptic(20);
  if(abakusSelection === undefined || (abakusSelection === null && !document.querySelector('.ab-row-selected'))) {
    // Seçim yapılmamış — uyar
    const ct=document.getElementById('change-toast');
    if(ct){ ct.textContent='Önce bir taksit veya nakit satırı seçin!'; ct.classList.add('show'); setTimeout(()=>ct.classList.remove('show'),2200); }
    return;
  }
  ...
  setTimeout(()=>{
    ...
    if(abakusSelection===null) {
      if(title) title.textContent='📲 Nakit — WhatsApp Teklif';
      if(info) {
      ...
      if(title) title.textContent='📲 '+abakusSelection.label+' — WhatsApp Teklif';
      if(info) {
        info.style.display='block';
    ...
    if(sendBtn) { sendBtn.dataset.mode='wa'; sendBtn.innerHTML='📲 WhatsApp Gönder'; }
    m.style.display='flex';
    requestAnimationFrame(()=>m.classList.add('open'));
  }, 150);
}

function openWaDirect() {} // artık kullanılmıyor
function saveProposalDirect() {} // artık kullanılmıyor
...
function finalizeProposal() {
  ...
  // Telefon zorunlu (WA için)
  if(!phone || phone.length!==11 || phone[0]!=='0') {
    alert('WhatsApp göndermek için 0 ile başlayan 11 haneli telefon giriniz.'); haptic(80); return;
  }
  ...
  const sendBtn=document.querySelector('#wa-modal .wa-send-btn');
  if(sendBtn) { sendBtn.dataset.mode=''; }
  abakusSelection=null;

  // Teklif olarak kaydet mi?
  setTimeout(()=>{
    if(confirm('Teklif "Tekliflerim" listesine kaydedilsin mi?')) {
      const prop = {
        ...
        abakus: null, user: currentUser?.Email||'-', durum: 'bekliyor', not: extraNote,
        tip: 'teklif'
      };
...
function renderProposals(container, forceAll) {
  ...
  target.innerHTML = myProps.map(p => {
    ...
    const actBtns = canAct && p.durum==='bekliyor'
      ...
      : canAct
        ? `<div class="proposal-actions"><span class="prop-closed-label">${p.durum==='onaylandi'?'✓ Onaylandı':'✕ İptal edildi'}</span>${waBtn}</div>`
        : '';
    // Admin için kullanıcı etiketi
...
function updatePropStatus(id, durum) {
  ...
  localStorage.setItem('aygun_proposals', JSON.stringify(proposals));
  renderProposals();
  // Admin panelindeki listeyi de güncelle
