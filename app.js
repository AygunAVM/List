...
function _renderMergedItem(m, idx) {
  ...
  if(hasBoth) {
    ...
    detailHtml = `Nakit: ${fmt(m.price.old)} → <strong>${fmt(m.price.new)}</strong>
      <span class="ci-detail-sep">·</span>
      <em class="ci-aciklama-inline">Açıklama güncellendi</em>`;
    badgeHtml = `<span class="change-badge ${up?'badge-price-up':'badge-price-down'}">${sign}${m.price.pct}%</span>
      <span class="change-badge badge-desc" style="margin-left:3px">Açıklama</span>`;
  } else if(m.price) {
    const up = m.price.diff > 0, sign = up ? '+' : '';
    emoji = up ? '📈' : '📉';
    detailHtml = `Nakit: ${fmt(m.price.old)} → <strong>${fmt(m.price.new)}</strong>`;
...
function _renderChangeItem(c, idx) {
  const isOld    = !!c.isOld;
  const mandatory = !isOld && _isMandatory(c.type);
  const optional  = !isOld && !_isMandatory(c.type);
  const doneCls   = isOld || optional ? 'change-item-done' : '';  // opsiyoneller baştan "okundu"
  const mandCls   = mandatory ? 'change-item-mandatory' : '';
  const optCls    = optional  ? 'change-item-optional'  : '';
...
function showChangePopup(changes, logKey) {
  ...
  if(lowItems.length) {
    ...
    verItems.forEach(c => {
      ...
      html += `<div class="change-item change-item-optional">
    ...
    // Opsiyonel değişimler
    optItems.forEach(c => { html += _renderChangeItem(c, itemIdx++); });
...
function _fbSavePopupSeen() {
  ...
  if(_db) {
    import('https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js').then(({setDoc,doc})=>{
      ...
      setDoc(doc(_db,'analytics',docId), { email, date:today, popupSeen:lastSeen, popupSeenTs:now }, {merge:true});
    }).catch(()=>{});
  }
}
...
function renderAdminUsers() {
  ...
  el.innerHTML = su.map(([email,s])=>{
    ...
    const verBadge = lastSeenVer
      ? `<span class="badge ${verUpToDate?'badge-ver-ok':'badge-ver-warn'}" title="Son popup: ${seenTs||'?'}">${lastSeenVer.split(' ')[0]} ${verUpToDate?'✓':'⚠️'}</span>`
      : `<span class="badge badge-ver-unknown" title="Popup hiç kapatılmamış">—</span>`;
