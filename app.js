...
function parseCampaigns(aciklama) {
  ...
  if (has_birlesen || has_kilitli || has_proje) {
    // ── ⎇ / 🔒︎ / ❖ ayraçlı format ──────────────────────────────
    const ayraclar = [];
...
function toggleCampaign(idx, campIdx) {
  ...
  if (camp.tip !== 'proje' && camp.tip !== 'bilgi' && itemHasProjeSelected) return;
...
function recalculateAllGroupCampaigns() {
  ...
  basket.forEach(item => {
    ...
    Object.entries(item._selectedCamps).forEach(([ci, sel]) => {
      ...
      if (camp.tutar > 0) {
        // Satır fiyatını proje fiyatına sabitle; tüm indirimler/iskontolar devre dışı
  ...
  basket.forEach((item, bi) => {
    ...
    Object.entries(item._selectedCamps).forEach(([ci, sel]) => {
      if (!sel) return;
      const camp = item._campaigns[parseInt(ci)];
      if (!camp || camp.tip === 'bilgi' || camp.tip === 'proje') return;
...
function renderCampaignPills(item, idx) {
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    const disabled = !sel && !isBilgi && (
      ...
      (!itemHasProje   && camp.tip === 'proje' && (itemHasKilitli || itemHasBirlesen)) // tam tersi
    );
    ...
    // Label: proje/bilgi için tam metin (tarih parantezi temizlenmiş), diğerleri grup+tutar
    const isProje2 = camp.tip === 'proje';
    const metinTemiz = camp.metin.replace(/\s*\[\d{2}\.\d{2}\.\d{2,4}\]\s*/g, '').trim();
    const label = (isBilgi || isProje2)
      ? (metinTemiz.length > 26 ? metinTemiz.slice(0,24) + '…' : metinTemiz)
