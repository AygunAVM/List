...
function _qfRenderTaksit() {
  ...
  TAK_LIST.forEach(n => {
    if (!hesap[n]) return;
    const aylik = Math.ceil(hesap[n].tahsilat/n);
    if (oncekiAylik !== null && aylik >= oncekiAylik) return;
    goster[n] = {...hesap[n], aylik}; oncekiAylik = aylik;
  });
  let rows = '';
...
function _calcAbakusKredi(resEl, zRows, nakit, kurum) {
  ...
  VADE_LIST.forEach(ay => {
    zRows.forEach(r => {
      ...
      const toplam = Math.round(nakit * (1 + vf / 100) / 50) * 50;
      const aylik  = Math.ceil(toplam / ay);
...
// Seçilen ödeme tipine göre her ürünün fatura bedelini hesaplar ve gösterir
function _renderFaturaTablosu(parsed) {
  ...
  const tip     = parsed.type || 'nakit';       // 'nakit' | 'taksit' | 'tek'
  const tahsilat = parsed.tahsilat || 0;         // toplam tahsilat
  const aylik   = parsed.aylik || 0;
  const taksit  = parsed.taksit || 0;
  const kartLabel = parsed.label || '';
  const zincir  = parsed.zincir || '';
  const ekInd   = parsed.ekIndirim || 0;

  // Toplam nakit (indirimler düşülmüş)
  const t = basketTotals();
  const totalItemDisc = basket.reduce((s,i) => s + (i.itemDisc||0), 0);
  const baseNakit = t.nakit - totalItemDisc - getDisc(t.nakit - totalItemDisc);
  const nakitFinal = Math.max(0, baseNakit - ekInd);

  // Ödeme tipine göre toplam tahsilat tutarı
  let toplamFatura;
  if (tip === 'nakit') {
    toplamFatura = nakitFinal;
  } else if (tip === 'tek') {
    toplamFatura = tahsilat || (parsed.nakit || nakitFinal);
  } else {
    toplamFatura = tahsilat; // taksit: komisyon dahil toplam
  }

  // Her ürün için oransal fatura bedeli
  const toplamNakit = basket.reduce((s,i) => s + Math.max(0,(i.nakit||0)-(i.itemDisc||0)), 0) || 1;
  let satirHTML = '';
  let kontrolToplam = 0;

  basket.forEach((item, idx) => {
    const itemNakit  = Math.max(0, (item.nakit||0) - (item.itemDisc||0));
    const oran       = itemNakit / toplamNakit;
    const faturaFiyat = Math.round(toplamFatura * oran);
    kontrolToplam   += faturaFiyat;

    // Komisyon farkı
    const komisyon = tip === 'nakit' ? 0 : Math.round(faturaFiyat - itemNakit);
    const komisyonStr = komisyon > 0
      ? `<span style="font-size:.56rem;color:#dc2626;font-weight:700">+${fmt(komisyon)} kom.</span>`
      : komisyon < 0
        ? `<span style="font-size:.56rem;color:#16a34a;font-weight:700">${fmt(komisyon)}</span>`
        : '';

    const renk = idx % 2 === 0 ? '#f8fafd' : '#ffffff';
    satirHTML += `
      <div style="display:flex;align-items:center;justify-content:space-between;
        background:${renk};border-radius:8px;padding:7px 10px;gap:6px">
        <div style="flex:1;min-width:0">
          <div style="font-size:.72rem;font-weight:700;color:#1e293b;
            white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${item.urun}</div>
          <div style="font-size:.58rem;color:#94a3b8;margin-top:1px">
            Liste: ${fmt(item.nakit||0)}${item.itemDisc>0?' · İnd: -'+fmt(item.itemDisc):''}
          </div>
        </div>
        <div style="text-align:right;flex-shrink:0">
          <div style="font-size:.84rem;font-weight:800;color:#0f172a">${fmt(faturaFiyat)}</div>
          ${komisyonStr}
        </div>
      </div>`;
  });
  ...
  let odemeAciklama = '';
  if (tip === 'nakit') {
    odemeAciklama = `<span style="background:#dcfce7;color:#15803d;border-radius:6px;padding:2px 8px;font-size:.62rem;font-weight:700">💵 Nakit</span>`;
  } else if (tip === 'tek') {
    odemeAciklama = `<span style="background:#eff6ff;color:#1d4ed8;border-radius:6px;padding:2px 8px;font-size:.62rem;font-weight:700">💳 ${kartLabel} · Tek Çekim</span>`;
  } else {
    odemeAciklama = `<span style="background:#faf5ff;color:#7c3aed;border-radius:6px;padding:2px 8px;font-size:.62rem;font-weight:700">
      💳 ${kartLabel} · ${taksit} Taksit
    </span>
    <span style="font-size:.62rem;color:#64748b;margin-left:4px">Aylık ${fmt(aylik)}</span>`;
  }
  ...
  topEl.innerHTML = `
    <div style="display:flex;flex-direction:column;gap:4px;width:100%">
      <div style="display:flex;align-items:center;justify-content:space-between">
        ${odemeAciklama}
        <span style="font-size:.86rem;font-weight:900;color:#dc2626">${fmt(toplamFatura)}</span>
      </div>
      ${tip !== 'nakit' ? `<div style="font-size:.58rem;color:#94a3b8;text-align:right">
        Nakit: ${fmt(nakitFinal)} · Komisyon: <span style="color:#dc2626;font-weight:700">+${fmt(Math.max(0,toplamFatura-nakitFinal))}</span>
      </div>` : ''}
    </div>`;
}
...
window.exportAbakusExcel = function() {
  ...
  if (abakusSelection) {
    tip = abakusSelection.taksit > 1 ? 'taksit' : 'tek';
    toplamFatura = abakusSelection.tahsilat;
    odemeBaslik = abakusSelection.label + ' / ' + abakusSelection.zincir
...
window.setTarihAralik = function(tip) {
  ...
  if (bas && bit) {
    cont.dataset.tarihBas = bas.toISOString().slice(0,10);
    cont.dataset.tarihBit = bit.toISOString().slice(0,10);
    // Input'ları güncelle
    const inpBas = document.getElementById('funnel-tarih-bas');
    const inpBit = document.getElementById('funnel-tarih-bit');
    if (inpBas) inpBas.value = cont.dataset.tarihBas;
    if (inpBit) inpBit.value = new Date(bit.getTime()-86400000).toISOString().slice(0,10);
  }
...
async function loadFunnelAnaliz(gunAralik = null, force = false) {
  ...
  if (tarihBas && tarihBit) {
    ...
    limitDateEnd = new Date(tarihBit + 'T23:59:59');
    // Gün farkını hesapla
      ...
      .map(([,s]) => {
        ...
        const bundleStr = bundleOran !== null
          ? `<div style="text-align:center;background:${bundleOran>=50?'#f0fdf4':'#fff7ed'};border-radius:8px;padding:6px 4px;border:1px solid ${bundleOran>=50?'#bbf7d0':'#fed7aa'}">
              ...
              <div style="font-size:.52rem;color:#94a3b8">${s.bundleYapilan}/${s.bundleFirsat} fırsat</div>
            </div>`
...
<div style="background:linear-gradient(160deg,#0f172a 0%,#1e293b 100%);border-radius:20px;padding:14px;margin-bottom:12px;box-shadow:0 4px 24px rgba(0,0,0,.18)">
  ...
  <div style="display:flex;align-items:center;gap:6px;margin-bottom:12px;flex-wrap:wrap">
    ...
    <input id="funnel-tarih-bit" type="date" value="${tarihBit}"
      style="padding:5px 9px;border-radius:10px;border:1.5px solid #334155;background:#1e293b;color:#f1f5f9;font-size:.68rem;font-family:inherit;outline:none;cursor:pointer">
    <button onclick="setTarihManuel()"
      style="padding:5px 13px;border-radius:10px;border:none;background:linear-gradient(135deg,#2563eb,#1d4ed8);color:#fff;font-size:.68rem;font-weight:700;cursor:pointer;font-family:inherit;box-shadow:0 2px 8px rgba(37,99,235,.35)">
      Uygula
    </button>
  </div>
...
async function fetchWeeklyStar() {
  ...
  try {
    ...
    dsSnap.docs.forEach(d => {
      ...
      const s = d.data().satis_sayisi || 0; if (!s) return;
      counts[email] = (counts[email] || 0) + s;
    });
    if (!Object.keys(counts).length) return null;
    const ranked = Object.entries(counts).sort((a,b)=>b[1]-a[1]).map(([email,count],i)=>({email,count,rank:i+1}));
