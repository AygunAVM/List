...
async function loadSepetAnaliz() {
  ...
  try {
    // ── Son 30 gün funnel_logs (sepet + sonuç verisi)
    const sinir = new Date(Date.now() - 30 * 86400000);
    const [funnelSnap, blurSnap] = await Promise.all([
      getDocs(query(collection(_db,'funnel_logs'), where('ts','>=',sinir), orderBy('ts','desc'))),
      getDocs(collection(_db,'fiyat_bakislari'))
    ]);

    const logs = [];
    funnelSnap.forEach(d => logs.push(d.data()));
    const blurKayitlar = [];
    blurSnap.forEach(d => blurKayitlar.push(d.data()));
    ...
    // Blur → Satış / Kaçan (blur bakan oturumlar)
    let blurSatis = 0, blurKacti = 0, blurSepet = 0;
    logs.forEach(l => {
      const hasBlur = (l.bakilanFiyatlar||l.blurUrunListesi||[]).length > 0;
      if (!hasBlur) return;
      blurSepet++;
      if (l.sonuc==='satis'||l.sonuc==='Satis') blurSatis++;
      if (l.sonuc==='kacti'||l.sonuc==='Kacti') blurKacti++;
    });
    const totBlurOturum = blurKayitlar.length; // anlık aktif blur oturumları

    // ── Saatlik diziler
    const saatSepet = Array(24).fill(0);
    const saatSatisA = Array(24).fill(0);
    const saatKactiA = Array(24).fill(0);
    const saatBlurA  = Array(24).fill(0);

    logs.forEach(l => {
      const h = l.saat ?? (l.ts?.toDate ? l.ts.toDate().getHours() : -1);
      if (h < 0 || h > 23) return;
      saatSepet[h]++;
      if (l.sonuc==='satis'||l.sonuc==='Satis') saatSatisA[h]++;
      if (l.sonuc==='kacti'||l.sonuc==='Kacti') saatKactiA[h]++;
      saatBlurA[h] += (l.bakilanFiyatlar||l.blurUrunListesi||[]).length;
    });
    blurKayitlar.forEach(b => {
      if (b.lastSeen?.toDate) {
        const h = b.lastSeen.toDate().getHours();
        if (h>=0 && h<=23) saatBlurA[h]++;
      }
    });

    // En yoğun saat (toplam aktivite)
    const saatToplam = saatSepet.map((v,h) => v + saatBlurA[h]);
    const enSaatIdx  = saatToplam.indexOf(Math.max(...saatToplam));
    const enBlurIdx  = saatBlurA.indexOf(Math.max(...saatBlurA));

    // ── Saatlik CSS heatmap
    const saatMaxHeat = Math.max(...saatSepet.map((v,h) => v + saatBlurA[h]), 1);
    const heatHtml = [...Array(24).keys()].map(h => {
      const tot   = saatSepet[h] + saatBlurA[h];
      const alpha = tot === 0 ? 0.05 : Math.max(0.08, (tot/saatMaxHeat) * 0.85);
      const sW    = saatMaxHeat===0?0:Math.round(saatSatisA[h]/saatMaxHeat*92);
      const kW    = saatMaxHeat===0?0:Math.round(saatKactiA[h]/saatMaxHeat*92);
      const bW    = saatMaxHeat===0?0:Math.round(saatBlurA[h]/saatMaxHeat*92);
      const tip   = `${h<10?'0':''+ h}:00 — ${saatSatisA[h]} satış · ${saatKactiA[h]} kaçan · ${saatBlurA[h]} blur`;
      return `<div style="display:flex;align-items:center;gap:4px;margin-bottom:2px;font-size:.58rem" title="${tip}">
        <span style="min-width:22px;text-align:right;color:var(--text-3)">${h<10?'0':''}${h}</span>
        <div style="flex:1;height:11px;border-radius:4px;background:rgba(99,102,241,${alpha.toFixed(2)});overflow:hidden;display:flex">
          <div style="width:${sW}%;background:#16a34a;height:100%"></div>
          <div style="width:${kW}%;background:#dc2626;height:100%"></div>
          <div style="width:${bW}%;background:#f59e0b;height:100%;opacity:.7"></div>
        </div>
        <span style="min-width:16px;font-size:.54rem;color:var(--text-3)">${tot||''}</span>
      </div>`;
    }).join('');
    ...
    cont.innerHTML = `
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:5px;padding:8px 10px 10px;border-bottom:1px solid var(--border)">
        <div style="text-align:center">
          <div style="font-size:1.05rem;font-weight:800;color:#16a34a">${totS}</div>
          <div style="font-size:.58rem;color:var(--text-3)">Satış</div>
        </div>
        <div style="text-align:center">
          <div style="font-size:1.05rem;font-weight:800;color:#dc2626">${totK}</div>
          <div style="font-size:.58rem;color:var(--text-3)">Kaçan</div>
        </div>
        <div style="text-align:center">
          <div style="font-size:1.05rem;font-weight:800;color:#2563eb">${donusum}%</div>
          <div style="font-size:.58rem;color:var(--text-3)">Dönüşüm</div>
        </div>
        <div style="text-align:center">
          <div style="font-size:1.05rem;font-weight:800;color:#f59e0b">${totBlurOturum}</div>
          <div style="font-size:.58rem;color:var(--text-3)">Blur Otur.</div>
        </div>
      </div>

      <!-- Blur → Sepet Dönüşüm KPI -->
      <div style="padding:7px 10px 8px;border-bottom:1px solid var(--border);display:flex;gap:6px;flex-wrap:wrap">
        <span style="background:#f0fdf4;border-radius:6px;padding:3px 8px;color:#16a34a;font-size:.65rem;font-weight:700">
          👁→🛒 ${blurSatis} satış (fiyat baktı, aldı)
        </span>
        <span style="background:#fef2f2;border-radius:6px;padding:3px 8px;color:#dc2626;font-size:.65rem;font-weight:700">
          👁→❌ ${blurKacti} kaçan (fiyat baktı, gitti)
        </span>
        <span style="background:#fffbeb;border-radius:6px;padding:3px 8px;color:#92400e;font-size:.65rem;font-weight:700">
          👁 ${totBlurOturum} aktif blur oturumu
        </span>
      </div>

      <!-- Saatlik CSS Heatmap -->
      <div style="padding:8px 10px 6px">
        <div style="font-size:.60rem;font-weight:700;color:var(--text-3);margin-bottom:5px;text-transform:uppercase;letter-spacing:.05em">⏰ Saatlik Yoğunluk Haritası</div>
        <div style="display:flex;gap:8px;font-size:.57rem;margin-bottom:5px">
          ...
          <span><span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:#f59e0b;margin-right:2px"></span>Blur</span>
          <span style="margin-left:auto;color:var(--text-3)">fare → detay</span>
        </div>
        ${heatHtml}
        <div style="display:flex;justify-content:space-between;font-size:.56rem;color:var(--text-3);margin-top:4px">
          <span>En yoğun: <b style="color:var(--text-1)">${enSaatIdx<10?'0':''}${enSaatIdx}:00</b></span>
          <span>En çok blur: <b style="color:#f59e0b">${enBlurIdx<10?'0':''}${enBlurIdx}:00</b></span>
        </div>
      </div>
    `;
...
logs.forEach(l => {
  ...
  if (h>=0) { 
    ...
    const _funnelStats = {
      ...
      // Personel
      personelHTML,
      // Saatlik diziler (raw — render fonksiyonunda kullanılır)
      saatSatis, saatKacti, saatBlur,
      // Saatlik barlar (hazır HTML)
      saatBar, blurBar
    };
