...
// ── Kampanya Eşik Arama Butonu ─────────────────────────────────────────────
// Her kampanya pill'inin yanında küçük 🔍 butonu.
// Basıldığında: sepeti kapatır, urunler sekmesine geçer,
// arama kutusuna tamamlayıcı ürünlerin ortak grubunu yazar.
function _campSearchBtn(item, camp, idx, ci) {
  // Sadece birlesen / kilitli kampanyalarda — bilgi, proje, bağımsız değil
  ...
  // Zaten seçiliyse butona gerek yok
  if (item._selectedCamps && item._selectedCamps[ci]) return '';
  ...
  // allProducts'ta bu grup+esik için farklı rol ve stoklu olanları bul
  const tamlar = [];
  const sepetKodlar = new Set(basket.map(i => String(i['Kod'] || i['kod'] || '')));
  for (const p of allProducts) {
    const pKod   = String(p['Kod'] || p['kod'] || '');
    if (sepetKodlar.has(pKod)) continue;
    const pNakit = parseFloat(String(p['Nakit'] || p['nakit'] || 0).replace(',','.')) || 0;
    if (iNakit > 0 && pNakit > iNakit * YON) continue;
    const stok = parseFloat(p['Stok'] || p['stok'] || 0) || 0;
    if (stok <= 0) continue;
    for (const pc of parseCampaigns(p['Açıklama'] || p['aciklama'] || '')) {
      if (pc.grup !== grup || pc.esik !== esik) continue;
      if (pc.rol === mevcutRol) continue;
      tamlar.push({ ad:(p['Ürün'] || p['urun'] || '').replace(/^[➥♲]\s*/,'').trim(), nakit:pNakit, stok });
      break;
    }
  }

  if (!tamlar.length) return '';  // tamamlayıcı yok → buton gösterme

  // Arama kelimesi: grup adı (KEA, KM, IKILI...) — tabloda kolayca filtreler
  ...
  return `<button type="button" title="${tamlar.length} tamamlayıcı ürün — ara"
    onclick="window._campSearch(${JSON.stringify(araKelime)})"
    style="${btnStyle}">🔍</button>`;
}

// Global arama fonksiyonu — sepeti kapatır, urunler sekmesine geçer, aratır
...
function renderCampaignPills(item, idx) {
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    return `<button type="button"${clickAttr} title="${camp.metin.replace(/"/g,'&quot;')}" `
      ...
      + `</button>`
      + _campSearchBtn(item, camp, idx, ci);
  }).join('');
...
function checkUpsellOpportunities() {
  // Kampanya önerileri artık pill butonlarının yanındaki 🔍 ile yapılıyor
