...
const APP_BUILD_VERSION = '20260729rev84';
...
function renderAdminUsers() {
  ...
  el.innerHTML = su.map(([email, s]) => {
    ...
    const staleBuild = isOnline && mySession?.appVersion && mySession.appVersion !== APP_BUILD_VERSION;
    ...
    return `<div class="user-row" style="border-left:3px solid ${isOnline?'#16a34a':isToday?'#f59e0b':'var(--border)'}">
      ...
      <div class="user-badges" style="display:flex;align-items:center;gap:4px;flex-wrap:wrap">
        ...
        ${staleBuild ? `<span class="badge" style="background:#fef2f2;color:#dc2626;font-weight:800" title="Bu cihaz eski bir sürüm çalıştırıyor (${mySession.appVersion}) — hard-refresh gerekiyor">⚠️ Eski sürüm</span>` : ''}
