...
function showChangePopup(changes, logKey) {
  ...
  let itemIdx = 0;
  let html = '';
  let inOptBlock = false, inMandBlock = false, inOldBlock = false;

  changes.forEach(c => {
    if(c.type === 'versiyon') {
      const saat = new Date(c.ts).toLocaleTimeString('tr-TR',{hour:'2-digit',minute:'2-digit'});
      const d = new Date(c.ts);
      const tarih = d.toLocaleDateString('tr-TR',{day:'2-digit',month:'2-digit',year:'2-digit'});
      const isOld = !!c.isOld;
      html += `<div class="change-ver-header ${isOld?'ver-old':'ver-new'}">
        <span class="change-ver-pill ${isOld?'pill-old':''}">${c.from} → ${c.to}</span>
        <span class="change-ver-time">${tarih} ${saat}</span>
        ${isOld ? '<span class="ver-done-badge">✓ Okundu</span>' : ''}
      </div>`;
      return;
    }
    ...
    // Bölüm başlıkları
    if(optional && !inOptBlock) {
      html += `<div class="cpd cpd-info-header">
        <div class="cpd-info-title">📋 Bilgi Amaçlı</div>
        <div class="cpd-info-desc">Versiyon, stok ve ürün adı değişimleri — okuyup geçebilirsiniz</div>
      </div>`;
      inOptBlock = true; inMandBlock = false; inOldBlock = false;
    } else if(mandatory && !inMandBlock) {
      html += '<div class="cpd cpd-mandatory"><span>⚠️ Dikkat — Tüm Değişimleri Okuyun</span><span class="cpd-sub">Her satıra tıklayarak onaylayın</span></div>';
      inMandBlock = true; inOptBlock = false; inOldBlock = false;
    } else if(isOld && !inOldBlock) {
      html += '<div class="cpd cpd-old"><span>📁 Önceki Oturumda Görüldü</span></div>';
      inOldBlock = true; inOptBlock = false; inMandBlock = false;
    }

    html += _renderChangeItem(c, itemIdx++);
  });
