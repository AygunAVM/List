...
function renderTable(searchVal) {
  ...
  // ── Kampanya ara filtresi (camp-ara-btn) ─────────────────────
  // Aynı grup+esik ama excludeRol dışındaki rolleri göster
  if (_campAraFilter) {
    ...
    data = data.filter(u => {
      ...
      while ((m = campRe.exec(ac)) !== null) {
        if (m[1].toUpperCase() === cfGrup && parseInt(m[2]) === cfEsik) {
          if (m[3].toUpperCase() === excludeRol) return false;
          return true;
        }
      }
      return false;
...
// ⎇ (birlesen), 🔒 (kilitli), ❖ (proje) destekler
function _campSearchBtn(item, camp, ci) {
  ...
  // Proje tipi için farklı renk tonu
  const isProje  = camp.tip === 'proje';
  const btnBg    = isProje ? '#2e1065' : '#172554';
  const btnColor = isProje ? '#c4b5fd' : '#93c5fd';
  const btnBdr   = isProje ? '#6d28d9' : '#1e40af';
  ...
  return `<button type='button'
    ...
    title='${camp.tip === "proje" ? "❖" : camp.tip === "kilitli" ? "🔒" : "⎇"} ${grup} eşik tamamlayıcı ürünleri ara'
