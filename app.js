...
function _renderSingleProp(p) {
  ...
  const canAct = isAdmin() || p.user===me;
    ...
    // Sepete Ekle: kendi teklifi + admin tüm teklifleri sepete alabilir
    if(isAdmin() || p.user === (currentUser?.Email||'')) {
      const isOtherUser = isAdmin() && p.user !== (currentUser?.Email||'');
      btns.push(`<button class="pact-btn haptic-btn" onclick="teklifSepeteEkle('${p.id}')" title="Güncel fiyat ve açıklamalarla sepete ekle" style="color:#16a34a;border-color:#bbf7d0;background:#f0fdf4;font-weight:700">${isOtherUser ? '🛒 Sepete Al' : '🛒'}</button>`);
    }
    if(canEdit) {
...
function renderSiparisBildirimBar() {
  ...
  // Admin için barı gizle — admin paneli zaten yeterli
  if (isAdmin()) {
    bar.style.display = 'none';
    return;
  }

  // Personel motivasyon analizi — proposals verisinden, hafif hesaplama
  try {
    const me      = currentUser?.Email || '';
    const bugun   = new Date().toISOString().slice(0, 10);
    const haftaOnce = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);

    const benimTeklifler = (proposals || []).filter(p => p.user === me);
    const bugunTeklif    = benimTeklifler.filter(p => (p.ts || '').slice(0, 10) === bugun).length;
    const bugunSatis     = benimTeklifler.filter(p => p.durum === 'satisDondu' && (p.ts || '').slice(0, 10) === bugun).length;
    const haftaTeklif    = benimTeklifler.filter(p => (p.ts || '').slice(0, 10) >= haftaOnce).length;
    const haftaSatis     = benimTeklifler.filter(p => p.durum === 'satisDondu' && (p.ts || '').slice(0, 10) >= haftaOnce).length;
    const donusum        = haftaTeklif > 0 ? Math.round((haftaSatis / haftaTeklif) * 100) : 0;

    // Motivasyon mesajını duruma göre seç
    let emoji, mesaj, renk, alt = '';

    if (bugunSatis >= 3) {
      emoji = '🔥'; renk = '#15803d';
      mesaj = `Bugün ${bugunSatis} satış kapattın!`;
      alt   = 'Müthiş performans, devam et!';
    } else if (bugunSatis === 2) {
      emoji = '💪'; renk = '#1d4ed8';
      mesaj = `Bugün ${bugunSatis} satış — bir tane daha!`;
      alt   = haftaTeklif > 0 ? `Bu hafta %${donusum} dönüşüm` : '';
    } else if (bugunSatis === 1) {
      emoji = '✅'; renk = '#0369a1';
      mesaj = `Bugün 1 satış yaptın.`;
      alt   = 'Bir teklif daha oluştur, ikinciye ulaş!';
    } else if (bugunTeklif >= 2) {
      emoji = '⚡'; renk = '#b45309';
      mesaj = `Bugün ${bugunTeklif} aktif teklif var.`;
      alt   = 'Birini satışa dönüştür!';
    } else if (haftaSatis > 0) {
      emoji = '📈'; renk = '#0369a1';
      mesaj = `Bu hafta ${haftaSatis} satış · %${donusum} dönüşüm`;
      alt   = 'Bugün yeni teklif oluştur!';
    } else if (haftaTeklif > 0) {
      emoji = '🎯'; renk = '#7c3aed';
      mesaj = `${haftaTeklif} teklifin var — takip et!`;
      alt   = 'Satışa dönüştürmek için ara!';
    } else {
      emoji = '🚀'; renk = '#7c3aed';
      mesaj = 'İlk teklifini oluştur!';
      alt   = 'Bugün bir müşteri kazan.';
    }
