...
function showPendingChanges(logKey) {
  ...
  // Yeni (isOld:false) değişimleri öneme + markaya göre sırala
  // Eski (isOld:true) değişimleri en alta koy
  const newItems    = allChanges.filter(c => !c.isOld && c.type !== 'versiyon');
  ...
  newItems.sort((a,b) => {
    const pa = _changePriority(a.type), pb = _changePriority(b.type);
    if(pa !== pb) return pa - pb;
    // Aynı öncelikteyse markaya göre (urun alanının ilk kelimesi)
    const ma = (a.urun||'').split(' ')[0], mb = (b.urun||'').split(' ')[0];
    return ma.localeCompare(mb,'tr');
  });

  // Yeni versiyonları tek başlık olarak öne al, sonra sıralı değişimler, sonra eskiler
  const sorted = [...newVerItems, ...newItems, ...oldVerItems, ...oldItems];
...
function showChangePopup(changes, logKey) {
  ...
  // Yeni değişimlere öncelik grubu ayracı ekle
  let lastPrioGroup = null;
  const withDividers = [];
  changes.forEach(c => {
    if(!c.isOld && c.type !== 'versiyon') {
      const grp = _isMandatory(c.type) ? 'mandatory' : 'optional';
      if(grp !== lastPrioGroup) {
        withDividers.push({ type: '_divider', grp });
        lastPrioGroup = grp;
      }
    }
    withDividers.push(c);
  ...
  let itemIdx = 0;
  list.innerHTML = withDividers.map(c => {

    if(c.type === '_divider') {
      if(c.grp === 'mandatory') return '<div class="change-priority-divider">🔴 Zorunlu — işaretleyerek okundu olarak işaretle</div>';
      return '<div class="change-priority-divider">🟡 Bilgi amaçlı — opsiyonel</div>';
    }

    if(c.type === 'versiyon') {
      const saat = new Date(c.ts).toLocaleTimeString('tr-TR',{hour:'2-digit',minute:'2-digit'});
      const oldCls = c.isOld ? 'ver-old' : 'ver-new';
      return `<div class="change-ver-header ${oldCls}">
        <span class="change-ver-pill ${c.isOld?'pill-old':''}">${c.from} → ${c.to}</span>
        <span class="change-ver-time">${saat}</span>
        ${c.isOld ? '<span class="ver-done-badge">✓ Okundu</span>' : ''}
      </div>`;
    }
    ...
    const idx = itemIdx++;
    const isOld   = !!c.isOld;
    const mandatory = !isOld && _isMandatory(c.type);
    const optional  = !isOld && !_isMandatory(c.type);
    const doneCls = isOld ? 'change-item-done' : '';
    const mandatoryCls = mandatory ? 'change-item-mandatory' : '';
    const optionalCls  = optional  ? 'change-item-optional'  : '';
    const chk = isOld
      ? `<span class="chk-box chk-done" data-idx="${idx}">✓</span>`
      : optional
        ? `<span class="chk-box chk-optional" data-idx="${idx}" onclick="toggleChangeItem(this)" title="Opsiyonel"></span>`
        : `<span class="chk-box" data-idx="${idx}" onclick="toggleChangeItem(this)"></span>`;

    let inner = '';
    if(c.type === 'price') {
      const up=c.diff>0, sign=up?'+':'';
      inner = `<span class="change-icon">${up?'📈':'📉'}</span>
        <div class="change-body">
          <span class="change-product">${c.urun}</span>
          <span class="change-detail">Nakit: ${fmt(c.old)} → <strong>${fmt(c.new)}</strong></span>
        </div>
        <span class="change-badge ${up?'badge-price-up':'badge-price-down'}">${sign}${c.pct}%</span>`;
    } else if(c.type === 'stok') {
      const up=c.diff>0, sign=up?'+':'';
      inner = `<span class="change-icon">${up?'📦':'⚠️'}</span>
        <div class="change-body">
          <span class="change-product">${c.urun}</span>
          <span class="change-detail">Stok: ${c.old} → <strong>${c.new}</strong></span>
        </div>
        <span class="change-badge ${up?'badge-stok-up':'badge-stok-down'}">Stok ${sign}${c.diff}</span>`;
    } else if(c.type === 'aciklama') {
      inner = `<span class="change-icon">📝</span>
        <div class="change-body">
          <span class="change-product">${c.urun}</span>
          <span class="change-detail">Açıklama → <em>${c.new||'(boş)'}</em></span>
        </div>
        <span class="change-badge badge-desc">Açıklama</span>`;
    } else if(c.type === 'urunadi') {
      inner = `<span class="change-icon">🏷️</span>
        <div class="change-body">
          <span class="change-product">${c.new}</span>
          <span class="change-detail">Ürün adı güncellendi</span>
        </div>
        <span class="change-badge badge-desc">Ad</span>`;
    } else if(c.type === 'info') {
      inner = `<span class="change-icon">ℹ️</span>
        <div class="change-body">
          <span class="change-product">Versiyon güncellendi</span>
          <span class="change-detail">${c.msg||''}</span>
        </div>
        <span class="change-badge" style="background:#f0f9ff;color:#0369a1">Bilgi</span>`;
    } else return '';

    return `<div class="change-item ${doneCls} ${mandatoryCls} ${optionalCls}" data-idx="${idx}">${chk}${inner}</div>`;
  }).join('');

  // Buton durumunu güncelle
  _updateChangeBtn();

  // logKey'i popup'a aktar — kapatılınca shown=true yapılacak
  const p = document.getElementById('change-popup');
...
function _updateChangeBtn() {
  requestAnimationFrame(() => {
    ...
    if(!btn) return;
    const allNew = document.querySelectorAll('#change-list .change-item:not(.change-item-done)');
    const ready  = allNew.length === 0;

    const markAllBtn = document.getElementById('change-mark-all-btn');
    if(markAllBtn) {
      markAllBtn.style.display = (isAdmin() && !ready) ? 'block' : 'none';
    }

    // Herkes işaretlemeden geçemez — sadece "Tümünü İşaretle" admin'e özel
    btn.disabled      = !ready;
...
function markAllChanges() {
  // Admin: sadece zorunlu olanları işaretle (🔴🟠)
  document.querySelectorAll('#change-list .change-item-mandatory:not(.change-item-done)').forEach(item => {
    ...
    if(chk) { chk.textContent = '✓'; chk.classList.add('chk-done'); }
  });
  _updateChangeBtn();
  haptic(18);
}
...
function closeChangePopup() {
  ...
  // Ürün tablosunu yeniden render et (popup açıkken tablo güncellendiyse)
...
function renderAdminUsers() {
  ...
  el.innerHTML = su.map(([email,s])=>{
    ...
    // Son görülen versiyonu localStorage'dan oku (her kullanıcı kendi cihazında)
    // Admin kendi cihazındaki veriyi görür; Firebase'e taşınmadı
    const seenArr = JSON.parse(localStorage.getItem('aygun_change_seen_'+email)||'[]');
    const lastSeenVer = seenArr.length ? seenArr[seenArr.length-1] : null;
    const currentVer  = window._currentVersion || '';
    const verUpToDate = lastSeenVer && currentVer && lastSeenVer === currentVer;
    const verBadge = lastSeenVer
      ? `<span class="badge" style="background:${verUpToDate?'#dcfce7;color:#15803d':'#fef3c7;color:#92400e'}" title="Son görülen versiyon">${lastSeenVer.split(' ')[0]}</span>`
      : `<span class="badge" style="background:#fee2e2;color:#dc2626" title="Versiyon bilinmiyor">—</span>`;
...
Object.assign(window, {
  ...
  clearAllPendingProps, logoutUser, toggleChangeItem, markAllChanges,
