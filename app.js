...
// ═══════════════════════════════════════════════════════════════
// ✨ EN İYİ FİYAT OPTİMİZASYON MOTORU  v2
// Sadece ⎇ (birlesen) ve 🔒 (kilitli) kampanyaları değerlendirir.
// Kurallar:
//   • Aynı üründe tek 🔒 (aynı gruptan), tek ⎇ veya karışık yok
//   • 🔒 farklı gruplar aynı üründe olabilir (IKILI + BDTVSP gibi)
//   • Brute-force yerine greedy + grup-bazlı atama
// ═══════════════════════════════════════════════════════════════
...
function optimizeCampaigns() {
  ...
  // 1. Sıfırla (proje/bagimsiz/bilgi dokunulmaz)
  basket.forEach(item => {
    item._selectedCamps = {};
    item._pendingGroups = {};
  ...
  // 2. Kandidat kampanyaları topla { bi, ci, camp }
  const kandidatlar = [];
  basket.forEach((item, bi) => {
    if (!item._campaigns) item._campaigns = parseCampaigns(item.aciklama || '');
    item._campaigns.forEach((camp, ci) => {
      ...
      kandidatlar.push({ bi, ci, camp });
    });
  });

  if (!kandidatlar.length) {
    _campToast('Optimize edilecek ⎇/🔒 kampanya bulunamadı.', 'info');
    return;
  }

  // 3. Grup bazında topla
  // Her grup için aday seçimler listesi
  const gruplar = {}; // grup → [{ bi, ci, camp }]
  kandidatlar.forEach(k => {
    const g = k.camp.grup;
    if (!gruplar[g]) gruplar[g] = [];
    gruplar[g].push(k);
  });

  // 4. Her grup için en iyi atamayı bağımsız hesapla
  // Sonra tüm grupların atamalarını birleştir (çakışma kontrolüyle)
  // Çakışma: aynı üründe aynı grup'tan 2 kez 🔒 atanamaz
  //          aynı üründe hem ⎇ hem 🔒 atanamaz

  // Atama durumu: bi → { kilitliGruplar: Set, birlesenVar: bool }
  const urunDurum = {}; // bi → { kilitliGruplar, birlesenVar }
  const sonSecimler = []; // { bi, ci, camp }

  // Grupları tutara göre büyükten küçüğe sırala (büyük indirimler önce)
  const siraliGruplar = Object.entries(gruplar)
    .sort((a, b) => {
      const tutarA = Math.max(...a[1].map(k => k.camp.tutar));
      const tutarB = Math.max(...b[1].map(k => k.camp.tutar));
      return tutarB - tutarA;
    });

  siraliGruplar.forEach(([grup, kandidatListesi]) => {
    const esik  = kandidatListesi[0].camp.esik || 1;
    const tip   = kandidatListesi[0].camp.tip;

    if (esik === 1) {
      // PAP benzeri: her ürün bağımsız
      kandidatListesi.forEach(k => {
        if (_optimAtanabilirMi(k, urunDurum)) {
          _optimAta(k, urunDurum, sonSecimler);
        }
      });
      return;
    }

    // Esik >= 2: greedy çiftler (birden fazla çift mümkün)
    // Rol bazlı mı? ANY mı?
    const hepsiANY = kandidatListesi.every(k => !k.camp.rol || k.camp.rol === 'ANY');

    if (hepsiANY) {
      // ANY: sırayla esik kadar ürün seç → çift
      const atanabilirler = kandidatListesi.filter(k => _optimAtanabilirMi(k, urunDurum));
      // Aynı üründen sadece 1 tane (en yüksek tutarlı)
      const urunBaziEn = {};
      atanabilirler.forEach(k => {
        if (!urunBaziEn[k.bi] || k.camp.tutar > urunBaziEn[k.bi].camp.tutar)
          urunBaziEn[k.bi] = k;
      });
      const liste = Object.values(urunBaziEn);
      for (let i = 0; i + esik <= liste.length; i += esik) {
        const cift = liste.slice(i, i + esik);
        cift.forEach(k => _optimAta(k, urunDurum, sonSecimler));
      }
    } else {
      // Rol bazlı: greedy — her çift için esik kadar farklı harfli ürün bul
      let kalan = kandidatListesi.filter(k => _optimAtanabilirMi(k, urunDurum));
      // Aynı üründen aynı rol için tek aday tut
      while (true) {
        ...
        for (const k of kalan) {
          ...
          if (!kullanilanH.has(r) && !kullanilanU.has(k.bi) && cift.length < esik) {
            cift.push(k); kullanilanH.add(r); kullanilanU.add(k.bi);
          } else {
            sonraki.push(k);
          }
        }
        if (cift.length === esik) {
          cift.forEach(k => _optimAta(k, urunDurum, sonSecimler));
          // Sonraki turda atanabilirlik yeniden kontrol et
          kalan = sonraki.filter(k => _optimAtanabilirMi(k, urunDurum));
        } else break;
      }
    }
  });
...
function _optimAtanabilirMi(k, urunDurum) {
  const d = urunDurum[k.bi];
  if (!d) return true;
  // Aynı üründe hem ⎇ hem 🔒 olamaz
  if (k.camp.tip === 'kilitli' && d.birlesenVar) return false;
  if (k.camp.tip === 'birlesen' && d.kilitliGruplar.size > 0) return false;
  // Aynı üründe aynı gruptan 2. 🔒 olamaz
  if (k.camp.tip === 'kilitli' && d.kilitliGruplar.has(k.camp.grup)) return false;
  return true;
}

function _optimAta(k, urunDurum, sonSecimler) {
  if (!urunDurum[k.bi]) urunDurum[k.bi] = { kilitliGruplar: new Set(), birlesenVar: false };
  if (k.camp.tip === 'kilitli') urunDurum[k.bi].kilitliGruplar.add(k.camp.grup);
  if (k.camp.tip === 'birlesen') urunDurum[k.bi].birlesenVar = true;
  sonSecimler.push(k);
}

// ═══════════════════════════════════════════════════════════════
// 💡 AKILLI ÇAPRAZ SATIŞ (UPSELL) MOTORU  v2
// cart-footer içinde sabit bir div — innerHTML sıfırlanmaz
// ═══════════════════════════════════════════════════════════════

function checkUpsellOpportunities() {
  // Bar sepet modalının footer'ında — cart-table-area değil cart-footer öncesi
  const CONTAINER_ID = 'upsell-bar-container';
  let container = document.getElementById(CONTAINER_ID);

  if (!container) {
    // İlk kez: cart-footer'ın önüne ekle
  ...
  // Grup bazında kaç ürünün bu grupta kampanyası var?
  const grupBilgi = {};
  basket.forEach((item, bi) => {
    if (!item._campaigns) item._campaigns = parseCampaigns(item.aciklama || '');
  ...
  Object.entries(grupBilgi).forEach(([grup, info]) => {
    ...
    // Tamamlanmış çiftler sonrası açık çifteki konum
    const acikPozisyon = seciliSayi % info.esik;
    const eksik = acikPozisyon === 0 ? info.esik : (info.esik - acikPozisyon);

    if (eksik === 1 && seciliSayi > 0) {
      // 1 ürün eksik — açık çift bekliyor
      firsatlar.push({ grup, kazanc: info.tutar, eksik: 1, tip: 'eksik1' });
    } else if (seciliSayi === 0 && sahipSayi >= info.esik) {
      // Hiç seçilmemiş ama yeterli ürün var — hazır
      firsatlar.push({ grup, kazanc: info.tutar, eksik: 0, tip: 'hazir' });
    } else if (eksik <= 2 && sahipSayi < info.esik) {
      // Sepete ürün eklense kazanılabilir
      firsatlar.push({ grup, kazanc: info.tutar, eksik, tip: 'ekle' });
    }
  });
  ...
  // En yüksek kazançlı fırsatı göster
  firsatlar.sort((a, b) => b.kazanc - a.kazanc);
  const f = firsatlar[0];
  const kazancFmt = f.kazanc >= 1000
    ? (f.kazanc/1000).toFixed(f.kazanc%1000===0?0:1)+'k₺'
    : f.kazanc + '₺';

  let mesaj, buton;
  if (f.tip === 'hazir') {
    mesaj = `<b>${f.grup}</b> kampanyası için yeterli ürün sepette!`;
    buton = `<button onclick="optimizeCampaigns()" style="background:#f59e0b;color:#0f172a;border:none;
      border-radius:6px;padding:5px 10px;font-weight:800;font-size:.65rem;cursor:pointer;
      white-space:nowrap;font-family:inherit">✨ Uygula +${kazancFmt}</button>`;
  } else if (f.tip === 'eksik1') {
    mesaj = `<b>${f.grup}</b> kampanyasına <b>1 ürün</b> ekle → <b>+${kazancFmt}</b> indirim`;
    buton = `<button onclick="optimizeCampaigns()" style="background:#f59e0b;color:#0f172a;border:none;
      border-radius:6px;padding:5px 10px;font-weight:800;font-size:.65rem;cursor:pointer;
      white-space:nowrap;font-family:inherit">✨ Optimize Et</button>`;
  } else {
    mesaj = `<b>${f.grup}</b> kampanyasına <b>${f.eksik} ürün</b> ekle → <b>+${kazancFmt}</b> kazan`;
    buton = '';
  }
  ...
  container.innerHTML = `
    <div style="display:flex;align-items:center;gap:8px;
      ...
      padding:8px 12px;font-size:.68rem;border-top:2px solid #f59e0b;">
      <span style="font-size:.9rem;flex-shrink:0">💡</span>
      <span style="flex:1;line-height:1.4">${mesaj}</span>
      ${buton}
      <button onclick="document.getElementById('upsell-bar-container').innerHTML=''"
        style="background:none;border:none;color:#64748b;font-size:.9rem;
        cursor:pointer;padding:0;flex-shrink:0;line-height:1">✕</button>
    </div>`;
}

// ── updateCartUI hook (sadece yeni call ekleniyor) ───────────────
const _origUpdateCartUI = updateCartUI;
window.updateCartUI = function() {
  _origUpdateCartUI.apply(this, arguments);
  try { checkUpsellOpportunities(); } catch(e) {}
};
