...
function renderTable(searchVal) {
  ...
  // Önemli: spread kopya YAPMA — allProducts.indexOf(u) bozulur → addToBasket(-1)
  // Rol bilgisini orijinal nesneye yaz, render sonrası temizle
    ...
    data = data.filter(u => {
      ...
      while ((m = campRe.exec(ac)) !== null) {
        if (m[1].toUpperCase() === cfGrup && parseInt(m[2]) === cfEsik) {
          ...
          u._campAraRol = rol; // orijinal nesneye yaz (indexOf çalışır)
          return true;
        }
      }
      return false;
  ...
  // ── Kampanya ara rol etiketlerini orijinal nesneden temizle ──
  if (_campAraFiltered) {
...
function _campSearchBtn(item, camp, ci) {
  ...
  if (secili && !pending) return ''; // tamamlanmış seçim — buton gizle
  ...
  const btnBg    = pending ? '#451a03' : isProje ? '#2e1065' : '#172554';
  const btnColor = pending ? '#fcd34d' : isProje ? '#c4b5fd' : '#93c5fd';
  const btnBdr   = pending ? '#d97706' : isProje ? '#6d28d9' : '#1e40af';
