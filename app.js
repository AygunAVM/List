...
function checkUpsellOpportunities() {
  ...
  // ── Açıklama metninden kampanya segmentlerini parse et ───────────
  function _parseCamps(ac) {
    ...
    while ((m = re.exec(ac)) !== null) {
      ...
      const mult = /k/i.test(m[0].charAt(m[1].length + 1)) ? 1000 : 1;
      out.push({
        grup : m[1].toUpperCase(),
        tutar: parseFloat(raw) * mult,
        esik : parseInt(m[3]),
        rol  : m[4].toUpperCase()
      });
    }
  ...
  // ── 1. Sepetteki kampanya durumu ─────────────────────────────────
  const grupBilgi = {};
  basket.forEach((item, bi) => {
    const itemNakit = parseFloat(String(item[nakitKey] || item.Nakit || 0).replace(',', '.')) || 0;
    (item._campaigns || []).forEach(camp => {
      if (camp.tip !== 'birlesen' && camp.tip !== 'kilitli') return;
      if (camp.tutar <= 0 || camp.esik < 2) return;
      if (camp.sonTarih && new Date() > camp.sonTarih) return;
      const g = camp.grup;
      if (!grupBilgi[g]) grupBilgi[g] = {
        esik: camp.esik, tutar: camp.tutar,
        roller: {}, sahipBi: new Set(), sepetNakitler: []
      };
      grupBilgi[g].sahipBi.add(bi);
      if (camp.tutar > grupBilgi[g].tutar) grupBilgi[g].tutar = camp.tutar;
      const rol = (camp.rol || 'ANY').toUpperCase();
      if (!grupBilgi[g].roller[bi]) {
        grupBilgi[g].roller[bi] = rol;
        grupBilgi[g].sepetNakitler.push(itemNakit);
      }
    });
  });
  ...
  Object.entries(grupBilgi).forEach(([grup, info]) => {
    ...
    // Tip A — Eşik zaten sağlanmış, sadece seçim lazım
    if (seciliSayi === 0 && sahipSayi >= info.esik) {
      const farkliRol = sepetRoller.has('ANY') ? info.esik : sepetRoller.size;
      if (farkliRol >= info.esik || sepetRoller.has('ANY')) {
        firsatlar.push({ grup, kazanc: info.tutar, tip: 'hazir', oncelik: 1 });
      }
      return;
    }

    // Tip B — Seçim devam ediyor, 1 ürün daha seç
    if (seciliSayi > 0) {
      const acikPos = seciliSayi % info.esik;
      if (acikPos > 0 && (info.esik - acikPos) === 1 && sahipSayi >= info.esik) {
        firsatlar.push({ grup, kazanc: info.tutar, tip: 'secim_eksik', oncelik: 2 });
      }
      return;
    }

    // Tip C — Ürün eklenerek tamamlanabilir
    if (sahipSayi > 0 && sahipSayi < info.esik) {
      const mevcutRoller = new Set(Object.values(info.roller));

      // Sepetteki ürünlerin ortalama nakit fiyatı (yön tespiti)
      const sf = info.sepetNakitler.filter(x => x > 0);
      const sepetMedyan = sf.length ? sf.reduce((a, b) => a + b, 0) / sf.length : 0;

      // allProducts'ta tamamlayıcı ürünleri bul
      ...
      for (const p of allProducts) {
        ...
        for (const pc of _parseCamps(p[acikKey] || '')) {
          if (pc.grup !== grup || pc.esik !== info.esik) continue;
          if (mevcutRoller.has(pc.rol)) continue;  // aynı rol → tamamlamaz
      ...
      if (!tamamlayicilar.length) return; // Gerçek tamamlayıcı yok → öneri gösterme

      // ── AKILLI SIRALAMA ─────────────────────────────────────────
      // Kural: Sepetteki üründen UCUZ olan önce önerilmeli.
      // Bulaşık makinesi → tost makinesi öner (mantıklı)
      // Tost makinesi → çamaşır makinesi önerme (anlamsız)
      // Kriterler: 1) Stoklu önce  2) Sepetten ucuz önce  3) En ucuz
      tamamlayicilar.sort((a, b) => {
        const aStoklu = a.stok > 0 ? 0 : 1;
        const bStoklu = b.stok > 0 ? 0 : 1;
        if (aStoklu !== bStoklu) return aStoklu - bStoklu;
        const aUcuz = sepetMedyan > 0 ? (a.nakit <= sepetMedyan ? 0 : 1) : 0;
        const bUcuz = sepetMedyan > 0 ? (b.nakit <= sepetMedyan ? 0 : 1) : 0;
        if (aUcuz !== bUcuz) return aUcuz - bUcuz;
        return a.nakit - b.nakit;
      });

      const enIyi     = tamamlayicilar[0];
      const stokluSayi = tamamlayicilar.filter(t => t.stok > 0).length;

      // Öncelik: tamamlayıcı sepetten ucuzsa daha akıllı öneri
      const oncelik = (sepetMedyan > 0 && enIyi.nakit > 0 && enIyi.nakit < sepetMedyan) ? 3 : 4;

      firsatlar.push({
        grup, kazanc: info.tutar, tip: 'ekle',
        tamamlayici: enIyi, tamamlayiciSayi: tamamlayicilar.length,
        stokluSayi, oncelik
      });
    }
  });
  ...
  if (f.tip === 'hazir') {
    ...
    mesaj = `<b>${f.grup}</b> için 1 ürün daha seç → <b>+${kFmt}</b>`;

  } else if (f.tip === 'ekle') {
    const t = f.tamamlayici;

    if (f.stokluSayi > 0) {
      const fFmt = t.nakit >= 1000
        ? (t.nakit / 1000).toFixed(t.nakit % 1000 === 0 ? 0 : 1) + 'k₺'
        : (t.nakit > 0 ? t.nakit + '₺' : '');
      const detay = [fFmt, t.stok > 0 ? t.stok + ' stok' : ''].filter(Boolean).join(' · ');

      mesaj = `<b>${f.grup}</b> tamamlanır: <b>${t.ad}</b>`
            + (detay ? ` <span style="opacity:.7;font-weight:400">(${detay})</span>` : '')
            + ` → <b>+${kFmt}</b>`;

      // Ara butonu: global fonksiyon üzerinden çalışır (closure sorunu yok)
      window._upsellAraKelime = t.ad.split(' ').slice(0, 3).join(' ');
      araBtn = `<button onclick="window._upsellAra()"
        style="background:#1e40af;color:#fff;border:none;border-radius:6px;
          padding:4px 9px;font-weight:800;font-size:.62rem;cursor:pointer;
          white-space:nowrap;font-family:inherit;flex-shrink:0">🔍 Ara</button>`;

    } else {
      mesaj = `<b>${f.grup}</b> tamamlanabilir — uygun stok yok (${f.tamamlayiciSayi} ürün)`;
    }
  }

  if (!mesaj) { container.innerHTML = ''; return; }

  // _upsellAra: modal kapat → urunler sekmesi → arama
  window._upsellAra = function() {
    const kelime = window._upsellAraKelime || '';
    if (!kelime) return;
    const cartModal = document.getElementById('cart-modal');
    if (cartModal) { cartModal.classList.remove('open'); cartModal.style.display = 'none'; }
