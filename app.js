...
function _qfRenderTaksit() {
  ...
  TAK_LIST.forEach(n => {
    ...
    const aylik = nInt === 1 ? hesap[n].tahsilat : Math.ceil(hesap[n].tahsilat / nInt);
    const tahsilatDuzeltilmis = nInt === 1 ? hesap[n].tahsilat : aylik * nInt; // ✅ tutarlı toplam
...
function _calcAbakusKredi(resEl, zRows, nakit, kurum) {
  ...
  VADE_LIST.forEach(ay => {
    zRows.forEach(r => {
      ...
      const aylik  = Math.ceil(_toplamHam / ay);
  ...
  VADE_LIST.forEach(ay => {
    zRows.forEach(r => {
      ...
      const aylik  = Math.ceil(toplamHam / ay);
      const toplam = aylik * ay; // ✅ toplam = aylık × ay → her zaman tutarlı
      hasRow = true;
...
function calcAbakus() {
  ...
  zRows.forEach(satir => {
    TAK.forEach(td => {
      ...
      const aylik    = td.n === 1 ? tahsilatHam : Math.ceil(tahsilatHam / td.n);
      const tahsilat = td.n === 1 ? tahsilatHam : aylik * td.n;
