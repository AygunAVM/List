...
async function openAbakus() {
  ...
  // QF planı varsa koru — sıfırlama
  const _qfPlanKorunan = abakusSelection && abakusSelection.kart ? {...abakusSelection} : null;
  abakusSelection = null;
  ...
  // QF planı varsa: kart dropdown'unu set et
  if (_qfPlanKorunan) {
    abakusSelection = _qfPlanKorunan; // calcAbakus'ta auto-select için geri yükle
    const ks = document.getElementById('ab-kart');
    if (ks) {
      const opts = Array.from(ks.options).map(o => o.value);
      if (opts.includes(_qfPlanKorunan.kart)) ks.value = _qfPlanKorunan.kart;
      else {
        const match = opts.find(o => o.toLowerCase().includes(_qfPlanKorunan.kart.toLowerCase()));
...
function openQuickFinance(urunIdx, nakitFiyat) {
  ...
  if (aciklama.includes('\u2756')) {
    kampanyaBanner = '<div class="qf-camp-banner" style="background:linear-gradient(135deg,#f3e8ff,#ede9fe);border-color:#7c3aed;color:#4c1d95">' +
      ...
      '<span style="font-size:.62rem">Sepete eklendiğinde Ödeme Seçenekleri\u2019nden proje fiyatını seçebilirsiniz. ' +
      'Aşağıdan taksit/kredi seçeneği de seçebilirsiniz.</span>' +
      '</div>';
  ...
  if (useJSON) {
    VADE_LIST.forEach(ay => {
      _ks.forEach(r => {
        ...
        rows += '<tr class="qf-taksit-row'+sel+'" onclick="_qfSelectPlan(this,\''+_qfKrediKurum+'\','+ay+','+toplam+',\''+_qfKrediKurum+'\')">'+
    ...
    if (fb) fb.vadeler.forEach(v => {
      ...
      rows += '<tr class="qf-taksit-row'+sel+'" onclick="_qfSelectPlan(this,\''+_qfKrediKurum+'\','+v.ay+','+toplam+',\''+_qfKrediKurum+'\')">'+
...
function qfAddToBasket() {
  ...
  if (_qfPlan) abakusSelection = {..._qfPlan};
  addToBasket(_qfUrunIdx);
  if (_qfPlan && basket.length > 0) {
    const item = basket[0];
    item._qfPlanLabel = _qfPlan.label;
    item._qfAylik     = _qfPlan.aylik;
    item._qfTaksit    = _qfPlan.taksit;
    item._qfKart      = _qfPlan.kart;
  }
  closeQuickFinance();
  if (_qfPlan) {
    // Kart dropdown'unu plana göre set et, sonra abaküsü aç
    setTimeout(() => {
      const ks = document.getElementById('ab-kart');
      if (ks && _qfPlan) {
        const opts = Array.from(ks.options).map(o=>o.value);
        if (opts.includes(_qfPlan.kart)) ks.value = _qfPlan.kart;
      }
      openAbakus();
    }, 80);
  }
...
if (!enKarliMap[td.n] || oran < enKarliMap[td.n].oran) {
  ...
  // QF planı varsa: bir tick sonra eşleşen satırı bul ve seç
  // (innerHTML render'ının tamamlanması için setTimeout(0) gerekli)
  if (abakusSelection && abakusSelection.kart) {
    const _planKart   = abakusSelection.kart;
    const _planTaksit = abakusSelection.taksit;
    setTimeout(() => {
      let matched = null;
      resEl.querySelectorAll('tr.ab-row-sel').forEach(tr => {
        try {
          const d = JSON.parse(tr.dataset.abrow || '{}');
          if (d.kart === _planKart && d.taksit === _planTaksit) matched = tr;
        } catch(e) {}
      });
      if (matched) {
        selectAbakusRow(matched);
        setTimeout(() => matched.scrollIntoView({ block: 'center', behavior: 'smooth' }), 150);
      }
    }, 0);
  }
}
...
function selectAbakusRow(rowEl) {
  ...
  try {
    ...
    if (actDiv) {
      ...
      // display:block sonrası bir frame bekle, sonra scroll et
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
