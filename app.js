...
function startFirebaseListeners() {
  ...
  window._propUnsub = _safeListener(
    'proposals',
    // ✅ Rev 7.0: limit(300) — sınırsız koleksiyonun her WebChannel
    // reconnect'inde (bkz. 404 hataları) baştan okunmasını engeller.
    // Eski/arşivlenmiş teklifler zaten runJanitor() ile temizleniyor,
    // canlı ekranda 300'den eskisine ihtiyaç yok.
    () => query(_colProp(), orderBy('ts', 'desc'), limit(300)),
    snap => {
      ...
      if (adminOpen) {
        ...
        // ✅ Rev 7.0: debounce — art arda gelen teklif/satış hareketlerinde
        // loadFunnelAnaliz'in her seferinde tekrar çalışmasını (ham funnel_logs +
        // daily_funnel_summary sorgusu) engeller. Son değişiklikten 30 sn sonra
        // en fazla 1 kez tetiklenir.
        if (activeTab === 'analiz') {
          clearTimeout(window._funnelAnalizDebounce);
          window._funnelAnalizDebounce = setTimeout(() => loadFunnelAnaliz(90, false), 30000);
        }
      }
      updateProposalBadge();
    },
  ...
  window._saleUnsub = _safeListener(
    'sales',
    // ✅ Rev 7.0: limit(300) — bkz. proposals listener'daki gerekçe.
    () => query(_colSales(), orderBy('ts', 'desc'), limit(300)),
    snap => {
