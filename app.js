...
function parseCampaigns(aciklama) {
  ...
  // ── ⎇ / 🔒︎ formatı: gerçek kampanya pilleri ─────────────────
  ...
  // ── ✦ formatı: tüm segmentler Bilgi pill (indirim yapılmaz) ──
  if (has_tulha) {
  ...
  // ── Ayraç yok: tüm metin tek bilgi segmenti ──────────────────
...
function _buildCampObj(metin, tip) {
  ...
  if (/KEA/i.test(metin) && tip !== 'bilgi') {
    grup = 'KEA';
    esik = esikOverride || 2;
    tip = 'birlesen';
  } else if (/KM/i.test(metin) && tip !== 'bilgi') {
    grup = 'KM';
    esik = esikOverride || 2;
    tip  = 'kilitli';
  } else if (/ANK|[Üü][çc]l[üu]|Ankastre/i.test(metin) && tip !== 'bilgi') {
    grup = 'ANK';
    esik = esikOverride || 3;
    tip  = 'birlesen';
