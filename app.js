...
function _qfRenderTekCekim() {
  ...
  const kartlar = {};
  allRates.filter(r => !r.Tip || r.Tip === 'kart').forEach(r => {
    if (!r.Kart) return;
...
function _qfRenderTaksit() {
  ...
  const kartSatirlari = allRates.filter(r => !r.Tip || r.Tip === 'kart');
...
function _qfRenderKredi() {
  ...
  const KREDI_FALLBACK = [
    { ad: 'TOMBank', vadeler: [{ay:6,faiz:2.89},{ay:12,faiz:3.49},{ay:18,faiz:3.89},{ay:24,faiz:4.29}] },
    { ad: 'Zip',     vadeler: [{ay:3,faiz:2.69},{ay:6,faiz:2.99},{ay:12,faiz:3.59}] },
  ];

  const krediSatirlari = allRates.filter(r => r.Tip === 'kredi');
  ...
  let rows = '';
  const VADE_LIST = [3,6,9,12,18,24,36,48];

  if (useJSON) {
    const satirlar = krediSatirlari.filter(r => r.Kart === _qfKrediKurum);
    VADE_LIST.forEach(ay => {
      satirlar.forEach(r => {
        const faiz = parseFloat(r[ay + 'Taksit']);
        if (isNaN(faiz) || faiz <= 0) return;
        const toplam = Math.round(nakit * Math.pow(1 + faiz/100, ay) / 50) * 50;
