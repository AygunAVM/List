...
async function fbSaveSale(sale) {
  try {
    ...
    // Daily stats: satış sayacını artır
    incrementDailyStat('satis_sayisi').catch(()=>{});
  ...
  document.addEventListener('visibilitychange', async () => {
    try {
      if (document.visibilityState === 'hidden') {
        ...
        // fetchLiveBasket içinde basket doluysa updateCartUI zaten çağrılır
      }
    } catch (err) {
...
async function showApp() {
  ...
  // Janitor: showApp'ta değil, Admin panelinde butona bağlı (Firebase kota koruması)
  // checkExpiredProposals: periyodik setInterval kaldırıldı — render sırasında isExpired() ile lazy
...
async function fetchLiveBasket() {
  ...
  // [DÜZELTME-1] Aktif oturum varsa (sepet doluysa) buluttan yükleme yapma.
  // iPhone'da arka plana geçip dönerken mevcut müşterinin sepeti
  // eski bir oturumun verisiyle ezilmesini önler.
  ...
  try {
    ...
    // ── Admin tarafından temizlendi bayrağı ─────────────────────
    if (data.cleared === true) {
      ...
      updateCartUI();
      // Bayrağı sil (bir daha tekrar tetiklenmesin)
...
function renderCampaignPills(item, idx) {
  ...
  const summaryRow = secilenSayi > 0
    ? `<div style="display:flex;align-items:center;gap:5px;margin-top:3px">`
      + (projeAktifPill
        ? (() => {
            // Satır özeti: "❖ EA+SFS · 159.999₺" — kimlik + fiyat birlikte
...
function renderSiparisBildirimBar() {
  ...
  try {
    ...
    const bugunTeklif = benimler.filter(p => (p.ts || '').slice(0, 10) === bugun).length;
    const bugunSatis  = benimler.filter(p => p.durum === 'satisDondu' && (p.ts || '').slice(0, 10) === bugun).length;
    const haftaTeklif = benimler.filter(p => (p.ts || '').slice(0, 10) >= haftaOnce).length;
    const haftaSatis  = benimler.filter(p => p.durum === 'satisDondu' && (p.ts || '').slice(0, 10) >= haftaOnce).length;
...
async function fetchWeeklyStar() {
  ...
  try {
    ...
    snap.docs.forEach(d => {
      const data = d.data();
      // ts hem Timestamp nesnesi hem string olabilir
      let tsDate;
      if (data.ts?.toDate) tsDate = data.ts.toDate();
      else if (typeof data.ts === 'string') tsDate = new Date(data.ts);
      else return;
...
async function renderMyStats() {
  ...
  // Hesaplamalar — 3 dönem
  const _calc = (tarihFiltre, satisTarih) => {
    const teklif = benimler.filter(p => (p.ts||'').slice(0,10) >= tarihFiltre || (p.ts||'').slice(0,10) === tarihFiltre).length;
    const satis  = benimler.filter(p => p.durum==='satisDondu' && (p.archivedAt||p.ts||'').slice(0,10) >= satisTarih).length;
    const ciro   = benimler.filter(p => p.durum==='satisDondu' && (p.archivedAt||p.ts||'').slice(0,10) >= satisTarih)
                           .reduce((s,p) => s + (p.nakit||0), 0);
    const blur   = blurData.toplam || 0;
    // Sepete atma: teklif sayısı blur'dan büyük olamaz → Math.min ile %100 sınırı
    const btb    = blur > 0 ? Math.min(100, (teklif / blur) * 100) : null;
    const bts    = teklif > 0 ? Math.min(100, (satis / teklif) * 100) : null;
    return { teklif, satis, ciro, blur, btb, bts };
  };

  const d = {
    bugun: (() => {
      const t = benimler.filter(p => (p.ts||'').slice(0,10) === bugun).length;
      const s = benimler.filter(p => p.durum==='satisDondu' && (p.archivedAt||p.ts||'').slice(0,10) === bugun).length;
      const c = benimler.filter(p => p.durum==='satisDondu' && (p.archivedAt||p.ts||'').slice(0,10) === bugun)
                        .reduce((sum,p) => sum + (p.nakit||0), 0);
      const blur = blurData.toplam || 0;
      return { teklif:t, satis:s, ciro:c, blur, btb: blur>0?Math.min(100,(t/blur)*100):null, bts: t>0?Math.min(100,(s/t)*100):null };
    })(),
    hafta: _calc(haftaOnce, haftaOnce),
    ay:    _calc(ayOnce, ayOnce),
  };
...
function _renderMyStatsUI() {
  ...
  const v = d[p];   // aktif dönem verileri

  const BTS_HEDEF = 30; // %

  // Motivasyon: aktif döneme göre
  const { emoji: motEmoji, mesaj: motMesaj, renk: motRenk } =
    _getMotivation(d.bugun.satis, d.bugun.teklif, d.hafta.satis, v.bts);

  const periodLabel = { bugun:'Bugün', hafta:'Bu Hafta', ay:'Bu Ay' }[p];

  // BTS rengi ve hedef çubuğu
  ...
  const btsHint     = v.bts === null ? 'Henüz teklif yok' :
                      btsVal >= BTS_HEDEF ? '🔥 Hedefi aştın! Kapanış ustası.' :
                      btsVal >= 15 ? `👍 İyi gidiyorsun — hedef %${BTS_HEDEF}` :
                      `⚠️ Her 5 tekliften en az 1'i satışa dönüşmeli`;

  // Sepete Atma Oranı rengi
  const btbVal  = v.btb !== null ? v.btb : 0;
  const btbWarn = v.btb !== null && btbVal < 10;
  const btbHint = v.btb === null ? 'Henüz fiyat sorgulaması yok' :
                  btbVal < 10 ? '⚠️ %10 altı — fiyat bakılan müşteriler sepet kurmadan ayrılıyor' :
                  btbVal < 30 ? `👍 Makul — %30'a ulaşmayı hedefle` :
                  '🔥 Mükemmel dönüşüm!';
  ...
  container.innerHTML = `
    ...
    <div class="ms-hero">
      ...
      ${benimYildiz ? `<div class="ms-star-badge">⭐ Haftanın Yıldızı<br><span>${star.count} satış</span></div>` : ''}
    </div>
    ...
    <!-- ① KAPANIŞ ORANI — Hero Kart (en üste, öne çıkarılmış) -->
    <div class="ms-bts-hero" style="border-left:4px solid ${btsRenk}">
      <div class="ms-bts-hero-top">
        <div>
          <div class="ms-bts-label">Satışa Dönüşme Oranı</div>
          <div class="ms-bts-sub">Satış ÷ Teklif × 100</div>
        </div>
        <div class="ms-bts-val" style="color:${btsRenk}">${v.bts !== null ? btsVal.toFixed(1) + '%' : '—'}</div>
      </div>
      <!-- Hedef çubuğu -->
      <div class="ms-bts-track-wrap">
        <div class="ms-bts-track">
          <div class="ms-bts-fill" style="width:${btsHedefPct}%;background:${btsRenk}"></div>
          <div class="ms-bts-target-line" style="left:100%"></div>
        </div>
        <div class="ms-bts-track-labels">
          <span style="color:${btsRenk};font-weight:700">${btsVal.toFixed(0)}%</span>
          <span>Hedef %${BTS_HEDEF}</span>
        </div>
      </div>
      <div class="ms-metric-hint" style="margin-top:6px">${btsHint}</div>
    </div>

    <!-- ② Segmented Control -->
    <div class="ms-seg-control">
      <button id="ms-seg-bugun" class="ms-seg-btn ${p==='bugun'?'ms-seg-active':''}" onclick="switchMyStatsPeriod('bugun')">Bugün</button>
      <button id="ms-seg-hafta"  class="ms-seg-btn ${p==='hafta' ?'ms-seg-active':''}" onclick="switchMyStatsPeriod('hafta')">Bu Hafta</button>
      <button id="ms-seg-ay"    class="ms-seg-btn ${p==='ay'   ?'ms-seg-active':''}" onclick="switchMyStatsPeriod('ay')">Bu Ay</button>
    </div>
    ...
    <div class="ms-metric-card ${btbWarn ? 'ms-metric-warn' : 'ms-metric-ok'}" style="margin-top:10px">
      ...
      ${v.blur > 0 ? `<div style="font-size:.60rem;color:var(--text-3);margin-top:5px">${periodLabel}: <strong>${v.blur}</strong> fiyat sorgulaması · <strong>${v.teklif}</strong> teklif</div>` : ''}
    </div>

    <!-- ⭐ Haftanın Yıldızı -->
    ${star ? (() => {
      const meRank = star.ranked?.find(r => r.email === me);
      const topN   = (star.ranked || []).slice(0, 3); // podiyum
      const total  = star.total || topN.length;

      // Mevcut kullanıcının sıralama bandı
      const rankBand = meRank
        ? (() => {
            ...
            return `<div class="ms-rank-band" style="border-left-color:${color}">
              ...
              <div class="ms-rank-info">
                ...
                <span class="ms-rank-detail">${meRank.rank === 1 ? 'Haftanın yıldızısın! 🔥' : `${meRank.rank}. sıradasın — lider ${star.ranked[0].email.split('@')[0]} (${star.ranked[0].count} satış)`}</span>
              </div>
        ...
        : `<div class="ms-rank-band ms-rank-zero">
            ...
            <div class="ms-rank-info">
              <span class="ms-rank-label">${total} kişi arasında</span>
              <span class="ms-rank-detail">Bu hafta henüz satış yok — ilk satışını yap!</span>
            </div>
          </div>`;

      // Podiyum tablosu (top 3)
...
// ── Blur istatistiklerini fiyat_bakislari'ndan çek ──────────
async function _getMyBlurStats(email) {
  if (!_db || !email) return { toplam: 0 };
  try {
    const snap = await getDoc(doc(_db, 'fiyat_bakislari', email));
    if (!snap.exists()) return { toplam: 0 };
    const data = snap.data();
    return { toplam: (data.revealedPrices || []).length };
  } catch (e) { return { toplam: 0 }; }
}
...
function _getMotivation(bugSatis, bugTeklif, hftSatis, bts) {
  ...
  return { emoji:'🚀', mesaj:'İlk teklifini oluştur ve yarışa gir!',                             renk:'#7c3aed' };
}

// ── ₺ kısalt (1.250.000 → 1,25M) ────────────────────────────
function _fmtK(v) {
