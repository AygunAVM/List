...
async function refreshAdminPanel() {
  ...
  const results = await Promise.allSettled([fetchProposalsOnce(), fetchSalesOnce(), fetchAnalyticsOnce()]);
  if (activeTab === 'siparis') await fetchSiparisOnce();
  switchAdminTab(activeTab);
  const ok = results.some(r => r.status === 'fulfilled' && r.value !== false);
  if (btn) {
    btn.classList.remove('loading');
    if (ok) {
      btn.classList.add('success');
...
function switchAdminTab(tab) {
  ...
  // ✅ Rev 8.0: sekme değişince ilgili veriyi tek seferlik tazele (cache'ten
  ...
  if(tab==='siparis')  {
    ...
    fetchSiparisOnce().then(() => { renderSiparisPanel(); updateSiparisBadge(); });
  }
  if(tab==='sepetler') {
    renderSepetDetay();
    fetchProposalsOnce().then(renderSepetDetay);
  }
  if(tab==='personel') {
    renderAdminUsers();
    Promise.allSettled([fetchProposalsOnce(), fetchSalesOnce(), fetchAnalyticsOnce()]).then(renderAdminUsers);
  }
  if(tab==='analiz')   {
  ...
  if(tab==='overview' || !tab) {
    Promise.allSettled([fetchProposalsOnce(), fetchSalesOnce(), fetchAnalyticsOnce()]).then(() => renderAdminPanel());
  }
}
