...
function renderTrendCards() {
  ...
  const vitrinHtml = vitrinAktif.map(v => {
    ...
    return `<div class="trend-card vitrin-card"
      ...
      <div class="trend-rank" style="color:rgba(167,139,250,.7);font-size:.58rem;font-weight:900">🛍️</div>
      <div class="trend-card-name">${v.urunAdi}</div>
      <div class="trend-card-meta">
        <span class="trend-card-price">${fmt(v.fiyat)}</span>
        ${primT ? `<span class="trend-card-score" style="background:rgba(245,158,11,.15);color:#f59e0b;border:1px solid rgba(245,158,11,.2)">${primT}</span>` : ''}
      </div>
      ${v.aciklama ? `<div style="font-size:.59rem;color:rgba(255,255,255,.35);line-height:1.3;margin:2px 0 4px;overflow:hidden;display:-webkit-box;-webkit-line-clamp:1;-webkit-box-orient:vertical">${v.aciklama}</div>` : ''}
      <button class="trend-card-add haptic-btn"
        onclick="event.stopPropagation();${oi >= 0 ? 'vitrinSepeteEkle(' + oi + ')' : ''}"
