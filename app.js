...
function vitrinUrunAra(query) {
  ...
  dropdown.style.display = 'block';
  // Her öğeye data-idx ile veriyi göm, click/touchend dinle
  dropdown.innerHTML = hits.map(({ u, idx }) => {
    const ad    = u[urunKey] || '';
    const nakit = nakitKey ? (parseFloat(u[nakitKey]) || 0) : 0;
    const not   = notKey ? (u[notKey] || '') : '';
    // JSON.stringify ile güvenli — tırnak/özel karakter sorununu önler
    return `<div class="vitrin-dd-item"
      data-idx="${idx}"
      data-ad="${ad.replace(/"/g,'&quot;')}"
      data-nakit="${nakit}"
      data-not="${(not||'').replace(/"/g,'&quot;')}"
      onclick="window.vitrinUrunSec(parseInt(this.dataset.idx), this.dataset.ad.replace(/&quot;/g,'\"'), parseFloat(this.dataset.nakit)||0, this.dataset.not.replace(/&quot;/g,'\"'))">
