...
// Vitrin ürün arama — fiyat listesindeki ürünlerden
let _vitrinSeciliIdx = -1; // seçilen ürünün allProducts indeksi

function vitrinUrunAra(query) {
  const dropdown = document.getElementById('vitrin-urun-dropdown');
  if (!dropdown) return;
  const q = (query || '').trim().toLowerCase();
  if (!q || q.length < 2) { dropdown.style.display = 'none'; _vitrinSeciliIdx = -1; return; }
  ...
  const notKey   = keys0.find(k => ['not','sipariş notu','aciklama','açıklama'].includes(k.toLowerCase())) || '';

  const sonuclar = allProducts
    .map((u, idx) => ({ u, idx }))
  ...
  dropdown.innerHTML = sonuclar.map(({ u, idx }) => {
    const ad    = u[urunKey] || '';
    const nakit = nakitKey ? (parseFloat(u[nakitKey]) || 0) : 0;
    const not   = notKey ? (u[notKey] || '') : '';
    return `<div class="vitrin-dd-item"
      onmousedown="event.preventDefault();vitrinUrunSec(${idx},${JSON.stringify(ad)},${nakit},${JSON.stringify(not)})">
      <span class="vitrin-dd-ad">${ad}</span>
      ${nakit > 0 ? `<span class="vitrin-dd-fiyat">${fmt(nakit)}₺</span>` : ''}
    </div>`;
  }).join('');
}

function vitrinUrunSec(idx, urunAdi, nakit, not) {
  _vitrinSeciliIdx = idx;
  const inputEl  = document.getElementById('vitrin-urun-input');
  const fiyatEl  = document.getElementById('vitrin-fiyat-input');
  const notEl    = document.getElementById('vitrin-aciklama-input');
  const dropdown = document.getElementById('vitrin-urun-dropdown');
...
async function saveVitrinUrun() {
  ...
  const productIdx = _vitrinSeciliIdx; // allProducts indeksi — sepete at için

  if (!urunAdi) { _campToast('Önce bir ürün seçin.', 'warn'); return; }
  ...
  try {
    ...
    if (document.getElementById('vitrin-aciklama-input'))
      document.getElementById('vitrin-aciklama-input').value = '';
...
function renderTrendCards() {
  ...
  // Vitrin kartları — productIdx ile doğru sepet bağlantısı
  const vitrinHtml = vitrinAktif.map(v => {
    // productIdx varsa kullan, yoksa isim ile eşleştirmeye çalış
    let oi = (typeof v.productIdx === 'number' && v.productIdx >= 0) ? v.productIdx : -1;
    if (oi < 0) {
      const keys0 = allProducts.length ? Object.keys(allProducts[0]) : [];
      const uk = keys0.find(k => k.toLowerCase() === 'urun') || '';
      oi = allProducts.findIndex(u => (u[uk] || '').toLowerCase() === v.urunAdi.toLowerCase());
    }
    return `<div class="trend-card vitrin-card"
      ...
      ${v.aciklama ? `<div style="font-size:.60rem;color:var(--text-3);margin:2px 0;line-height:1.3;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical">${v.aciklama}</div>` : ''}
  ...
  const trendHtml = trendData.map((item, i) => {
    ...
    return `<div class="trend-card"
      ...
      <div class="trend-card-meta">
        ${fiyat !== '—' ? `<span class="trend-card-price">${fiyat}₺</span>` : ''}
...
function renderTrendFullList() {
  ...
  if (vitrinAktif.length) {
    ...
    html += vitrinAktif.map(v => {
      // productIdx varsa kullan, yoksa isim eşleştir
      let oi = (typeof v.productIdx === 'number' && v.productIdx >= 0) ? v.productIdx : -1;
      if (oi < 0) {
        const keys0 = allProducts.length ? Object.keys(allProducts[0]) : [];
        const uk = keys0.find(k => k.toLowerCase() === 'urun') || '';
      ...
      return `<div class="trend-full-card" style="border-color:#fca5a5;border-left-width:3px">
        <div class="trend-full-rank" style="color:#e11d48;font-size:.80rem">🛍️</div>
        <div class="trend-full-name">${v.urunAdi}</div>
        <div class="trend-full-price">${fmt(v.fiyat)}₺</div>
        ${v.kampanya ? `<div class="trend-full-meta" style="color:#92400e;font-weight:700;background:#fef3c7;border-radius:4px;padding:1px 6px;display:inline-block">${v.kampanya}</div>` : ''}
        ${v.aciklama ? `<div style="font-size:.66rem;color:var(--text-2);margin-top:2px;line-height:1.4;grid-column:1/-1">${v.aciklama}</div>` : ''}
  ...
  // ── Popüler / Trend bölümü ──
  if (trendData.length) {
    html += `<div style="grid-column:1/-1;display:flex;align-items:center;gap:8px;margin-top:${vitrinAktif.length ? '12px' : '0'};margin-bottom:4px">
      <span style="font-size:.72rem;font-weight:800;color:var(--text-1)">🔥 Popüler Ürünler</span>
      <span style="font-size:.60rem;color:var(--text-3)">son 14 gün · blur sayısına göre</span>
    </div>`;
    html += trendData.map((item, i) => {
      ...
      return `<div class="trend-full-card">
        ...
        ${fiyat !== '—' ? `<div class="trend-full-price">${fiyat}₺</div>` : ''}
        ${item.addCnt > 0 ? `<div class="trend-full-meta">${item.addCnt}× görüntülendi</div>` : ''}
        <button class="trend-full-add haptic-btn"
          onclick="${oi >= 0 ? 'addToBasket(' + oi + ')' : ''}">+ Sepete Ekle</button>
      </div>`;
    }).join('');
