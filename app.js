...
//   Tv                       → sepette "Tv" gamı / kodu / adı geçen ürün varsa
//   Tv+CepTelefonu           → "Tv" VEYA "Cep Telefonu" (OR, + ile)
//   S26&CepTelefonu          → "S26" VE "Cep Telefonu" aynı üründe (AND, & ile)
//   *~Adaptör~Epilasyon      → herkese ama bu gamlar sepette varsa gösterme (~ ile)
//   Tv+Cep~Adaptör           → (Tv OR Cep) AND NOT Adaptör
//   SM-A175F                 → tire/slash içeren kod tam eşleşme
//
// Eşleşme kuralları:
//   Kod   → TAM eşleşme (===)  — kısmi eşleşme kabul edilmez
//   Gam   → kısmi (includes)   — "Buzdolabı" → "Büro Tipi Buzdolabı"
//   Ürün  → kısmi (includes)   — "S26" → "S26 U (12/256)"
// ─────────────────────────────────────────────────────────────────────────────
function _bundleMatches(condition, basket) {
  ...
  // Boşluksuz normalize — "Cep Telefonu" ile "CepTelefonu" eşdeğer
  const ns = s => (s || '').toLowerCase().replace(/\s+/g, '');

  // Tek bir terimin sepetteki herhangi bir ürünle eşleşip eşleşmediği
  const termHits = (term, bskt) => {
    const t = ns(term);
    return bskt.some(item =>
      String(item.kod ?? '').toLowerCase() === t ||  // Kod: TAM
  ...
  // ── AND koşulları: '&' ile ayrılmış — HEPSİ aynı üründe olmalı ──────────
  // Örn: "S26&CepTelefonu" → sepette hem S26 adı geçen hem Cep Telefonu gamı olan
  // bir ürün olmalı (aynı satırda, tek ürün üzerinde her iki koşul)
  if (includePart.includes('&')) {
    const andTerms = includePart.split('&').map(s => s.trim()).filter(Boolean);
    // Sepetteki her ürünü tek tek kontrol et — tüm AND terimleri o üründe geçiyor mu?
  ...
  // ── OR koşulları: '+' ile ayrılmış — herhangi biri yeterli ─────────────
  const orTerms = includePart.split('+').map(s => s.trim()).filter(Boolean);
  return orTerms.some(t => termHits(t, basket));
}
...
function _getBundleSuggestions() {
  ...
  results.sort((a, b) => {
    const score = r => {
      if (r.condition === '*') return 0;
      if (basket.some(i => String(i.kod??'').toLowerCase() === r.condition.toLowerCase())) return 2;
      return 1;
    };
