...
document.addEventListener('DOMContentLoaded', () => {
  // ── Kampanya ara butonları — event delegation ─────────────
  document.addEventListener('click', function(e) {
    ...
    if (typeof switchMainTab === 'function') {
      ...
      const tu = document.getElementById('tab-urunler');
      const tt = document.getElementById('tab-trend');
...
// Kampanya ara filtresi rol renkleri — pastel, ayırt edilebilir
...
function renderTable(searchVal) {
  ...
  if (_campAraFilter) {
    ...
    data = data.reduce((acc, u) => {
      ...
      while ((m = campRe.exec(ac)) !== null) {
        if (m[1].toUpperCase() === cfGrup && parseInt(m[2]) === cfEsik) {
          ...
          if (rol === excludeRol) return acc; // sepetteki ürünün rolü → hariç tut
          acc.push({ ...u, _campAraRol: rol }); // rol bilgisini taşı
          return acc;
        }
      }
      return acc;
    ...
    _campAraFilter = null; // tek seferlik
  }
...
window.deleteSelectedItems = function() {
  ...
  // Seçili yoksa → tümünü sil
  const tumunuSil = checkboxes.length === 0;
  const indices = tumunuSil
    ? Array.from(allCheckboxes).map(cb => parseInt(cb.value)).sort((a, b) => b - a)
    : Array.from(checkboxes).map(cb => parseInt(cb.value)).sort((a, b) => b - a);
...
// ⎇ (birlesen), 🔒 (kilitli), ❖ (proje) — pending durumunda da göster
function _campSearchBtn(item, camp, ci) {
  const desteklenen = ['birlesen', 'kilitli', 'proje'];
  if (!desteklenen.includes(camp.tip)) return '';
  if (!camp.tutar || camp.tutar <= 0 || !camp.esik || camp.esik < 2) return '';
  if (camp.sonTarih && new Date() > camp.sonTarih) return '';

  // Seçili VE aktif (pending değil) ise butona gerek yok
  ...
  // Seçili + tamamlanmış (pending değil) → zaten çalışıyor, buton gizle
  if (secili && !pending) return '';
  ...
  const sepetKodlar = new Set(
    (typeof basket !== 'undefined' ? basket : []).map(b => String(b['Kod'] || ''))
  );
  ...
  for (const p of urunler) {
    ...
    let m;
    while ((m = campRe.exec(ac)) !== null) {
      if (m[1].toUpperCase() === grup &&
          parseInt(m[2]) === esik &&
          m[3].toUpperCase() !== mevcutRol) {
        bulunan = true; break;
      }
    }
    if (bulunan) break;
  ...
  // Pending durumunda daha belirgin — turuncu ton
  const btnBg    = isPending ? '#451a03' : isProje ? '#2e1065' : '#172554';
  const btnColor = isPending ? '#fcd34d' : isProje ? '#c4b5fd' : '#93c5fd';
  const btnBdr   = isPending ? '#d97706' : isProje ? '#6d28d9' : '#1e40af';

  return `<button type='button'
    class='camp-ara-btn'
    data-grup='${grup}'
    data-esik='${esik}'
    data-exclude-rol='${mevcutRol}'
    title='${grup} eşik tamamlayıcı ürünleri ara'
    style='display:inline-flex;align-items:center;justify-content:center;
      width:15px;height:15px;border-radius:3px;
      background:${btnBg};color:${btnColor};
      border:1px solid ${btnBdr};cursor:pointer;
...
function _diffJson(oldJson, newJson) {
  ...
  newJson.data.forEach(p => {
    ...
    const urunAdi   = p[urunKey]||p.Kod||'?';
      const rolRozet  = p._campAraRol ? _rolRozetHTML(p._campAraRol) : '';
    // Nakit fiyat
