...
function toggleCartDiscPanel() {
  ...
  // Satır indirimi kapatılınca tüm satır indirimlerini sıfırla
  if(isOpen) {
    basket.forEach(i => { i.itemDisc = 0; });
    saveBasket();
  }
  // Re-render ile panel durumu değişsin
  updateCartUI();
  // Panel durumunu tersine çevir — updateCartUI render eder
  requestAnimationFrame(() => {
    const p2 = document.getElementById('cart-disc-panel');
    if(p2) p2.dataset.open = isOpen ? '0' : '1';
    updateCartUI();
  });
}
...
function updateCartUI() {
  ...
  let rows='';
  if(isAdmin()) {
    // ── Admin sepeti: Nakit + satır indirimi (toggle ile) ────
    // Satır indirimi paneli açık mı?
    const discOpen = document.getElementById('cart-disc-panel')?.dataset.open === '1';
    basket.forEach((item,idx) => {
      ...
      rows+=`<tr>`+
        ...
        `<td class="${item.stok===0?'cart-stok-0':''}" style="font-size:.72rem">${item.stok}</td>`+
        ...
        (discOpen ? `<td><input type="number" class="item-disc-input" min="0" value="${itemDisc||''}" placeholder="0"
          onchange="setItemDisc(${idx},this.value)" style="width:60px;padding:3px 5px;border:1px solid var(--border);border-radius:5px;font-size:.70rem;text-align:right;background:var(--surface)"></td>`+
          `<td class="cart-price${itemDisc>0?' cart-price-net':''}">${fmt(nakitNet)}</td>` : '')+
        `<td><button class="remove-btn haptic-btn" onclick="removeFromBasket(${idx})">×</button></td></tr>`;
    });
    const totalItemDisc = basket.reduce((s,i)=>s+(i.itemDisc||0),0);
    let dr='';
    if(discountAmount>0) {
      dr=`<tr class="discount-row"><td colspan="${discOpen?5:3}" style="text-align:right;font-size:.69rem">İndirim ${discountType==='PERCENT'?'%'+discountAmount:fmt(discountAmount)}</td>`+
        `<td class="cart-price" style="color:#16a34a">-${fmt(getDisc(t.nakit-totalItemDisc))}</td><td></td></tr>`;
    }
    const nakitFinal = basket.reduce((s,i)=>s+Math.max(0,i.nakit-(i.itemDisc||0)),0) - getDisc(t.nakit - totalItemDisc);
    const tot=`<tr class="total-row"><td colspan="${discOpen?5:3}" style="text-align:right;font-weight:800;font-size:.78rem">NET TOPLAM</td>`+
      `<td class="cart-price" style="font-weight:800">${fmt(Math.max(0,nakitFinal))}</td><td></td></tr>`;
    const discBtn=`<div id="cart-disc-panel" data-open="${discOpen?1:0}" style="padding:4px 12px 2px;display:flex;align-items:center;gap:8px">
      <button class="btn-reset haptic-btn" onclick="toggleCartDiscPanel()" style="font-size:.68rem;padding:4px 10px;background:${discOpen?'#dbeafe':'var(--surface-2)'};border:1px solid ${discOpen?'#93c5fd':'var(--border)'};border-radius:6px;color:${discOpen?'#1d4ed8':'var(--text-2)'}">
        ${discOpen?'✓ Satır İndirimi Aktif':'＋ Satır İndirimi'}
      </button>
      ${discOpen&&totalItemDisc>0?`<span style="font-size:.65rem;color:#16a34a">Toplam satır ind: ${fmt(totalItemDisc)}</span>`:''}
    </div>`;
    const heads = discOpen
      ? `<th>Ürün</th><th>Stok</th><th>Açıklama</th><th>Nakit</th><th>İndirim</th><th>Net</th><th></th>`
      : `<th>Ürün</th><th>Stok</th><th>Açıklama</th><th>Nakit</th><th></th>`;
    area.innerHTML=discBtn+`<table class="cart-table"><thead><tr>${heads}</tr></thead><tbody>${rows}${dr}${tot}</tbody></table>`;
  } else {
...
function _renderSingleProp(p) {
  ...
  const canAct = isAdmin() || p.user===me;
    ...
    // Not: teklifi görebilen herkes not ekleyebilir (canAct)
    if(canAct) btns.push(`<button class="pact-btn pact-note haptic-btn" onclick="openPropNote('${p.id}')" title="Not Ekle"><svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M2 12V14h2l7.5-7.5-2-2L2 12zm12.7-7.3a1 1 0 000-1.4l-1-1a1 1 0 00-1.4 0L11 3.6l2.4 2.4 1.3-1.3z" fill="currentColor"/></svg>${noteDot}</button>`);
    if(canEdit) {
...
function showChangePopup(changes, logKey) {
  ...
  if(lowItems.length) {
    // Versiyon aralığı özeti — ilk ve son versiyon
    const firstVer = verItems.length ? verItems[0] : null;
    const lastVer  = verItems.length ? verItems[verItems.length-1] : null;
    let verRangeStr = '';
    if(firstVer && lastVer && firstVer !== lastVer) {
      verRangeStr = firstVer.from + ' → ' + lastVer.to;
    } else if(firstVer) {
      verRangeStr = firstVer.from + ' → ' + firstVer.to;
    }
      ...
      <div class="section-header section-header-low">
        <div class="sh-left">
          ...
          <div>
            ...
            <div class="sh-sub">${verRangeStr ? verRangeStr + ' · ' : ''}Stok · Ürün adı</div>
          </div>
        </div>
      </div>
      <div class="section-items">`;

    // Versiyon satırları — artık gösterilmiyor (başlıkta özetlendi)

    // Bilgi amaçlı: sadece stok göster, versiyon/ürün adı başlıkta özetlendi
    optItems.filter(c => c.type === 'stok').forEach(c => { html += _renderChangeItem(c, itemIdx++, true); });
...
function markAllChanges() {
  ...
  // Bilgi bölümü "Okudum" butonunu bul ve onayla
