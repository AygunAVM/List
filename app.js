...
import { getFirestore, collection, collectionGroup, doc, deleteDoc,
         ...
         query, orderBy, serverTimestamp, where, limit, // <-- 'where' burada olmalı
...
const Basket = {
  ...
  // _shiftCampaignState: kullanılmıyor, gelecek sürümde kaldırılacak
...
function _renderFaturaTablosu(parsed) {
  ...
  // tip: nakit rowda type=nakit yazılı; kartlı rowlarda type yoksa taksit sayısından belirle
  ...
  // Her ürün için fatura bedeli = nakit ağırlığına oransal dağılım
  // Nakit toplam (her ürün kendi payını belirliyor)
  // ❖ Proje kampanyası varsa _projeNakit öncelikli; itemDisc proje fiyatında uygulanmaz
  ...
  basket.forEach((item, idx) => {
    ...
    // Son ürüne kalan farkı ver (yuvarlama hatası birikmesini önler)
    ...
    const komisyon = faturaFiyat - itemNakit;

    // Komisyon göstergesi
    let komisyonStr = '';
    if (tip !== 'nakit' && komisyon > 0) {
    ...
    satirHTML += `
      <div style="display:flex;align-items:center;justify-content:space-between;
        ...
        <div style="text-align:right;flex-shrink:0;display:flex;flex-direction:column;align-items:flex-end;gap:3px">
          <b style="font-size:.90rem;font-weight:900;color:${tip==='nakit'?'#0f172a':'#dc2626'}">${fmt(faturaFiyat)}</b>
          ${komisyonStr}
        </div>
      </div>`;
  });
  ...
  // Alt özet
  const komisyonToplam = tip === 'nakit' ? 0 : Math.max(0, toplamFatura - nakitFinal);
  let odemeBadge = '';
  if (tip === 'nakit') {
  ...
  topEl.innerHTML = `
    <div style="width:100%;display:flex;flex-direction:column;gap:6px">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px">
        ${odemeBadge}
        <span style="font-size:.96rem;font-weight:900;color:${tip==='nakit'?'#0f172a':'#dc2626'}">${fmt(toplamFatura)}</span>
      </div>
      ${tip !== 'nakit' ? `
...
function buildPremiumPDF(docType, data) {
  ...
  const isTekCekim = data.odemeTipi === 'tek_cekim';
  const isTaksit   = !isNakit && !isTekCekim;
  ...
  const logoHTML  = `<img src="${logoUrl}" alt="Aygün AVM"
    ...
    onerror="this.outerHTML='<div style=\\'font-size:1.6rem;font-weight:900;color:#D01F2E;\\'>aygün<span style=\\'font-weight:400;color:#475569;\\'> AVM</span></div>'">`;

  // ── TEMEL HESAPLAR ───────────────────────────────────────────────
  const urunler       = data.urunler || [];
  const _ekInd        = Number(data.ekIndirim       || 0);
  const _satırInd     = Number(data.toplamIndirim   || 0); // satır+alt indirim
  const _toplamInd    = _satırInd + _ekInd;
  const toplamOdeme   = Number(data.toplamOdeme     || 0); // komisyon/nakit fark dahil nihai tutar
  const aylikTaksit   = Number(data.aylikTaksit     || 0);
  const taksitSayisi  = Number(data.taksitSayisi    || 1);
  const kartAdi       = data.kartAdi || 'Kart';

  // Nakit toplam (indirimler düşülmüş, komisyon YOK)
  // ❖ Proje kampanyası varsa _projeNakit öncelikli; itemDisc proje fiyatında uygulanmaz
  ...
  const nakitToplam  = urunNakitler.reduce((s,v) => s+v, 0) || 1;

  // PDF'te her ürünün göstereceği fiyat:
  // - Nakit: nakit bedel (indirim düşülmüş)
  // - Kartlı: toplamOdeme'yi nakit ağırlıklarına oransal dağıt (komisyon yedirilmiş)
  const urunFaturaFiyatlari = urunler.map((u, idx) => {
    if (isNakit) return urunNakitler[idx];
  ...
  let urunlerHTML = '';
  urunler.forEach((u, i) => {
    let marka = '', gam = '', urunAdi = u.urun || '—';
    if (window.allProducts?.length && u.kod) {
      const op = window.allProducts.find(p => p.Kod === u.kod);
      if (op) { marka = op.Marka||''; gam = op.Gam||op.gam||''; urunAdi = op.Urun||op.urun||u.urun; }
    }
    const markaStr = marka && marka !== '-' ? `<span style="color:#64748b;font-weight:600">${marka} </span>` : '';
    const gamStr   = gam   && gam   !== '-' ? `<span style="font-size:.8em;color:#94a3b8"> (${gam})</span>` : '';
    const faturaFiyat = urunFaturaFiyatlari[i];
    const itemDisc    = u._projeNakit !== undefined ? 0 : Number(u.itemDisc || 0); // ❖ Proje fiyatında satır indirimi ayrıca uygulanmaz
    ...
    if (isNakit && itemDisc > 0) {
      ...
      fiyatHTML = `
        <div style="text-decoration:line-through;color:#94a3b8;font-size:.82em">${fmt(listefiyat)}</div>
        <div style="font-weight:800;color:#15803d;font-size:1.05em">${fmt(faturaFiyat)}</div>
        <div style="font-size:.72em;color:#16a34a">-${fmt(itemDisc)} indirim</div>`;
    } else {
      fiyatHTML = `<div style="font-weight:800;color:#0f172a;font-size:1.05em">${fmt(faturaFiyat)}</div>`;
    }
    // Taksitli: aylık göster
    if (isTaksit && taksitSayisi > 1) {
      const aylik = yuvarlaKademe(faturaFiyat / taksitSayisi, 1);
      fiyatHTML += `<div style="font-size:.75em;color:#7c3aed;margin-top:2px">${taksitSayisi}×${fmt(aylik)}</div>`;
    }

    const zebra = i % 2 === 0 ? '#ffffff' : '#f8fafd';
    urunlerHTML += `
      <tr style="background:${zebra}">
        <td style="padding:13px 10px;text-align:center;color:#94a3b8;font-weight:700;font-size:.88em;width:36px">${i+1}</td>
        <td style="padding:13px 12px">
          <div style="font-weight:600;color:#0f172a;font-size:.95em;line-height:1.3">${markaStr}${urunAdi}${gamStr}</div>
          ${u.kod ? `<div style="font-size:.72em;color:#94a3b8;margin-top:2px;font-family:'DM Mono',monospace">${u.kod}</div>` : ''}
        </td>
        <td style="padding:13px 10px;text-align:center;color:#475569;font-size:.9em;width:42px">${u.adet||1}</td>
        <td style="padding:13px 14px;text-align:right;font-family:'DM Mono',monospace;white-space:nowrap">${fiyatHTML}</td>
      </tr>`;
  });

  // ── ÖDEME BLOĞU ─────────────────────────────────────────────────
  const komisyon = isNakit ? 0 : Math.max(0, toplamOdeme - (nakitToplam > 0 ? nakitToplam : toplamOdeme));

  let odemeHTML = '';
  if (isNakit) {
    odemeHTML = `
      <tr><td class="rl">Liste Toplamı</td><td class="rr">${fmt(nakitToplam + _toplamInd)}</td></tr>
      ${_toplamInd > 0 ? `<tr style="color:#15803d"><td class="rl">Toplam İndirim</td><td class="rr" style="font-weight:700">-${fmt(_toplamInd)}</td></tr>` : ''}
      ${data.pazarlikNotu && _ekInd > 0 ? `<tr><td class="rl" style="font-size:.78em;color:#92400e">↳ ${data.pazarlikNotu}</td><td class="rr" style="font-size:.78em;color:#92400e">-${fmt(_ekInd)}</td></tr>` : ''}
    ...
    odemeHTML = `
      <tr><td class="rl">Nakit Bedel</td><td class="rr">${fmt(nakitToplam)}</td></tr>
      ${komisyon > 0 ? `<tr><td class="rl" style="color:#dc2626">Kart Komisyonu</td><td class="rr" style="color:#dc2626;font-weight:700">+${fmt(komisyon)}</td></tr>` : ''}
    ...
    odemeHTML = `
      <tr><td class="rl">Nakit Bedel</td><td class="rr">${fmt(nakitToplam)}</td></tr>
      ${komisyon > 0 ? `<tr><td class="rl" style="color:#dc2626">Vade Farkı / Komisyon</td><td class="rr" style="color:#dc2626;font-weight:700">+${fmt(komisyon)}</td></tr>` : ''}
  ...
  const notHtml   = data.not ? `
    <div style="background:#fffbeb;border-left:4px solid #f59e0b;border-radius:0 10px 10px 0;padding:12px 16px;margin-top:16px">
      <div style="font-size:.72em;font-weight:700;color:#92400e;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">Not</div>
      <div style="font-size:.88em;color:#78350f">${_esc(data.not)}</div>
    </div>` : '';
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  body{font-family:'Inter',-apple-system,BlinkMacSystemFont,sans-serif;background:#e2e8f0;color:#0f172a;padding:28px;line-height:1.55;font-size:14px}
  .page{max-width:860px;margin:0 auto;background:#fff;border-radius:20px;overflow:hidden;box-shadow:0 24px 64px rgba(0,0,0,.12)}

  /* HEADER */
  .hdr{background:linear-gradient(135deg,#0f172a 0%,#1e293b 60%,#0f2744 100%);padding:32px 40px 28px;display:flex;align-items:center;justify-content:space-between;gap:20px;flex-wrap:wrap}
  .hdr-meta{text-align:right}
  .doc-tip{font-size:.65rem;font-weight:800;color:rgba(255,255,255,.45);letter-spacing:.12em;text-transform:uppercase;margin-bottom:6px}
  .doc-no{font-size:1.6rem;font-weight:900;color:#fff;letter-spacing:-.02em}
  .doc-tarih{font-size:.78rem;color:rgba(255,255,255,.55);margin-top:4px}

  /* BANT */
  .info-band{background:#f8fafd;border-bottom:1px solid #e8ecf5;padding:18px 40px;display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px}
  .info-item label{font-size:.60rem;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.08em;display:block;margin-bottom:3px}
  .info-item span{font-size:.88rem;font-weight:600;color:#1e293b}

  /* ÜRÜNLER */
  .section-title{padding:18px 40px 10px;font-size:.65rem;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.1em}
  table{width:100%;border-collapse:collapse}
  thead th{background:#0f172a;color:rgba(255,255,255,.5);font-size:.60rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em;padding:9px 12px;text-align:left}
  ...
  /* ÖDEME */
  .odeme-wrap{padding:24px 40px 32px;display:grid;grid-template-columns:1fr auto;gap:32px;align-items:start}
  .odeme-table{min-width:280px}
  .odeme-table table{border:1.5px solid #e2e8f0;border-radius:14px;overflow:hidden}
  .rl{padding:11px 14px;font-size:.85rem;color:#475569;border-bottom:1px solid #f1f5f9;white-space:nowrap}
  .rr{padding:11px 16px;font-size:.85rem;font-weight:600;color:#0f172a;text-align:right;border-bottom:1px solid #f1f5f9;font-family:'DM Mono',monospace;white-space:nowrap}
  .grand-row .rl{background:#0f172a;color:rgba(255,255,255,.7);font-weight:700;border:none}
  .grand-row .rr{background:#0f172a;border:none}
  .total-amt{color:#D01F2E!important;font-size:1.18rem!important;font-weight:900!important}
  .badge-nakit{background:#dcfce7;color:#15803d;border-radius:6px;padding:2px 9px;font-size:.78em;font-weight:700}
  .badge-kart{background:#eff6ff;color:#1d4ed8;border-radius:6px;padding:2px 9px;font-size:.78em;font-weight:700}
  .badge-taksit{background:#f5f3ff;color:#7c3aed;border-radius:6px;padding:2px 9px;font-size:.78em;font-weight:700}

  /* FOOTER */
  .footer{background:#f8fafd;border-top:1px solid #e8ecf5;padding:18px 40px;display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap}
  .footer-note{font-size:.72rem;color:#94a3b8;max-width:460px;line-height:1.5}
  .footer-brand{font-size:.80rem;font-weight:800;color:#D01F2E}

  /* KOŞULLAR */
  .kosullar{padding:0 40px 24px}
  .kosul-box{background:#f8fafd;border:1px solid #e8ecf5;border-radius:12px;padding:16px 20px}
  .kosul-box h4{font-size:.68rem;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px}
  .kosul-box p{font-size:.76rem;color:#64748b;line-height:1.6}

  /* GEÇERLİLİK */
  .validity-chip{display:inline-flex;align-items:center;gap:6px;background:#fef3c7;color:#92400e;border:1px solid #fde68a;border-radius:8px;padding:6px 12px;font-size:.78rem;font-weight:700;margin:0 40px 20px}
...
<div class="page">
  ...
  <div class="info-band">
    <div class="info-item">
      <label>Müşteri</label>
      <span>${_esc(data.musteriIsim || '—')}</span>
    </div>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th style="width:42px;text-align:center">Adet</th>
  ...
  ${isTeklif && sureBitis ? `<div style="padding:20px 40px 0">
    <div class="validity-chip">⏰ Teklif Geçerlilik: ${sureBitis} tarihine kadar</div>
  </div>` : ''}

  ${notHtml ? `<div style="padding:16px 40px 0">${notHtml}</div>` : ''}

  <!-- ÖDEME TABLOSU -->
  <div class="section-title" style="padding-top:20px">💳 Ödeme Özeti</div>
  <div style="padding:0 40px 28px">
    <table class="odeme-table" style="max-width:400px;margin-left:auto">
  ...
  <div class="kosullar">
    <div class="kosul-box">
      ...
      <p>
        ${isTeklif ? 'Bu teklif, yukarıda belirtilen geçerlilik tarihine kadar geçerlidir. Stok ve fiyatlar değişkenlik gösterebilir.' : 'Ürünlerin teslim alınmasından sonra iade ve değişim koşulları için mağazamızı ziyaret ediniz.'}
        Kampanya ve indirimler birleştirilemez. Aygün AVM tüm hakları saklıdır.
      </p>
    </div>
  </div>
  ...
  <!-- FOOTER -->
  <div class="footer">
    <div class="footer-note">Aygün AVM • Bu belge elektronik olarak oluşturulmuştur. İmza gerektirmez.</div>
    <div class="footer-brand">aygün AVM</div>
  </div>

  <!-- YAZDIR BUTONU (baskıda görünmez) -->
  <div class="no-print" style="padding:16px 40px 24px;text-align:center">
    <button onclick="window.print()" style="padding:12px 32px;background:linear-gradient(135deg,#D01F2E,#b91c1c);color:#fff;border:none;border-radius:12px;font-size:.92rem;font-weight:700;cursor:pointer;font-family:inherit;box-shadow:0 4px 14px rgba(208,31,46,.35)">
      🖨 Yazdır / PDF Kaydet
    </button>
  </div>
...
async function loadSepetAnaliz() {
  ...
  try {
    ...
    const snap  = await getDocs(query(collection(_db,'funnel_logs'), where('ts','>=',sinir), orderBy('ts','desc'), limit(2000)));
...
window.paylasBilgi = function() {
  ...
  const tarihStr = tarihBas ? `${tarihBas}${tarihBit && tarihBit !== tarihBas ? ' → ' + tarihBit : ''}` : 'Son 7 Gün';

  // Personel kartlarından özet al
  const pItems = cont.querySelectorAll('[data-personel-id]');
  let satirlar = [];
  pItems.forEach(el => {
    const ad   = el.dataset.personelAd || '?';
    const sO   = el.dataset.satisOran  || '0';
    const satis = el.dataset.satisAdet || '0';
    const top  = el.dataset.toplamAdet || '0';
    const mag  = el.dataset.magaza     || '';
    const rozet = el.dataset.rozet     || '';
    satirlar.push(`${rozet} ${ad}${mag?' ('+mag+')':''}: ${satis}/${top} müşteri — %${sO} dönüşüm`);
  });

  const metin = satirlar.length > 0
    ? `📊 *Personel Performans Özeti*\n📅 ${tarihStr}\n\n${satirlar.join('\n')}\n\n_Aygün AVM Analiz_`
    : `📊 *Personel Performans Özeti*\n📅 ${tarihStr}\n\nAnaliz verileri için admin panelini kontrol edin.\n\n_Aygün AVM Analiz_`;
  ...
  if (navigator.share) {
    ...
    navigator.clipboard.writeText(metin).then(() => {
      ...
      if (btn) { btn.textContent = '✅ Kopyalandı'; setTimeout(() => btn.textContent = '📤 Haftalık Bilgi', 2000); }
    }).catch(() => {
      prompt('Aşağıdaki metni kopyalayın:', metin);
    });
  }
};
...
async function renderMyStats() {
  ...
  // daily_stats'tan doğrudan satis_sayisi oku
...
function _renderMyStatsUI() {
  ...
  const { d, me, ad, bugun, star, benimYildiz } = _myStatsCache;
  ...
  const btbAcil = btbRaw !== null && btbVal < 5 && (v.blur||0) > 10; // Çok düşük dönüşüm + yüksek blur = acil
  ...
  container.innerHTML = `
    ...
    ${star ? (() => {
      ...
      // ── SEKMELİ LİG: Personel / Mağaza ─────────────────────
      const _ligSekmesi = window._ligAktifSekme || 'personel';

      // PODYUM (ilk 3)
      const podyumHtml = (() => {
        const top3 = (star.ranked||[]).slice(0,3);
        if (!top3.length) return '';
        const [bir, iki, uc] = top3;
        const _kart = (r, boyut, madalya, renk, border) => {
          if (!r) return '<div></div>';
          const isMe = r.email === me;
          const ad = r.email.split('@')[0];
      ...
      // MAĞAZA LİGİ
      const magazaLigiHtml = (star.magazaLigi||[]).length > 1 ? `
        <div style="background:#0f172a;border-radius:16px;padding:12px 14px;margin-top:10px;border:1px solid #1e293b">
          ...
          ${(star.magazaLigi||[]).map((m,i)=>{
            const medals = ['🥇','🥈','🥉'];
            const isMyMag = (getMagazaTipi()===m.mag);
            const bar = Math.round((m.satis / (star.magazaLigi[0]?.satis||1)) * 100);
      ...
      // TAM SIRALAMASI (4. ve altı)
      const altListeHtml = (() => {
        const altlar = (star.ranked||[]).slice(3);
          ...
          ${altlar.map((r,i) => {
            ...
            return `<div style="display:flex;align-items:center;gap:10px;padding:8px 14px;border-top:1px solid #1e293b;${isMe?'background:#1e2a1a':''}">
              ...
              <div style="flex:1;min-width:0">
                ...
                ${bts!==null?`<div style="font-size:.54rem;color:#475569">BTS %${bts} · ${r.teklif} teklif</div>`:''}
              </div>
