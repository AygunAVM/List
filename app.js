// ═══════════════════════════════════════════════════════════════
//  Rev 9.0 YENİLİKLERİ (KALICI TAZELİK BEKÇİSİ — asıl maliyet düzeltmesi):
//  · KÖK NEDEN: Rev 8.0 "canlı dinlemeyi kapat" dedi ama "sadece kullanıcı
//    isteyince çek" ile "her sayfa açılışında/girişte bir kez çek"i
//    birbirine karıştırdı. startFirebaseListeners() her login/reload'da
//    proposals+sales+siparis (300'er limit) + analytics (admin: 90 gün)
//    + motd'u KOŞULSUZ yeniden çekiyordu — ~900+ okuma / her açılış.
//  · _checkForNewBuild() her deploy sonrası açık TÜM sekmeleri birkaç
//    dakika içinde otomatik location.reload() yaptığından, her deploy =
//    her açık cihazda bir tam re-fetch patlaması demekti. Sık deploy
//    attıkça (V8.0→V8.9) maliyet de artıyordu — düzeltme çabasının
//    kendisi maliyeti büyütüyordu.
//  · _propLastFetchTs zaten tutuluyordu ama SADECE "3 dk önce güncellendi"
//    etiketi için okunuyordu (kozmetik) — hiçbir yerde "tekrar çekme"
//    kararına girmiyordu. Üstelik bellek-içiydi, her reload'da sıfırlanıyordu.
//  · ÇÖZÜM: fetchProposalsOnce/fetchSalesOnce/fetchSiparisOnce/
//    fetchAnalyticsOnce/fetchMotdOnce artık gerçek bir force parametresi
//    ve localStorage'a yazılan bir zaman damgası kullanıyor (_FETCH_TTL_MS
//    = 4 dk). TTL dolmadıysa ağa HİÇ gidilmiyor — reload'lar arasında da
//    korunuyor. "Yenile" butonları (refreshProposals/refreshAdminPanel)
//    force=true ile TTL'i bilerek görmezden geliyor; otomatik girişler ve
//    sekme değişimleri (switchAdminTab) force=false ile TTL'e uyuyor.
//  · siparis/analytics/motd artık proposals/sales gibi localStorage'a
//    yazılıyor ki TTL penceresi içinde bir reload olduğunda (stale-
//    while-revalidate) ekran boş kalmasın.
//  · Beklenen etki: maliyet artık (personel × günlük açılış sayısı) ile
//    değil, (personel × TTL penceresi başına en fazla 1) ile orantılı —
//    deploy sıklığı veya kaç kez sayfa açıldığı artık faturayı büyütmüyor.
//
// ═══════════════════════════════════════════════════════════════
//  AYGÜN AVM — app.js  (Rev 8.0 — Canlı Dinlemeden İstek Üzerine Çekmeye)
...
//  okunuyor (Spark kota tasarrufu, sınırsız tarih aralığı desteği —
...
//
//  Rev 8.0 YENİLİKLERİ (MİMARİ DEĞİŞİKLİK — iş akışına göre yeniden tasarım):
//  · İş akışı netleşti: teklif/satış/sipariş verisinin geç görünmesinin
//    hiçbir önemi yok (admin panel günde 1 kez kontrol ediliyor), asıl
//    kritik olan blur hızı (zaten etkilenmiyor) ve saatlik trafik analizi
//    (zaten daily_funnel_summary üzerinden ucuz).
//  · proposals/sales/siparis/analytics/motd artık 7/24 açık onSnapshot
//    TUTMUYOR. Bunun yerine: uygulama açılışında bir kez, ilgili ekran
//    açıldığında (openProposals, switchAdminTab, openAdmin) bir kez, ve
//    "Yenile" butonuna basıldığında (refreshProposals/refreshAdminPanel)
//    getDocs() ile tek seferlik çekiliyor.
//  · Arkaplanda hiç canlı bağlantı tutulmadığı için, kronik WebChannel
//    404/reconnect sorunu bu koleksiyonlar için artık HİÇ okuma/network
//    maliyeti üretemez — kaybedecek bir "watch" bağlantısı yok.
//  · sessions ve live_baskets küçük/tek-doküman ölçekli ve gerçekten
//    anlık olması faydalı olduğu için canlı dinlemede bırakıldı.
//  · Artık kullanılmayan _safeListener() retry/backoff sarmalayıcısı
//    kaldırıldı (proposals/sales/siparis/analytics onSnapshot kullanmıyor).
//  · Admin panelindeki 60 saniyelik otomatik yenileme kaldırıldı — panel
//    günde bir kez kısa süre açılıyor, sürekli arkaplan yenilemesinin
//    değeri yok. Panel açılışında ve "Yenile" butonunda güncel veri çekiliyor.
//
//  Rev 7.4 YENİLİKLERİ (Pazar/kapalı gün analizinden çıkan ince ayrıntılar):
//  · siparis listener'ına eksik olan limit(300) eklendi (proposals/sales'ta
//    yapılmıştı, siparis unutulmuştu).
//  · analytics koleksiyonu dinleyicisi KRİTİK şekilde düzeltildi: hiçbir
//    limit/filtre olmadan, hem admin hem HER PERSONEL cihazına tüm
//    çalışanların tüm geçmiş kayıtlarını indiriyordu (koleksiyon her gün
//    büyüdüğünden maliyeti de sürekli büyüyordu). Artık personel sadece
//    kendi email'iyle eşleşen kayıtları dinliyor; admin raporlama için
//    tüm koleksiyonu almaya devam ediyor.
//
...
//    "Listen/channel 404" hatalarını azaltmak için denenmişti, ama
//    bazı kullanıcılarda tam tersi etki yaptı (bağlantı türü keşfi
//    için attırdığı ekstra istek de aynı endpoint'e gidiyor ve bazı
//    ağlarda/uzantılarda engelleniyor). Kanıtlanmamış faydası için
//    risk eklemeye değmedi. Bu 404 hatası zaten _safeListener()
//    içindeki retry mantığıyla telafi ediliyor ve genelde zararsızdır.
//
//  Rev 7.1 YENİLİKLERİ (watermark tabanlı özet takibi):
//  · _ensureSummariesExist artık sabit "son 30 gün" penceresi yerine
//    meta/funnelWatermark dokümanında kalıcı bir işaret tutuyor.
//    Admin panele 30+ gün girilmese bile hiçbir gün kalıcı olarak
//    atlanmaz — sadece gecikmeli tamamlanır (maxCatchUp=90 ile
//    tek seferlik okuma patlaması sınırlanır).
//
//  Rev 7.0 YENİLİKLERİ (okuma kotası acil müdahale):
//  · _ensureSummariesExist artık günde SADECE 1 kez çalışıyor
//    (localStorage kilidi) — önceden "analiz" sekmesine her tıklamada
//    365 ayrı getDoc() okuması yapıyordu, bu tek başına kotanın büyük
//    kısmını tüketiyordu.
//  · proposals / sales realtime listener'larına limit(300) eklendi —
//    WebChannel 404 sonrası yeniden bağlanmalarda TÜM koleksiyonun
//    yeniden okunmasının önüne geçer.
//  · proposals değişince tetiklenen loadFunnelAnaliz çağrısı debounce
//    edildi (art arda teklif hareketlerinde tekrar tekrar ağır sorgu
//    çalışmasın diye).
//
//  Rev 6.0 YENİLİKLERİ:
//  · FOMO Motoru: Teklif ürününün stoğu ≤3'e düşünce kart uyarısı
//  · Price Drop Hunter: Güncel fiyat/kampanya değişimi teklif kartına yansır
//  · Müşteri Isı Skoru (0-100): Otomatik önceliklendirme (🔥/📞/❄️)
//  · Proposals sıralama: Isı skoru + stok aciliyeti birleşik
//  · Personel filtresi: Arama çubuğuna personel dropdown eklendi
//  · Admin Dönüşüm Endeksi: Teklif → Satış oranı + Unutulan Teklif Skoru
// ═══════════════════════════════════════════════════════════════

// ─── FİREBASE BAŞLATMA ──────────────────────────────────────────
import { initializeApp }                           from 'https://www.gstatic.com/firebasejs/12.10.0/firebase-app.js';
import { initializeFirestore, collection, collectionGroup, doc, deleteDoc,
...
// ✅ Rev 8.3 — QA doğrulama etiketi: F12 konsolunda hangi build'in
// çalıştığını hemen görmek için (bkz. index.html'deki app.js?v=... ve
// service-worker.js'deki CACHE_VERSION ile eşleşmeli). Aynı zamanda
// sessions dokümanına da yazılır (bkz. _checkAndRegisterSession) —
// admin panelinde "Personel" sekmesinde hangi cihazın eski kod
// çalıştırdığını artık TAHMİN ETMEYE gerek kalmadan görebilirsiniz.
// ✅ Rev 8.8 — Sürüm şeması değişti: 'V{major}.{minor}-{YYYYMMDD}-{SSSS}'.
// Bu tek string HER YERDE aynı: app.js sabiti, index.html'deki
// app.js?v=... query'si, service-worker.js CACHE_VERSION'ı,
...
const APP_BUILD_VERSION = 'V9.1-20260808-0939';
...
// etikete çevrilip ana ekrandaki küçük rozete (#app-version-badge,
// index.html) yazılır. Herkes (admin dahil) kendi cihazının hangi
// build'de olduğunu tek bakışta, F12 açmadan görebilir.
function _formatBuildLabel(v) {
  const m = /^V(\d+\.\d+)-(\d{4})(\d{2})(\d{2})-(\d{2})(\d{2})$/.exec(v || '');
  if (!m) return v || '';
  const [, ver, yyyy, mm, dd, hh, mi] = m;
  return `V${ver} · ${dd}.${mm}.${yyyy} ${hh}:${mi}`;
}
try {
  const _vb = document.getElementById('app-version-badge');
  if (_vb) _vb.textContent = _formatBuildLabel(APP_BUILD_VERSION);
} catch(_) {}

// ─── BUILD TAZELİK KONTROLÜ (Rev 8.6) ──────────────────────────
// ✅ Sorun: mobil/masaüstü tarayıcılarda bir sekmeyi/PWA'yı "kapatıp
// tekrar açmak" çoğu zaman JS'i gerçekten sonlandırmıyor — işletim
// sistemi sekmeyi sadece arkaplana atıyor, "tekrar açılınca" da GERÇEK
// bir sayfa yüklemesi/network isteği OLMUYOR; aynı donmuş JS kaldığı
// yerden devam ediyor. Service Worker'ın kendi güncelleme mekanizması
// (saatte bir reg.update()) sadece sekme fiilen canlıyken/JS timer'ları
// çalışırken tetiklenir — bu yüzden bu senaryoyu yakalayamıyordu ve
// "kapatıp açtırıyorum yine eski yazıyor" şikayeti buradan geliyordu.
// Çözüm: tarayıcının/SW'nin kendi önbellek sezgilerine bel bağlamak
// yerine, sekme HER görünür/odaklı hale geldiğinde küçük, her zaman
// taze (cache:'no-store') bir version.json dosyasını biz kontrol
// ediyoruz. Sunucudaki build farklıysa kullanıcıyı bilgilendirip kısa
// bir gecikmeyle sayfayı otomatik yeniliyoruz.
let _buildCheckInFlight = false;
async function _checkForNewBuild() {
  if (_buildCheckInFlight) return;
  _buildCheckInFlight = true;
  try {
    const r = await fetch('./version.json?t=' + Date.now(), { cache: 'no-store' });
    if (!r.ok) return;
    const data = await r.json();
    if (data?.build && data.build !== APP_BUILD_VERSION) {
      console.warn('Yeni sürüm bulundu: ' + data.build + ' (bu sekme: ' + APP_BUILD_VERSION + ') — sayfa yenileniyor.');
      try { showToast('🔄 Yeni sürüm mevcut, sayfa birazdan otomatik yenilenecek…', 'warning'); } catch(_){}
      setTimeout(() => location.reload(), 2500);
    }
  } catch (e) { /* ağ hatası — sessiz geç, bir sonraki kontrolde tekrar denenecek */ }
  finally { _buildCheckInFlight = false; }
}
function _startBuildFreshnessWatchdog() {
  if (window._buildWatchdogStarted) return;
  window._buildWatchdogStarted = true;
  // Sekme görünür hale geldiğinde (kapalıyken tekrar açma / uygulama
  // değiştirmeden dönme dahil) HEMEN kontrol et — asıl çözüm burası.
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') _checkForNewBuild();
  });
  // Bazı mobil tarayıcılar bfcache'ten geri dönüşü visibilitychange
  // yerine/yanında pageshow ile bildirir.
  window.addEventListener('pageshow', (e) => { if (e.persisted) _checkForNewBuild(); });
  window.addEventListener('focus', _checkForNewBuild);
  // Sekme uzun süre önde açık kalsa bile 10 dk'da bir de kontrol et
  // (Firestore maliyeti YOK — sadece küçük bir statik dosya isteği).
  setInterval(_checkForNewBuild, 10 * 60 * 1000);
  _checkForNewBuild();
}
// ⛔ Rev 7.2 — GERİ ALINDI: Rev 7.0'da denenen experimentalAutoDetectLongPolling,
// bazı kullanıcılarda "Listen/channel ... 404" hatasını AZALTMAK yerine
// ARTIRDI (bu ayar bağlantı türünü belirlemek için ekstra bir "keşif"
// isteği attırıyor, bu istek de aynı Listen/channel endpoint'ine gidiyor
// ve bazı ağlarda/tarayıcı uzantılarında engelleniyor). Kanıtlanmış bir
// fayda olmadan risk eklediği için kaldırıldı — normal initializeFirestore
// ile devam ediyoruz. Bu 404 hatası zaten _safeListener() tarafından
// retry ile telafi ediliyor ve genelde zararsızdır (bağlantı kendini
// birkaç saniye içinde toparlar).
// ✅ Rev 8.3 — Kalıcı (IndexedDB) local cache açıldı. Önceden {} ile
// başlatılıyordu (sadece bellek-içi cache) — bu yüzden her sayfa
// yüklemesi/hard-refresh, hâlâ canlı dinlenen koleksiyonların
// (sessions, live_baskets, vitrin) TAMAMINI sıfırdan server'dan
// okuyordu. Artık tarayıcı diskinde kalıcı — aynı sekmede/aynı
// cihazda tekrar tekrar reload yapmak (Ctrl+Shift+R dahil) baştan
// tam koleksiyon okumasına yol açmıyor, sadece değişen dokümanlar
// okunuyor. persistentMultipleTabManager ile birden fazla sekme/
// pencere aynı anda açıksa da hepsi aynı diskteki cache'i paylaşır.
...
// ═══════════════════════════════════════════════════════════════
//  Rev 8.0 — CANLI DİNLEME → İSTEK ÜZERİNE ÇEKME (on-demand fetch)
// ═══════════════════════════════════════════════════════════════
// İş akışı netleşti: teklif/satış/sipariş verisinin GEÇ görünmesinin
// hiçbir önemi yok (admin panelini günde 1 kez kontrol ediyorsunuz).
// Asıl kritik olan blur hızı (zaten Firestore'u beklemiyor, etkilenmedi)
// ve saatlik trafik analizi (zaten daily_funnel_summary üzerinden ucuz).
// Bu yüzden proposals/sales/siparis/analytics/motd artık 7/24 açık bir
// onSnapshot bağlantısı TUTMUYOR — sadece ilgili ekran açıldığında veya
// "Yenile" butonuna basıldığında BİR KEZ getDocs() ile çekiliyor. Arkaplanda
// hiç canlı bağlantı olmadığı için, kronik WebChannel 404/reconnect sorunu
// bu koleksiyonlar için artık HİÇ okuma maliyeti üretemez.
// (sessions ve live_baskets küçük/tek-doküman ölçekli olduğu ve gerçekten
// anlık olması faydalı olduğu için canlı dinlemede bırakıldı.)

// ═══════════════════════════════════════════════════════════════
//  Rev 9.0 — GERÇEK TAZELİK BEKÇİSİ (persistent TTL guard, SWR)
// ═══════════════════════════════════════════════════════════════
// V8.x'te _propLastFetchTs sadece bir UI etiketi için tutuluyordu
// (kozmetik) — hiçbir yerde "tekrar çekme" kararı için kullanılmıyordu.
// Üstelik bellek-içiydi: her reload'da sıfırlanıyordu. _checkForNewBuild()
// her deploy sonrası TÜM açık sekmeleri otomatik location.reload()
// yaptığından, her reload sıfırdan 300+300+300+... okuma tekrarlıyordu
// — bu, günlük sabit maliyetin asıl kaynağıydı (bkz. Rev 9.0 notları).
// Bu sürümde:
//  1) Zaman damgaları localStorage'a taşındı (reload'lar arasında hayatta
//     kalır — asıl kaldıraç burası).
//  2) TTL süresi dolmadıysa fetch TAMAMEN atlanır (sadece cache'ten değil,
//     ağa hiç gidilmez).
//  3) "Yenile" butonları force=true ile çağırır ve TTL'i görmezden gelir.
//  4) sipariş/analytics/motd de artık proposals/sales gibi localStorage'a
//     yazılıyor — reload sonrası TTL içindeyse ekran boş kalmasın diye.
const _FETCH_TTL_MS = 4 * 60 * 1000; // 4 dakika — bkz. proje notları

function _fetchIsFresh(tsKey) {
  const ts = Number(localStorage.getItem(tsKey) || 0);
  return ts > 0 && (Date.now() - ts) < _FETCH_TTL_MS;
}
function _markFetched(tsKey) {
  localStorage.setItem(tsKey, String(Date.now()));
}

async function fetchProposalsOnce(force = false) {
  if (!force && _fetchIsFresh('aygun_proposals_ts') && proposals.length) {
    window._propLastFetchTs = Number(localStorage.getItem('aygun_proposals_ts'));
    return true; // taze — ağa hiç gidilmedi
  }
  try {
    const snap = await getDocs(query(_colProp(), orderBy('ts', 'desc'), limit(300)));
    proposals = snap.docs.map(d => d.data());
    localStorage.setItem('aygun_proposals', JSON.stringify(proposals));
    window._propLastFetchTs = Date.now();
    _markFetched('aygun_proposals_ts');
    updateProposalBadge();
    EventBus.emit(EV.PROPOSAL_CHANGED);
    EventBus.emit(EV.CART_UPDATED, { source: 'firestore:proposals' });
    return true;
  } catch(e) {
    console.warn('fetchProposalsOnce:', e);
    try {
      const cached = localStorage.getItem('aygun_proposals');
      if (cached && !proposals.length) { proposals = JSON.parse(cached); updateProposalBadge(); }
    } catch(e2) {}
    return false;
  }
}

async function fetchSalesOnce(force = false) {
  if (!force && _fetchIsFresh('aygun_sales_ts') && sales.length) {
    return true; // taze — ağa hiç gidilmedi
  }
  try {
    const snap = await getDocs(query(_colSales(), orderBy('ts', 'desc'), limit(300)));
    sales = snap.docs.map(d => d.data());
    localStorage.setItem('aygun_sales', JSON.stringify(sales));
    _markFetched('aygun_sales_ts');
    return true;
  } catch(e) {
    console.warn('fetchSalesOnce:', e);
    try { const c = localStorage.getItem('aygun_sales'); if(c && !sales.length) sales = JSON.parse(c); } catch(e2) {}
    return false;
  }
}

async function fetchSiparisOnce(force = false) {
  if (!force && _fetchIsFresh('aygun_siparis_ts') && window._siparisData && window._siparisData.length) {
    return true; // taze — ağa hiç gidilmedi
  }
  try {
    const snap = await getDocs(query(_colSiparis(), orderBy('ts', 'desc'), limit(300)));
    window._siparisData = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    localStorage.setItem('aygun_siparis', JSON.stringify(window._siparisData));
    _markFetched('aygun_siparis_ts');
    updateSiparisBadge();
    try { renderSiparisBildirimBar(); } catch(e) {}
    return true;
  } catch(e) {
    console.warn('fetchSiparisOnce:', e);
    try {
      const c = localStorage.getItem('aygun_siparis');
      if (c && (!window._siparisData || !window._siparisData.length)) window._siparisData = JSON.parse(c);
    } catch(e2) {}
    return false;
  }
}

async function fetchAnalyticsOnce(force = false) {
  // Not: admin ve personel görünümü farklı sorgu şekli kullandığından
  // (admin: son 90 gün / personel: sadece kendi email'i), tazelik anahtarı
  // role göre ayrı tutuluyor — aksi halde admin'in taze saydığı bir cache
  // personel girişinde yanlışlıkla "taze" sayılabilirdi.
  const tsKey = isAdmin() ? 'aygun_analytics_admin_ts' : 'aygun_analytics_self_ts';
  const dataKey = isAdmin() ? 'aygun_analytics_admin' : 'aygun_analytics_self';
  if (!force && _fetchIsFresh(tsKey) && window._fbAnalytics && Object.keys(window._fbAnalytics).length) {
    return true; // taze — ağa hiç gidilmedi
  }
  try {
    // ✅ Rev 8.1: admin sorgusu son 90 günle sınırlı — bu koleksiyona her
    // gün her kullanıcı için 1 kayıt ekleniyor (silinmiyor), sınırsız
    // çekmek zamanla admin panel açılışının maliyetini artırırdı.
    const cutoff = _istanbulTarihEkle(_bugunIstanbul(), -90);
    const q = isAdmin()
      ? query(_colAnalytics(), where('date', '>=', cutoff))
      : query(_colAnalytics(), where('email', '==', currentUser?.Email || '__none__'));
    const snap = await getDocs(q);
    window._fbAnalytics = {};
    snap.docs.forEach(d => { window._fbAnalytics[d.id] = d.data(); });
    localStorage.setItem(dataKey, JSON.stringify(window._fbAnalytics));
    _markFetched(tsKey);
    return true;
  } catch(e) {
    console.warn('fetchAnalyticsOnce:', e);
    try {
      const c = localStorage.getItem(dataKey);
      if (c && (!window._fbAnalytics || !Object.keys(window._fbAnalytics).length)) window._fbAnalytics = JSON.parse(c);
    } catch(e2) {}
    return false;
  }
}

async function fetchMotdOnce(force = false) {
  if (!force && _fetchIsFresh('aygun_motd_ts') && _motdMessages && _motdMessages.length) {
    return true; // taze — ağa hiç gidilmedi
  }
  try {
    const snap = await getDocs(query(_colMotd(), orderBy('ts', 'desc')));
    _motdMessages = snap.docs
      .filter(d => d.id !== '_greeting')
      .map(d => ({ id: d.id, ...d.data() }))
      .filter(m => m.aktif !== false);
    localStorage.setItem('aygun_motd', JSON.stringify(_motdMessages));
    _markFetched('aygun_motd_ts');
    const greetingDoc = snap.docs.find(d => d.id === '_greeting');
    if (greetingDoc) {
      const g = greetingDoc.data().metin || '';
      localStorage.setItem(_GREETING_KEY, g);
    }
    _startMotdTicker();
    return true;
  } catch(e) {
    console.warn('fetchMotdOnce:', e);
    try {
      const c = localStorage.getItem('aygun_motd');
      if (c && (!_motdMessages || !_motdMessages.length)) { _motdMessages = JSON.parse(c); _startMotdTicker(); }
    } catch(e2) {}
    return false;
  }
}

// ── Teklifler modalı: tazelik göstergesi ────────────────────────
// "Yenile" butonu artık sadece dönmüyor — basılınca dolu kırmızı
// zemine geçip check işaretine dönüyor, ve altta "az önce güncellendi"
// / "3 dk önce güncellendi" gibi canlı bir etiket her an tazeliği
// gösteriyor. Bu, verinin ne zaman çekildiğini her an görünür kılar.
function _propFreshTick() {
  const dot = document.getElementById('prop-fresh-dot');
  const label = document.getElementById('prop-fresh-label');
  if (!dot || !label) return;
  const ts = window._propLastFetchTs;
  if (!ts) { label.textContent = 'güncelleniyor…'; dot.className = 'prop-fresh-dot loading'; return; }
  const saniye = Math.floor((Date.now() - ts) / 1000);
  let metin;
  if (saniye < 10)       metin = 'az önce güncellendi';
  else if (saniye < 60)  metin = saniye + ' sn önce güncellendi';
  else if (saniye < 3600) metin = Math.floor(saniye/60) + ' dk önce güncellendi';
  else                    metin = Math.floor(saniye/3600) + ' sa önce güncellendi';
  label.textContent = metin;
  dot.className = 'prop-fresh-dot' + (saniye * 1000 > _FETCH_TTL_MS ? ' stale' : '');
}
function _startPropFreshTicker() {
  _stopPropFreshTicker();
  _propFreshTick();
  window._propFreshTimer = setInterval(_propFreshTick, 5000);
}
function _stopPropFreshTicker() {
  if (window._propFreshTimer) { clearInterval(window._propFreshTimer); window._propFreshTimer = null; }
}

// Teklifler modalı için elle "Yenile" — ekranı açık tutup başka bir
// cihazdaki değişikliği görmek isteyen personel için.
async function refreshProposals() {
  const btn = document.getElementById('prop-refresh-btn');
  const dot = document.getElementById('prop-fresh-dot');
  const label = document.getElementById('prop-fresh-label');
  if (btn) btn.classList.add('loading');
  if (dot) dot.className = 'prop-fresh-dot loading';
  if (label) label.textContent = 'güncelleniyor…';
  const ok = await fetchProposalsOnce(true); // ✅ Rev 9.0: manuel Yenile — TTL'i görmezden gel
  ...
  _startPropFreshTicker();
}

// Admin paneli için elle "Yenile" — aktif sekmeye göre ilgili veriyi çeker.
// ✅ Rev 8.1: Önceden burada bir kez fetch edilip SONRA switchAdminTab()
// çağrılıyordu — o da tabına göre AYNI veriyi tekrar çekiyordu. Yani her
// Yenile basışı veriyi 2 KEZ okuyordu. Artık tek fetch: switchAdminTab
// zaten kendi promise'ini döndürüyor, biz sadece onu bekliyoruz.
async function refreshAdminPanel() {
  const btn = document.getElementById('admin-refresh-btn');
  if (btn) btn.classList.add('loading');
  const activeTab = document.querySelector('.admin-tab.active')?.dataset?.tab || 'overview';
  const ok = await switchAdminTab(activeTab, true); // ✅ Rev 9.0: manuel Yenile — TTL'i görmezden gel
  if (btn) {
    btn.classList.remove('loading');
    if (ok) {
      btn.classList.add('success');
...
function startFirebaseListeners() {
  // ✅ Rev 8.0: _safeListener() (WebChannel retry/backoff yardımcısı) burada
  // kaldırıldı — proposals/sales/siparis/analytics/motd artık onSnapshot
  // kullanmıyor, tek seferlik fetchXOnce() fonksiyonlarına geçti (aşağıda).
  // sessions ve live_baskets hâlâ canlı dinleniyor ama onlar zaten kendi
  // basit onSnapshot + err callback'iyle yönetiliyor, ayrı bir retry
  // sarmalayıcıya ihtiyaçları yok (küçük ölçekli, tek doküman/koleksiyon).
  // ── Proposals / Sales / Sipariş / Analytics / Motd ─────────────
  // ✅ Rev 8.0: Artık 7/24 açık onSnapshot YOK. Teklif/satış/sipariş
  // verisinin geç görünmesi iş akışında bir sorun teşkil etmiyor
  // (admin panel günde 1 kez kontrol ediliyor) — bu yüzden bunlar
  // sadece uygulama açılışında BİR KEZ çekiliyor; ilgili ekran
  // açıldığında (openProposals, switchAdminTab) ve "Yenile" butonuna
  // basıldığında (refreshProposals/refreshAdminPanel) tekrar çekilir.
  // Arkaplanda canlı bağlantı tutulmadığı için kronik WebChannel 404/
  // reconnect sorunu bu koleksiyonlar için artık HİÇ maliyet üretmiyor.
  fetchProposalsOnce();
  fetchSalesOnce();
  fetchSiparisOnce();
  fetchAnalyticsOnce();
  ...
  // Sessions listener (admin eş zamanlı oturum izleme) — küçük/tek
  // doküman ölçekli olduğu ve zorla-çıkış tespiti gerçekten anlık
  // olması gerektiği için canlı dinlemede bırakıldı.
  _startSessionListener();

  // ✅ Rev 8.6 — build tazelik izleyicisi: sekme arkaplandan öne her
  // dönüşünde otomatik sürüm kontrolü yapar (bkz. yukarıdaki tanım).
  _startBuildFreshnessWatchdog();

  // --- Live Baskets Listener ---
  // Admin: tüm sepetleri dinle | Personel: sadece kendi sepetini dinle (güvenlik + performans)
  // Küçük ölçekli (aktif sepet sayısı kadar) olduğu için canlı dinlemede bırakıldı.
...
// ✅ YENİ: Hızlı filtre değişimlerinde bayat (stale) sonuçların ekranı ezmesini
// önlemek için her çağrıya artan bir kimlik veriyoruz; render anında sadece
// "en son başlatılan" istek kazanır.
let _funnelReqToken = 0;
...
let messages  = [];
// ✅ Rev 9.0: siparis/analytics/motd de artık proposals/sales gibi
// localStorage'dan hydrate ediliyor — TTL penceresi içinde bir reload
// olduğunda fetch atlanınca ekran boş kalmasın diye (bkz. fetchXOnce).
window._siparisData = (() => { try { return JSON.parse(localStorage.getItem('aygun_siparis')) || []; } catch(e) { return []; } })();
window._fbAnalytics = (() => { try { return JSON.parse(localStorage.getItem('aygun_analytics_admin')) || JSON.parse(localStorage.getItem('aygun_analytics_self')) || {}; } catch(e) { return {}; } })();
let _motdMessages  = (() => { try { return JSON.parse(localStorage.getItem('aygun_motd')) || []; } catch(e) { return []; } })(); // Admin tarafından girilen kayan yazılar
...
const Basket = {
  ...
  // ✅ Rev 9.2: Basket.setItemDisc metodu kaldırıldı — hiçbir yerden
  // çağrılmıyordu. Sepette manuel satır indirimi gerçekte global
  // setItemDisc(idx, val) fonksiyonu üzerinden yapılıyor (bkz. aşağıda,
  // index.html'deki onblur/onkeydown/onclick bağlamaları oraya gidiyor).
  // İki ayrı fonksiyonun aynı işi yapıyormuş gibi durması karışıklık
  // yaratıyordu; kullanılmayan olan kaldırıldı.
  ...
  // Sync: kampanyaları yeniden hesapla (bu saveBasket'i de içerir) + UI güncelle
  // ✅ Rev 9.2: eskiden yalnızca "✨ En İyi" butonu recalculate'i tetikliyordu.
  // Sepet değişikliklerinde (ekle/çıkar) tetiklenmediği için, bir ürün
  // silindiğinde kalan üründeki artık-geçersiz kampanya indirimi ekranda
  // donmuş kalıyordu. add/removeAt/removeMany hepsi _sync() üzerinden
  // geçtiği için tek bir yerden düzeltmek yeterli.
  _sync() {
...
async function loadData() {
  ...
  try {
    ...
    setTimeout(() => {
      ...
      // ✅ Rev 9.2: renderKategoriGrid çağrısı kaldırıldı — fonksiyon
      // hiçbir yerde tanımlı değildi, try/catch onu sessizce yutuyordu.
...
function recalculateAllGroupCampaigns() {
  ...
  // ✅ Rev 9.2: eskiden bu blok `item._campaigns` dolu olmasını şart koşuyordu.
  // Basket.removeAt() kalan ürünlerin _campaigns'ini null'a çektiği için
  // (bkz. removeAt), bu şart silme sonrası HİÇ tutmuyor ve eski kampanya
  // indirimi (_campDisc/itemDisc) donmuş kalıyordu — bir ürün silindiğinde
  // sepette kalan diğer ürünün artık geçersiz olan indirimi kaybolmuyordu.
  // Koşul kaldırıldı: _campaigns yoksa zaten _campDisc de 0/undefined
  // olduğundan bu blok o üründe no-op çalışır, zararsızdır.
  basket.forEach(item => {
...
function setItemDisc(idx, val) {
  ...
  // ✅ Rev 9.2: recalculate manuel kısmı (itemDisc - campDisc farkından) doğru
  // şekilde geri çıkarıp koruyor, ardından kampanya payını güncel sepete göre
  // tazeler — saveBasket() zaten recalculate içinde çağrılıyor.
  recalculateAllGroupCampaigns();
  updateCartUI();
  // Panel güncelleme
...
function setNakitOverride(idx, val) {
  ...
  // ✅ Rev 9.2: tutarlılık için diğer sepet-değişikliği yollarıyla aynı
  // desende kampanyaları tazele (saveBasket() içeride çağrılıyor).
  recalculateAllGroupCampaigns();
  updateCartUI();
}
...
let currentPropPersonel = ''; // '' = tümü, email = personel filtresi
...
function openProposals() {
  ...
  // ✅ Rev 8.0: modal açılır açılmaz güncel veriyi çek (cache'ten değil).
  // Zaten cache'ten hemen render ettik (yukarıda), bu güncelleme geldiğinde
  // sessizce tekrar render eder — kullanıcı boş ekran görmez.
  const _btn = document.getElementById('prop-refresh-btn');
  if (_btn) _btn.classList.add('loading');
  const _dot = document.getElementById('prop-fresh-dot');
  if (_dot) _dot.className = 'prop-fresh-dot loading';
  fetchProposalsOnce().then(ok => {
    try { renderProposals(); } catch(e) {}
    if (_btn) { _btn.classList.remove('loading'); if (ok) { _btn.classList.add('success'); setTimeout(()=>_btn.classList.remove('success'), 900); } }
    _startPropFreshTicker();
  });
}
...
function filterProposals(filter) {
  ...
  // Personel filtresi dropdown değeri
  const pSel = document.getElementById('prop-personel-select');
  if(pSel) currentPropPersonel = pSel.value || '';
  renderProposals(null, false, q);
}

// Personel dropdown'ını doldur (admin için — bekliyor + tümünden unique user'lar)
function _buildPersonelSelect() {
  const sel = document.getElementById('prop-personel-select');
  if(!sel || !isAdmin()) return;
  const users = [...new Set(proposals.map(p=>p.user).filter(Boolean).filter(u=>u!=='-'))].sort();
  const cur = sel.value;
...
// ── Müşteri Isı Skoru (Lead Scoring) 0-100 ──────────────────────
// Faktörler: teklifin yaşı, ürün adedi, toplam tutar, not sayısı,
// müşterinin önceki satışları, bekliyor durumu.
function _calcHeatScore(p) {
  if(p.durum !== 'bekliyor') return 0;
  let score = 50; // temel puan

  // 1. Yaş (taze = daha sıcak)
  const ageDays = (Date.now() - new Date(p.ts||0).getTime()) / 86400000;
  ...
  // 2. Ürün adedi (çok ürün = yüksek niyet)
  const urunAdet = (p.urunler||[]).length;
  score += Math.min(urunAdet * 4, 16);

  // 3. Toplam tutar (değerli teklif = öncelikli)
  const tutar = (p.urunler||[]).reduce((s,u)=>s+(parseFloat(u.nakit)||0),0);
  if      (tutar >= 50000) score += 15;
  else if (tutar >= 20000) score += 10;
  else if (tutar >= 5000)  score += 5;

  // 4. Takip notları (not varsa = sıcak ilişki, ama çok not = sorunlu)
  const notSayisi = (p.adminNot||[]).length;
  if      (notSayisi === 0) score -= 5;  // hiç aranmamış
  else if (notSayisi === 1) score += 8;
  else if (notSayisi === 2) score += 4;
  else                      score -= notSayisi * 3; // çok aranıp sonuç yok

  // 5. Müşteri önceki satış geçmişi (proposals içinde aynı phone'dan satisDondu var mı)
  if(p.phone && p.phone !== '—') {
    const cleanPhone = (p.phone||'').replace(/\D/g,'');
    const prevSales = sales.filter(s=>(s.phone||'').replace(/\D/g,'')===cleanPhone).length;
    score += Math.min(prevSales * 8, 16);
  }

  // 6. Stok aciliyeti — ürün stoğu düşükse ekstra bonus
  if(window._cachedUrunler) {
    const hasLowStock = (p.urunler||[]).some(u => {
      const found = window._cachedUrunler.find(pr=>{
        const k=Object.keys(pr).find(kk=>(kk||'').toLowerCase()==='urun');
        return k && pr[k]===u.urun;
      });
      return found && Number(found.Stok||0) > 0 && Number(found.Stok||0) <= 3;
    });
    if(hasLowStock) score += 12;
  }

  return Math.max(0, Math.min(100, Math.round(score)));
}

// Isı skoru etiket/renk
function _heatBadge(score) {
  if(score >= 80) return { icon:'🔥', label:'Acil Ara', color:'#dc2626', bg:'#fef2f2' };
  if(score >= 40) return { icon:'📞', label:'Sıcak',    color:'#d97706', bg:'#fffbeb' };
  return               { icon:'❄️', label:'Soğuk',    color:'#6b7280', bg:'#f3f4f6' };
}

// Stok uyarısı: teklifteki ürünlerin güncel stoğunu kontrol et
function _propStockWarning(p) {
  if(p.durum !== 'bekliyor') return null;
  if(!window._cachedUrunler) return null;
  const low = [];
  (p.urunler||[]).forEach(u=>{
    const found = window._cachedUrunler.find(pr=>{
      const k=Object.keys(pr).find(kk=>(kk||'').toLowerCase()==='urun');
    ...
    if(!found) return;
    const stok = Number(found.Stok||0);
    if(stok > 0 && stok <= 3) low.push({ urun: u.urun, stok });
    if(stok === 0) low.push({ urun: u.urun, stok: 0 });
  });
  return low.length ? low : null;
}

// Fiyat/kampanya değişimi kontrolü: teklifte kaydedilen fiyat vs güncel
function _propPriceChange(p) {
  if(p.durum !== 'bekliyor') return null;
  if(!window._cachedUrunler) return null;
  const changed = [];
  (p.urunler||[]).forEach(u=>{
    ...
    if(!found) return;
    const teklifFiyat = parseFloat(u.nakit||0);
    const guncelFiyat = parseFloat(found.Nakit||found.nakit||0);
    if(!teklifFiyat || !guncelFiyat) return;
    const diff = guncelFiyat - teklifFiyat;
    const pct  = Math.abs(diff/teklifFiyat*100);
    // Kampanya değişimi (Kampanya alanı varsa)
    const teklifKamp  = (u._campaigns||[]).map(c=>c.grup+'_'+c.tutar).sort().join('|');
    const guncelKamps = (found.Kampanyalar||found.kampanyalar||[]);
    const guncelKampStr = Array.isArray(guncelKamps)
      ? guncelKamps.map(c=>(c.grup||c.Grup||'')+'_'+(c.tutar||c.Tutar||0)).sort().join('|')
      : '';
    const kampanyaDegisti = teklifKamp !== guncelKampStr && guncelKampStr !== '';
    if(pct >= 1 || kampanyaDegisti) {
      changed.push({ urun: u.urun, teklifFiyat, guncelFiyat, diff, pct: pct.toFixed(1), kampanyaDegisti });
    }
  });
...
function renderProposals(container, forceAll, searchQ) {
  ...
  if(isAdmin() && currentPropPersonel) {
    myProps = myProps.filter(p => p.user === currentPropPersonel);
  }
  // Arama filtresi
  if(searchQ) {
  ...
  // Personel dropdown'ını doldur (admin için)
  _buildPersonelSelect();
  ...
  // Bekliyor filtresi: ısı skoruna göre önceliklendir; diğerlerinde yeniden eskiye
  if(currentPropFilter === 'bekliyor') {
    myProps.sort((a,b) => _calcHeatScore(b) - _calcHeatScore(a));
  } else {
...
function _renderSingleProp(p) {
  ...
  const canAct = isAdmin() || p.user===me;
    ...
    // ── Isı Skoru ─────────────────────────────────────────────────
    const heatScore = _calcHeatScore(p);
    const heatInfo  = _heatBadge(heatScore);

    // ── Stok Uyarısı (FOMO) ───────────────────────────────────────
    const stockWarn = _propStockWarning(p);

    // ── Fiyat/Kampanya Değişimi ───────────────────────────────────
    const priceChg = _propPriceChange(p);
    ...
    // Fiyat/Kampanya Değişimi: "Revize Et" butonu
    if(priceChg && (isAdmin() || p.user===me)) {
      btns.push(`<button class="pact-btn haptic-btn" onclick="teklifRevizeSepet('${p.id}')" title="Fiyat/kampanya değişti — revize et" style="color:#0369a1;border-color:#bae6fd;background:#f0f9ff;font-weight:700;animation:prop-pulse 1.8s infinite">🟢 Revize</button>`);
    }
    if(canEdit) {
    ...
    // ── Stok uyarı bandı ──────────────────────────────────────────
    const stockBanner = stockWarn
      ? `<div class="prop-fomo-banner">⚠️ Stok Kritik: ${stockWarn.map(s=>s.stok===0?`${s.urun} <b>TÜKENDİ</b>`:`${s.urun} — son <b>${s.stok} adet</b>`).join(' · ')}</div>`
      : '';

    // ── Fiyat değişim bandı ───────────────────────────────────────
    const priceBanner = priceChg
      ? `<div class="prop-price-banner">🟢 Güncelleme: ${priceChg.map(c=>c.diff<0?`${c.urun} <b>−${Math.abs(c.diff).toLocaleString('tr-TR')}₺ (%${c.pct})</b>`:`${c.urun} <b>+${c.diff.toLocaleString('tr-TR')}₺</b>`).join(' · ')} — revize et</div>`
      : '';

    // ── Isı skoru rozeti (sadece bekliyor) ────────────────────────
    const heatTag = p.durum==='bekliyor'
      ? `<span class="proposal-tag prop-heat-tag" style="background:${heatInfo.bg};color:${heatInfo.color};font-weight:700" title="Isı Skoru: ${heatScore}/100">${heatInfo.icon} ${heatInfo.label} <span style="opacity:.6;font-size:.60rem">${heatScore}</span></span>`
      : '';

    return `<div class="proposal-card status-card-${p.durum||'bekliyor'}${p.durum==='satisDondu'?' prop-card-converted':''}" id="pcard-${p.id}">
      ${stockBanner}${priceBanner}
      ...
      <div class="proposal-body">
        <div class="proposal-row">
          ...
          ${userTag}${gizliTag}${sureTag}${heatTag}
...
// ── Yardımcı: Popup engellenirse PDF'i sayfa içinde göster ──────
// ✅ Rev 9.2: _showPdfInline daha önce hiç tanımlanmamıştı — popup
// engelleyici devredeyken _openPdfWindow bu satıra düşünce ReferenceError
// fırlatıyor ve PDF hiç açılmıyordu. Şimdi tam ekran bir iframe overlay
// içinde PDF'i gösteriyor; yazdırma ve kapatma butonları var.
function _showPdfInline(html) {
  const eski = document.getElementById('_pdf-inline-overlay');
  if (eski) eski.remove();

  const overlay = document.createElement('div');
  overlay.id = '_pdf-inline-overlay';
  overlay.style.cssText = 'position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.65);'
    + 'display:flex;flex-direction:column;align-items:center;justify-content:center;padding:16px;';

  const bar = document.createElement('div');
  bar.style.cssText = 'width:100%;max-width:860px;display:flex;justify-content:space-between;'
    + 'align-items:center;margin-bottom:8px;gap:8px;';
  bar.innerHTML =
    '<span style="color:#fff;font-size:.78rem;opacity:.85;flex:1">'
    + 'Tarayıcınız açılır pencereyi engelledi — PDF burada gösteriliyor.</span>'
    + '<button id="_pdf-inline-print" style="padding:6px 14px;border:none;border-radius:6px;'
    + 'background:#fff;color:#1e293b;font-weight:600;cursor:pointer;font-size:.78rem;white-space:nowrap">Yazdır</button>'
    + '<button id="_pdf-inline-close" style="padding:6px 14px;border:none;border-radius:6px;'
    + 'background:#374151;color:#fff;font-weight:600;cursor:pointer;font-size:.78rem;white-space:nowrap">Kapat ✕</button>';

  const iframe = document.createElement('iframe');
  iframe.style.cssText = 'width:100%;max-width:860px;flex:1;background:#fff;border:none;border-radius:8px;';
  iframe.srcdoc = html;
  ...
  bar.querySelector('#_pdf-inline-close').onclick = () => overlay.remove();
  bar.querySelector('#_pdf-inline-print').onclick = () => {
    try { iframe.contentWindow.focus(); iframe.contentWindow.print(); } catch(e) {}
  };
  overlay.addEventListener('click', (e) => { if (e.target === overlay) overlay.remove(); });
}
window._showPdfInline = _showPdfInline;
...
async function loadSepetAnaliz() {
  ...
  try {
    // Son 30 günlük veri: hybrid fetcher (ham bugün/dün + özet öncesi)
    const _30gunBas = _istanbulTarih(new Date(Date.now() - 30 * 86400000));
    const _30gunBit = _istanbulTarih(new Date());
    const logs = await _fetchFunnelHybrid(_30gunBas, _30gunBit);
...
window.setTarihAralik = function(tip) {
  const cont = document.getElementById('funnel-analiz-konteynir');
  if (!cont) return;
  // ✅ DÜZELTME: Artık tarayıcının yerel saat dilimine bağımlı Date metodları
  // (getFullYear/getMonth/getDate) KULLANILMIYOR. Tüm hesap Istanbul tarih
  // string'i üzerinden yapılıyor — _fetchFunnelHybrid ile AYNI "bugün" tanımı.
  const bugunStr = _bugunIstanbul();
  let basStr, bitGosterimStr;
  if (tip === 'bugun') {
    // "Bugün" artık dünü gösterir — özet dün gece oluşur
    basStr = _istanbulTarihEkle(bugunStr, -1);
    bitGosterimStr = basStr;
  } else if (tip === 'hafta') {
    basStr = _istanbulTarihEkle(bugunStr, -6);
    bitGosterimStr = bugunStr;
  } else if (tip === '30') {
    basStr = _istanbulTarihEkle(bugunStr, -29);
    bitGosterimStr = bugunStr;
  } else if (tip === '90') {
    basStr = _istanbulTarihEkle(bugunStr, -89);
    bitGosterimStr = bugunStr;
  } else if (tip === '365') {
    basStr = _istanbulTarihEkle(bugunStr, -364);
    bitGosterimStr = bugunStr;
  } else if (tip === 'tumveri') {
    // Tüm veri: uygulama başlangıcından bugüne (5 yıl geriye kadar)
    basStr = _istanbulTarihEkle(bugunStr, -365 * 5);
    bitGosterimStr = bugunStr;
  }
  if (basStr && bitGosterimStr) {
    // Firebase sorgusu için bitGosterim+1 gün kullanılır (gece 00:00 sınırı)
    const bitQueryStr = _istanbulTarihEkle(bitGosterimStr, 1);
    cont.dataset.tarihBas = basStr;
    cont.dataset.tarihBit = bitGosterimStr; // ✅ görüntüde doğru tarih
    cont.dataset.tarihBitQuery = bitQueryStr; // Firebase için +1 gün
    ...
    if (inpBit) inpBit.value = bitGosterimStr;
  }
...
// ═══════════════════════════════════════════════════════════════
//  SATIŞ HUNİSİ ANALİZ — Aggregation Mimarisi
//  Ham log → günlük özet (daily_funnel_summary) sistemi.
//  Spark planı kota tasarrufu: 3.000 okuma/analiz → 30 okuma/analiz.
//  90 gün/yıllık/her türlü aralık limitssiz desteklenir.
// ═══════════════════════════════════════════════════════════════

// ── Yardımcı: İstanbul tarih string'i (YYYY-MM-DD) ───────────
function _istanbulTarih(d) {
...
// ── Yardımcı: Istanbul tarih string'ine N gün ekle/çıkar ─────
// Tarayıcının yerel saat dilimine bağımlı DEĞİLDİR — saf string/gün
// aritmetiği yapar (UTC öğlen referansıyla DST/timezone kaymalarını önler).
// setTarihAralik VE _fetchFunnelHybrid AYNI "bugün" tanımını kullansın diye
// her iki yerde de bu fonksiyon kullanılır.
function _istanbulTarihEkle(tarihStr, gunSayisi) {
  // Öğlen (12:00 UTC) referans alınır ki gün sınırı kaymaları (DST vb.) sorun çıkarmasın
  const d = new Date(tarihStr + 'T12:00:00Z');
  d.setUTCDate(d.getUTCDate() + gunSayisi);
  return d.toISOString().slice(0, 10);
}

// ── Yardımcı: "Bugün" — her zaman Istanbul saat dilimine göre ─
function _bugunIstanbul() {
  return _istanbulTarih(new Date());
}

// ── Gün Sonu Özet Yazıcı ─────────────────────────────────────
// Bir günün funnel_logs kayıtlarını okur, özetler ve
// daily_funnel_summary/{tarih} dokümanına yazar.
// Özet zaten varsa hiç dokunmaz (idempotent).
async function _pushDailyFunnelSummary(tarih) {
  if (!_db) return;
  try {
    // Guard: Özet zaten var mı?
    const summaryRef = doc(_db, 'daily_funnel_summary', tarih);
    const existing = await getDoc(summaryRef);
    if (existing.exists()) return; // Zaten oluşturulmuş

    // O günün ham loglarını çek
    const baslangic = new Date(tarih + 'T00:00:00+03:00');
    const bitis     = new Date(tarih + 'T23:59:59+03:00');
    const q = query(
    ...
    const snap = await getDocs(q);
    if (snap.empty) {
      // Boş gün için placeholder yaz — bir daha kontrol edilmesin
      await setDoc(summaryRef, { tarih, bos: true, olusturulma: serverTimestamp(), oturumlar: [] });
      return;
    }
    ...
    snap.forEach(d => {
      const l = d.data();
      const pid = l.personelId || 'bilinmiyor';
      if (!byP[pid]) {
        byP[pid] = {
          personelId:  pid,
          personelAd:  l.personelAd || pid.split('@')[0],
          funnelRol:   l.funnelRol  || 'saha',
          magazaTipi:  l.magazaTipi || '',
          gun:         l.gun ?? 0,
          oturumSayisi: 0,
          satis: 0, kacti: 0, bekliyor: 0, hareketsizlik: 0,
          toplamTutar: 0, toplamSure: 0,
          altinSepet: 0, gumusSepet: 0, standartSepet: 0,
          toplamBlur: 0, intentL3Plus: 0,
      ...
      const sonuc = l.sonuc || '';
      const neden  = l.neden || '';
      const HAREKETSIZ = 'Hareketsizlik (Arka Plan)';
      if (sonuc === 'satis') p.satis++;
      else if (sonuc === 'kacti' && neden === HAREKETSIZ) p.hareketsizlik++;
      else if (sonuc === 'kacti') p.kacti++;
      else p.bekliyor++;
      p.toplamTutar += l.toplamTutar || 0;
      p.toplamSure  += l.sure        || 0;
      const kat = l.sepetKategorisi || 'Standart';
      if (kat === 'Altin')      p.altinSepet++;
      else if (kat === 'Gumus') p.gumusSepet++;
      else                      p.standartSepet++;
      const blurListesi = l.blurUrunListesi || l.bakilanFiyatlar || [];
      p.toplamBlur += blurListesi.length;
      if ((l.intentLevel || 0) >= 3) p.intentL3Plus++;
      if (l.bundleVarMi)  { p.bundleFirsat++;  if (l.bundleYapildi) p.bundleYapilan++; }
      const saat = l.saat ?? -1;
      if (saat >= 0 && saat <= 23) p.saatDagilimi[saat] = (p.saatDagilimi[saat] || 0) + 1;
      if (sonuc === 'kacti' && neden && neden !== HAREKETSIZ) {
        p.kacisNedenleri[neden] = (p.kacisNedenleri[neden] || 0) + 1;
      }
      blurListesi.forEach(u => {
        if (u) p.blurUrunSayac[u] = (p.blurUrunSayac[u] || 0) + 1;
      });
      // Ürün listesi (satış/sepet için)
      (l.urunler || []).forEach(u => {
        const ad = u.urun || u;
        if (ad) p.blurUrunSayac['__sepet__' + ad] = (p.blurUrunSayac['__sepet__' + ad] || 0) + 1;
      });
    });

    // Özet dizisini oluştur
    const oturumlar = Object.values(byP).map(p => {
      const topBlurUrunler = Object.entries(p.blurUrunSayac)
        .filter(([k]) => !k.startsWith('__sepet__'))
        .sort((a, b) => b[1] - a[1])
        .slice(0, 15)
        .map(([urun, sayi]) => ({ urun, sayi }));
      const { blurUrunSayac, ...rest } = p;
      return {
        ...rest,
        topBlurUrunler,
        ortaTutar: p.satis > 0 ? Math.round(p.toplamTutar / p.satis) : 0,
      };
    });
    ...
    await setDoc(summaryRef, {
      ...
      personelSayisi: oturumlar.length,
      toplamOturum:   snap.size,
      oturumlar,
    });
    console.log(`📦 daily_funnel_summary/${tarih} oluşturuldu (${snap.size} log → ${oturumlar.length} personel).`);
  } catch (e) {
    console.warn(`_pushDailyFunnelSummary(${tarih}):`, e);
  }
}

// ── Lazy Push: Admin analiz sekmesi açılınca eksik günleri doldur ─
// Bugün dahil edilmez (gün bitmemiş); dünden geriye gider.
//
// ✅ Rev 7.0 — KRİTİK OKUMA KOTASI DÜZELTMESİ:
// Önceki sürümde bu fonksiyon "analiz" sekmesine her tıklamada
// varsayılan 365 gün için TEK TEK getDoc() çağırıyordu. Özet zaten
// var olsa bile bu bir Firestore okuması sayılıyor, yani her sekme
// tıklaması = 365 okuma. Günde onlarca kez tıklanınca kısa sürede
// yüz binlerce okumaya ulaşıp Spark limitini zorluyordu.
//
// ✅ Rev 7.1 — "SU İŞARETİ" (WATERMARK) MİMARİSİ:
// Sabit "son 30 gün" penceresi bir açık verdi: admin panele 30
// günden uzun süre girmezse (tatil, sezon kapanışı, uzun izin vb.),
// o aralıktaki günler pencerenin dışına kayar ve BİR DAHA ASLA otomatik
// olarak yakalanmaz — ham funnel_logs verisi hâlâ dursa da özet hiç
// üretilmez ve "Tüm Veri" raporunda kalıcı bir delik oluşur.
// Çözüm: kayan pencere yerine, en son hangi güne kadar özetlerin
// tamamlandığını meta/funnelWatermark dokümanında kalıcı olarak
// (cihazdan bağımsız, Firestore'da) işaretliyoruz. Her çalışmada
// sadece işaretten bugüne kadar olan GERÇEK boşluk taranır:
//   · Normal günde boşluk çoğunlukla 1 gündir → maliyet ~1-2 okuma.
//   · Uzun bir aradan sonra boşluk büyükse, tek seferde devasa bir
//     okuma patlaması yaşanmasın diye maxCatchUp ile sınırlanır;
//     kalan kısmı bir sonraki çalışmalar tamamlar.
//   · Hiçbir gün kalıcı olarak atlanmaz — sadece gecikmeli tamamlanır.
// Günde en fazla 1 kez çalışması için localStorage kilidi korunuyor.
async function _ensureSummariesExist(maxCatchUp = 90) {
  if (!currentUser || !_db) return;

  const bugunStr = _bugunIstanbul();
  const KILIT_ANAHTARI = 'aygun_summaries_checked_' + bugunStr;
  if (localStorage.getItem(KILIT_ANAHTARI)) {
    // Bugün zaten kontrol edildi — tekrar Firestore'a gitme.
    return;
  }

  try {
    const wmRef  = doc(_db, 'meta', 'funnelWatermark');
    const wmSnap = await getDoc(wmRef);                 // +1 okuma

    // İşaret hiç yoksa (ilk kurulum / eski sürümden geçiş) güvenli
    // varsayılan: son 30 günden başla. Böylece ilk çalıştırmada da
    // 5 yıl geriye gidip devasa bir okuma yapılmaz.
    const baslangicTarih = wmSnap.exists()
      ? _istanbulTarihEkle(wmSnap.data().sonTamamlananTarih, 1)
      : _istanbulTarihEkle(bugunStr, -30);

    // Bugüne kadar olan gerçek boşluk (gün sayısı), üst sınırla sınırlı.
    const bosluk = Math.round((new Date(bugunStr) - new Date(baslangicTarih)) / 86400000);
    const gunSayisi = Math.max(0, Math.min(bosluk, maxCatchUp));
    ...
    for (let i = 0; i < gunSayisi; i++) {
      isler.push(_pushDailyFunnelSummary(_istanbulTarihEkle(baslangicTarih, i)));
    }
    // Sessiz — hepsi paralel, hata olsa devam
    const results = await Promise.allSettled(isler);
    const yazilan = results.filter(r => r.status === 'fulfilled').length;

    // İşareti sadece gerçekten taranan son güne taşı (maxCatchUp'a takıldıysa
    // tamamı bitmemiş olabilir — kalanı bir sonraki çalıştırma tamamlayacak).
    const yeniSonTarih = _istanbulTarihEkle(baslangicTarih, gunSayisi - 1);
    await setDoc(wmRef, { sonTamamlananTarih: yeniSonTarih, guncellenme: serverTimestamp() }, { merge: true }); // +1 yazma

    console.log(`✅ _ensureSummariesExist: ${gunSayisi} gün tarandı (${baslangicTarih} → ${yeniSonTarih}).`);
    localStorage.setItem(KILIT_ANAHTARI, '1');
  } catch (e) {
    // ✅ Rev 8.3: Bu hata genelde _initFirebaseWatchdog()'un
    // disableNetwork()→enableNetwork() döngüsüyle çakışan ~1-2 sn'lik
    // pencerede oluşur (bkz. "client is offline"), gerçek bir sorun
    // değildir. Bir kere, kısa bir gecikmeyle sessizce tekrar dene;
    // yine olmazsa normal davranışa dön (kilit koyma, bir sonraki
    // açılışta tekrar denensin).
    const offlineHatasi = /client is offline/i.test(e?.message || '');
    if (offlineHatasi && !_ensureSummariesExist._retried) {
      _ensureSummariesExist._retried = true;
      console.warn('_ensureSummariesExist: geçici "offline" hatası, 3 sn sonra tekrar denenecek.');
      setTimeout(() => { _ensureSummariesExist._retried = false; _ensureSummariesExist(maxCatchUp); }, 3000);
      return;
    }
    console.warn('_ensureSummariesExist:', e);
    // Kilidi koymuyoruz — hata varsa yarın (ya da bir sonraki açılışta) tekrar denensin.
  }
}

// ── Özet Okuyucu ──────────────────────────────────────────────
// Tarih aralığı için daily_funnel_summary dokümanlarını çeker.
// 1 yıllık analiz = max 365 doküman okuma (çok verimli).
async function _fetchDailySummaries(baslangicTarih, bitisTarih) {
  const q = query(
    collection(_db, 'daily_funnel_summary'),
  ...
  const snap = await getDocs(q);
  const sonuc = [];
  snap.forEach(d => sonuc.push(d.data()));
  return sonuc;
}

// ── Hybrid Veri Çekici ────────────────────────────────────────
// Bugün/dün → ham funnel_logs (henüz özet yok)
// Önceki günler → daily_funnel_summary (özet)
// Özet dokümanlarındaki oturumlar[] dizisini
// _renderFunnelHTML'in beklediği log formatına dönüştürür.
async function _fetchFunnelHybrid(baslangicTarih, bitisTarih) {
  // ✅ DÜZELTME: setTarihAralik ile AYNI "bugün" tanımı (Istanbul string aritmetiği)
  const bugun = _bugunIstanbul();
  const dun   = _istanbulTarihEkle(bugun, -1);

  // ── A: Bugün / Dün için ham log çek (henüz özet yok) ────────
  const hamLogs = [];
  const hamGunler = [bugun, dun].filter(t => t >= baslangicTarih && t <= bitisTarih);
  if (hamGunler.length > 0) {
    const hamBas = new Date(hamGunler[hamGunler.length - 1] + 'T00:00:00+03:00');
    const hamBit = new Date(hamGunler[0] + 'T23:59:59+03:00');
    try {
      const q = query(
        collection(_db, 'funnel_logs'),
        where('ts', '>=', hamBas),
        where('ts', '<=', hamBit),
        orderBy('ts', 'desc'),
        limit(1000) // Bugün+dün için 1000 yeterli
      );
      const snap = await getDocs(q);
      snap.forEach(d => hamLogs.push({ ...d.data(), _kaynak: 'ham' }));
      console.log(`🔍 Hybrid ham log: [${hamBas.toISOString()} → ${hamBit.toISOString()}] → ${snap.size} kayıt bulundu.`);
    } catch (e) {
      // ⚠️ Artık sessizce yutmuyoruz — composite index hatası, izin hatası
      // veya başka bir Firestore sorunu konsolda görünür hale gelir.
      console.error('❌ Hybrid ham log çekme hatası (funnel_logs):', e.code || e.message, e);
    }
  } else {
    console.log(`🔍 Hybrid: [${baslangicTarih} → ${bitisTarih}] aralığı bugün/dün dışında, ham log atlandı.`);
  }

  // ── B: Önceki günler için özet çek ──────────────────────────
  // Özet aralığı: baslangic → dün-1 (dün zaten hamda var)
  const ozetBitis = _istanbulTarihEkle(bugun, -2);
  const ozetLogs  = [];
  if (baslangicTarih <= ozetBitis) {
    const summaries = await _fetchDailySummaries(baslangicTarih, ozetBitis);
    summaries.forEach(s => {
      if (s.bos) return; // Boş gün placeholder
      (s.oturumlar || []).forEach(o => {
        // Her personel-gün özetini, render fonksiyonunun beklediği tekil
        // log formatına dönüştür. _isAggregate=true → hesaplama dalları bunu bilir.
        ozetLogs.push({
          _kaynak:         'ozet',
          _isAggregate:    true,
          tarih:           s.tarih,
          ...
          personelAd:      o.personelAd,
          funnelRol:       o.funnelRol,
          magazaTipi:      o.magazaTipi,
          // Sonuç alanları (tekil log formatı uyumluluğu için en yüksek çıktıyı ver)
          sonuc:           o.satis > 0 ? 'satis' : o.kacti > 0 ? 'kacti' : 'bekliyor',
          neden:           '',
          // Aggregate sayılar — hesaplama katmanı _isAggregate true'sa bunları kullanır
          _satis:          o.satis,
          _kacti:          o.kacti,
          _bekliyor:       o.bekliyor,
          _hareketsizlik:  o.hareketsizlik || 0,
          _oturumSayisi:   o.oturumSayisi,
          // Metrikler
          toplamTutar:     o.toplamTutar   || 0,
          toplamSure:      o.toplamSure    || 0,
          derinlik:        o.oturumSayisi  || 0,
          benzersizUrun:   0,
          benzersizBlurSayisi: o.toplamBlur > 0 ? Math.round(o.toplamBlur / Math.max(o.oturumSayisi, 1)) : 0,
          bakilanFiyatlar: [],
          blurUrunListesi: (o.topBlurUrunler || []).map(u => u.urun),
          urunler:         [],
          intentLevel:     o.intentL3Plus > 0 ? 3 : 1,
          sepetKategorisi: o.altinSepet  > 0 ? 'Altin'
                         : o.gumusSepet > 0 ? 'Gumus' : 'Standart',
          bundleVarMi:     o.bundleFirsat  > 0,
          bundleYapildi:   o.bundleYapilan > 0,
          saat:            -1, // Özette saat dağılımı ayrı tutulur
          _saatDagilimi:   o.saatDagilimi  || {},
          _kacisNedenleri: o.kacisNedenleri || {},
          _topBlurUrunler: o.topBlurUrunler || [],
          indirimVarMi:    false,
        });
      });
    });
  ...
  // ── C: Birleştir ve döndür ───────────────────────────────────
  const tumLogs = [...hamLogs, ...ozetLogs];
  tumLogs._tavanaTakildi = false; // Artık tavan yok
  return tumLogs;
}

// Geriye dönük uyumluluk takma adı (eski çağrılar için)
async function _fetchFunnelLogsPaged(baseConstraints) {
  // Bu fonksiyon artık doğrudan çağrılmıyor.
  // loadFunnelAnaliz hybrid sistemi kullanıyor.
  console.warn('_fetchFunnelLogsPaged eskidi, _fetchFunnelHybrid kullanın.');
  return [];
}
...
async function loadFunnelAnaliz(gunAralik = null, force = false) {
  ...
  // Cooldown kaldırıldı — aggregation sistemi çok hızlı (eski 5 dk bekleme gereksiz)
  // Ardışık hızlı tıklamalar için sadece reqToken koruması yeterli.
  const simdi = Date.now();
  
  // ✅ DÜZELTME: Eski "zaten çalışıyorsa hiç başlama" engeli kaldırıldı.
  // Bu engel, hızlı ardışık filtre tıklamalarında (örn. 30 gün → 90 gün → Özel)
  // sonraki tıklamaların sorgusunu hiç başlatmadan iptal ediyor, ekran butonları
  // güncellenmiş görünse de rakamlar ÖNCEKİ filtreden kalıyordu.
  // Artık her çağrı kendi kimliğini (token) alır; render anında sadece EN SON
  // başlatılan istek ekrana yazar — daha eski biri tamamlanırsa sessizce yutulur.
  const _myReqToken = ++_funnelReqToken;
  _isFunnelLoading = true;
  _lastFunnelLoadTime = simdi;
  
  cont.innerHTML = '<div class="admin-empty" style="padding:24px">⏳ Analiz yükleniyor…</div>';
  ...
  if (tarihBas && tarihBit) {
    ...
    // Default: son 90 gün (artık özet tabanlı — hızlı ve limitssiz)
    const g = gunAralik || 90;
    const _defBugun = new Date();
    const _defBas   = new Date(_defBugun.getTime() - g*86400000);
    limitDate    = _defBas; // hybrid fetcher artık bunu kullanmıyor ama uyumluluk için bırak
    limitDateEnd = null;
    gosterimLabel = `Son ${g} gün`;
    gunAralik = g;
    // Dataset'e yaz ki render tarafı bilsin
    cont.dataset.tarihBas = _istanbulTarih(_defBas);
    cont.dataset.tarihBit = _istanbulTarih(_defBugun);
  ...
  // ✅ DÜZELTME: aktifFiltre/aktifMagaza artık sorgudan ÖNCE okunuyor; böylece
  // "veri yok" durumunda da doğru/aktif filtre paneli render edilebiliyor.
  let aktifFiltre = cont.dataset.funnelFiltre || 'saha';
  const aktifMagaza = cont.dataset.magazaFiltre || 'hepsi';

  try {
    // ── Hybrid veri çekici: bugün/dün ham log + öncesi özet dokümanlar ──
    // Artık startAfter sayfalama yok; tavan yok; yıllık/çok yıllık analiz sorunsuz.
    const allLogs = await _fetchFunnelHybrid(
      cont.dataset.tarihBas,
      cont.dataset.tarihBit || _istanbulTarih(new Date())
    );

    // ✅ DÜZELTME: Bu sırada kullanıcı başka bir filtreye tıklayıp daha yeni
    // bir istek başlattıysa, bu eski (bayat) isteğin sonucu ekrana YAZILMAZ.
    if (_myReqToken !== _funnelReqToken) return;

    if (!allLogs.length) {
      // ✅ DÜZELTME: Filtre paneli SİLİNMİYOR — kullanıcı her zaman tarih/rol/
      // mağaza filtresini değiştirebilir, ekranda kilitli kalmaz.
      cont.innerHTML = _renderFunnelFilterPanelHTML(cont, aktifFiltre, 0, gosterimLabel)
        + `<div class="admin-empty" style="margin-top:10px">📭 ${gosterimLabel} için veri yok.<br><span style="font-size:.72rem;color:var(--text-3)">Sepet kapatılınca burada görünecek.</span></div>`;
      return;
    }
    ...
    if (logs.length === 0) {
      // ✅ DÜZELTME: Burada da filtre paneli korunuyor — "Destek" gibi boş
      // sonuç veren bir filtrede artık ekran kilitlenmiyor, kullanıcı aynı
      // panelden tek tıkla başka bir role/tarihe geçebiliyor.
      cont.innerHTML = _renderFunnelFilterPanelHTML(cont, aktifFiltre, 0, gosterimLabel)
        + `<div class="admin-empty" style="margin-top:10px">📭 "${aktifFiltre === 'saha' ? '👷 Saha' : aktifFiltre === 'destek' ? '🖥 Destek' : aktifFiltre === 'admin' ? '👑 Admin' : '🌐 Tümü'}" filtresinde veri yok.<br><span style="font-size:.72rem;color:var(--text-3)">Yukarıdaki butonlardan farklı bir filtre deneyin.</span></div>`;
      return;
    }
    ...
    // ── GENEL SAYILAR ──────────────────────────────────────────
    // Aggregate-aware: özet satırlarında _satis/_kacti/_hareketsizlik kullan
    const HAREKETSIZLIK_NEDEN = 'Hareketsizlik (Arka Plan)';
    function _logSatis(l)        { return l._isAggregate ? (l._satis        || 0) : (l.sonuc === 'satis' ? 1 : 0); }
    function _logKacti(l)        { return l._isAggregate ? (l._kacti        || 0) : (l.sonuc === 'kacti' && l.neden !== HAREKETSIZLIK_NEDEN ? 1 : 0); }
    function _logHareketsiz(l)   { return l._isAggregate ? (l._hareketsizlik|| 0) : (l.sonuc === 'kacti' && l.neden === HAREKETSIZLIK_NEDEN ? 1 : 0); }
    function _logOturum(l)       { return l._isAggregate ? (l._oturumSayisi || 1) : 1; }

    const totN = logs.reduce((s,l) => s + _logOturum(l), 0);
    const totS = logs.reduce((s,l) => s + _logSatis(l), 0);
    const totK = logs.reduce((s,l) => s + _logKacti(l), 0);
    const totHareketsiz = logs.reduce((s,l) => s + _logHareketsiz(l), 0);

    // Gerçek dönüşüm oranı (hareketsizlik hariç)
    const donusumGercek = totN === 0 ? 0 : ((totS / totN) * 100).toFixed(1);

    // ── MOMENTUM ──────────────────────────────────────────────
    const s7S = son7Logs.reduce((s,l) => s + _logSatis(l), 0);
    const o7S = onc7Logs.reduce((s,l) => s + _logSatis(l), 0);
...
logs.forEach(l => {
  ...
  const p = pMap[eid];
  // Aggregate-aware sayaçlar
  const _oturum  = _logOturum(l);
  const _lSatis  = _logSatis(l);
  const _lKacti  = _logKacti(l);
  p.toplam   += _oturum;
  p.satis    += _lSatis;
  p.kacti    += _lKacti;
  ...
  p.blurToplam      += l._isAggregate
    ? (l.benzersizBlurSayisi || 0) * _oturum
    : (l.bakilanFiyatlar || []).length;
  if (l.bundleVarMi)  { p.bundleFirsat += _oturum; if(l.bundleYapildi) p.bundleYapilan += _oturum; }
  // L3 pazarlık analizi (intentLevel >= 3 → abaküs açıldı)
  if ((l.intentLevel || 0) >= 3) {
    p.l3Giris += _oturum;
    p.l3Satis += _lSatis;
    p.l3Kacti += _lKacti;
    p.l3Ciro  += _lKacti > 0 ? (l.toplamTutar || 0) : 0;
  }
  const k = l.sepetKategorisi||'Standart';
  if (k==='Altin') p.altin += _oturum; else if(k==='Gumus') p.gumus += _oturum; else p.standart += _oturum;
  // Saatlik grafik: özet satırlarında _saatDagilimi kullan
  if (l._isAggregate && l._saatDagilimi) {
    Object.entries(l._saatDagilimi).forEach(([h, n]) => {
      const hi = parseInt(h, 10);
      if (hi >= 0 && hi <= 23) {
        saatSatis[hi] += _lSatis > 0 ? Math.round(_lSatis * n / Math.max(_oturum,1)) : 0;
        saatKacti[hi] += _lKacti > 0 ? Math.round(_lKacti * n / Math.max(_oturum,1)) : 0;
      }
    });
  } else {
    const h = l.saat ?? -1;
    ...
    // ── L3 GLOBAL İSTATİSTİKLER (aggregate-aware) ──────────────
    const l3Toplam    = logs.reduce((s,l) => s + ((l.intentLevel||0)>=3 ? _logOturum(l) : 0), 0);
    const l3Satis     = logs.reduce((s,l) => s + ((l.intentLevel||0)>=3 ? _logSatis(l)  : 0), 0);
    // Hareketsizlik kaçışını gerçek L3 kaçışından ayır
    const l3Kacti          = logs.reduce((s,l) => s + ((l.intentLevel||0)>=3 ? _logKacti(l)       : 0), 0);
    const l3KactiHareketsiz= logs.reduce((s,l) => s + ((l.intentLevel||0)>=3 ? _logHareketsiz(l)  : 0), 0);
...
<div data-personel-id="1"
  ...
  <div style="padding:12px 14px">
    ...
    const _funnelStats = {
      ...
      aktifMagaza,
      gosterimLabel,
      // ✅ YENİ: Güvenlik tavanına (FUNNEL_MAX_PAGES) ulaşıldıysa kullanıcıyı uyar
      tavanaTakildi: !!allLogs._tavanaTakildi
    };
    window._funnelCache = _funnelStats; // paylasBilgi doğrudan okur
    // ✅ DÜZELTME: Hesaplama sırasında (örn. urunler.json fetch'i) geçen
    // sürede daha yeni bir filtre isteği başlamış olabilir — son kontrol.
    if (_myReqToken !== _funnelReqToken) return;
    _renderFunnelHTML(cont, aktifFiltre, _funnelStats);

        console.log(`✅ Funnel analiz tamamlandı: ${totN} oturum işlendi (ham+özet hybrid).`);

  } catch(e) {
    console.error('loadFunnelAnaliz:', e);
    // ✅ DÜZELTME: Hata durumunda da filtre paneli korunuyor; ayrıca bu
    // sırada daha yeni bir istek başladıysa bayat hata mesajını basma.
    if (_myReqToken === _funnelReqToken) {
      cont.innerHTML = _renderFunnelFilterPanelHTML(cont, aktifFiltre, 0, gosterimLabel || '')
        + `<div class="admin-empty" style="color:#dc2626;margin-top:10px">⚠️ Veri çekilemedi: ${e.message}</div>`;
    }
  } finally {
...
// ═══════════════════════════════════════════════════════════════
// ✅ DÜZELTME: Filtre paneli (tarih/rol/mağaza butonları) artık ayrı bir
// fonksiyonda üretiliyor. Önceden bu HTML doğrudan _renderFunnelHTML içinde
// üretiliyordu ve "veri yok" durumlarında cont.innerHTML komple silinince bu
// butonlar da yok oluyor, kullanıcı filtreyi geri değiştirecek hiçbir kontrol
// bulamıyordu (ör. "Destek" filtresinde sıkışma sorunu). Artık "veri yok"
// ekranı da bu paneli koruyarak gösteriliyor.
function _renderFunnelFilterPanelHTML(cont, aktifFiltre, totN, gosterimLabel) {
  const aktifMagaza   = cont.dataset.magazaFiltre || 'hepsi';
...
function _renderFunnelHTML(cont, aktifFiltre, s) {
  const { gunAralik, gosterimLabel,
    ...
    funnelBlur, funnelSepet, funnelL3, funnelTeklif, funnelSatis,
    _tavsiyeListesi, _tavsiyeKurallari, tavanaTakildi } = s;

  cont.innerHTML = _renderFunnelFilterPanelHTML(cont, aktifFiltre, totN, gosterimLabel) + `
      <!-- Genel Özet -->
...
window.openAdmin = async function() {
  ...
  try {
    // ✅ Rev 8.0: panel açılırken güncel veriyi çek (cache'ten değil)
    await Promise.allSettled([fetchProposalsOnce(), fetchSalesOnce(), fetchAnalyticsOnce()]);
  ...
  // ✅ Rev 8.0: 60 saniyelik otomatik yenileme KALDIRILDI — admin panel
  // günde bir kez, kısa süre açılıp kontrol ediliyor; panel açıkken bile
  // sürekli arkaplanda yenilemenin bir değeri yok, sadece gereksiz maliyet
  // olurdu. Panel açılışında zaten güncel veri çekiliyor (yukarıda);
  // panel açık kalırken tekrar güncellemek isterseniz "Yenile" butonu
  // (refreshAdminPanel()) kullanılır.
...
async function switchAdminTab(tab, force = false) {
  ...
  // ✅ Rev 8.1: sekme değişince ilgili veriyi tek seferlik tazele (cache'ten
  // hemen render + arkadan güncel veri gelince tekrar render — boş ekran yok).
  // Bu fonksiyon artık bir Promise DÖNDÜRÜYOR — refreshAdminPanel() gibi
  // çağıranlar KENDİ ayrı bir fetch yapmak yerine bunu bekleyebilir (aksi
  // halde aynı veri iki kez çekiliyordu — bkz. Rev 8.1 notu).
  let fetchPromise = Promise.resolve(true);
  if(tab==='proposals') {
    renderProposals(document.getElementById('admin-proposals-list'), true);
    renderArchivedProposals();
    fetchPromise = fetchProposalsOnce(force).then(ok => {
      renderProposals(document.getElementById('admin-proposals-list'), true);
      renderArchivedProposals();
      return ok;
    });
  ...
  if(tab==='siparis')  {
    ...
    fetchPromise = fetchSiparisOnce(force).then(ok => { renderSiparisPanel(); updateSiparisBadge(); return ok; });
  }
  if(tab==='sepetler') {
    renderSepetDetay();
    fetchPromise = fetchProposalsOnce(force).then(ok => { renderSepetDetay(); return ok; });
  }
  if(tab==='personel') {
    renderAdminUsers();
    fetchPromise = Promise.allSettled([fetchProposalsOnce(force), fetchSalesOnce(force), fetchAnalyticsOnce(force)])
      .then(results => { renderAdminUsers(); return results.some(r => r.status==='fulfilled' && r.value !== false); });
  }
  if(tab==='analiz')   {
    // Analiz sekmesi: Funnel + Ürün Popülerliği + Uyuyan Stok
    // Eksik günlük özetleri arka planda lazy oluştur (sessiz, hata olsa devam)
    // ✅ Rev 7.1: watermark tabanlı — günde 1 kez çalışır, sadece gerçek
    // boşluğu tarar, hiçbir gün kalıcı olarak atlanmaz (bkz. fonksiyon tanımı).
    _ensureSummariesExist().catch(() => {});
    loadFunnelAnaliz();
    renderAdminProducts();
  ...
  if(tab==='overview' || !tab) {
    fetchPromise = Promise.allSettled([fetchProposalsOnce(force), fetchSalesOnce(force), fetchAnalyticsOnce(force)])
      .then(async results => { await renderAdminPanel(); return results.some(r => r.status==='fulfilled' && r.value !== false); });
  }
  return fetchPromise;
}
...
async function renderAdminPanel() {
  ...
  const today=new Date().toISOString().split('T')[0];
  // Özet modeli: admin panelinde "bugün" yerine "dün" gösterilir
  // (bugünün özeti gece oluştuğundan, sabah açıldığında dün en güncel veridir)
  const _dunDate = new Date(); _dunDate.setDate(_dunDate.getDate()-1);
  const dun = _dunDate.toISOString().split('T')[0];
  ...
  // Dünkü login sayısı (özet modeli: dün en güncel özettir)
  const dunData=data[dun]||{};
  let dunLogins=0; Object.values(dunData).forEach(r=>dunLogins+=r.logins||0);
  // Bugün de varsa ekle (ham veri, henüz özete girmedi)
  const todayData=data[today]||{};
  let todayLogins=0; Object.values(todayData).forEach(r=>todayLogins+=r.logins||0);

  // Dün aktif kullanıcı (login yapan)
  const dunActive=Object.keys(dunData).filter(u=>(dunData[u].logins||0)>0).length;
  const todayActive=Object.keys(todayData).filter(u=>(todayData[u].logins||0)>0).length;

  document.getElementById('stat-logins').innerHTML    = `${tL}<span class="stat-today">+${dunLogins||todayLogins} dün</span>`;
  ...
  // Personel bugün
  renderPersonelBugun(data, dun, today); // dün birincil, bugün fallback
...
function renderPersonelBugun(data, today, bugunFallback) {
  const el = document.getElementById('admin-personel-bugun');
  if(!el) return;
  // Dünkü veri tercih edilir; yoksa bugünkü ham veriye düş (henüz özetlenmemiş)
  const todayData = Object.keys(data[today]||{}).length > 0
    ? data[today]
    : (bugunFallback ? data[bugunFallback] || {} : {});

  // Veri yoksa son 7 günün verilerini göster
  if(Object.keys(todayData).length === 0) {
    ...
    const html = `
        ...
        <table style="width:100%; border-collapse:collapse; font-size:.75rem">
          <thead>
            <tr style="background:var(--surface-2); border-bottom:2px solid var(--border)">
              ...
              <th style="padding:8px 6px; text-align:center" title="Sepet oturumu sayısı">🛒 İşlem</th>
              <th style="padding:8px 6px; text-align:center">Teklif</th>
              <th style="padding:8px 6px; text-align:center">Satış</th>
              <th style="padding:8px 6px; text-align:center" title="Teklif → Satış dönüşüm oranı">🎯 Dön.</th>
              <th style="padding:8px 6px; text-align:center" title="Bekliyor / Kapatılmamış teklif sayısı — düşük iyi">😴 Unk.</th>
          ...
          <tbody>
            ${sortedUsers.map(user => {
              ...
              const activityScore = (user.logins||0) + (user.basketSessions||0)*0.5 + (user.proposals||0)*2;
              const scoreColor = activityScore===0?'#94a3b8':activityScore<3?'#f59e0b':'#16a34a';
              const cr = user.proposals > 0 ? ((user.sales/user.proposals)*100).toFixed(0) : 0;
              const crColor = cr>=50?'#16a34a':cr>=25?'#d97706':'#dc2626';
              const forgot = proposals.filter(p=>p.user===user.email && p.durum==='bekliyor').length;
              const forgotColor = forgot===0?'#16a34a':forgot<=2?'#d97706':'#dc2626';
              return `
              <tr style="border-bottom:1px solid var(--border)">
                ...
                <td style="padding:8px 6px; text-align:center; font-weight:700; color:var(--red)">${user.basketSessions||0}</td>
                <td style="padding:8px 6px; text-align:center">${user.proposals||0}</td>
                <td style="padding:8px 6px; text-align:center">${user.sales||0}</td>
                <td style="padding:8px 6px; text-align:center; font-weight:700; color:${crColor}">%${cr}</td>
                <td style="padding:8px 6px; text-align:center; font-weight:700; color:${forgotColor}" title="${forgot} bekliyor teklif">${forgot===0?'✓':forgot}</td>
      ...
      <div style="font-size:.65rem; color:var(--text-3); margin-top:8px">ℹ️ Bugün veri yok, son 7 gün · 🎯 Dön.=Teklif→Satış oranı · 😴 Unk.=Kapatılmamış bekliyor</div>
    `;
    el.innerHTML = html;
    return;
  }
  ...
  const html = `
    <div class="admin-section-header" style="margin-bottom:12px">📈 Personel Performansı (Dün — ${today})</div>
    <div style="overflow-x:auto">
      <table style="width:100%; border-collapse:collapse; font-size:.75rem">
        <thead>
          <tr style="background:var(--surface-2); border-bottom:2px solid var(--border)">
            ...
            <th style="padding:8px 6px; text-align:center">Satış</th>
            <th style="padding:8px 6px; text-align:center" title="Teklif → Satış dönüşüm oranı">🎯 Dön.</th>
            <th style="padding:8px 6px; text-align:center" title="Açık bekliyor teklif sayısı — 0 ideal">😴 Unk.</th>
    ...
    <div style="font-size:.65rem; color:var(--text-3); margin-top:6px">🎯 Dön.=Teklif→Satış · 😴 Unk.=Bekliyor teklif adedi (0=mükemmel, kırmızı=takip gerekli)</div>
  `;
  el.innerHTML = html;
}
...
// ✅ Rev 8.3 — KRİTİK DÜZELTME (okuma/maliyet patlamasının asıl kaynağı):
// Eskiden her girişte YENİ bir doküman açılıyordu (ses_email_timestamp)
// ve kullanıcı "Çıkış" butonuna basmadan sekmeyi/tarayıcıyı kapatırsa
// (çoğu personel öyle yapar) o doküman SONSUZA KADAR kalıyordu — hiçbir
// temizlik yoktu. Aylar içinde `sessions` koleksiyonu yüzlerce/binlerce
// yetim kayda büyüyor, admin panelindeki sınırsız/filtresiz
// onSnapshot(collection(_db,'sessions')) dinleyicisi de HER sayfa
// yüklemesinde (hard-refresh dahil, önceden persistence de kapalıydı)
// bunların TAMAMINI server'dan yeniden okuyordu — 7.4K+/dk okuma
// patlamasının ana kaynağı buydu. Artık email başına TEK doküman var
// (doc id = email), üzerine yazılıyor (upsert). Koleksiyon boyutu artık
// en fazla personel sayısı kadar, otomatik büyümüyor.
async function _checkAndRegisterSession(email, rol) {
  if(!_db || rol==='admin') return; // Admin kısıtlanmaz
  try {
    const sessionRef = doc(_db,'sessions', email);
    // Bu login'e özel cihaz kimliği. Aynı email başka bir cihazda/sekmede
    // tekrar giriş yaparsa doküman üzerine yazılır ve deviceId değişir;
    // eski sekme bunu _startSessionListener() içinde fark edip kendini
    // otomatik kapatır (aşağıda).
    const deviceId = 'dev_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8);
    localStorage.setItem('_aygun_device_id', deviceId);

    const twoMinAgo = new Date(Date.now() - 120000).toISOString();
    // Aynı email için hâlâ aktif (son 2 dk içinde heartbeat atmış)
    // başka bir cihaz var mı?
    const existingSnap = await getDoc(sessionRef);
    if (existingSnap.exists()) {
      const s = existingSnap.data();
      if (s.lastSeen > twoMinAgo && s.deviceId && s.deviceId !== deviceId) {
        const warn = await ayConfirm(
          '⚠️ Bu hesap başka bir cihazda zaten açık!\n' +
          'Cihaz: ' + (s.device || 'Bilinmiyor') + '\nDevam etmek istiyor musunuz?'
        );
        if (!warn) { currentUser = null; localStorage.removeItem('aygun_user'); return; }
      }
    }
    // Oturumu kaydet — tek doküman üzerine yazılır (yeni doküman AÇILMAZ)
    ...
    // Heartbeat — her 60 sn'de güncelle (appVersion de tazelenir —
    // gece yarısı SW güncellemesi olsa da bir sonraki heartbeat'te
    // yeni sekme zaten yeni build'i yükleyeceği için burada ekstra
    // maliyet oluşturmaz, aynı yazmanın üstüne bindirilir)
    ...
    window._sessionHeartbeat = setInterval(()=>{
      ...
      setDoc(sessionRef,{lastSeen:new Date().toISOString(), appVersion: APP_BUILD_VERSION},{merge:true}).catch(()=>{});
    },60000);
...
function _startSessionListener() {
  ...
  // Personel: kendi session belgesini (doc id = email) dinle —
  // forceLogout VE cihaz devralma (başka yerden giriş) kontrolü
  if(!isAdmin()) {
    const myEmail = currentUser?.Email;
    if(myEmail) {
      if(window._mySessionUnsub) window._mySessionUnsub();
      window._mySessionUnsub = onSnapshot(
        doc(_db, 'sessions', myEmail),
        (snap) => {
          if(!snap.exists()) {
            if(currentUser) ayAlert('⚠️ Oturumunuz sonlandırıldı. Lütfen tekrar giriş yapın.').then(() => logoutUser(true));
            return;
          }
          const s = snap.data();
          if(s.forceLogout === true) {
            if(currentUser) ayAlert('⚠️ Yönetici tarafından oturumunuz kapatıldı. Lütfen tekrar giriş yapın.').then(() => logoutUser(true));
            return;
          }
          // ✅ Rev 8.3: Aynı hesapla başka bir cihazdan/sekmeden giriş
          // yapılınca doküman üzerine yazılır ve deviceId değişir —
          // eski sekme burada bunu fark edip nazikçe çıkış yapar.
          const myDeviceId = localStorage.getItem('_aygun_device_id');
          if(currentUser && myDeviceId && s.deviceId && s.deviceId !== myDeviceId) {
            ayAlert('⚠️ Hesabınız başka bir cihazda açıldı. Bu oturum kapatılıyor.').then(() => logoutUser(true));
          }
        },
  ...
  // Admin: tüm oturumları izle (Rev 8.3 sonrası en fazla personel
  // sayısı kadar doküman — önceden sınırsız büyüyordu)
...
function renderAdminUsers() {
  ...
  el.innerHTML = su.map(([email, s]) => {
    ...
    const userCR = s.proposals > 0 ? Math.round((s.sales/s.proposals)*100) : null;
    const crBadgeColor = userCR === null ? '' : userCR >= 50 ? '#16a34a' : userCR >= 25 ? '#d97706' : '#dc2626';
    const crBadgeBg = userCR === null ? '' : userCR >= 50 ? '#dcfce7' : userCR >= 25 ? '#fef3c7' : '#fef2f2';
    const isOnline = Object.values(activeSessions).some(sess => sess.email === email);
    const isToday  = s.lastSeen === today;
    const mySession = Object.values(activeSessions).find(sess => sess.email === email);
    // ✅ Rev 8.3 — bu kullanıcı şu an açıksa ve eski bir app.js build'i
    // çalıştırıyorsa (sessions dokümanındaki appVersion, mevcut
    // APP_BUILD_VERSION ile eşleşmiyorsa) burada görünür hale gelir —
    // artık "hard-refresh gerekiyor mu?" sorusunu tahmin etmenize gerek yok.
    // ✅ Rev 8.4 düzeltmesi: appVersion alanı HİÇ YOKSA (bu doküman
    // rev84 öncesi bir login ile açıldıysa — heartbeat sadece
    // {lastSeen} merge ettiği için appVersion'ı asla yazmaz) bu durum
    // bizzat en net "eski kod çalışıyor" sinyalidir. Önceki halde
    // `mySession?.appVersion &&` ön-koşulu bu durumu (alan undefined
    // olduğu için) yanlışlıkla GİZLİYORDU — rozet hiç çıkmıyordu.
    const staleBuild = isOnline && !!mySession && mySession.appVersion !== APP_BUILD_VERSION;
    ...
    return `<div class="user-row" style="border-left:3px solid ${isOnline?'#16a34a':isToday?'#f59e0b':'var(--border)'}">
      ...
      <div class="user-badges" style="display:flex;align-items:center;gap:4px;flex-wrap:wrap">
        ...
        ${userCR !== null ? `<span class="badge" style="background:${crBadgeBg};color:${crBadgeColor};font-weight:800" title="Teklif→Satış Dönüşüm Oranı">🎯%${userCR}</span>` : ''}
        ${pending ? `<span class="badge" style="background:#fef3c7;color:#92400e" title="${pending} açık bekliyor teklif">${pending}⏳</span>` : ''}
        ${staleBuild ? `<span class="badge" style="background:#fef2f2;color:#dc2626;font-weight:800" title="Bu cihaz eski bir sürüm çalıştırıyor (${mySession.appVersion || 'rev84 öncesi / bilinmiyor'}) — hard-refresh gerekiyor">⚠️ Eski sürüm</span>` : ''}
...
// ✅ Rev 8.7 — Toplu güncelleme: şu an çevrimiçi VE eski build çalıştıran
// TÜM oturumları tek tıkla zorla çıkışa uğratır (bkz. logoutUser'daki
// silent=true → location.reload() düzeltmesi). Bu, "personeli eski
// sürümden nasıl kurtarırım" sorusunun asıl pratik cevabıdır — rozet
// sadece kimin eski olduğunu gösterir, bu buton onları fiilen günceller.
async function adminForceLogoutAllStale() {
  if(!isAdmin()) return;
  try {
    // ✅ Rev 8.9 düzeltmesi: doküman YOLU için data() içindeki 'email'
    // alanına güvenmek yerine, koleksiyondaki GERÇEK doküman ID'sini
    // (Object.entries anahtarı) kullanıyoruz. Rev 8.3 öncesinden kalma
    // bazı eski/bozuk kayıtlarda 'email' alanı boş olabiliyordu — bu da
    // doc(_db,'sessions', undefined) çağrısında Firestore SDK'nın
    // "Cannot read properties of undefined (reading 'indexOf')"
    // hatasıyla patlamasına sebep oluyordu. Doküman ID'si ise her zaman
    // vardır (aksi halde zaten listede olamaz).
    const entries = Object.entries(window._fbSessions || {})
      .filter(([id, s]) => s && s.appVersion !== APP_BUILD_VERSION);
    if(entries.length === 0) {
      await ayAlert('✅ Şu an çevrimiçi kimse eski sürüm çalıştırmıyor.');
      return;
    }
    const names = entries.map(([id, s]) => (s.email || id || '?').split('@')[0]).join(', ');
    if(!(await ayConfirm(entries.length + ' kişi eski sürümde görünüyor: ' + names + '\nHepsi zorla çıkış yapıp sayfaları otomatik yenilenecek. Devam edilsin mi?'))) return;
    let ok = 0, failed = 0;
    for(const [id, s] of entries) {
      try {
        if(s && s.email) {
          // Normal (Rev 8.3+) kayıt: forceLogout bayrağını koy — canlı
          // sekme bunu yakalayıp kendini gerçekten reload edecek
          // (bkz. _startSessionListener + logoutUser(silent)).
          await updateDoc(doc(_db,'sessions', id), { forceLogout: true, forceLogoutAt: serverTimestamp() });
          setTimeout(async () => { try { await deleteDoc(doc(_db,'sessions', id)); } catch(e){} }, 5000);
        } else {
          // 'email' alanı olmayan bozuk/yetim eski kayıt — kimseye
          // bildirim gitmez (zaten kimse dinlemiyor), doğrudan temizle.
          await deleteDoc(doc(_db,'sessions', id));
        }
        ok++;
      } catch(e) { console.warn('adminForceLogoutAllStale — ' + id + ':', e); failed++; }
    }
    haptic(22);
    showToast('🔄 ' + ok + ' cihaz güncellemeye zorlandı' + (failed ? ' (' + failed + ' başarısız)' : '') + '.', failed ? 'warning' : 'success');
    renderAdminUsers();
  } catch(e) { console.warn('adminForceLogoutAllStale:', e); await ayAlert('İşlem başarısız: ' + e.message); }
}
...
// ─── ÇIKIŞ ────────────────────────────────────────────────────
// ✅ Rev 8.3: silent=true olduğunda (forceLogout / başka cihazdan giriş
// gibi otomatik tetiklenen çıkışlarda) onay penceresi sorulmaz ve — daha
// önemlisi — Firestore'daki session dokümanı SİLİNMEZ. Çünkü bu durumda
// doküman zaten ya admin'in forceLogout akışı tarafından 5 sn sonra
// silinecek ya da yeni cihazın deviceId'siyle güncellenmiş durumda;
// eski sekme kendi email'iyle silme çağrısı yaparsa yeni cihazın
// oturumunu da silmiş olur.
async function logoutUser(silent = false) {
  if(!silent) {
    haptic(22);
    if(!(await ayConfirm('Çıkış yapmak istediğinize emin misiniz?'))) return;
  }
  // Oturumu Firestore'dan temizle — sadece kullanıcının kendi isteğiyle
  // çıkış yaptığı normal akışta (silent değilken).
  if(_db && currentUser && !silent) {
    deleteDoc(doc(_db, 'sessions', currentUser.Email)).catch(()=>{});
  }
  // ✅ Rev 8.7 — KRİTİK DÜZELTME: silent=true (admin'in "zorla çıkış"
  // yaptırması VEYA başka cihazdan giriş nedeniyle oturumun elden
  // alınması) ile tetiklenen çıkışlarda, önceden sadece login ekranı
  // gösteriliyor, sayfa HİÇ yenilenmiyordu. Bu, cihaz eski bir app.js
  // build'inde donmuş kaldıysa, kullanıcı tekrar giriş yapsa bile aynı
  // eski koda geri dönmesi anlamına geliyordu — yani "zorla çıkış"
  // özelliği, personeli eski sürümden gerçekten KURTARMIYORDU.
  // Artık silent modda gerçek bir location.reload() yapılıyor: bu,
  // network-first kuralı sayesinde index.html + app.js'i sunucudan
  // taze çeker. Admin panelinden bir kullanıcıya "zorla çıkış"
  // uyguladığınızda artık o cihazı GERÇEKTEN güncel build'e taşımış
  // olursunuz — saatlik otomatik kontrolü beklemenize gerek kalmaz.
  if(silent) {
    try { showToast('Oturum sonlandırıldı, sayfa yenileniyor…', 'info'); } catch(_){}
    setTimeout(() => location.reload(), 800);
    return;
  }
  currentUser = null;
  localStorage.removeItem('aygun_user');
  localStorage.removeItem('_aygun_device_id');
...
Object.assign(window, {
  ...
  // ✅ Rev 9.2: openMessages daha önce hiç tanımlanmamıştı. #user-msg-bar
  // şu an .visible class'ı hiçbir yerden eklenmediği için görünmüyor
  // (display:none kalıyor), o yüzden bugüne kadar hata vermedi — ama
  // özellik ileride aktifleşirse ReferenceError fırlatırdı. Mesaj
  // ekranı henüz yapılmadığından güvenli bir stub bırakıldı.
});

function openMessages() {
  // Mesaj özelliği henüz uygulanmadı — bkz. Rev 9.2 notu yukarıda.
}
...
// ✅ Rev 9.2: window.renderKategoriGrid ataması kaldırıldı — renderKategoriGrid
// hiçbir yerde tanımlı değildi, bu satır her zaman boş bir no-op fonksiyon
// atıyordu (kod kirliliği, işlevsel etkisi yoktu).
