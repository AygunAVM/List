...
// Hangi ürünlere manuel fiyat müdahalesi yapılmış? Map<urunAdi, {eskiFiyat, yeniFiyat, not, kim, ts}>
  ...
  data.forEach(u => {
    ...
    tr.innerHTML = 
      ...
      (isSahaPersonel() || isDestek()
        ...
        : (() => {
            // Admin görünümü: Manuel fiyat override varsa ⚠️ ikonu + mavi font
            const _ov = window._adminOverrides || JSON.parse(localStorage.getItem('aygun_admin_overrides') || '{}');
            const urunAdi = u[urunKey] || '';
            const hasOverride = !!_ov[urunAdi];
            const priceStyle = hasOverride ? ' style="color:#2563eb;font-weight:700"' : '';
            const overrideBadge = hasOverride
              ? `<span class="_admin-override-badge" title="Manuel Fiyat — Not: ${(_ov[urunAdi]?.not||'').replace(/"/g,'&quot;')} (${(_ov[urunAdi]?.kim||'-').split('@')[0]})" style="margin-left:3px;font-size:.7em;cursor:help">⚠️</span>`
              : '';
            return (
              '<td class="td-price"' + priceStyle + '>' + fmt(u[kartKey]) + overrideBadge + '</td>' +
              '<td class="td-price"' + priceStyle + '>' + fmt(u['4T AWM']) + overrideBadge + '</td>' +
              '<td class="td-price"' + priceStyle + '>' + fmt(u[cekKey]) + overrideBadge + '</td>' +
              '<td class="td-price"' + priceStyle + '>' + fmt(u.Nakit) + overrideBadge + '</td>'
            );
          })()) +
...
logs.forEach(l => {
  ...
  const h = l.saat ?? -1;
  // Güvenli saat kontrolü: 'saat' alanı null/undefined/NaN ise -1 döner ve aşağıda filtrelenir.
  // serverTimestamp henüz resolve olmamış (null) loglar h===-1 ile saatlik grafiğe dahil edilmez.
  if (h >= 0 && h <= 23) { 
    ...
    // Her log kaydı = 1 benzersiz müşteri oturumu (funnel_logs'a her kapanışta 1 kayıt gider)
    // Bu nedenle count(logs) zaten session-based sayımdır — event-based değil.
    // Aynı oturum birden fazla kez sayılmasın diye personelId+tarih+saat üçlüsünden
    // benzersizlik sağlanır (nadir duplicate koruması).
    ...
    // Blur: benzersiz oturumlardan en az 1 fiyat açılmış olanlar
    const funnelBlur = _uniqueLogs.filter(l =>
      (l.benzersizBlurSayisi || 0) > 0 ||
      (l.blurUrunListesi || l.bakilanFiyatlar || []).length > 0
    ).length;

    // Sepet: benzersiz oturumlardan sepete en az 1 ürün eklenmiş olanlar
    // count(distinct sessionId) mantığı — 10 tıklama 1 oturum sayılır
    ...
    // Her ürün için yalnızca 1 öneri tutulur. Yeni bir kural daha yüksek
    // önceliğe sahipse eski öneriyi override eder (duplicate oluşmaz).
    // Öncelik sırası (yüksekten düşüğe):
    //   1. Dinamik if-else (davranışsal kural — en güncel)
    //   2. Özelleştirilebilir kural listesi (kural sırasına göre)
    // Aynı ürün için birden fazla kural tetiklendiğinde son tetiklenen
    // değil, öncelik sıralamasındaki ilk kural kazanır (priority-based).
    const _tavsiyeMap = new Map(); // key: u.ad (ürün adı), value: tavsiye objesi
    ...
    direktenDonenler.concat(sessizDegerler).concat(olduStok).forEach(u => {
      ...
      // Önce dinamik if-else mantığını dene (en yüksek öncelik)
      const dinamik = _dinamikOneri(u);
      if (dinamik) {
        // Ürün için henüz bir öneri yoksa veya mevcut öneri daha düşük öncelikliyse override et
        if (!_tavsiyeMap.has(u.ad)) {
          _tavsiyeMap.set(u.ad, { ...entry, kural: { ...dinamik, durum: dinamik.oneri }, _oncelik: 1 });
        }
        return; // Dinamik kural tetiklendi — özelleştirilebilir kurallara bakma
      }

      // Sonra özelleştirilebilir kurallara bak (daha düşük öncelik)
      _tavsiyeKurallari.filter(k => k.aktif).forEach(kural => {
        try {
          if (kural.test(u)) {
            // Aynı ürün için daha önce bir özelleştirilebilir kural kaydedilmemişse ekle
            // (Map sayesinde aynı ürünün üzerine yazılmaz — ilk eşleşen kural önceliklidir)
            if (!_tavsiyeMap.has(u.ad)) {
    ...
    // Map'ten diziye çevir, blur'a göre sırala, en fazla 15 öneri
...
window.adminHizliDuzenle = async function(urunAdi) {
  ...
  // ── Admin Override Map: localStorage'a kaydet ─────────────────
  // Bu map, ürün tablosunda fiyatın yanında ⚠️ ikonunu göstermek için kullanılır.
  // Böylece yönetici, "sistem fiyatı" ile "manuel fiyat" arasındaki farkı
  // Excel'e bakmadan anlık görebilir.
  try {
    const _overrideMap = JSON.parse(localStorage.getItem('aygun_admin_overrides') || '{}');
    _overrideMap[urunAdi] = {
      eskiFiyat: Number(String(eskiFiyat).replace(/\D/g,'')),
      yeniFiyat: Number(yeniFiyat.replace(/\D/g,'')),
      not,
      kim: currentUser?.Email || '-',
      ts: new Date().toISOString()
    };
    localStorage.setItem('aygun_admin_overrides', JSON.stringify(_overrideMap));
    window._adminOverrides = _overrideMap; // Bellek kopyası (filterData hızlı erişim için)
  } catch(e) { console.warn('Override map kaydedilemedi:', e); }

  // Fiyat gösterimini tablodan da yansıt (açık hücreleri güncelle)
  document.querySelectorAll('.td-price').forEach(cell => {
    const cellUrun = cell.dataset.urun;
    if (cellUrun === urunAdi && window._adminOverrides?.[urunAdi]) {
      if (!cell.querySelector('._admin-override-badge')) {
        const badge = document.createElement('span');
        badge.className = '_admin-override-badge';
        badge.title = `Manuel Fiyat — Not: ${not} (${currentUser?.Email?.split('@')[0]||'-'})`;
        badge.style.cssText = 'margin-left:4px;font-size:.65em;color:#2563eb;cursor:help';
        badge.textContent = '⚠️';
        cell.appendChild(badge);
        cell.style.color = '#2563eb'; // Manuel fiyat → mavi font
      }
    }
  });
