...
function calcAbakus() {
  ...
  // ── Pazarlık değerlerini innerHTML yenilenmeden ÖNCE kaydet ──────
  const _ekIndirimEl = document.getElementById('ab-ek-indirim');
  const _pazarlikEl  = document.getElementById('ab-pazarlik-notu');
  const _savedIndirim = _ekIndirimEl ? _ekIndirimEl.value : '';
  const _savedNot     = _pazarlikEl  ? _pazarlikEl.value  : '';
  ...
  zRows.forEach(satir => {
    TAK.forEach(td => {
      ...
      // Hassas taksit: tam bölüm + kalan kuruş ilk taksitte
      const _aylikTaban = td.n === 1 ? tahsilat : Math.floor(tahsilat / td.n);
      const _aylikKalan = td.n === 1 ? 0 : (tahsilat - _aylikTaban * td.n);
      const aylik = _aylikTaban + _aylikKalan; // Gösterimde ilk taksit (max fark 1₺)
if (!enKarliMap[td.n] || oran < enKarliMap[td.n].oran) {
  ...
  zRows.forEach(satir => {
    ...
    TAK.forEach(td => {
      ...
      const _zTaban = td.n === 1 ? tahsilatZ : Math.floor(tahsilatZ / td.n);
      const _zKalan = td.n === 1 ? 0 : (tahsilatZ - _zTaban * td.n);
      const aylik = _zTaban + _zKalan;
  ...
  // ── innerHTML sonrası: input değerlerini geri yükle ─────────────
  const newIndirimEl = resEl.querySelector('#ab-ek-indirim') || document.getElementById('ab-ek-indirim');
  const newNot       = resEl.querySelector('#ab-pazarlik-notu') || document.getElementById('ab-pazarlik-notu');
  if (newIndirimEl && _savedIndirim !== '') { newIndirimEl.value = _savedIndirim; }
  if (newNot       && _savedNot     !== '') { newNot.value = _savedNot; }

  // Uyarı elementini güncelle (innerHTML sonrası yeniden oluştu)
  const uyariEl = resEl.querySelector('#ab-ek-indirim-uyari') || document.getElementById('ab-ek-indirim-uyari');
  if (uyariEl) uyariEl.style.display = (ekIndirim > 0 && ekIndirim > nakit * 0.07) ? 'inline' : 'none';
