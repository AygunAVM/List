...
// Artık: (a) günde sadece 1 kez çalışır (localStorage kilidi),
// (b) varsayılan aralık 365 → 30 güne indirildi — geçmiş özetler
// kalıcı olduğundan zaten sadece "dün oluştu mu" kontrolüne ihtiyaç var.
async function _ensureSummariesExist(sonKacGun = 30) {
  if (!currentUser || !_db) return;
  ...
  const isler = [];
  for (let i = 1; i <= sonKacGun; i++) {
    const tarih = _istanbulTarihEkle(bugunStr, -i);
    isler.push(_pushDailyFunnelSummary(tarih));
  }
  ...
  console.log(`✅ _ensureSummariesExist: ${sonKacGun} gün kontrol edildi.`);

  // Kilidi başarılı denemeden sonra koy — yarım kalan/hatalı bir çalışma
  // günü kilitleyip eksik özet bırakmasın.
  try { localStorage.setItem(KILIT_ANAHTARI, '1'); } catch(e) {}
}
...
function switchAdminTab(tab) {
  ...
  if(tab==='analiz')   {
    ...
    // ✅ Rev 7.0: artık günde 1 kez çalışır (localStorage kilidi, bkz. fonksiyon
    // tanımı) ve varsayılan 30 gün taranır — tekrar tekrar 365 okuma yapılmaz.
