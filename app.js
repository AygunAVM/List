...
function toggleCampaign(idx, campIdx) {
  ...
  // ═══════════════════════════════════════════════════════════
  if (camp.rol && camp.rol !== 'ANY') {
    const grupAdi = camp.grup;
    const ayniHarfVar = basket.some((b, bi) => {
      ...
      return Object.entries(b._selectedCamps).some(([ci, sel]) => {
        ...
        return bc && bc.grup === grupAdi && bc.rol === camp.rol;
      });
    });
    if (ayniHarfVar) {
      _campToast(
...
function recalculateAllGroupCampaigns() {
  ...
  Object.entries(grupSecimler).forEach(([grup, secimler]) => {
    ...
    // Eşik: esik kadar FARKLI ürün + FARKLI harfler seçilmeli
    const secilenUrunler = [...new Set(secimler.map(s => s.basketIdx))];
    const secilenRoller  = secimler.map(s => s.camp.rol || 'ANY');
    const farkliRoller   = secilenRoller.every(r => r === 'ANY')
      ? true
      : new Set(secilenRoller.filter(r => r !== 'ANY')).size === secilenRoller.filter(r => r !== 'ANY').length;

    const esikSaglandi = secilenUrunler.length >= esik && farkliRoller;
    ...
    // Grup indirim tutarı = pill tutarı (tüm pill'ler aynı tutarı taşır)
    const grupTutar = Math.max(...secimler.map(s => s.camp.tutar || 0));
    if (grupTutar <= 0) return;

    // Orantılı dağıtım: ürün nakit fiyatına göre
    const grupToplamNakit = secilenUrunler.reduce((s, bi) => s + (basket[bi]?.nakit || 0), 0);

    if (grupToplamNakit <= 0) {
      // Eşit dağıt
      const esitPay = Math.round(grupTutar / secilenUrunler.length);
      secilenUrunler.forEach(bi => {
        basket[bi].itemDisc  = (basket[bi].itemDisc  || 0) + esitPay;
        basket[bi]._campDisc = (basket[bi]._campDisc || 0) + esitPay;
      });
    } else {
      let dagitilan = 0;
      secilenUrunler.forEach((bi, i) => {
        const urun    = basket[bi];
        const agirlik = urun.nakit / grupToplamNakit;
        const pay     = i === secilenUrunler.length - 1
          ? grupTutar - dagitilan              // son ürün yuvarlama farkını alır
