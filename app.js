...
function _renderFaturaTablosu(parsed) {
  ...
  basket.forEach((item, idx) => {
    ...
    const altEtiket = tip === 'proje' && item._projeNakit !== undefined
      ? `<span style="color:#15803d;font-weight:700">❖ Proje Fiyatı</span>`
  ...
  let odemeBadge = '';
  if (tip === 'proje') {
    odemeBadge = `<span style="background:#dcfce7;color:#15803d;border-radius:6px;padding:3px 9px;font-size:.63rem;font-weight:700">❖ ${projeLabel2 || 'Proje Fiyatı'}</span>`;
  } else if (tip === 'nakit') {
...
function buildPremiumPDF(docType, data) {
  ...
  const urunFaturaFiyatlari = urunler.map((u, idx) => {
    ...
    if (isNakit || isProje || data.odemeTipi === 'tek_cekim') return urunNakitler[idx];
  ...
  let odemeHTML = '';
  if (isProje) {
    // ❖ Proje: ürün fiyatları zaten proje fiyatı, toplam = proje toplamı, komisyon yok
    const _projeSatirInd = (listeToplam - indirimliToplam) > 0
    ...
    odemeHTML = `
      ...
      ${_projeSatirInd}
      <tr><td class="rl">Ödeme Şekli</td><td class="rr"><span class="badge-nakit">❖ ${projeLabel || 'Proje Fiyatı'}</span></td></tr>
