...
function vitrinUrunAra(query) {
  ...
  dropdown.innerHTML = hits.map(({ u, idx }) => {
    ...
    return `<div class="vitrin-dd-item"
      onmousedown="event.preventDefault();window.vitrinUrunSec(${idx},${JSON.stringify(ad)},${nakit},${JSON.stringify(not)})">
...
function renderTrendCards() {
  ...
  if (!vitrinAktif.length && !trendData.length) {
    // Fallback: prim sıralamasıyla göster
    const keys0 = allProducts.length ? Object.keys(allProducts[0]) : [];
    const urunKey = keys0.find(k => k.toLowerCase().replace(/\s/g,'') === 'urun') ||
    ...
    if (subEl) subEl.textContent = 'yüksek prim';
  ...
  if (!vitrinAktif.length && !trendData.length) {
    ...
    container.innerHTML =
      `<div style="grid-column:1/-1;display:flex;align-items:center;gap:8px;margin-bottom:4px">
        ...
        <span style="font-size:.60rem;color:var(--text-3)">prim sıralaması</span>
      </div>` +
      fallback.map(u => {
        const oi    = allProducts.indexOf(u);
        const ad    = u[urunKey] || '';
