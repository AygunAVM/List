...
const Basket = {
  ...
  totals() {
    return basket.reduce((t, i) => {
      ...
      t.awm   += i.awm   || 0;
      t.tek   += i.tek   || 0;
      // ❖ Proje fiyatı varsa, sonra manuel override, sonra ham nakit
      const efektif = i._projeNakit !== undefined ? i._projeNakit
                    : i._nakitOverride !== undefined ? i._nakitOverride
                    : (i.nakit || 0);
      t.nakit += efektif;
      return t;
...
async function _lazyMarkExpired(id) {
  ...
  localStorage.setItem('aygun_proposals', JSON.stringify(proposals));
  await fbUpdateProp(id, { durum: 'sureDoldu', archivedAt: proposals[idx].archivedAt });
}
...
function adminGreetingSave() {
  const inp = document.getElementById('greeting-input');
  if (!inp) return;
  const val = inp.value.trim();
  if (val) {
    localStorage.setItem(_GREETING_KEY, val);
  } else {
    localStorage.removeItem(_GREETING_KEY);
  }
  // Arama kutusunu hemen güncelle
  _startMotdTicker();
  ayAlert('✅ Karşılama metni güncellendi: "' + _getGreeting() + '"');
}
...
function renderCampaignPills(item, idx) {
  ...
  const summaryRow = secilenSayi > 0
    ? `<div style="display:flex;align-items:center;gap:5px;margin-top:3px">`
      + (projeAktifPill
        ? (() => {
            const projeInd = item._projeNakit !== undefined ? Math.round(item.nakit - item._projeNakit) : 0;
            const projeIndStr = projeInd > 0
              ? `-${projeInd >= 1000 ? (projeInd/1000).toFixed(projeInd%1000===0?0:1)+'k' : projeInd}₺`
              : fmt(item._projeNakit) + '₺';
            return `<span style="font-size:.57rem;background:#f3e8ff;color:#7c3aed;border-radius:3px;padding:1px 7px;font-weight:700">❖ ${projeIndStr}</span>`;
          })()
        : campDisc > 0
...
function _buildAdminCartHTML() {
  ...
  basket.forEach((item, idx) => {
    ...
    const efektifFiyat = projeAktif  ? item._projeNakit
                       : hasOverride ? item._nakitOverride
                       : item.nakit;
        ...
        ${projeAktif
          ? (() => {
              ...
              const projeIndStr = projeInd > 0
                ? `-${projeInd >= 1000 ? (projeInd/1000).toFixed(projeInd%1000===0?0:1)+'k' : projeInd}₺`
                : fmt(item._projeNakit);
              return `<span style="font-size:.60rem;color:#7c3aed;font-weight:700">❖ ${projeIndStr}</span>`;
            })()
