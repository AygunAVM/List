...
async function logSessionResult(sonuc, neden) {
  ...
  // Sepet kategorisi (Bundle puanlama)
  const sepetDerini = basket.length;
  const sepetKategorisi = sepetDerini >= 3 ? 'Altin' : sepetDerini === 2 ? 'Gumus' : 'Standart';

  // Bundle kontrolü — açıklamalı ürün var mı?
  ...
  const bundleYapildi = bundleVarMi && sepetDerini > 1;
  ...
  try {
    await addDoc(collection(_db, 'funnel_logs'), {
      ...
      sepetKategorisi,                   // 'Altin' | 'Gumus' | 'Standart'
      toplamTutar,
      sure,
