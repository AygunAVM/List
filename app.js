...
document.addEventListener('click', e => {
  if (e.target.closest('.haptic-btn,.add-btn,.remove-btn,.btn-login,.cart-trigger'))
    haptic();
  // Global aktivite tetikleyici — sepet doluysa hareketsizlik sayacını sıfırla
...
if (typeof fetchLiveBasket === 'function') fetchLiveBasket(); 
  ...
  if (searchEl) {
    ...
    searchEl.placeholder = ad ? 'Mart Yıldızı, ' + ad + ' — Ürün arama' : 'Ürün arama';
  }
...
async function fetchLiveBasket() {
  ...
  try {
    ...
    if (snap.exists()) {
      const remote = snap.data().basket || [];
      if (remote.length > 0 && basket.length === 0) {
        basket = remote;
        updateCartUI();
        if (!window._sessionTimer) resetSessionTimer();
        // Session verilerini de geri yükle
        const sd = snap.data().sessionData;
        if (sd) _sessionData = { ...sd, startTime: sd.startTime || Date.now() };
      }
    }
...
// ─── FİYAT GÖSTER WRAPPER (data-attr tabanlı, apostrof güvenli) ─
function _fyGos(el) {
  if (!el) return;
  const urun  = el.dataset.urun  || '';
  const fiyat = parseFloat(el.dataset.fiyat) || 0;
  fiyatGoster(el, urun, fiyat);
}

function fiyatGoster(el, urunAdi, fiyat) {
  if(!el) return;
  el.textContent = fiyat ? fiyat.toLocaleString('tr-TR')+' ₺' : '—';
  el.classList.remove('price-blur');
  el.onclick = null;
  if(_sessionData && urunAdi && !_sessionData.revealedPrices.includes(urunAdi)) _sessionData.revealedPrices.push(urunAdi);
  if(basket.length>0) resetSessionTimer();
}
...
function addToBasket(idx) {
  ...
  saveBasket();
// updateLiveBasket ve resetSessionTimer saveBasket + global listener üzerinden tetiklenir
...
function saveBasket() {
  ...
  if (currentUser && _db) {
    ...
    // 2) Canlı sepet bulutu — her değişimde güncelle (cihazlar arası + veri kaybı önleme)
    if (basket.length > 0) {
      setDoc(doc(_db, 'live_baskets', email), {
        basket,
        lastActive: serverTimestamp(),
        personel: email,
        personelAd: currentUser.Ad || email.split('@')[0],
        sessionData: {
...
async function aySonucSecimi(sonuc, detay = "") {
  ...
  const personelAd2 = currentUser?.Ad || currentUser?.Email?.split('@')[0] || 'Bilinmeyen';
...
async function loadFunnelAnaliz() {
  ...
  try {
    ...
    const gecenH    = new Date(Date.now()-7*86400000).toISOString().split('T')[0];
    const gecenHdun = new Date(Date.now()-8*86400000).toISOString().split('T')[0];

    const logsB  = logs.filter(l=>(l.tarih||'').startsWith(bugun));
    const logsGH = logs.filter(l=>(l.tarih||'').startsWith(gecenH));

    // ── PUANLAMA MOToru ─────────────────────────────────────────
    // Her personele puan: Satış=3p, Teklif=1p, Kaçan=0p, Bundle=+1p, Derinlik≥3=+0.5p
    function puanHesapla(s) {
      return (s.Satis||s.satis||0)*3 + (s.Teklif||s.teklif||0)*1 +
             (s.bundleYapilan||0)*1 +
             (s.derinlik3plus||0)*0.5;
    }
    function rozet(puan, toplam) {
      if (toplam < 3) return { emoji:'🌱', label:'Başlangıç', color:'#94a3b8' };
      const oran = puan / Math.max(toplam*3, 1);
      if (oran >= 0.75) return { emoji:'🥇', label:'Altın',   color:'#f59e0b' };
      if (oran >= 0.50) return { emoji:'🥈', label:'Gümüş',  color:'#64748b' };
      if (oran >= 0.25) return { emoji:'🥉', label:'Bronz',   color:'#b45309' };
      return { emoji:'🎯', label:'Gelişiyor', color:'#6366f1' };
    }

    // ── PERSONEL STATİSTİKLERİ ──────────────────────────────────
    const pMap = {};
    const saatKacti  = Array(24).fill(0);
    const saatSatis  = Array(24).fill(0);
    const saatTeklif = Array(24).fill(0);

    logs.forEach(l => {
      const eid = l.personelId || '?';
      if (!pMap[eid]) pMap[eid] = {
        ad: l.personelAd || eid.split('@')[0],
        toplam:0, Satis:0, Teklif:0, Kacti:0,
        derinlikToplam:0, tutarToplam:0,
        bundleFirsat:0, bundleYapilan:0, derinlik3plus:0,
        bugün:0, gecenHafta:0
      };
      const p = pMap[eid];
      p.toplam++;
      const s = l.sonuc || '';
      if (s==='Satis'||s==='satis')   { p.Satis++;  }
      if (s==='Teklif'||s==='teklif') { p.Teklif++; }
      if (s==='Kacti'||s==='kacti')   { p.Kacti++;  }
      ...
      if ((l.derinlik||0) >= 3) p.derinlik3plus++;
      if ((l.tarih||'').startsWith(bugun))   p.bugün++;
      if ((l.tarih||'').startsWith(gecenH))  p.gecenHafta++;

      // Saatlik yoğunluk
      const h = l.saat ?? (l.ts?.toDate ? l.ts.toDate().getHours() : -1);
      if (h >= 0) {
        if (s==='Satis'||s==='satis')   saatSatis[h]++;
        if (s==='Teklif'||s==='teklif') saatTeklif[h]++;
        if (s==='Kacti'||s==='kacti')   saatKacti[h]++;
      }
    });

    // ── GENEL TOPLAMLAR ─────────────────────────────────────────
    const totS = logs.filter(l=>l.sonuc==='Satis'||l.sonuc==='satis').length;
    const totT = logs.filter(l=>l.sonuc==='Teklif'||l.sonuc==='teklif').length;
    ...
    // ── MOMENTUM ─────────────────────────────────────────────────
    const mFark = logsGH.length===0
      ? (logsB.length>0?100:0)
      : ((logsB.length-logsGH.length)/logsGH.length*100).toFixed(1);
    const mSatisB  = logsB.filter(l=>l.sonuc==='Satis'||l.sonuc==='satis').length;
    const mSatisGH = logsGH.filter(l=>l.sonuc==='Satis'||l.sonuc==='satis').length;
    const mDonusB  = logsB.length===0?0:((mSatisB/logsB.length)*100).toFixed(1);
    const mDonusGH = logsGH.length===0?0:((mSatisGH/logsGH.length)*100).toFixed(1);
    const mIcon = parseFloat(mFark)>0?'📈':parseFloat(mFark)<0?'📉':'➡️';
    const mCol  = parseFloat(mFark)>0?'#22c55e':parseFloat(mFark)<0?'#ef4444':'#94a3b8';

    // ── SAATLİK YOĞUNLUK CHARTLABEL ─────────────────────────────
    const saatLabels = [...Array(24).keys()].map(h=>(h<10?'0':'')+h+':00');
    const saatMax = Math.max(...saatKacti.map((v,i)=>v+saatSatis[i]+saatTeklif[i]),1);

    // ── PERSONEL KARTLARI HTML ───────────────────────────────────
    const personelHTML = Object.entries(pMap)
      .sort((a,b) => puanHesapla(b[1]) - puanHesapla(a[1]))
      .map(([,s]) => {
        const puan   = puanHesapla(s);
        const roz    = rozet(puan, s.toplam);
        const sO     = s.toplam===0?0:((s.Satis/s.toplam)*100).toFixed(0);
        const tO     = s.toplam===0?0:((s.Teklif/s.toplam)*100).toFixed(0);
        ...
        const aT     = s.toplam===0?0:Math.round(s.tutarToplam/s.toplam).toLocaleString('tr-TR');
        const kCol   = parseFloat(kO)>50?'#dc2626':parseFloat(kO)>30?'#f59e0b':'#16a34a';
        const mBug   = s.bugün;
        const mGec   = s.gecenHafta;
        const mB2    = mGec===0?(mBug>0?'↑ Yeni':'—'):mBug>mGec?`↑ ${mBug-mGec} fazla`:`↓ ${mGec-mBug} az`;
        const mB2C   = mBug>=mGec?'#22c55e':'#ef4444';

        return `<div style="background:var(--surface);border:1.5px solid var(--border);border-radius:18px;padding:16px;position:relative;overflow:hidden;">
          <!-- Rozet bandı -->
          <div style="position:absolute;top:12px;right:12px;background:${roz.color}18;border:1px solid ${roz.color}44;border-radius:20px;padding:3px 10px;font-size:.65rem;font-weight:700;color:${roz.color}">${roz.emoji} ${roz.label}</div>
          <!-- Personel başlık -->
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;padding-right:80px;">
            <div class="user-avatar" style="width:40px;height:40px;font-size:.75rem;flex-shrink:0">${s.ad.slice(0,2).toUpperCase()}</div>
            <div>
              <div style="font-weight:800;font-size:.92rem">${s.ad}</div>
              <div style="font-size:.64rem;color:var(--text-3)">${s.toplam} müşteri · ${puan.toFixed(1)} puan</div>
            </div>
          </div>
          <!-- Dönüşüm bar -->
          <div style="display:flex;height:26px;border-radius:10px;overflow:hidden;margin-bottom:10px;gap:1px">
            <div style="flex:${s.Satis};background:#16a34a;display:flex;align-items:center;justify-content:center;font-size:.62rem;color:#fff;font-weight:800;min-width:${s.Satis>0?'24px':'0'}">${s.Satis>0?sO+'%':''}</div>
            <div style="flex:${s.Teklif};background:#2563eb;display:flex;align-items:center;justify-content:center;font-size:.62rem;color:#fff;font-weight:800;min-width:${s.Teklif>0?'24px':'0'}">${s.Teklif>0?tO+'%':''}</div>
            <div style="flex:${s.Kacti||1};background:#dc2626;display:flex;align-items:center;justify-content:center;font-size:.62rem;color:#fff;font-weight:800;min-width:${s.Kacti>0?'24px':'0'}">${s.Kacti>0?kO+'%':''}</div>
          </div>
          <div style="display:flex;gap:4px;font-size:.62rem;color:var(--text-3);margin-bottom:12px;justify-content:flex-start">
            <span style="color:#16a34a">✅ ${s.Satis} sat</span>
            <span style="margin:0 4px">·</span>
            <span style="color:#2563eb">📩 ${s.Teklif} tek</span>
            <span style="margin:0 4px">·</span>
            <span style="color:#dc2626">❌ ${s.Kacti} kaç</span>
          </div>
          <!-- İstatistik grid -->
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:6px;font-size:.64rem;">
            <div style="background:#f8fafc;border-radius:8px;padding:7px 4px;text-align:center"><span style="font-weight:800;font-size:.82rem">📦 ${aD}</span><div style="color:var(--text-3);margin-top:2px">Ort.Derinlik</div></div>
            <div style="background:#f8fafc;border-radius:8px;padding:7px 4px;text-align:center"><span style="font-weight:800;font-size:.82rem">🎯 ${bB}</span><div style="color:var(--text-3);margin-top:2px">Bundle</div></div>
            <div style="background:#f8fafc;border-radius:8px;padding:7px 4px;text-align:center"><span style="font-weight:800;font-size:.80rem;color:${kCol}">${kO}%</span><div style="color:var(--text-3);margin-top:2px">Kaçan</div></div>
            <div style="background:#f8fafc;border-radius:8px;padding:7px 4px;text-align:center"><span style="font-weight:800;font-size:.80rem;color:${mB2C}">${mB2}</span><div style="color:var(--text-3);margin-top:2px">vs Geçen H.</div></div>
          </div>
          ${s.derinlik3plus>0?`<div style="margin-top:8px;font-size:.64rem;background:#eff6ff;border-radius:8px;padding:5px 8px;color:#1d4ed8">💡 ${s.derinlik3plus}x 3+ ürün sepeti — yüksek ilgi</div>`:''}
        </div>`;
      }).join('');

    // ── SAATLİK YOĞUNLUK HARİTASI ───────────────────────────────
    const saatHarita = saatLabels.map((lbl,h) => {
      const top = saatKacti[h]+saatSatis[h]+saatTeklif[h];
      const w   = top===0?0:Math.round((top/saatMax)*100);
      const sW  = top===0?0:Math.round((saatSatis[h]/Math.max(top,1))*w);
      const tW  = top===0?0:Math.round((saatTeklif[h]/Math.max(top,1))*w);
      const kW  = Math.max(0, w-sW-tW);
      return `<div style="display:flex;align-items:center;gap:6px;margin-bottom:3px;font-size:.62rem;">
        <span style="min-width:34px;color:var(--text-3);text-align:right">${lbl}</span>
        <div style="flex:1;height:12px;border-radius:6px;overflow:hidden;background:#f1f5f9;display:flex">
          <div style="width:${sW}%;background:#16a34a;height:100%"></div>
          <div style="width:${tW}%;background:#2563eb;height:100%"></div>
          <div style="width:${kW}%;background:#dc2626;height:100%"></div>
        </div>
        <span style="min-width:20px;color:var(--text-2);text-align:left">${top||''}</span>
      </div>`;
    }).join('');

    cont.innerHTML = `
      <!-- GENEL ÖZET -->
      <div style="background:linear-gradient(135deg,#1e293b,#0f172a);border-radius:16px;padding:16px;margin-bottom:14px;color:#fff">
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;text-align:center;margin-bottom:12px">
          <div><div style="font-size:1.5rem;font-weight:800">${totN}</div><div style="font-size:.64rem;opacity:.6">Müşteri</div></div>
          <div><div style="font-size:1.5rem;font-weight:800;color:#22c55e">${totS}</div><div style="font-size:.64rem;opacity:.6">Satış</div></div>
          <div><div style="font-size:1.5rem;font-weight:800;color:#f59e0b">${totT}</div><div style="font-size:.64rem;opacity:.6">Teklif</div></div>
          <div><div style="font-size:1.5rem;font-weight:800;color:#ef4444">${totK}</div><div style="font-size:.64rem;opacity:.6">Kaçan</div></div>
        </div>
        <div style="display:flex;align-items:center;justify-content:space-between;padding-top:10px;border-top:1px solid rgba(255,255,255,.12);font-size:.76rem">
          <span>🎯 Dönüşüm: <b style="color:#22c55e">${donusum}%</b></span>
          <span>${mIcon} <b style="color:${mCol}">Geçen haftaya göre ${Math.abs(mFark)}% ${parseFloat(mFark)>0?'artış':'azalış'}</b></span>
        </div>
        <div style="font-size:.64rem;opacity:.45;text-align:center;margin-top:6px">Bugün: ${logsB.length} müşteri (${mDonusB}% dönüşüm) · Geçen hafta bu gün: ${logsGH.length} (${mDonusGH}%)</div>
      </div>

      <!-- PERSONEL KARTLARI -->
      <div style="font-size:.7rem;font-weight:700;color:var(--text-2);margin-bottom:8px;text-transform:uppercase;letter-spacing:.06em">👤 Personel Analizi</div>
      ...
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:14px">
        <div style="font-size:.7rem;font-weight:700;color:var(--text-2);margin-bottom:10px;text-transform:uppercase;letter-spacing:.06em">⏰ Saatlik Yoğunluk Haritası</div>
        <div style="display:flex;gap:12px;font-size:.62rem;margin-bottom:8px">
          <span><span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:#16a34a;margin-right:3px"></span>Satış</span>
          <span><span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:#2563eb;margin-right:3px"></span>Teklif</span>
          <span><span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:#dc2626;margin-right:3px"></span>Kaçan</span>
        </div>
        ${saatHarita}
      </div>
    `;
...
Object.assign(window, {
  ...
  updateLiveBasket, 
  fetchLiveBasket,
  aySonucSecimi, gosterKactiNedenleri, fiyatGoster, _fyGos, // Eksiksiz eklendi
  openMessages: ()=>{},
