...
function toggleCampaign(idx, campIdx) {
  ...
  // ❖ proje, ⎇ ve 🔒 ile aynı anda seçilebilir — engel kaldırıldı
...
function recalculateAllGroupCampaigns() {
  ...
  basket.forEach(item => {
    ...
    // _pendingGroups her turda sıfırdan hesaplanmalı — stale true/false kalmasın
    item._pendingGroups = {};
...
function renderCampaignPills(item, idx) {
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    const disabled = !sel && !isBilgi && (
      ...
      // ❖ proje, ⎇ ve 🔒 ile aynı anda seçilebilir — fiyat override ayrı, indirim ayrı
    );
    ...
    if (isBilgi) {
      ...
      // ❖ Proje tamamlandı — mor/lila ton, çizgi yok (fiyat override aktif, indirim değil)
      style = `background:#f5f3ff;color:#6d28d9;border-bottom:2px solid #a78bfa;font-weight:800;cursor:pointer;`;
    } else if (sel) {
      ...
      style = tarihDolmus
        ...
        : disabled
          ? `background:#f1f5f9;color:#94a3b8;border-bottom:1.5px solid #e2e8f0;font-weight:600;cursor:not-allowed;opacity:.40;`
    ...
    // disabled durumda hover'da sebep göster
    const disabledTitle = disabled
      ? (ayniProjeGrupVar
          ? 'Bu üründe aynı proje grubundan seçim zaten var.'
          : camp.tip === 'kilitli' && itemHasKilitli
            ? 'Bu üründe zaten bir 🔒 kampanyası seçili.'
            : camp.tip === 'bagimsiz' && itemHasBagimsiz
              ? 'Bu üründe zaten bir ⌗ kampanyası seçili.'
              : 'Bu kampanya seçilemiyor.')
      : null;
    ...
    return `<button type="button"${clickAttr} title="${disabledTitle || camp.metin.replace(/"/g,'&quot;')}" `
