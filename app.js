...
async function openAbakus() {
  ...
  // Sepetteki ilk ürünün QF planı varsa kart dropdown'unu set et (item bazlı)
  const _itemPlan = basket.length > 0 && basket[0]._qfKart ? basket[0] : null;
  if (_itemPlan) {
...
function qfAddToBasket() {
  ...
  // Plan bilgisini geçici global'e al — addToBasket sonrası item'a yazılacak
  const _planSnapshot = _qfPlan ? {..._qfPlan} : null;

  addToBasket(_qfUrunIdx);

  // Sepete eklenen item'a plan bilgisini yaz (item bazlı, global değil)
  if (_planSnapshot && basket.length > 0) {
    const item = basket[0];
    item._qfPlanLabel = _planSnapshot.label;
    item._qfAylik     = _planSnapshot.aylik;
    item._qfTaksit    = _planSnapshot.taksit;
    item._qfKart      = _planSnapshot.kart;
    item._qfZincir    = _planSnapshot.zincir;
  }

  // abakusSelection'a DOKUNMA — global state kirletilmesin
  closeQuickFinance();

  // Başarı bildirimi — kullanıcı sepetten abaküsü açar, plan otomatik seçilir
  if (_planSnapshot) {
    Swal.fire({
      toast: true, position: 'top', timer: 2000, timerProgressBar: true,
      showConfirmButton: false, icon: 'success',
      title: _planSnapshot.label + ' planıyla sepete eklendi'
    });
  }
}
