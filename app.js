...
function renderCampaignPills(item, idx) {
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    } else if (sel && pending && isProje) {
      // ❖ Proje — eşik bekleniyor: turuncu pending (sepettekiyle aynı uyarı)
      style = `background:rgba(251,191,36,.22);color:#b45309;border-bottom:2px solid #f59e0b;font-weight:800;cursor:pointer;box-shadow:0 0 0 1px #fde68a;`;
    } else if (sel && pending) {
      style = `background:rgba(251,191,36,.18);color:#b45309;border-bottom:2px solid #f59e0b;font-weight:700;cursor:pointer;`;
    } else if (sel && !pending && isProje) {
      // ❖ Proje eşik TAMAMLANDI — sepetteki 🟢 yeşil+çizili ile birebir aynı
      style = `background:#dcfce7;color:#15803d;border-bottom:2px solid #4ade80;font-weight:800;text-decoration:line-through;text-decoration-color:#4ade80;cursor:pointer;box-shadow:0 0 0 1px #86efac;`;
    } else if (sel) {
    ...
    ? `<div style="display:flex;align-items:center;gap:5px;margin-top:3px">`
      + (projeAktifPill
        ...
        : campDisc > 0
          ...
          : `<span style="font-size:.57rem;background:rgba(251,191,36,.28);color:#92400e;border-radius:4px;padding:2px 8px;font-weight:700;border:1.5px solid #f59e0b">${pendingSummaryText}</span>`)
