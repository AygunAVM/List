  ...
  window._dataPollingTimer = setInterval(async () => {
    ...
    try {
      ...
      if(newV && !seen.includes(newV)) {
        ...
        checkChanges(json);
        // Ürün listesini güncelle (sessizce)
        filterData();
      }
...
function renderProposals(container, forceAll, searchQ) {
  ...
  // Aynı müşteriye birden fazla teklif varsa gruplandır
  const custGroups = {};
  myProps.forEach(p => {
    const key = (p.custName||'?').toLowerCase().trim() + '_' + (p.phone||'');
    if(!custGroups[key]) custGroups[key] = { custName: p.custName, phone: p.phone, items: [] };
    custGroups[key].items.push(p);
  });
  // Birden fazla teklifi olan müşteriler için grup başlığı ekle
  window._custGroups = custGroups;
...
if(isAdmin()) btns.push(`<button class="pact-btn pact-edit haptic-btn" onclick="openEditProp('${p.id}')" title="Düzenle"><svg width="13" height="13" viewBox="0 0 16 16" fill="none"><rect x="1" y="11" width="14" height="1.5" rx=".75" fill="currentColor"/><path d="M10.5 2.5l2 2-7 7-2.5.5.5-2.5 7-7z" stroke="currentColor" stroke-width="1.3" stroke-linejoin="round"/></svg></button>`);
    ...
    return `<div class="proposal-card status-card-${p.durum||'bekliyor'}" id="pcard-${p.id}">
      <div class="proposal-card-header">
        ...
        <span class="proposal-name">${p.custName}</span>${grpCount>1?`<span class="prop-grp-badge" title="Bu müşteriye ${grpCount} teklif verilmiş">×${grpCount}</span>`:''}
...
function printTeklif(id) {
  ...
  const indRow = (p.indirim && p.indirim > 0)
    ? `<tr><td class="ol">İndirim</td><td class="or" style="color:#16a34a">-${p.indirimTip==='PERCENT'?'%'+p.indirim:fmt(p.indirim)}</td></tr>`
    : '';
  ...
  const urunRows = (p.urunler||[]).map((u,i) =>
    `<tr class="${i%2===0?'row-even':'row-odd'}">
      ...
      <td class="u-fiyat">${fmt(u.nakit||u.fiyat||0)}</td>
    </tr>`
  ).join('');
...
<title>Aygün AVM — Teklif ${p.id||''}</title>
<style>
  @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap');
  *{margin:0;padding:0;box-sizing:border-box;}
  body{font-family:'Inter',sans-serif;background:#f8fafc;color:#1e293b;font-size:13px;}
  .page{max-width:720px;margin:0 auto;background:#fff;min-height:100vh;padding:0;}

  /* Header */
  .hdr{background:linear-gradient(135deg,#0f172a 0%,#1e293b 100%);padding:28px 36px 24px;display:flex;align-items:center;justify-content:space-between;}
  .hdr-brand{color:#fff;}
  .hdr-brand .brand-name{font-size:22px;font-weight:800;letter-spacing:-.5px;}
  .hdr-brand .brand-sub{font-size:11px;color:#94a3b8;margin-top:2px;letter-spacing:.5px;text-transform:uppercase;}
  .hdr-right{text-align:right;}
  .hdr-right .doc-type{color:#94a3b8;font-size:11px;text-transform:uppercase;letter-spacing:1px;}
  .hdr-right .doc-no{color:#fff;font-size:18px;font-weight:700;margin-top:2px;}
  .hdr-right .doc-date{color:#64748b;font-size:11px;margin-top:3px;}

  /* Accent bar */
  .accent{height:3px;background:linear-gradient(90deg,#3b82f6,#8b5cf6,#ec4899);}
  ...
  .info-card{background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:10px;padding:16px;}
  .info-card h4{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:#64748b;margin-bottom:10px;}
  .info-row{display:flex;gap:6px;margin-bottom:5px;font-size:12px;}
  .info-lbl{color:#64748b;min-width:72px;flex-shrink:0;}
  ...
  .urun-table thead th{color:#94a3b8;font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.6px;padding:9px 12px;text-align:left;}
  .urun-table thead th:last-child{text-align:right;}
  .u-no{width:36px;color:#94a3b8!important;font-size:11px;}
  .u-ad{font-weight:500;color:#1e293b;}
  .u-fiyat{text-align:right;font-weight:600;color:#1e293b;white-space:nowrap;}
  .row-even{background:#fff;}
  .row-odd{background:#f8fafc;}
  .urun-table tbody td{padding:9px 12px;border-bottom:1px solid #f1f5f9;font-size:12px;}

  /* Ödeme */
  .odeme-wrap{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px;}
  .odeme-table{width:100%;border-collapse:collapse;}
  .odeme-table tr:not(:last-child) td{border-bottom:1px solid #f1f5f9;}
  .ol{padding:8px 12px;font-size:11px;color:#64748b;background:#f8fafc;width:45%;}
  .or{padding:8px 12px;font-size:12px;font-weight:600;color:#1e293b;background:#fff;}
  .total-cell{font-size:15px!important;font-weight:800!important;color:#0f172a!important;}
  .odeme-card{background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:10px;overflow:hidden;}
  .odeme-card h4{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:#64748b;padding:12px 14px 8px;background:#fff;border-bottom:1px solid #f1f5f9;}

  /* Not */
  .not-box{background:#fffbeb;border:1.5px solid #fcd34d;border-radius:8px;padding:12px 14px;margin-bottom:20px;font-size:12px;color:#92400e;}
  .not-box span{font-weight:700;margin-right:4px;}

  /* Footer */
  .footer{margin-top:8px;padding:16px 36px 20px;border-top:1.5px solid #f1f5f9;}
  .footer-text{font-size:11px;color:#94a3b8;line-height:1.6;text-align:center;}
  .footer-sig{display:grid;grid-template-columns:1fr 1fr;gap:40px;margin-top:20px;}
  .sig-box{border-top:1.5px solid #e2e8f0;padding-top:8px;text-align:center;}
  .sig-lbl{font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:.6px;}
  ...
  @media(max-width:600px){
    .info-grid,.odeme-wrap,.footer-sig{grid-template-columns:1fr;}
    .body,.hdr,.footer{padding-left:18px;padding-right:18px;}
  }
</style>
</head>
<body>
<div class="page">
  <div class="hdr">
    <div class="hdr-brand">
      <div class="brand-name">Aygün AVM</div>
      <div class="brand-sub">Fiyat Teklifi</div>
    </div>
    <div class="hdr-right">
      <div class="doc-type">Teklif</div>
      <div class="doc-no">#${(p.id||'').slice(-6).toUpperCase()}</div>
      <div class="doc-date">Tarih: ${today}</div>
    </div>
  </div>
  ...
  <div class="body">
    ...
    <table class="urun-table">
      <thead><tr><th class="u-no">#</th><th>Ürün Adı</th><th style="text-align:right">Birim Fiyat</th></tr></thead>
...
function resendProposalWa(id) {
  ...
  const dn=p.indirim>0?`\n_İndirim: ${p.indirimTip==='PERCENT'?'%'+p.indirim:fmt(p.indirim)}_`:'';
  // Ödeme bloğu — kayıtlı odeme string + tahsilat
  const odemeBlok = '* `'+p.odeme+'`\n*Toplam* '+fmt(p.tahsilat||p.nakit||0);
...
function showPendingChanges(logKey) {
  ...
  const hasNew = log.some(e => !e.shown);
  if(!hasNew) return;
...
function _renderMergedItem(m, idx) {
  // Hem fiyat hem açıklama değişmişse tek satırda göster
  const hasBoth = m.price && m.aciklama;
  let detailHtml = '';
  let badgeHtml  = '';
  let emoji      = '';
  ...
  if(hasBoth) {
    ...
    emoji = up ? '📈' : '📉';
    const newAciklama = m.aciklama.new || '(boş)';
    detailHtml = `<span class="ci-row">Nakit: ${fmt(m.price.old)} → <strong>${fmt(m.price.new)}</strong></span>`
      + `<span class="ci-row ci-aciklama-block">📝 ${newAciklama}</span>`;
    badgeHtml = `<span class="change-badge ${up?'badge-price-up':'badge-price-down'}">${sign}${m.price.pct}%</span>`;
  } else if(m.price) {
    const up = m.price.diff > 0, sign = up ? '+' : '';
    emoji = up ? '📈' : '📉';
    detailHtml = `Nakit: ${fmt(m.price.old)} → <strong>${fmt(m.price.new)}</strong>`;
    badgeHtml  = `<span class="change-badge ${up?'badge-price-up':'badge-price-down'}">${sign}${m.price.pct}%</span>`;
  } else if(m.aciklama) {
    emoji = '📝';
    detailHtml = `Açıklama → <em>${m.aciklama.new||'(boş)'}</em>`;
    badgeHtml  = `<span class="change-badge badge-desc">Açıklama</span>`;
  }

  return `<div class="change-item change-item-mandatory" data-idx="${idx}" onclick="toggleChangeItemRow(this)">
    <span class="ci-emoji">${emoji}</span>
    <div class="ci-body">
      <span class="ci-urun">${m.urun}</span>
      <span class="ci-detail">${detailHtml}</span>
    </div>
    ${badgeHtml}
  </div>`;
}
...
function showChangePopup(changes, logKey) {
  ...
  const oldItems  = changes.filter(c => c.isOld);

  let html = '';
  let itemIdx = 0;

  // ── BÖLÜM 1: Düşük önem (versiyon + bilgi amaçlı) ──────────
  const lowItems = [...verItems, ...optItems];
  if(lowItems.length) {
      ...
      <div class="section-header section-header-low">
        <div class="sh-left">
          ...
          <div>
            ...
            <div class="sh-sub">Versiyon · Stok · Ürün adı</div>
          </div>
        </div>
      </div>
      <div class="section-items">`;

    // Versiyon satırları — başta normal (soluk değil)
    verItems.forEach(c => {
      const saat  = new Date(c.ts).toLocaleTimeString('tr-TR',{hour:'2-digit',minute:'2-digit'});
      const tarih = new Date(c.ts).toLocaleDateString('tr-TR',{day:'2-digit',month:'2-digit',year:'2-digit'});
      html += `<div class="change-item change-item-info">
        <span class="ci-emoji">🔄</span>
        <div class="ci-body">
          <span class="ci-urun">Versiyon ${c.from} → ${c.to}</span>
          <span class="ci-detail">${tarih} · ${saat}</span>
        </div>
        <span class="change-badge" style="background:#dbeafe;color:#1e40af;font-size:.60rem">Versiyon</span>
      </div>`;
    });

    // Bilgi amaçlı değişimler — başta normal
    optItems.forEach(c => { html += _renderChangeItem(c, itemIdx++, true); });
  ...
  // ── BÖLÜM 2: Yüksek önem (fiyat + açıklama) ────────────────
  if(mandItems.length) {
    // Aynı ürünün fiyat + açıklama değişimlerini tek satırda birleştir
    const mergedMap = new Map();
    mandItems.forEach(c => {
      const key = c.urun || c.new || '?';
      if(!mergedMap.has(key)) mergedMap.set(key, { urun:key, gam:c.gam||'', marka:c.marka||'', price:null, aciklama:null });
    ...
    html += `<div class="section-block section-high">
      <div class="section-header section-header-high">
        ...
        <div>
          ...
          <div class="sh-sub">Her satıra tıklayarak onaylayın</div>
        </div>
      </div>
      <div class="section-items">`;

    mergedItems.forEach(m => {
      html += _renderMergedItem(m, itemIdx++);
    });

    html += `</div></div>`;
  }

  // ── Önceki oturum (gizli, sadece veri) ─────────────────────
  // oldItems gösterilmiyor — sadece log için saklanıyor
...
function _doUpdateChangeBtn() {
  ...
  { // scope
    ...
    if(markAllBtn) markAllBtn.style.display = (isAdmin() && mandatoryLeft > 0) ? 'block' : 'none';
    ...
    if(canClose) {
      ...
      btn.textContent = '↑ Bilgi bölümünü onaylayın';
    } else {
      btn.textContent = mandatoryLeft + ' önemli değişim kaldı';
    }
  } // scope
}

function markAllChanges() {
  document.querySelectorAll('#change-list .change-item-mandatory:not(.change-item-done)').forEach(item => {
...
function closeChangePopup() {
  ...
  // Firebase'e son görülen versiyon + popup kapatma zamanını yaz
  _fbSavePopupSeen();
...
async function renderAdminPanel() {
  ...
  // Kritik Stok — sadece bir kez buton göster (flag ile)
  const _stokEl = document.getElementById('admin-stok-uyari');
  if(_stokEl && !_stokEl.dataset.initialized) {
    _stokEl.dataset.initialized = '1';
    _stokEl.innerHTML='<button class="btn-stok-load haptic-btn" onclick="renderStokUyari()">🔍 Stok Durumunu Kontrol Et</button>';
  }
...
function renderStokUyari() {
  ...
  if(!el) return;
  // allProducts zaten loadData() ile yüklü olmalı
  const rows = (allProducts && allProducts.length) ? allProducts
             : (window._cachedUrunler && window._cachedUrunler.length) ? window._cachedUrunler
             : null;
  if(rows) { window._cachedUrunler = rows; _doStokUyari(el, rows); return; }
  // Yoksa fetch et
...
function renderAdminUsers() {
  ...
  el.innerHTML = su.map(([email,s])=>{
    ...
    // Firebase analytics'ten popupSeen verisini çek
    let lastSeenVer = null, popupSeenTs = null;
    if(window._fbAnalytics) {
      // En son tarihin kaydına bak
      const userDocs = Object.values(window._fbAnalytics)
        .filter(d => d.email === email && d.popupSeen)
        .sort((a,b) => (b.date||'').localeCompare(a.date||''));
      if(userDocs.length) {
        lastSeenVer = userDocs[0].popupSeen;
        popupSeenTs = userDocs[0].popupSeenTs;
      }
    }
    // Yoksa localStorage fallback (admin kendi cihazı)
    if(!lastSeenVer) {
      const seenArr = JSON.parse(localStorage.getItem('aygun_change_seen_'+email)||'[]');
      lastSeenVer = seenArr.length ? seenArr[seenArr.length-1] : null;
    }
    const currentVer  = window._currentVersion || '';
    const verUpToDate = lastSeenVer && currentVer && lastSeenVer === currentVer;
    const seenTs = popupSeenTs ? new Date(popupSeenTs).toLocaleString('tr-TR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}) : null;
    // currentAppVer: kullanıcının popup'u kapattığı andaki uygulama versiyonu
    let userAppVer = null;
    if(window._fbAnalytics) {
      const userDocs2 = Object.values(window._fbAnalytics)
        .filter(d => d.email === email && d.currentAppVer)
        .sort((a,b) => (b.date||'').localeCompare(a.date||''));
      if(userDocs2.length) userAppVer = userDocs2[0].currentAppVer;
    }
    const appVer  = window._currentVersion || '';
    // Versiyon karşılaştırması: kullanıcının gördüğü ver == mevcut ver?
    const isUpToDate = userAppVer && appVer && userAppVer === appVer;
    const verLabel   = userAppVer ? userAppVer.split(' ')[0] : (lastSeenVer ? lastSeenVer.split(' ')[0] : null);
    const verColor   = !verLabel ? 'badge-ver-unknown'
                     : isUpToDate ? 'badge-ver-ok' : 'badge-ver-warn';
    const verIcon    = !verLabel ? '—' : isUpToDate ? '✓' : '⚠️';
    const verTitle   = seenTs ? `Son popup: ${seenTs}` : 'Popup henüz kapatılmamış';
    const verBadge   = `<span class="badge ${verColor} ver-badge-admin" title="${verTitle}">${verLabel||'—'} ${verIcon}</span>`;

    // Versiyon durumu satırı
    const verStatusLine = !isUpToDate && verLabel && appVer
      ? `<div class="user-ver-warn">📋 ${verLabel} → güncel: ${appVer.split(' ')[0]}</div>`
      : '';
...
function renderSepetDetay() {
  ...
  // Her kullanıcının basket localStorage'da kendi cihazında.
  // Admin panelinde göstereceğimiz: tüm kullanıcılara ait
  // proposals içindeki "bekliyor" tekliflerindeki ürünleri sepet olarak sun
  // + mevcut admin cihazındaki basket varsa onu da göster
  ...
  // 2. Firestore proposals'dan her kullanıcının bekleyen ürünleri (diğer cihazlar)
  if(proposals && proposals.length > 0) {
    const bekliyor = proposals.filter(p => p.durum==='bekliyor' || p.durum==='sureDoldu');
    const byUser = {};
    bekliyor.forEach(p => {
      if(!byUser[p.user]) byUser[p.user] = { items: new Map() };
      (p.urunler||[]).forEach(u => {
        if(!u.urun) return;
        const cur = byUser[p.user].items.get(u.urun) || { qty:0, price: u.nakit||u.fiyat||0 };
        cur.qty++;
        byUser[p.user].items.set(u.urun, cur);
      });
    });

    Object.entries(byUser).forEach(([user, d]) => {
      if(!d.items.size) return;
      const ini = user.split('@')[0].slice(0,2).toUpperCase();
      const rows = Array.from(d.items.entries()).map(([urun, v]) =>
        '<div class="sepet-item-row">' +
        '<span class="sepet-item-urun">' + urun + '</span>' +
        '<span class="sepet-item-qty">x' + v.qty + '</span>' +
        '<span class="sepet-item-price">' + fmt(v.price) + '</span>' +
        '</div>'
      ).join('');
      html_parts.push(
        '<div class="sepet-user-block">' +
        ...
        '<span class="stok-badge sk" style="background:#dbeafe;color:#1e40af">' + d.items.size + ' ürün (tekliften)</span>' +
        '</div>' + rows + '</div>'
      );
    });
  }
...
async function openSiparisNot(urunAdi, urunIdx) {
  ...
  if(ct) {
    const el = document.createElement('div'); el.className='toast-item';
    el.innerHTML='<span>📦</span><span style="flex:1">Siparis notu eklendi: '+urunAdi+'</span><button class="toast-close" onclick="this.parentElement.remove()">x</button>';
...
Object.assign(window, {
  ...
  clearAllPendingProps, logoutUser, toggleChangeItem, toggleChangeItemRow, markAllChanges, confirmSection, printTeklif,
