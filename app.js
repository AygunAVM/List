...
async function showApp() {
  ...
  if (searchEl) {
    ...
    searchEl.placeholder = ad ? 'Champ, ' + ad + ' — Ürün arama' : 'Ürün arama';
  }
...
async function loadFunnelAnaliz(gunAralik = 90, force = false) {
  ...
  try {
    ...
    // Her ürün için: blur, sepet, satis, en sık kaçış nedeni + yüzdesi
    ...
    logs.forEach(l => {
      ...
      (l.urunler || []).forEach(u => {
        const ad = u.urun || u; if (!ad) return;
        if (!urunKacisMatris[ad]) urunKacisMatris[ad] = { blur:0, sepet:0, satis:0, nedenler:{} };
        urunKacisMatris[ad].sepet++;
        if (l.sonuc === 'satis') urunKacisMatris[ad].satis++;
      });
    });
    // Her ürün için en sık kaçış nedeni hesapla
    ...
    const top3Pahali = Object.entries(fiyatiPahali).sort((a,b) => b[1]-a[1]).slice(0,3);

    // ── HUNİ ADIMLARI (Blur→Sepet→Abaküs→Teklif→Satış) ──────
...
function _renderFunnelHTML(cont, aktifFiltre, s) {
  ...
  const { gunAralik, totN, totS, totK, donusumGercek,
    ...
    l3Toplam, l3Satis, l3Kacti, l3KayipCiro, l3Donusum,
    l3NedenSirali, l3UrunSirali, saatL3Kacti, l3SaatMax,
    personelHTML, saatBar, blurBar, saatBlur,
    saatSatis = [], saatKacti = [],
    urunKacisMatris = {}, huniBlur = 0, huniSepet = 0,
    huniAbakus = 0, huniTeklif = 0, huniSatis = 0, huniKayiplar = [] } = s;
...
function switchAdminTab(tab) {
  ...
  if(tab==='analiz')   {
    // Fiyat düzeltme butonu (bir kez inject)
    setTimeout(() => {
      if (!document.getElementById('_fiyat-btn-inject')) {
        const hdrRow = document.querySelector('#tab-analiz .admin-section > div:first-child');
        if (hdrRow) {
          const btn = document.createElement('button');
          btn.id = '_fiyat-btn-inject';
          btn.className = 'btn-reset';
          btn.style.cssText = 'background:#1d4ed8;color:#fff;border-color:#1d4ed8;font-size:.68rem;padding:4px 12px;margin-left:8px';
          btn.innerHTML = '💲 Fiyat Düzelt';
          btn.onclick = () => openFiyatEditor();
          hdrRow.appendChild(btn);
        }
      }
    }, 50);
...
// ════════════════════════════════════════════════════════════════
// FİYAT DÜZENLEME ARACI — Admin paneli, Analiz sekmesi altında
// Düzenleme → Sipariş Notu olarak kaydedilir (push hatırlatıcısı)
// ════════════════════════════════════════════════════════════════

function openFiyatEditor() {
  if (!isAdmin()) return;
  // Mevcut modalı temizle
  document.getElementById('_fiyat-editor-modal')?.remove();

  const modal = document.createElement('div');
  modal.id = '_fiyat-editor-modal';
  modal.className = 'modal-overlay';
  modal.style.cssText = 'display:flex;z-index:10001;align-items:flex-end;justify-content:center;padding:0';

  modal.innerHTML = `
    <div style="background:#fff;border-radius:28px 28px 0 0;max-width:480px;width:100%;
      padding:0 0 env(safe-area-inset-bottom,20px);box-shadow:0 -12px 48px rgba(0,0,0,.2);font-family:inherit;max-height:90vh;overflow-y:auto">

      <!-- Başlık -->
      <div style="padding:16px 18px 0;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;background:#fff;z-index:1;border-bottom:1px solid #f1f5f9">
        <div style="display:flex;align-items:center;gap:10px">
          <span style="font-size:1.4rem">💲</span>
          <div>
            <div style="font-weight:800;font-size:.96rem;color:#0f172a">Fiyat Düzeltme Notu</div>
            <div style="font-size:.68rem;color:#94a3b8;margin-top:1px">Düzeltme Sipariş Notuna işlenir → push hatırlatıcı</div>
          </div>
        </div>
        <button onclick="document.getElementById('_fiyat-editor-modal').remove()"
          style="background:#f1f5f9;border:none;border-radius:50%;width:30px;height:30px;cursor:pointer;
            font-size:.9rem;color:#64748b;display:flex;align-items:center;justify-content:center">✕</button>
      </div>
      ...
      <div style="padding:16px 18px">
        ...
        <div style="margin-bottom:12px">
          <label style="font-size:.72rem;font-weight:700;color:#475569;display:block;margin-bottom:4px">📦 Ürün Adı</label>
          <input id="_fe-urun" list="_fe-urun-list" placeholder="Ürün adı yazın veya seçin…"
            style="width:100%;padding:10px 12px;border:1.5px solid #e2e8f0;border-radius:12px;
              font-family:inherit;font-size:.82rem;color:#1e293b;box-sizing:border-box;outline:none"
            oninput="_feFiyatDoldur(this.value)"
            onfocus="this.style.borderColor='#2563eb'" onblur="this.style.borderColor='#e2e8f0'">
          <datalist id="_fe-urun-list">
            ${(allProducts||[]).slice(0,200).map(p => {
              const keys = Object.keys(p);
              const uKey = keys.find(k => k.toLowerCase()==='urun'||k.toLowerCase().includes('ürün')) || '';
              return uKey && p[uKey] ? `<option value="${p[uKey].replace(/"/g,'&quot;')}">` : '';
            }).join('')}
          </datalist>
        </div>

        <!-- Mevcut fiyat (otomatik dolar) -->
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px">
          <div>
            <label style="font-size:.72rem;font-weight:700;color:#475569;display:block;margin-bottom:4px">📋 Mevcut Nakit Fiyat</label>
            <input id="_fe-eski" type="text" readonly placeholder="—"
              style="width:100%;padding:10px 12px;border:1.5px solid #e2e8f0;border-radius:12px;
                font-family:inherit;font-size:.82rem;background:#f8fafc;color:#94a3b8;box-sizing:border-box">
          </div>
          <div>
            <label style="font-size:.72rem;font-weight:700;color:#1e293b;display:block;margin-bottom:4px">✏️ Yeni Fiyat (₺)</label>
            <input id="_fe-yeni" type="number" min="0" placeholder="0"
              style="width:100%;padding:10px 12px;border:1.5px solid #2563eb;border-radius:12px;
                font-family:inherit;font-size:.82rem;color:#1e293b;box-sizing:border-box;outline:none"
              oninput="_feGuncelleFark()"
              onfocus="this.style.borderColor='#1d4ed8'" onblur="this.style.borderColor='#2563eb'">
          </div>
        </div>

        <!-- Fark göstergesi -->
        <div id="_fe-fark" style="display:none;background:#f8fafc;border-radius:10px;padding:9px 12px;margin-bottom:12px;font-size:.76rem;font-weight:700;text-align:center"></div>

        <!-- Değişiklik tipi -->
        <div style="margin-bottom:12px">
          <label style="font-size:.72rem;font-weight:700;color:#475569;display:block;margin-bottom:6px">🏷 Değişiklik Tipi</label>
          <div style="display:flex;gap:6px;flex-wrap:wrap">
            ${['Geçici İndirim','Kampanya','Maliyet Güncellemesi','Hata Düzeltme','Diğer'].map(t =>
              `<button class="_fe-tip-btn" data-tip="${t}" onclick="_feSelectTip(this)"
                style="padding:6px 12px;border:1.5px solid #e2e8f0;border-radius:20px;
                  font-family:inherit;font-size:.71rem;font-weight:600;cursor:pointer;
                  background:#fff;color:#475569;transition:all .1s">${t}</button>`
            ).join('')}
          </div>
        </div>
        ...
        <div style="margin-bottom:16px">
          <label style="font-size:.72rem;font-weight:700;color:#475569;display:block;margin-bottom:4px">📝 Açıklama (isteğe bağlı)</label>
          <textarea id="_fe-not" rows="2" placeholder="Örnek: Rakip kampanyasına karşı, 15 Ağustos'a kadar geçerli…"
            style="width:100%;padding:10px 12px;border:1.5px solid #e2e8f0;border-radius:12px;
              font-family:inherit;font-size:.80rem;color:#1e293b;resize:none;box-sizing:border-box;outline:none"
            onfocus="this.style.borderColor='#2563eb'" onblur="this.style.borderColor='#e2e8f0'"></textarea>
        </div>

        <!-- Kaydet -->
        <button onclick="_feSipKaydet()"
          style="width:100%;background:linear-gradient(135deg,#1d4ed8,#2563eb);color:#fff;
            border:none;border-radius:16px;padding:15px;font-family:inherit;font-weight:800;
            font-size:.92rem;cursor:pointer;letter-spacing:-.01em;
            box-shadow:0 4px 16px rgba(37,99,235,.35);transition:filter .12s"
          onmouseover="this.style.filter='brightness(1.08)'" onmouseout="this.style.filter=''">
          📦 Sipariş Notuna Kaydet
        </button>
        <div style="text-align:center;font-size:.62rem;color:#94a3b8;margin-top:8px">
          Not, 📦 Sipariş sekmesine işlenecek · JSON dosyasını güncellemeyi unutma!
        </div>
      </div>
    </div>`;
  ...
  window._feFiyatSecim = null;
}

// Ürün seçilince mevcut fiyatı doldur
function _feFiyatDoldur(urunAdi) {
  const guncel = (allProducts||[]).find(p => {
    const keys = Object.keys(p);
    const uKey = keys.find(k => k.toLowerCase()==='urun'||k.toLowerCase().includes('ürün')) || '';
    return p[uKey] === urunAdi;
  });
  const eskiEl = document.getElementById('_fe-eski');
  if (eskiEl) {
    if (guncel) {
      const nakit = parseFloat(guncel.Nakit || guncel.nakit || 0);
      eskiEl.value = nakit ? nakit.toLocaleString('tr-TR') + ' ₺' : '—';
      window._feMevcutFiyat = nakit;
      window._feSeciliUrun  = urunAdi;
    } else {
      eskiEl.value = '—';
      window._feMevcutFiyat = 0;
      window._feSeciliUrun  = urunAdi;
    }
  }
  _feGuncelleFark();
...
// Fark hesapla
function _feGuncelleFark() {
  const eski   = window._feMevcutFiyat || 0;
  const yeniEl = document.getElementById('_fe-yeni');
  const farkEl = document.getElementById('_fe-fark');
  if (!yeniEl || !farkEl) return;
  const yeni = parseFloat(yeniEl.value) || 0;
  if (!yeni || !eski) { farkEl.style.display='none'; return; }
  const fark = yeni - eski;
  const pct  = eski ? ((fark/eski)*100).toFixed(1) : 0;
  farkEl.style.display = 'block';
  farkEl.style.background = fark > 0 ? '#fef2f2' : '#f0fdf4';
  farkEl.style.color       = fark > 0 ? '#dc2626'  : '#16a34a';
  farkEl.textContent = `${fark > 0 ? '▲ +' : '▼ '}${Math.abs(fark).toLocaleString('tr-TR')} ₺ (${fark>0?'+':''}${pct}%)`;
}

// Tip seç
function _feSelectTip(btn) {
  document.querySelectorAll('._fe-tip-btn').forEach(b => {
  ...
  window._feSecilenTip = btn.dataset.tip;
}

// Sipariş Notuna Kaydet
async function _feSipKaydet() {
  const urunAdi = document.getElementById('_fe-urun')?.value?.trim();
  const yeniF   = parseFloat(document.getElementById('_fe-yeni')?.value) || 0;
  const not     = document.getElementById('_fe-not')?.value?.trim() || '';
  const tip     = window._feSecilenTip || 'Diğer';
  const eskiF   = window._feMevcutFiyat || 0;

  if (!urunAdi) { await ayAlert('Ürün adı zorunludur.'); return; }
  if (!yeniF)   { await ayAlert('Yeni fiyat girilmeli.'); return; }

  const fark    = yeniF - eskiF;
  const farkStr = eskiF ? `${eskiF.toLocaleString('tr-TR')} ₺ → ${yeniF.toLocaleString('tr-TR')} ₺ (${fark>0?'+':''}${fark.toLocaleString('tr-TR')} ₺)` : `Yeni fiyat: ${yeniF.toLocaleString('tr-TR')} ₺`;

  // Sipariş notu içeriği
  const notMetin = [
    `[${tip}] ${urunAdi}`,
    `Fiyat: ${farkStr}`,
    not ? `Not: ${not}` : null,
    `⚠️ urunler.json dosyasını güncellemeyi unutma!`,
  ].filter(Boolean).join('\n');

  const yeni = {
    id:    uid(),
    ts:    new Date().toISOString(),
    urun:  `💲 FİYAT DEĞİŞİKLİĞİ — ${urunAdi}`,
    not:   notMetin,
    user:  currentUser?.Email || '-',
    durum: 'bekliyor',
    tip:   'fiyat_degisikligi',   // özel tip etiketi
    eskiFiyat: eskiF,
    yeniFiyat: yeniF,
  };
  ...
  if (ct) {
    ...
    el.innerHTML = `<span>💲</span><span style="flex:1"><b>${urunAdi}</b> fiyat notu eklendi → 📦 Sipariş sekmesi</span><button class="toast-close" onclick="this.parentElement.remove()">×</button>`;
    ct.appendChild(el);
    setTimeout(() => el.remove(), 4000);
  }

  // Admin panelde Sipariş sekmesine geç
  if (document.getElementById('admin-modal')?.classList.contains('open')) {
    switchAdminTab('siparis');
  }
}
...
Object.assign(window, {
  ...
  openFiyatEditor, _feFiyatDoldur, _feGuncelleFark, _feSelectTip, _feSipKaydet,
