...
function parseCampaigns(aciklama) {
  ...
  const has_kilitli  = str.includes(KILITLI);
  const has_tulha    = str.includes('✦');

  if (has_birlesen || has_kilitli) {
    // ── ⎇ / 🔒︎ ayraçlı format ───────────────────────────────────
    const ayraclar = [];
    for (let i = 0; i < str.length; i++) {
      if (str[i] === '⎇') {
        ayraclar.push({ idx: i, len: 1, tip: 'birlesen' });
      } else if (str.slice(i, i + KILITLI.length) === KILITLI) {
        ayraclar.push({ idx: i, len: KILITLI.length, tip: 'kilitli' });
      }
    }
    let onceki = 0;
    ayraclar.forEach(a => {
...
function _buildCampObj(metin, tip) {
  ...
  if (yeniFormat && tip !== 'bilgi') {
    ...
    return { metin, tutar, tip, grup, esik, rol };
  }
...
// Tıklama akışı:
//   1. Pill'e tıkla → seçim state'ini güncelle
...
// Şık uyarı toast — kampanya çakışmaları ve aşım için
function _campToast(msg, tip) {
  ...
  if (!ct) return;
  const el = document.createElement('div');
  el.className = 'toast-item camp-toast';
  el.style.cssText = 'background:#fff;border-left:4px solid ' + renk + ';border-radius:8px;padding:8px 14px;'
    + 'display:flex;align-items:center;gap:8px;font-size:.76rem;box-shadow:0 4px 12px rgba(0,0,0,.12);'
    + 'animation:slideInRight .25s ease;margin-bottom:4px';
  el.innerHTML = '<span style="font-size:1rem">' + ikon + '</span><span style="flex:1;color:#1e293b">' + msg + '</span>';
  ct.appendChild(el);
...
function toggleCampaign(idx, campIdx) {
  ...
  // Aynı ürün içi mevcut seçimler
  const mevcutKilitli  = Object.entries(item._selectedCamps).some(
    ([ci, sel]) => sel && item._campaigns[parseInt(ci)]?.tip === 'kilitli'
  );
  const mevcutBirlesen = Object.entries(item._selectedCamps).some(
  ...
  if (isSelected) {
    ...
    // MADDE 7: Çakışma uyarısı — aynı üründe tür karışımı engeli
    if (mevcutKilitli && camp.tip === 'birlesen') {
      _campToast('Bu üründe 🔒 kilitli kampanya aktif — birleşebilir (⎇) seçilemez.', 'warn');
      return;
    }
    if (mevcutBirlesen && camp.tip === 'kilitli') {
      _campToast('Bu üründe ⎇ kampanya aktif — kilitli (🔒) seçilemez.', 'warn');
      return;
    }

    // MADDE 5: Maksimum eşik aşımı kontrolü
    if (camp.tip !== 'bilgi' && camp.esik > 1) {
      const grupAdi = camp.grup || 'DIGER';
      const seciliUrunSayisi = new Set(
        basket.flatMap((b, bi) => {
          if (!b._campaigns || !b._selectedCamps) return [];
          return Object.entries(b._selectedCamps)
            .filter(([ci, sel]) => sel && b._campaigns[parseInt(ci)]?.grup === grupAdi)
            .map(() => bi);
        })
      ).size;
      if (seciliUrunSayisi >= camp.esik && !item._selectedCamps[campIdx]) {
        const zatenBuUrun = Object.entries(item._selectedCamps).some(
          ([ci, sel]) => sel && item._campaigns[parseInt(ci)]?.grup === grupAdi
        );
        if (!zatenBuUrun) {
          _campToast(
            grupAdi + ' kampanyası en fazla ' + camp.esik + ' ürünle kullanılabilir. '
            + 'Fazladan ürün eklenemez.',
            'warn'
          );
          return;
        }
    ...
    // MADDE 6: Kilitli grup tamamlandıktan sonra başka gruba giriş engeli
    if (camp.tip !== 'bilgi') {
      const grupAdi = camp.grup || 'DIGER';
      const grupTamamlandi = basket.some(b => {
        if (!b._pendingGroups) return false;
        return b._pendingGroups[grupAdi] === false; // false = tamamlandı
      });
      if (grupTamamlandi) {
        // Bu grupta zaten bu ürünün seçimi var mı? Varsa aynı ürüne 2. pill seçimi OK
        const buUrunGrupSecimi = Object.entries(item._selectedCamps).some(
          ([ci, sel]) => sel && item._campaigns[parseInt(ci)]?.grup === grupAdi
        );
        if (!buUrunGrupSecimi && camp.tip === 'kilitli') {
          _campToast(grupAdi + ' kampanyası zaten tamamlandı — yeni seçim eklenemez.', 'warn');
          return;
        }
      }
    }
    ...
    if (camp.tip === 'kilitli') {
      // 🔒 seçildi → aynı üründeki ⎇ seçimleri temizle
      Object.keys(item._selectedCamps).forEach(ci => {
        if (item._selectedCamps[ci] && item._campaigns[parseInt(ci)]?.tip === 'birlesen') {
          delete item._selectedCamps[ci];
        }
      });
    } else if (camp.tip === 'birlesen') {
      // ⎇ seçildi → aynı üründeki 🔒 seçimleri temizle
      Object.keys(item._selectedCamps).forEach(ci => {
        if (item._selectedCamps[ci] && item._campaigns[parseInt(ci)]?.tip === 'kilitli') {
          delete item._selectedCamps[ci];
        }
      });
    }
  ...
  // Tüm sepeti tarayıp grup bazında indirimi hesapla ve dağıt
  recalculateAllGroupCampaigns();
  updateCartUI();
}

// ─── GLOBAL GRUP HESAPLAMA MOTORU ──────────────────────────────
// Her tıklamada çalışır. Sepetteki tüm seçimleri grup bazında toplar,
// eşik kontrolü yapar, ağırlıklı oranlama ile itemDisc dağıtır.
function recalculateAllGroupCampaigns() {
  // Sadece kampanya kaynaklı kısmı (_campDisc) sıfırla
  // Manuel satır indirimi (manuelDisc) korunur
  basket.forEach(item => {
    if (item._campaigns && item._campaigns.length > 0) {
      const manuelDisc = (item.itemDisc || 0) - (item._campDisc || 0);
      item._campDisc  = 0;
      item.itemDisc   = Math.max(0, manuelDisc); // manuel kısmı koru
    }
  });

  // Tüm seçili pill'leri gruplarına göre topla
  // Yapı: { 'KEA': [ {basketIdx, campIdx, camp} ], 'KM': [...], 'PAP': [...] }
  const grupSecimler = {};
  ...
  // Her grup için eşik + ROL kontrolü yap ve indirimi dağıt
  Object.entries(grupSecimler).forEach(([grup, secimler]) => {
    // BILGI grubu: indirim yok
    if (grup === 'BILGI' || secimler[0]?.camp?.tip === 'bilgi') return;

    const esik = secimler[0]?.camp?.esik || 1;
    const grupUrunler = [...new Set(secimler.map(s => s.basketIdx))];
    const urunSayisi  = grupUrunler.length;

    // ROL kontrolü: 'ANY' olmayan rollerin hepsi farklı olmalı
    const rollerList = secimler.map(s => s.camp.rol || 'ANY');
    const rollerFarkli = rollerList.every(r => r === 'ANY') ||
      (new Set(rollerList.filter(r => r !== 'ANY')).size === rollerList.filter(r => r !== 'ANY').length);

    const esikSaglandi = urunSayisi >= esik && rollerFarkli;

    // Pending durumunu güncelle
    secimler.forEach(s => {
      if (!s.item._pendingGroups) s.item._pendingGroups = {};
      s.item._pendingGroups[grup] = !esikSaglandi;
    });

    if (!esikSaglandi) return;

    // ── İndirim hesabı ──────────────────────────────────────────
    // DOĞRU MANTIK: Grup indirimi SABİT bir tutardır.
    // PAP (esik=1): her ürün kendi pill tutarını alır (tekil).
    // Grup (esik>=2): grup tutarı tüm seçili ürünlere ORANLI dağıtılır.
    //
    // "Grup tutarı" = seçilen pill'lerin MAX tutarı
    // (KEA -3k: ürün1=3k, ürün2=3k → grup tutarı 3k, her ürün oranı kadar alır)
    const grupToplamIndirim = Math.max(...secimler.map(s => s.camp.tutar || 0));

    if (grupToplamIndirim <= 0) return;

    const grupToplamNakit = grupUrunler.reduce((s, bi) => s + (basket[bi]?.nakit || 0), 0);

    if (grupToplamNakit <= 0) {
      const esitPay = Math.round(grupToplamIndirim / grupUrunler.length);
      grupUrunler.forEach(bi => {
        basket[bi].itemDisc  = (basket[bi].itemDisc  || 0) + esitPay;
        basket[bi]._campDisc = (basket[bi]._campDisc || 0) + esitPay;
      });
    } else {
      let dagitilan = 0;
      grupUrunler.forEach((bi, i) => {
        const urun    = basket[bi];
        const agirlik = urun.nakit / grupToplamNakit;
        const pay     = i === grupUrunler.length - 1
          ? grupToplamIndirim - dagitilan
          : Math.round(agirlik * grupToplamIndirim);
  ...
  // Basket state'ini kaydet (localStorage + live_baskets)
  saveBasket();
}
...
// ─── GRUP RENK PALETİ (Premium) ─────────────────────────────────
// Aynı grup = aynı renk tonu — personel renk eşleştirme yapar
const _GRUP_PALETTE = {
  'PAP':   { bg:'#f0fdf4', bd:'#22c55e', tx:'#15803d' },
  'KEA':   { bg:'#eff6ff', bd:'#3b82f6', tx:'#1d4ed8' },
  'KM':    { bg:'#fff7ed', bd:'#f97316', tx:'#c2410c' },
  'IKILI': { bg:'#faf5ff', bd:'#a855f7', tx:'#7e22ce' },
  'ANK':   { bg:'#fff1f2', bd:'#f43f5e', tx:'#be123c' },
  'BILGI': { bg:'#f5f3ff', bd:'#8b5cf6', tx:'#6d28d9' },
  'DIGER': { bg:'#f8fafc', bd:'#64748b', tx:'#334155' },
};

// Kampanya pilllerini HTML olarak render et — Premium görünüm
// Aynı grup = aynı renk, duruma göre vurgu + eşik rozeti
function renderCampaignPills(item, idx) {
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    const pal     = _GRUP_PALETTE[camp.grup] || _GRUP_PALETTE['DIGER'];

    // Devre dışı: aynı üründe tür çakışması
    const disabled = !sel && !isBilgi && (
      (itemHasKilitli  && camp.tip === 'birlesen') ||
      (itemHasBirlesen && camp.tip === 'kilitli')
    );

    let bg, border, color, textDeco = '', icon, ring = '';
    if (isBilgi) {
      bg = pal.bg; border = pal.bd; color = pal.tx; icon = 'ℹ️';
    } else if (sel && !pending) {
      bg = '#dcfce7'; border = '#16a34a'; color = '#15803d';
      textDeco = 'line-through'; icon = '✓';
      ring = 'box-shadow:0 0 0 2px #86efac;';
    } else if (sel && pending) {
      bg = '#fffbeb'; border = '#f59e0b'; color = '#92400e';
      icon = '⏳'; ring = 'box-shadow:0 0 0 2px #fde68a;';
    } else if (disabled) {
      bg = '#f1f5f9'; border = '#e2e8f0'; color = '#94a3b8';
      icon = locked ? '🔒' : '⎇';
    } else {
      bg = pal.bg; border = pal.bd; color = pal.tx;
      icon = locked ? '🔒' : '⎇';
    }

    // Eşik rozeti — pill içinde küçük sayı (sadece seçilmemiş ve esik>1)
    const esikBadge = (camp.esik > 1 && !isBilgi && !sel)
      ? '<span style="display:inline-flex;align-items:center;justify-content:center;'
        + 'width:13px;height:13px;border-radius:50%;background:' + (disabled ? '#94a3b8' : border)
        + ';color:#fff;font-size:.44rem;font-weight:900;margin-left:3px;flex-shrink:0">'
        + camp.esik + '</span>'
      : '';

    const tutarStr = (camp.tutar > 0 && !isBilgi)
      ? '<span style="font-weight:800;font-size:.62rem;margin-left:3px;color:'
        + (sel && !pending ? '#15803d' : color) + '">-'
        + (camp.tutar >= 1000 ? (camp.tutar/1000).toFixed(camp.tutar%1000===0?0:1)+'k' : camp.tutar)
        + '₺</span>'
      : '';

    const cursor  = (disabled || isBilgi) ? 'not-allowed' : 'pointer';
    const esikTip = camp.esik > 1 ? ' | ' + camp.esik + ' ürün gerekli' : '';
    const tipStr  = locked ? 'Kilitli — birleşmez' : 'Birleşebilir — toplanır';

    return '<button type="button"'
      + ' onclick="' + (disabled || isBilgi ? 'void(0)' : 'toggleCampaign(' + idx + ',' + ci + ')') + '"'
      + ' title="' + camp.metin.replace(/"/g, '&quot;') + ' — ' + tipStr + esikTip + '"'
      + ' style="display:inline-flex;align-items:center;gap:2px;padding:3px 8px;border-radius:20px;'
      + 'font-size:.60rem;font-weight:600;white-space:nowrap;cursor:' + cursor + ';user-select:none;'
      + 'background:' + bg + ';border:1.5px solid ' + border + ';color:' + color + ';'
      + 'margin:2px 3px 2px 0;text-decoration:' + textDeco + ';'
      + 'opacity:' + (disabled ? 0.38 : 1) + ';transition:all .18s;' + ring + '">'
      + '<span style="font-size:.70rem;line-height:1">' + icon + '</span>'
      + '<span style="margin-left:2px">' + (camp.metin.length > 26 ? camp.metin.slice(0, 24) + '…' : camp.metin) + '</span>'
      + tutarStr + esikBadge
      + '</button>';
  }).join('');
  ...
  const summaryRow = secilenSayi > 0
    ? '<div style="display:flex;align-items:center;gap:6px;margin-top:4px;flex-wrap:wrap">'
      + (campDisc > 0
        ? '<span style="font-size:.58rem;background:#dcfce7;border:1px solid #86efac;color:#15803d;'
          + 'border-radius:20px;padding:2px 8px;font-weight:700;display:inline-flex;align-items:center;gap:3px">'
          + '✓ '
          + (campDisc >= 1000 ? (campDisc/1000).toFixed(campDisc%1000===0?0:1)+'k' : campDisc)
          + '₺ kampanya uygulandı</span>'
        : '<span style="font-size:.57rem;color:#92400e;background:#fffbeb;border:1px solid #fde68a;'
          + 'border-radius:20px;padding:2px 8px;font-weight:600">⏳ Eşik tamamlanmayı bekliyor…</span>')
      + '<button type="button" onclick="clearAllCampaigns(' + idx + ')"'
        + ' style="margin-left:auto;padding:2px 8px;border-radius:12px;font-size:.55rem;cursor:pointer;'
        + 'background:#fee2e2;border:1px solid #fca5a5;color:#dc2626;font-family:inherit;'
        + 'display:inline-flex;align-items:center;gap:3px">✕ Temizle</button>'
      + '</div>'
    : '';

  return '<div class="camp-pills-wrap" style="margin-top:5px">' + pills + summaryRow + '</div>';
}
...
function _buildAdminCartHTML() {
  ...
  basket.forEach((item, idx) => {
    ...
    rows += `<tr class="${hasDisc ? 'row-has-disc' : ''}">
      ...
      <td style="font-size:.63rem;color:var(--text-3);max-width:160px">
        <div style="word-break:break-word;margin-bottom:2px">${item.aciklama || '—'}</div>
...
if (!enKarliMap[td.n] || oran < enKarliMap[td.n].oran) {
  ...
  if (nakitRow) nakitRow.dataset.abrow = JSON.stringify({ type: 'nakit', nakit: nakitFinal, nakitBrut: nakit, ekIndirim });
...
async function finalizeAksiyon() {
  ...
  // Pazarlık indirimi — önce abakusSelection'dan, yoksa DOM input'tan oku
  // Modal kapalı olsa bile ab-ek-indirim inputu DOM'da kalır
  const _ekIndirimF = (() => {
    if (abakusSelection?.ekIndirim > 0) return abakusSelection.ekIndirim;
    const el = document.getElementById('ab-ek-indirim');
  ...
  const canAct = isAdmin() || p.user===me;
    ...
    return `<div class="proposal-card status-card-${p.durum||'bekliyor'}${p.durum==='satisDondu'?' prop-card-converted':''}" id="pcard-${p.id}">
      ...
      <div class="proposal-body">
        ...
        ${(() => {
          const pItemDisc = (p.urunler||[]).reduce((s,u) => s+(u.itemDisc||0), 0);
          const pAltInd   = p.indirim || 0;
          const pEkInd    = p.ekIndirim || 0;
          const parts = [];
          if (pItemDisc > 0)  parts.push('<span style="color:#16a34a">Satır: -'+fmt(pItemDisc)+'₺</span>');
          if (pAltInd   > 0)  parts.push('<span style="color:#f97316">Alt: -'+fmt(pAltInd)+'₺</span>');
          if (pEkInd    > 0)  parts.push('<span style="color:#8b5cf6">Pazarlık: -'+fmt(pEkInd)+'₺</span>');
          if (!parts.length)  return '';
          return '<div style="font-size:.60rem;margin-top:3px;display:flex;gap:6px;flex-wrap:wrap;align-items:center">'
            + '<span style="color:var(--text-3)">İndirimler:</span>' + parts.join('') + '</div>';
        })()}
        ${adminNotes}
      </div>
...
async function teklifSepeteEkle(propId) {
  ...
  (p.urunler || []).forEach(tu => {
    ...
    yeniUrunler.push({ urun: tu.urun, nakit, dk, awm, tek, aciklama, stok, kod,
      ...
      itemDisc: 0,               // kampanya indirimlerini sıfırla — aciklama güncel geldi
      _campDisc: 0,
      _campaigns: null,          // aciklama değişmiş olabilir, yeniden parse edilecek
      _selectedCamps: {},
      _pendingGroups: {}
    });
  });
