...
// Son 14 günde en çok blur açılan ürünler (sadece açılma sayısı, prim yok)
function _trendSkoru() {
  ...
  const urunKey = keys0.find(k => k.toLowerCase() === 'urun') ||
                  keys0.find(k => k.toLowerCase().includes('urun')) || '';
...
// Nakit fiyatı RAW sayı olarak döndürür (₺ EKLEMEZ — fmt() zaten ₺ ekliyor)
function _nakitFiyat(u) {
  const keys = Object.keys(u);
  const nakitKey = keys.find(k => k.toLowerCase() === 'nakit') || '';
  return nakitKey ? (parseFloat(u[nakitKey]) || 0) : 0;
}

// Ürün adından marka önekini çıkar
...
// Vitrin ürününü allProducts'ta bul — productIdx varsa kullan
function _vitrinOi(v) {
  ...
  return allProducts.findIndex(u => (u[uk] || '').toLowerCase() === v.urunAdi.toLowerCase());
}

// ── Vitrin Arama (admin paneli) ─────────────────────────────
...
async function saveVitrinUrun() {
  ...
  const productIdx = _vitrinSeciliIdx;

  if (!urunAdi) { _campToast('Önce listeden bir ürün seçin.', 'warn'); return; }
...
function renderTrendCards() {
  ...
  // Vitrin varsa daha az trend, yoksa daha fazla
  ...
  const vitrinHtml = vitrinAktif.map(v => {
    ...
    return `<div class="trend-card vitrin-card"
      ...
      <div class="trend-rank" style="color:#e11d48;font-size:.60rem;font-weight:900">🛍️</div>
      <div class="trend-card-name">${v.urunAdi}</div>
      ${v.aciklama ? `<div style="font-size:.60rem;color:var(--text-3);margin:1px 0 3px;line-height:1.3">${v.aciklama}</div>` : ''}
  ...
  // Popüler ürün kartları — marka yok, isim + fiyat + prim
  const trendHtml = trendData.map((item, i) => {
    ...
    return `<div class="trend-card"
      ...
      ${primStr ? `<div style="font-size:.58rem;color:#b45309;font-weight:700;margin:1px 0 3px">${primStr}</div>` : ''}
      <button class="trend-card-add haptic-btn"
        onclick="event.stopPropagation();${oi >= 0 ? 'addToBasket(' + oi + ')' : ''}"
        title="Sepete ekle">+ Sepete</button>
    </div>`;
...
function renderTrendFullList() {
  ...
  if (vitrinAktif.length) {
    ...
    html += vitrinAktif.map(v => {
      ...
      return `<div class="trend-full-card" style="border-left:3px solid #e11d48">
        ...
        ${v.aciklama ? `<div style="font-size:.66rem;color:var(--text-2);margin-top:2px;line-height:1.4">${v.aciklama}</div>` : ''}
  ...
  if (trendData.length) {
    html += `<div style="grid-column:1/-1;display:flex;align-items:center;gap:8px;margin-top:${vitrinAktif.length ? '12px' : '0'};margin-bottom:4px">
      ...
      <span style="font-size:.60rem;color:var(--text-3)">son 14 gün · görüntülenmeye göre</span>
    </div>`;
