...
function _startMotdTicker() {
  ...
  const ad = currentUser?.Ad || currentUser?.Email?.split('@')[0] || '';
  const staticPlaceholder = ad ? 'En İyisiniz, ' + ad + ' — Ürün arama' : 'Ürün arama';
...
function checkUpsellOpportunities() {
  ...
  // ── Alan anahtarları ────────────────────────────────────────────
  const urunKey  = (() => {
    if (!allProducts.length) return 'Ürün';
    const k = Object.keys(allProducts[0]);
    return k.find(x => x === 'Ürün') || k.find(x => /ürün|urun/i.test(x)) || k[1] || 'Ürün';
  })();
  const acikKey  = 'Açıklama';
  const stokKey  = 'Stok';
  const kodKey   = 'Kod';
  const nakitKey = 'Nakit';

  // ── Para format ─────────────────────────────────────────────────
  function _fmt(t) {
    if (t >= 1000) { const k = t/1000; return (k%1===0 ? k.toFixed(0) : k.toFixed(1))+'k'; }
    return String(t);
  }

  // ── Açıklama → kampanya listesi ─────────────────────────────────
  function _parseCamps(ac) {
    const out = [];
    const re  = /([A-ZÇŞĞÜÖİ0-9_+]+)\s+[-–]?\s*([\d.,]+)([kK])?\s*\((\d+)\/([A-Z])\)/g;
    let m;
    while ((m = re.exec(ac)) !== null) {
      const raw = parseFloat(m[2].replace(',','.'));
      out.push({ grup:m[1].toUpperCase(), tutar:m[3]?raw*1000:raw, esik:parseInt(m[4]), rol:m[5].toUpperCase() });
    }
    return out;
  }

  // ── 1. Sepet kampanya envanteri ─────────────────────────────────
  // grup → { esik, tutar, rol (sepetteki), sahipBi, sepetOrt }
  const grupBilgi = {};
  basket.forEach((item, bi) => {
    const iNakit = parseFloat(String(item[nakitKey]||0).replace(',','.')) || 0;
    (item._campaigns||[]).forEach(camp => {
      if (camp.tip !== 'birlesen' && camp.tip !== 'kilitli') return;
      if (camp.tutar <= 0 || camp.esik < 2) return;
      if (camp.sonTarih && new Date() > camp.sonTarih) return;
      const g = camp.grup;
      if (!grupBilgi[g]) grupBilgi[g] = { esik:camp.esik, tutar:camp.tutar, roller:{}, sahipBi:new Set(), nakitler:[] };
      grupBilgi[g].sahipBi.add(bi);
      if (camp.tutar > grupBilgi[g].tutar) grupBilgi[g].tutar = camp.tutar;
      if (!grupBilgi[g].roller[bi]) { grupBilgi[g].roller[bi] = (camp.rol||'ANY').toUpperCase(); grupBilgi[g].nakitler.push(iNakit); }
    });
  });

  // ── 2. Seçili kampanyalar ───────────────────────────────────────
  const grupSecili = {};
  basket.forEach((item, bi) => {
    Object.entries(item._selectedCamps||{}).forEach(([ci,sel]) => {
      if (!sel) return;
  ...
  const sepetKodlar = new Set(basket.map(i => String(i[kodKey]||'')));

  // ── 3. Her grup için en iyi 2 tamamlayıcıyı bul ────────────────
  // YÖN FİLTRESİ: tamamlayıcı sepet ortalamasının 3 katından pahalıysa atla
  const YON = 3.0;
  function _findTop2(grup, esik, mevcutRoller, sepetOrt) {
    const results = [];
    for (const p of allProducts) {
      const pKod   = String(p[kodKey]||'');
      if (sepetKodlar.has(pKod)) continue;
      const pNakit = parseFloat(String(p[nakitKey]||0).replace(',','.')) || 0;
      if (sepetOrt > 0 && pNakit > sepetOrt * YON) continue;
      for (const pc of _parseCamps(p[acikKey]||'')) {
        if (pc.grup !== grup || pc.esik !== esik) continue;
        if (mevcutRoller.has(pc.rol)) continue;
        const stok = parseFloat(p[stokKey]) || 0;
        if (stok <= 0) continue;  // sadece stoklu öner
        results.push({ ad:(p[urunKey]||'').replace(/^[➥♲]\s*/,'').trim(), kod:pKod, nakit:pNakit, stok });
        break;
      }
    }
    results.sort((a,b) => a.nakit - b.nakit);
    return results.slice(0,2);
  }

  // ── 4. Öneri listesi üret ───────────────────────────────────────
  // Öncelik: en az eksik ürün + en düşük kazanç (kolay → güç)
  // Aynı tamamlayıcı setini öneren grupları tekilleştir (dedup)

  const oneriListesi = []; // [{ label, tutar, eksik, top2 }]

  // Tip A: Hazır (seçim yok)
  Object.entries(grupBilgi).forEach(([grup, info]) => {
    const seciliSayi = (grupSecili[grup]||new Set()).size;
    const sahipSayi  = info.sahipBi.size;
    const sepetRoller = new Set(Object.values(info.roller));
    const farkliRol = sepetRoller.has('ANY') ? info.esik : sepetRoller.size;
    if (seciliSayi === 0 && sahipSayi >= info.esik && (farkliRol >= info.esik || sepetRoller.has('ANY'))) {
      oneriListesi.push({ grup, tutar:info.tutar, eksik:0, tip:'hazir', top2:[] });
    }
  });
  ...
  Object.entries(grupBilgi).forEach(([grup, info]) => {
    ...
    if (seciliSayi > 0) {
      ...
      if (acikPos > 0 && (info.esik - acikPos) === 1 && info.sahipBi.size >= info.esik) {
        oneriListesi.push({ grup, tutar:info.tutar, eksik:1, tip:'secim_eksik', top2:[] });
      }
    }
  });

  // Tip C: Ürün eklenmeli
  Object.entries(grupBilgi).forEach(([grup, info]) => {
    const seciliSayi = (grupSecili[grup]||new Set()).size;
    if (seciliSayi > 0) return;
    const sahipSayi  = info.sahipBi.size;
    if (sahipSayi === 0 || sahipSayi >= info.esik) return;
    const eksik        = info.esik - sahipSayi;
    const mevcutRoller = new Set(Object.values(info.roller));
    const nakitler     = info.nakitler.filter(x=>x>0);
    const sepetOrt     = nakitler.length ? nakitler.reduce((a,b)=>a+b,0)/nakitler.length : 0;
    const top2         = _findTop2(grup, info.esik, mevcutRoller, sepetOrt);
    if (!top2.length) return;
    oneriListesi.push({ grup, tutar:info.tutar, eksik, tip:'ekle', top2 });
  });

  if (!oneriListesi.length) { container.innerHTML=''; return; }

  // ── 5. Sırala: kolay önce ──────────────────────────────────────
  // hazir(0) < secim_eksik(1) < ekle(eksik sayısı)
  // Aynı eksik sayısında: en düşük kazanç önce (en erişimlisi)
  const tipSkor = { hazir:0, secim_eksik:1 };
  oneriListesi.sort((a,b) => {
    const sa = a.tip==='ekle' ? 2+a.eksik : tipSkor[a.tip];
    const sb = b.tip==='ekle' ? 2+b.eksik : tipSkor[b.tip];
    return sa - sb || a.tutar - b.tutar;
  });

  // ── 6. Dedup: aynı top2 set'ini paylaşan grupları birleştir ────
  // Örn: 3KEA ve 4KEA aynı 2 ürünü öneriyorsa sadece 3KEA göster
  const seen = new Map(); // kod_seti → ilk grup
  const dedupListesi = [];
  for (const o of oneriListesi) {
    if (o.tip !== 'ekle') { dedupListesi.push(o); continue; }
    const setKey = o.top2.map(t=>t.kod).sort().join(',');
    if (!seen.has(setKey)) { seen.set(setKey, true); dedupListesi.push(o); }
  }

  // ── 7. En iyi 2 öneriyi göster ─────────────────────────────────
  const goster = dedupListesi.slice(0,2);

  // ── 8. HTML üret ───────────────────────────────────────────────
  // Global arama fonksiyonu (closure sorunu yok)
  window._upsellAra = function(kelime) {
    if (!kelime) return;
    const cm = document.getElementById('cart-modal');
    if (cm) { cm.classList.remove('open'); cm.style.display='none'; }
    if (typeof switchMainTab==='function') switchMainTab('urunler');
    requestAnimationFrame(() => {
  ...
  function _oneriHTML(o, idx) {
    if (o.tip === 'hazir') {
      return `<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
        <span style="flex-shrink:0">💡</span>
        <span>Sepette <b>${o.grup}</b> kampanyası tamamlanabilir → <b>+${_fmt(o.tutar)}</b></span>
      </div>`;
    }
    if (o.tip === 'secim_eksik') {
      return `<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
        <span style="flex-shrink:0">💡</span>
        <span><b>${o.grup}</b> için 1 ürün daha seç → <b>+${_fmt(o.tutar)}</b></span>
      </div>`;
    }
    // ekle tipi — 2 ürün butonu
    const eksikStr = o.eksik === 1 ? '1 ekle' : o.eksik+' ekle';
    const btnler = o.top2.map(t => {
      const araKelime = t.ad.split(' ').slice(0,3).join(' ');
      const fFmt = t.nakit>0 ? _fmt(t.nakit) : '';
      const stokStr = t.stok>0 ? t.stok+' st' : '';
      const detay = [fFmt, stokStr].filter(Boolean).join('·');
      return `<button onclick="window._upsellAra(${JSON.stringify(araKelime)})"
        style="background:#1e3a5f;color:#93c5fd;border:1px solid #1e40af;border-radius:5px;
          padding:3px 8px;font-weight:700;font-size:.61rem;cursor:pointer;
          white-space:nowrap;font-family:inherit;flex-shrink:0;line-height:1.4">
        🔍 ${t.ad}${detay?' <span style="opacity:.65;font-weight:400">('+detay+')</span>':''}
      </button>`;
    }).join('');
    return `<div style="display:flex;align-items:flex-start;gap:6px;flex-wrap:wrap;">
      <span style="flex-shrink:0;margin-top:2px">💡</span>
      <div style="flex:1;min-width:0">
        <span style="line-height:1.5"><b>${o.grup}</b> ${eksikStr} → <b>+${_fmt(o.tutar)}</b></span>
        <div style="display:flex;gap:5px;flex-wrap:wrap;margin-top:4px">${btnler}</div>
      </div>
    </div>`;
  }

  const satirlar = goster.map((o,i) => _oneriHTML(o,i)).join(
    '<div style="height:1px;background:#1e293b;margin:5px 0"></div>'
  );

  container.innerHTML = `<div style="background:#0f172a;color:#f8fafc;padding:8px 12px;
      font-size:.67rem;border-top:2px solid #f59e0b;">
    ${satirlar}
    <button onclick="document.getElementById('${CONTAINER_ID}').innerHTML=''"
      style="position:absolute;top:6px;right:8px;background:none;border:none;
        color:#64748b;font-size:.85rem;cursor:pointer;padding:0">✕</button>
  </div>`;
  // ✕ butonu için container'a relative position ver
...
function renderTrendCards() {
  ...
  const vitrinHtml = vitrinAktif.map(v => {
    ...
    return `<div class="trend-card vitrin-card"
      ...
      <div class="trend-rank" style="color:#e11d48;font-size:.58rem;font-weight:900">🛍️</div>
      ...
      ${primT ? `<div style="font-size:.58rem;color:#b45309;font-weight:700;margin:1px 0 2px">${primT}</div>` : ''}
      ${v.aciklama ? `<div style="font-size:.60rem;color:var(--text-3);line-height:1.3;margin:1px 0 2px">${v.aciklama}</div>` : ''}
      <button class="trend-card-add haptic-btn"
        onclick="event.stopPropagation();${oi >= 0 ? 'vitrinSepeteEkle(' + oi + ')' : ''}"
        title="Sepete ekle — sepet açılır">+ Sepete</button>
    </div>`;
  }).join('');
...
function renderTrendFullList() {
  ...
  if (vitrinAktif.length) {
    ...
    html += vitrinAktif.map(v => {
      ...
      const prim  = oi >= 0 ? _primDeger(allProducts[oi]) : 0;
      const primT = _primStr(prim);
      return `<div class="trend-full-card" style="border-left:3px solid #e11d48">
