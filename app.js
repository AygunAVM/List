...
window.exportAbakusExcel = function() {
  ...
  const urunNakitler = basket.map(item => Math.max(0,(item.nakit||0)-(item.itemDisc||0)));
  const toplamNakit = urunNakitler.reduce((s,v) => s+v, 0) || 1;
...
function buildPremiumPDF(docType, data) {
  ...
  const urunNakitler = urunler.map(u => Math.max(0, Number(u.nakit || u.fiyat || 0) - Number(u.itemDisc || 0)));
