...
function renderSepetDetay() {
  ...
  if(!bekliyor.length) { el.innerHTML='<div class="admin-empty">Bekleyen teklif bulunamadı</div>'; return; }
...
// ─── SİPARİŞ NOTU ────────────────────────────────────────────
const SIPARIS_KEY = 'aygun_siparis_notlari';

function getSiparisNotlari() {
  return JSON.parse(localStorage.getItem(SIPARIS_KEY)||'[]');
}
function saveSiparisNotlari(list) {
  localStorage.setItem(SIPARIS_KEY, JSON.stringify(list));
}

function openSiparisNot(urunAdi, urunIdx) {
  haptic(16);
  const not = prompt(`"${urunAdi}" için sipariş notu:\n(Adet, muadil veya açıklama girebilirsiniz)`, '');
  ...
  list.unshift({
    ...
    durum: 'bekliyor' // bekliyor | tamamlandi
  });
  saveSiparisNotlari(list);
  renderSiparisPanel();
  // Toast
  const ct = document.getElementById('change-toast');
  if(ct) {
...
function renderSiparisPanel() {
  ...
  if(!el) return;
  const list = getSiparisNotlari();
  if(!list.length) { el.innerHTML='<div class="admin-empty">Sipariş notu yok</div>'; return; }
  el.innerHTML = list.map(s => `
    <div class="siparis-row ${s.durum==='tamamlandi'?'siparis-done':''}">
      ...
      <div style="display:flex;flex-direction:column;gap:4px;align-items:flex-end">
        ${s.durum==='bekliyor'
          ? `<button class="pact-btn pact-green haptic-btn" style="font-size:.65rem;padding:4px 8px" onclick="siparisToggle('${s.id}')">✓ Tamamlandı</button>`
          : `<span style="font-size:.65rem;color:#10b981;font-weight:700">✓ Tamamlandı</span>`}
...
function siparisToggle(id) {
  const list = getSiparisNotlari();
  const idx = list.findIndex(s=>s.id===id);
  if(idx===-1) return;
  list[idx].durum = list[idx].durum==='tamamlandi'?'bekliyor':'tamamlandi';
  saveSiparisNotlari(list);
  renderSiparisPanel();
}
function siparisDelete(id) {
  const list = getSiparisNotlari().filter(s=>s.id!==id);
  saveSiparisNotlari(list);
  renderSiparisPanel();
  updateSiparisBadge();
}
function clearSiparisNotlari() {
  if(!confirm('Tüm sipariş notları silinsin mi?')) return;
  saveSiparisNotlari([]);
  renderSiparisPanel();
  updateSiparisBadge();
}
