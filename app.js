// ═══════════════════════════════════════════════════════════════
//  AYGÜN AVM — app.js  (Rev 2.1)
//  Düzeltmeler:
//    • Abaküs: Oran sütunu yok, Nakit seçeneği işaretlenebilir
//    • WhatsApp butonu görünürlük sorunu giderildi
//    • Admin mesajları: tam çalışır hale getirildi
//    • Admin paneli: tüm kullanıcı teklifleri görüntülenir
//    • Karşılama ekranı: header ℹ️ ile tekrar açılabilir
//    • Teklif & Satış Belgesi ayrıştırıldı
//    • Admin paneli: profesyonel görünüm iyileştirmeleri
//    • Teklif listesinde WhatsApp ile yeniden gönder butonu
// ═══════════════════════════════════════════════════════════════
...
function selectAbakusRow(rowEl) {
  ...
  if(waBtn) {
    ...
    if(abakusSelection===null) {
      ...
      const manEl=document.getElementById('ab-nakit');
      if(manEl && manEl.value!=='') { const mn=parseFloat(manEl.value.replace(',','.')); if(!isNaN(mn)&&mn>0) nakit=mn; }
      waBtn.innerHTML=`<span>📲</span><span>Nakit Teklif</span><strong>${fmt(nakit)}</strong><span class="ab-wa-oran">Nakit</span>`;
    } else {
...
function openWaFromAbakus() {
  ...
  // Kısa gecikmeyle aç — closeAbakus animasyonu bitsin
  setTimeout(()=>{
    ...
    if(abakusSelection===null) {
      if(title) title.textContent='💵 Nakit Teklif';
      if(info) {
        info.style.display='block';
        info.innerHTML=`<div class="wa-ab-info-box">
          <span class="wa-ab-chip wa-ab-nakit">💵 Nakit</span>
          <span class="wa-ab-chip wa-ab-tahsilat">${fmt(nakit)}</span>
        </div>`;
      }
    } else {
      ...
      if(info) {
        ...
        info.innerHTML=`<div class="wa-ab-info-box">
          ...
          <span class="wa-ab-chip wa-ab-tahsilat">${fmt(abakusSelection.tahsilat)}</span>
...
// Doğrudan sepetten teklif gönder (Abaküs olmadan, nakit)
// ─── TEKLİF KAYDET (Sepetten, müşteri adı modal ile) ─────────────
function saveProposalDirect() {
  ...
  // Başlığı güncelle
  const title=document.getElementById('wa-modal-title');
  if(title) title.textContent='💾 Teklif Kaydet';
  const info=document.getElementById('wa-abakus-info');
  if(info) {
    const t=basketTotals();
    const nakit=t.nakit-getDisc(t.nakit);
    info.style.display='block';
    info.innerHTML=`<div class="wa-ab-info-box"><span class="wa-ab-chip">💵 Sepet Toplamı</span><span class="wa-ab-chip wa-ab-tahsilat">${fmt(nakit)}</span></div>`;
  }
  // Gönder butonunu güncelle
  const sendBtn=document.querySelector('#wa-modal .wa-send-btn');
  if(sendBtn) { sendBtn.dataset.mode='save'; sendBtn.innerHTML='💾 Kaydet &amp; WhatsApp Gönder'; }
  m.style.display='flex';
  requestAnimationFrame(()=>m.classList.add('open'));
}
...
function finalizeProposal() {
  ...
  const sendBtn=document.querySelector('#wa-modal .wa-send-btn');
  const isSaveMode = sendBtn && sendBtn.dataset.mode === 'save';
  ...
  if(phone && (phone.length!==11||phone[0]!=='0')) {
    alert('0 ile başlayan 11 haneli telefon giriniz'); haptic(80); return;
  }
  ...
  const prop = {
    ...
    custName, phone: phone||'—', urunler: basket.map(i=>({...i})),
  ...
  // WhatsApp gönder (telefon girilmişse)
  if(phone && phone.length===11 && phone[0]==='0') {
  ...
  if(ct){ ct.textContent='✓ Teklif kaydedildi — '+custName; ct.classList.add('show'); setTimeout(()=>ct.classList.remove('show'),2500); }

  closeWaModal();
  ['cust-name','cust-phone','extra-info'].forEach(id=>{ const el=document.getElementById(id); if(el) el.value=''; });
  if(sendBtn) { sendBtn.dataset.mode=''; sendBtn.innerHTML='📲 Teklifi Gönder &amp; Kaydet'; }
  abakusSelection=null;
}
...
function renderProposals(container, showAll) {
  ...
  if(!target) return;
  // Admin her teklife erişir, user sadece kendi tekliflerine
  const myProps = (isAdmin() && showAll)
    ? proposals
  ...
  target.innerHTML=myProps.map(p => {
    ...
    const waResend = `<button class="btn-wa-resend haptic-btn" onclick="resendProposalWa('${p.id}')" title="WhatsApp ile tekrar gönder">📲 WA</button>`;
    const adminBtns = canAct && p.durum==='bekliyor'
      ...
      : (canAct && p.durum!=='bekliyor'
          ? `<div class="proposal-actions"><span style="font-size:.70rem;color:var(--text-3)">${p.durum==='onaylandi'?'✓ Onaylandı':'✕ İptal edildi'}</span>${waResend}</div>`
          : '');
    // Admin görünümünde kullanıcı adı göster
    const userTag = showAll && isAdmin() ? `<span class="proposal-tag">👤 ${p.user}</span>` : '';
...
function checkUnreadMessages() {
  ...
  if(bar) {
    bar.classList.toggle('visible', unread>0);
    const t=document.getElementById('user-msg-bar-text');
    if(t) t.textContent=`${unread} yeni mesajınız var`;
  }
}
...
function openMessages() {
  ...
  renderMessages();
  if(isAdmin()) {
    const ca=document.getElementById('msg-compose-area');
    if(ca) {
...
function renderMessages() {
  ...
  const myMsgs = isAdmin()
    ? messages
    : messages.filter(m=>m.to==='all'||m.to===currentUser?.Email);
  if(!myMsgs.length) {
    body.innerHTML='<div class="empty-cart" style="height:120px;"><span class="empty-cart-icon">✉</span>Mesaj yok</div>';
    return;
  }
  body.innerHTML=myMsgs.map(msg=>{
    const ismine = msg.from === (currentUser?.Email||'') || msg.from === 'Admin';
...
async function loadMsgRecipients() {
  ...
  // proposal'lardan
  proposals.forEach(p=>{ if(p.user) userSet.add(p.user); });
  // analytics'ten
  const data=JSON.parse(localStorage.getItem('analytics_local')||'{}');
  Object.values(data).forEach(byUser=>Object.keys(byUser).forEach(email=>userSet.add(email)));
  // kullanicilar.json'dan (en güvenilir)
  try {
    const resp = await fetch('data/kullanicilar.json?t='+Date.now());
    const users = JSON.parse((await resp.text()).replace(/^\uFEFF/,'').trim());
    const arr = Array.isArray(users) ? users : (users.data||[]);
    arr.forEach(u=>{ if(u.Email) userSet.add(u.Email.toLowerCase().trim()); });
  } catch(e) { /* offline */ }

  sel.innerHTML='<option value="all">Herkese</option>'+
    [...userSet].filter(u=>u && u!==currentUser?.Email).map(u=>`<option value="${u}">${u}</option>`).join('');
}
...
function sendAdminMessage() {
  ...
  if(!toEl || !textEl) { console.error('Mesaj formu elemanları bulunamadı.'); return; }
  const to=(toEl.value||'all').trim();
  const text=(textEl.value||'').trim();
  if(!text) { haptic(60); textEl.focus(); textEl.style.border='1.5px solid var(--red)'; setTimeout(()=>textEl.style.border='',1200); return; }
  const msg={
    id:uid(),
    ts:new Date().toISOString(),
    from: currentUser?.Email || 'Admin',
    to, text, read:false
  };
  ...
  haptic(20);
  // Başarı bildirimi
  const btn=document.querySelector('#msg-compose-area .msg-send-btn');
  if(btn) {
  ...
  // Tost göster
  const ct=document.getElementById('change-toast');
  if(ct){ ct.textContent=`✓ Mesaj gönderildi → ${to==='all'?'Herkes':to}`; ct.classList.add('show'); setTimeout(()=>ct.classList.remove('show'),2200); }
}
...
async function renderAdminPanel() {
  ...
  const todayActiveUsers=Object.keys(todayData).filter(u=>(todayData[u].logins||0)>0).length;
  ...
  document.getElementById('stat-sales').innerHTML     = `${sales.length}`;
  document.getElementById('stat-users').innerHTML     = `${au.size}<span class="stat-today">${todayActiveUsers} aktif</span>`;
...
function renderAdminUsers() {
  ...
  el.innerHTML=su.map(([email,s])=>{
    ...
    return `<div class="user-row">
      ...
      <div class="user-action-btns"><button class="btn-user-msg haptic-btn" onclick="quickMsgUser('${email}')">✉</button></div>
    </div>`;
...
function renderAdminMessages() {
  ...
  if(!el) return;
  if(!messages.length) { el.innerHTML='<div class="admin-empty">Mesaj yok</div>'; return; }
  el.innerHTML=messages.map(m=>{
    const unread=!m.read;
    return `<div class="admin-msg-row ${unread?'unread':''}">
      <div class="admin-msg-header">
        <span class="msg-from">${m.from}</span>
        <span class="admin-msg-to">→ ${m.to==='all'?'Herkese':m.to}</span>
        <span class="msg-time">${fmtDate(m.ts)}</span>
      </div>
...
function quickMsgUser(email) {
  ...
  setTimeout(()=>{
    ...
    setTimeout(()=>{
      ...
      if(sel) { loadMsgRecipients(); setTimeout(()=>{ sel.value=email||'all'; }, 50); }
