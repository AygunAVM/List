...
function addToBasket(idx) {
  ...
  // Hiyerarşi guard: fiyatı gösterilmeden sepete eklenen ürünü blur'lanmış say
  {
    const _gKeys = Object.keys(p);
    const _gUrunK = _gKeys.find(k => (k||'').toLowerCase() === 'urun') || '';
    const _gAdi = p[_gUrunK] || '';
    if (_gAdi && _sessionData && !_sessionData.blurUrunler[_gAdi]) {
      _sessionData.blurUrunler[_gAdi] = true;
      if (!_sessionData.revealedPrices.includes(_gAdi)) _sessionData.revealedPrices.push(_gAdi);
      if (!_blurredThisSession[_gAdi]) {
        const _gIdx = allProducts.findIndex(pr => {
          const _kk = Object.keys(pr).find(k2 => (k2||'').toLowerCase() === 'urun') || '';
          return pr[_kk] === _gAdi;
        });
        if (_gIdx >= 0) _blurredThisSession[_gAdi] = _gIdx;
      }
      localStorage.setItem('_sd', JSON.stringify(_sessionData));
    }
  }
...
logs.forEach(l => {
  ...
  if (h >= 0 && h <= 23) { 
    ...
    // Tutarsızlık uyarısı kaldırıldı — eski kayıtlarda benzersizBlurSayisi eksik olabilir, Math.min zaten kırpıyor
