...
function vitrinUrunAra(query) {
  ...
  dropdown.innerHTML = hits.map(({ u, idx }) => {
    ...
    // fmt() zaten ₺ ekliyor — extra ₺ EKLEME
...
function vitrinUrunSec(idx, urunAdi, nakit, not) {
  ...
  if (el('vitrin-urun-input'))              el('vitrin-urun-input').value  = urunAdi;
  if (el('vitrin-fiyat-input') && nakit>0)  el('vitrin-fiyat-input').value = nakit;
  if (el('vitrin-aciklama-input'))          el('vitrin-aciklama-input').value = not || '';
  const dd = el('vitrin-urun-dropdown');
  ...
  if (!vitrinAktif.length && !trendData.length) {
    container.innerHTML = `<div style="grid-column:1/-1;padding:32px 16px;text-align:center">
      ...
      <div style="font-size:.78rem;color:var(--text-3);line-height:1.6">
        Admin panelinden vitrin ürünü ekleyin.<br>
        Popüler ürünler sepete eklendikçe burada görünür.
      </div>
    </div>`;
    return;
  }
...
async function saveVitrinUrun() {
  ...
  const productIdx = _vitrinSeciliIdx; // allProducts index — sepete at için

  if (!urunAdi || _vitrinSeciliIdx < 0) { _campToast('Listeden bir ürün seçin.', 'warn'); return; }
