...
logs.forEach(l => {
  ...
  if (h>=0) { 
    ...
    const personelHTML = Object.entries(pMap)
      ...
      .map(([,s]) => {
        ...
        return `<div style="background:var(--surface);border:1.5px solid var(--border);border-radius:16px;padding:14px;position:relative">
          ...
          <!-- Detay satırı — kompakt -->
          <div style="display:flex;gap:6px;margin-top:6px;font-size:.56rem;color:var(--text-3)">
            <span>🥇 Altın: ${s.altin}</span>
            <span>🥈 Gümüş: ${s.gumus}</span>
            <span style="margin-left:auto">Kaçan: ${s.kacti} (${kO}%)</span>
          </div>
        </div>`;
      }).join('');
    ...
    // Her aşamayı benzersiz oturum üzerinden say — mükerrer kayıt önler
    // Blur: bu oturumda en az 1 fiyat açılmış
    const _blurOturumSet = new Set(
      logs.filter(l => (l.benzersizBlurSayisi || 0) > 0 || (l.blurUrunListesi||l.bakilanFiyatlar||[]).length > 0)
          .map((_,i) => i)  // indeks = oturum kimliği (log dizisi tekil)
    );
    const funnelBlur = _blurOturumSet.size;

    // Sepet: bu oturumda en az 1 ürün sepete eklenmiş (derinlik > 0 veya urunler dolu)
    const funnelSepet = Math.min(funnelBlur,
      logs.filter(l => (l.derinlik || 0) > 0 || (l.urunler||[]).length > 0).length
    );

    // L3/Abaküs: intentLevel >= 3 olan oturumlar, Sepet'i geçemez
    const funnelL3 = Math.min(funnelSepet, l3Toplam);

    // Teklif: aktif proposals (L3'ü geçemez)
    const funnelTeklif = Math.min(funnelL3, proposals.filter(p => p.durum !== 'iptal').length);

    // Satış: gerçek satış oturumları, Teklif'i geçemez
    const funnelSatis = Math.min(funnelTeklif > 0 ? funnelTeklif : funnelL3, totS);
