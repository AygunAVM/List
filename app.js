...
function resendProposalWa(id) {
  ...
  const exp = new Date(); // Varsayılan: bugün
...
async function showApp() {
  ...
  // Placeholder + kayan yazı — _startMotdTicker hem statik placeholder'ı set eder hem ticker'ı başlatır
  _startMotdTicker();
...
function _startMotdTicker() {
  ...
  const staticPlaceholder = ad ? 'Liste Yanlıştır, ' + ad + ' — Ürün arama' : 'Ürün arama';
...
// ES Module → global bağlantı: login butonu onclick için hemen window'a at
...
function renderCampaignPills(item, idx) {
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    if (isBilgi) {
      ...
      // Seçili + eşik bekliyor — saydam amber
    ...
    // Pill içi rozet: seçilmemiş+eşikli → xN, pending → +N
    let pillRozet = '';
...
function _renderMergedItem(m, idx, isRequired=true) {
  ...
  // Başlık ikonu ve Badge'lar — önce hesapla, sonra kullan
