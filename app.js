...
async function openAbakus() {
  ...
  if (abakusSelection && abakusSelection.kart) {
    ...
    if (ks) {
      ...
      if (opts.includes(abakusSelection.kart)) ks.value = abakusSelection.kart;
      else { const m = opts.find(o=>o.toLowerCase().includes(abakusSelection.kart.toLowerCase())); if(m) ks.value=m; }
    }
  }
  calcAbakus();
}
...
function _qfRenderTaksit() {
  ...
  Object.entries(goster).forEach(([n,v]) => {
    ...
    rows += '<tr class="qf-taksit-row'+sel+'" onclick="_qfSelectPlan(this,\''+_qfTaksitKart+'\','+n+','+v.tahsilat+',\'+(v.zincir||_qfTaksitKart)+'\')">'+
