...
function updateCartUI() {
  ...
  if(isAdmin()) {
    ...
    const discOpen = !!window._cartDiscOpen;
    basket.forEach((item,idx) => {
      ...
      rows+=`<tr>`+
        ...
        (discOpen ? `<td><input type="number" class="item-disc-input" min="0" value="${itemDisc||''}" placeholder="0"
          onblur="setItemDisc(${idx},this.value)" onkeydown="if(event.key==='Enter'){setItemDisc(${idx},this.value);this.blur()}" style="width:60px;padding:3px 5px;border:1px solid var(--border);border-radius:5px;font-size:.70rem;text-align:right;background:var(--surface)"></td>`+
