...
async function showApp() {
  ...
  if (searchEl) {
    ...
    searchEl.placeholder = ad ? 'Liste Hatalıdır Kullanma!, ' + ad + ' — Ürün arama' : 'Ürün arama';
  }
...
function toggleCampaign(idx, campIdx) {
  ...
  if (camp.tip !== 'proje') {
    ...
    if (camp.tip === 'kilitli' && itemHasKilitli) {
      const eskiEntry = Object.entries(item._selectedCamps).find(([ci2, sel2]) => {
        if (!sel2) return false;
        const c2 = item._campaigns[parseInt(ci2)];
        return c2 && c2.tip === 'kilitli' && c2.grup === camp.grup;
      });
      if (eskiEntry) {
        delete item._selectedCamps[parseInt(eskiEntry[0])];
      }
    }
  }
...
function _renderMergedItem(m, idx, isRequired=true) {
  ...
  // Başlık ikonu ve Badge'lar — tanımla, sonra kullan
  ...
  return `<div class="change-item ${mandCls}" data-idx="${idx}" ${clickEvt}>
    <span class="ci-emoji">${emoji2}</span>
    <div class="ci-body">
      <span class="ci-urun">${m.urun}</span>
      <span class="ci-detail">${details}</span>
    </div>
    <div class="ci-badges">${badges2}${readTag}</div>
  </div>`;
}
