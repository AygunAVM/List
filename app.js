...
function showApp() {
  ...
  if(searchEl) {
    ...
    searchEl.placeholder = ad ? 'KasırG ' + ad + ' — Ürün arama' : 'Ürün arama';
  }
}
...
function showChangePopup(changes, logKey) {
  ...
  if(mandItems.length) {
    ...
    // Satış kullanıcıları için: %20'si kadarını (min 2) random seç, kalan görüntülenir ama onaylanmaz
    const isSatisUser = !isAdmin();
    let randomSelected = null;
    let requiredCount = mergedItems.length;
    if(isSatisUser && mergedItems.length > 2) {
      requiredCount = Math.max(1, Math.ceil(mergedItems.length * 0.10));
      if(requiredCount < mergedItems.length) {
        const shuffled = [...Array(mergedItems.length).keys()].sort(() => Math.random() - 0.5);
        randomSelected = new Set(shuffled.slice(0, requiredCount));
      }
    }
    const subLabel = isSatisUser && randomSelected
      ? `${requiredCount} tanesini onaylayın (${mergedItems.length} değişim var — okuyarak geçin)`
    ...
    mergedItems.forEach((m, mIdx) => {
      ...
      html += _renderMergedItem(m, itemIdx++, isRequired);
    });
...
function renderAdminUsers() {
  // _fbAnalytics henüz yüklenmediyse 1 sn sonra retry
  if(!window._fbAnalytics || Object.keys(window._fbAnalytics).length === 0) {
    const el2 = document.getElementById('admin-user-list');
    if(el2) el2.innerHTML = '<div class="admin-empty">⏳ Veriler yükleniyor...</div>';
  ...
  el.innerHTML = su.map(([email,s])=>{
    ...
    // Tüm analytics kayıtlarından bu kullanıcıya ait en güncel kaydı bul
    let bestDoc = null;
    if(window._fbAnalytics) {
      const uDocs = Object.values(window._fbAnalytics)
        .filter(d => d.email === email)
        .sort((a,b) => (b.date||'').localeCompare(a.date||''));
      if(uDocs.length) bestDoc = uDocs[0];
    }
    // localStorage fallback (admin kendi cihazı veya test)
    if(!bestDoc) {
      const localAnal = JSON.parse(localStorage.getItem('analytics_local')||'{}');
      const dates = Object.keys(localAnal).sort().reverse();
      for(const d of dates) {
        if(localAnal[d][email]) { bestDoc = {...localAnal[d][email], date:d, email}; break; }
      }
    }
    const userAppVer  = bestDoc?.currentAppVer || bestDoc?.popupSeen || null;
    const popupSeenTs = bestDoc?.popupSeenTs || null;
    const appVer      = window._currentVersion || '';
    const isUpToDate  = userAppVer && appVer && userAppVer.split(' ')[0] === appVer.split(' ')[0];
    const verLabel    = userAppVer ? userAppVer.split(' ')[0] : null;
...
function renderSepetDetay() {
  ...
  // 2. Firebase analytics'te basket snapshot varsa göster
  //    Yoksa: personelin bekleyen tekliflerini "Teklif Sepeti" olarak sun
  //    Admin bu kısımdan teklif sepetlerini silebilir
  const myEmail = currentUser?.Email || '';
  if(window._fbAnalytics) {
    const basketByUser = {};
    Object.values(window._fbAnalytics).forEach(rec => {
      if(!rec.email || rec.email === myEmail) return; // kendi sepetimiz zaten üstte
      if(rec.basketSnapshot && rec.basketSnapshot.length > 0) {
        if(!basketByUser[rec.email] || (rec.date||'') > (basketByUser[rec.email].date||'')) {
          basketByUser[rec.email] = rec;
        }
      }
    });
    Object.entries(basketByUser).forEach(([email, rec]) => {
      const items = rec.basketSnapshot || [];
      const ini = email.split('@')[0].slice(0,2).toUpperCase();
      ...
      html_parts.push(
        ...
        '<span style="font-weight:700">' + email.split('@')[0] + '</span>' +
        '<span class="stok-badge sk" style="background:#dbeafe;color:#1e40af">' + items.length + ' ürün</span>' +
        '</div>' + rows + '</div>'
      );
    });
  }

  // 3. Bekleyen teklifleri olan personeller — teklif bazlı sepet özeti
  if(proposals && proposals.length > 0) {
    const allPropUsers = [...new Set(proposals.filter(p=>p.durum==='bekliyor').map(p=>p.user).filter(u=>u&&u!==myEmail))];
    allPropUsers.forEach(user => {
      const userProps = proposals.filter(p=>p.user===user&&p.durum==='bekliyor');
      if(!userProps.length) return;
      const ini = user.split('@')[0].slice(0,2).toUpperCase();
      const propRows = userProps.map(p => {
        const urunNames = (p.urunler||[]).map(u=>u.urun||'?').join(', ');
        return '<div class="sepet-item-row" style="border-left:2px solid #fde68a;padding-left:8px">' +
          '<span class="sepet-item-urun" style="font-size:.72rem">' + urunNames + '</span>' +
          '<span class="sepet-item-price">' + fmt(p.nakit||0) + '</span>' +
          '<button onclick="deleteProp(\'' + p.id + '\')" style="margin-left:6px;background:#fee2e2;border:none;border-radius:4px;padding:2px 7px;font-size:.65rem;cursor:pointer;color:#dc2626">Sil</button>' +
          '</div>';
      }).join('');
      html_parts.push(
        '<div class="sepet-user-block">' +
        ...
        '<span class="stok-badge sk" style="background:#fef3c7;color:#92400e">' + userProps.length + ' bekleyen teklif</span>' +
        '<button onclick="clearUserProps(\'' + user + '\')" style="margin-left:auto;background:#fee2e2;border:none;border-radius:6px;padding:3px 10px;font-size:.65rem;cursor:pointer;color:#dc2626">Hepsini Sil</button>' +
        '</div>' + propRows + '</div>'
      );
    });
  }
...
Object.assign(window, {
  ...
  resetProductStats, exportBasketToExcel, renderUyuyanStok, deleteProp, renderSepetDetay, clearUserProps,
