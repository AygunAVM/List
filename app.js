...
function toggleCampaign(idx, campIdx) {
  ...
  if (camp.tip !== 'proje') {
    ...
    // Aynı üründe aynı GRUPTAN 🔒: radio-button davranışı — eskiyi bırak, yenisini seç
    if (camp.tip === 'kilitli' && itemHasKilitli) {
      const eskiKilitliEntry = Object.entries(item._selectedCamps).find(([ci2, sel2]) => {
        if (!sel2) return false;
        const c2 = item._campaigns[parseInt(ci2)];
        return c2 && c2.tip === 'kilitli' && c2.grup === camp.grup;
      });
      if (eskiKilitliEntry) {
        // Eski 🔒'yı sil — yeni seçim aşağıda eklenir
        delete item._selectedCamps[parseInt(eskiKilitliEntry[0])];
      }
    }
  }
...
function renderCampaignPills(item, idx) {
    ...
    if (!isBilgi && esikGerekli >= 2) {
      if (sel && pending && kalanEsik > 0) {
        pillRozet = `<span style="font-size:.52rem;background:#d97706;color:#fff;`
          + `padding:1px 5px;border-radius:3px;font-weight:800;margin-left:3px">+${kalanEsik}</span>`;
      } else if (!sel) {
        pillRozet = `<span style="font-size:.50rem;background:rgba(0,0,0,.10);color:inherit;`
          + `padding:1px 4px;border-radius:3px;font-weight:700;margin-left:2px;opacity:.8">x${esikGerekli}</span>`;
      }
    }
  ...
  // Pending olan ilk grubun kalanEsik sayısını hesapla
