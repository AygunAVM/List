...
function _doPrintTeklif(p) {
  ...
  // Nakit net tutar (kartlı işlemde "liste nakit fiyat" gösterilir, kart tahsilatı ayrı)
  const nakitNet        = toplamNakit - toplamIndirim;

  // ── Ürün satırları ───────────────────────────────────────────
  // Kart işleminde: ürün fiyatı = nakit + satır indirimi (yani liste fiyatı)
  // Üstü çizili liste fiyatı → net fiyat gösterimi
  const urunRows = (p.urunler||[]).map((u,i) => {
    ...
    const listeFiyat  = ab ? nakitFiyat + itemDisc : nakitFiyat; // kartlı işlemde liste = nakit+itemDisc
  ...
  // Sadece toplam indirim varsa göster — tek satır
  ...
  if(ab) {
    ...
    if(taksitSay <= 1) {
      // Tek çekim: İndirimler + Ödeme Şekli + Toplam
      odemeRows = `
        ${indirimRow}
      ...
      // Taksitli: İndirimler + Ödeme Şekli + Taksit Sayısı + Aylık + Toplam
  ...
  // ... (LOGO_B64 ve HTML kısmı mevcut koddan aynı kalır, sadece odemeRows değişti)
