...
function renderProposals(container, forceAll, searchQ) {
  ...
  // Aynı müşteriye (isim + telefon eşleşmesi) birden fazla teklif varsa gruplandır
  const custMap = new Map();
  myProps.forEach(p => {
    const key = (p.custName||'').trim().toLowerCase() + '||' + (p.phone||'').trim();
    if(!custMap.has(key)) custMap.set(key, []);
    custMap.get(key).push(p);
  });

  // Grupları render et: çoklu teklif varsa başlık + collapse
  let renderHtml = '';
  const renderedIds = new Set();

  custMap.forEach((group, key) => {
    if(group.length > 1) {
      // Çoklu teklif — grup başlığı
      const rep = group[0];
      const bekCnt = group.filter(p => p.durum==='bekliyor'||p.durum==='sureDoldu').length;
      const grpId = 'grp_' + key.replace(/[^a-z0-9]/g,'_').slice(0,20);
      ...
      // Her teklifi render et
      group.forEach(p => {
        renderHtml += _renderSingleProp(p);
      });
      renderHtml += `</div></div>`;
    }
  });

  // Geri kalan teklifler (gruplanmamış)
  myProps.forEach(p => {
...
Object.assign(window, {
  ...
  clearAllPendingProps, logoutUser, toggleChangeItem, toggleChangeItemRow, markAllChanges, confirmSection, printTeklif, togglePropGroup,
