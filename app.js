...
const APP_BUILD_VERSION = 'V8.9-20260730-0316';
...
async function fetchProposalsOnce() {
  try {
...
async function fetchSiparisOnce() {
  try {
  ...
  } catch(e) { console.warn('fetchSiparisOnce:', e); }
}

async function fetchAnalyticsOnce() {
  try {
    // ✅ Rev 8.1: admin sorgusu artık son 90 günle sınırlı. Bu koleksiyona
    // her gün her kullanıcı için 1 kayıt ekleniyor (silinmiyor) — sınırsız
    // çekmek zamanla (aylar geçtikçe) admin panel açılışının maliyetini
    // sürekli artıracaktı. Panelin gösterdiği istatistikler zaten son
    // 7-90 günlük trendlere odaklı, 90 günden eskisine ihtiyaç yok.
  ...
  } catch(e) { console.warn('fetchAnalyticsOnce:', e); }
}

async function fetchMotdOnce() {
  try {
    ...
    _startMotdTicker();
  } catch(e) { console.warn('fetchMotdOnce:', e); }
}
...
function _propFreshTick() {
  ...
  dot.className = 'prop-fresh-dot' + (saniye > 300 ? ' stale' : '');
}
...
async function refreshProposals() {
  ...
  const ok = await fetchProposalsOnce();
...
async function refreshAdminPanel() {
  ...
  const ok = await switchAdminTab(activeTab);
  if (btn) {
    btn.classList.remove('loading');
    if (ok) {
      btn.classList.add('success');
...
async function switchAdminTab(tab) {
  ...
  if(tab==='proposals') {
    ...
    renderArchivedProposals();
    fetchPromise = fetchProposalsOnce().then(ok => {
      renderProposals(document.getElementById('admin-proposals-list'), true);
      renderArchivedProposals();
      return ok;
    });
  ...
  if(tab==='siparis')  {
    ...
    fetchPromise = fetchSiparisOnce().then(ok => { renderSiparisPanel(); updateSiparisBadge(); return ok; });
  }
  if(tab==='sepetler') {
    renderSepetDetay();
    fetchPromise = fetchProposalsOnce().then(ok => { renderSepetDetay(); return ok; });
  }
  if(tab==='personel') {
    renderAdminUsers();
    fetchPromise = Promise.allSettled([fetchProposalsOnce(), fetchSalesOnce(), fetchAnalyticsOnce()])
