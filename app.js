...
function showApp() {
  ...
  if(searchEl) {
    ...
    searchEl.placeholder = ad ? 'Efsane, ' + ad + ' — Ürün arama' : 'Ürün arama';
  }
}
...
function renderTable(searchVal) {
  ...
  data.forEach(u => {
    ...
    const addBtnCls = stok === 0 ? 'add-btn add-btn-stok0' : stok <= 3 ? 'add-btn add-btn-stokaz' : stok <= 10 ? 'add-btn add-btn-stokorta' : 'add-btn add-btn-stokbol';
    tr.innerHTML =
      `<td><button class="${addBtnCls} haptic-btn" onclick="addToBasket(${oi})">+</button></td>`+
...
async function loadAnalyticsData() {
  ...
  if(window._fbAnalytics && Object.keys(window._fbAnalytics).length > 0) {
    ...
    Object.values(window._fbAnalytics).forEach(fbRec => {
      ...
      // Firebase verisi varsa öncelik Firebase'de — çünkü tüm cihazlardan toplanan veri
      const existing = merged[date][email] || {};
      merged[date][email] = {
        logins:      Math.max(fbRec.logins      || 0, existing.logins      || 0),
        proposals:   Math.max(fbRec.proposals   || 0, existing.proposals   || 0),
        sales:       Math.max(fbRec.sales       || 0, existing.sales       || 0),
