...
function checkChanges(json) {
  ...
  // Daha önce bu versiyon işlendi mi?
  const seen = JSON.parse(localStorage.getItem(seenKey)||'[]');
  if(seen.includes(vKey)) return; // Bu versiyon zaten loglandı

  // Önceki snapshot var mı?
  const lastJson = JSON.parse(localStorage.getItem(lastKey)||'null');
  if(lastJson) {
    const changes = _diffJson(lastJson, json);
    if(changes.length > 0) {
      // Bu versiyon geçişini log'a ekle
...
function showPendingChanges(logKey) {
  ...
  //   - shown:false  → yeni, işaretsiz göster
  //   - shown:true   → zaten görülmüş, çizili göster (bağlam için)
  // Sadece yeni olan varsa popup aç
  const hasNew = log.some(e => !e.shown);
  if(!hasNew) return;

  // Tüm girişleri göster (eski=çizili, yeni=işaretsiz)
  const allChanges = [];
  log.forEach(entry => {
    allChanges.push({
      type:'versiyon',
      ...
      to: entry.toV,
      ts: entry.ts,
      isOld: entry.shown   // eski versiyon → çizili başlık
    });
    entry.changes.forEach(c => allChanges.push({ ...c, isOld: entry.shown }));
    // Yeni olanları shown=true yap (popup açılınca)
...
function _updateChangeBtn() {
  ...
  // Admin için "Tümünü İşaretle" butonu göster/gizle
  ...
  btn.textContent      = ready ? '✓ Anladım, Kapat' : 'Tüm değişimleri işaretleyin (' + allNew.length + ' kaldı)';
}
