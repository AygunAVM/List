...
function updateCartUI() {
  ...
  basket.forEach((item,idx) => {
    const itemDisc = item.itemDisc || 0; // satır bazlı indirim
    ...
    rows+=`<tr>`+
      ...
      `<td><input type="number" class="item-disc-input" min="0" value="${itemDisc||''}" placeholder="0"
        oninput="setItemDisc(${idx},this.value)" style="width:60px;padding:3px 5px;border:1px solid var(--border);border-radius:5px;font-size:.70rem;text-align:right"></td>`+
  ...
  // Genel indirim satırı
  let dr='';
  const totalItemDisc = basket.reduce((s,i)=>s+(i.itemDisc||0),0);
  if(discountAmount>0) {
    dr=`<tr class="discount-row">
      <td colspan="3" style="text-align:right;font-size:.69rem;color:var(--text-2)">Genel İndirim ${discountType==='PERCENT'?'%'+discountAmount:fmt(discountAmount)}</td>
      <td class="cart-price" style="text-decoration:line-through;opacity:.5">${fmt(t.nakit)}</td>
      <td></td>
      <td class="cart-price" style="color:#16a34a">-${fmt(getDisc(t.nakit-totalItemDisc))}</td>
      <td></td></tr>`;
  }
  ...
  const tot=`<tr class="total-row">
    <td colspan="3" style="text-align:right;font-weight:800;font-size:.78rem">NET TOPLAM</td>
    <td class="cart-price" style="text-decoration:${(discountAmount>0||totalItemDisc>0)?'line-through':'none'};opacity:${(discountAmount>0||totalItemDisc>0)?.5:1}">${fmt(t.nakit)}</td>
    <td></td>
    <td class="cart-price" style="font-weight:800;color:var(--text-1)">${fmt(Math.max(0,nakitFinal))}</td>
    <td></td></tr>`;
  area.innerHTML=`<table class="cart-table"><thead><tr>
    <th>Ürün</th><th>Stok</th><th>Açıklama</th>
    <th>Nakit</th><th>Satır İnd.</th><th>Net</th><th></th>
...
Object.assign(window, {
  ...
  clearAllPendingProps, logoutUser, toggleChangeItem, toggleChangeItemRow, markAllChanges, confirmSection, printTeklif, togglePropGroup, setItemDisc,
