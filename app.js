...
function toggleCampaign(idx, campIdx) {
  ...
  // KURAL 1b: kilitli kampanya — sepette ayni grupta baska kilitli secilmis olmamali
  if (camp.tip === 'kilitli') {
    const grupAdi = camp.grup;
    const sepetteAyniGrupKilitli = basket.some((b, bi) => {
      ...
      return Object.entries(b._selectedCamps).some(([ci, sel]) => {
        ...
        return bc && bc.grup === grupAdi && bc.tip === 'kilitli';
      });
    });
    if (sepetteAyniGrupKilitli) {
      _campToast(
        camp.grup + ' grubunda zaten 🔒 kampanya seçili. '
        + 'Bu grup için sepette yalnızca bir 🔒 kampanya kullanılabilir.',
        'warn'
      );
      return;
    }
  ...
  // ═══════════════════════════════════════════════════════════
  // KURAL 3: Eşik (esik) aşılamaz — fazla ürün seçimi engeli
  // ═══════════════════════════════════════════════════════════
  if (camp.esik > 1) {
    const grupAdi = camp.grup;
    // Bu grupta kaç farklı ürün seçilmiş?
    ...
    if (grupSeciliUrunler.size >= camp.esik && !buUrunGrupta) {
      _campToast(
        camp.grup + ' kampanyası en fazla ' + camp.esik + ' ürün ile kullanılabilir.',
        'warn'
      );
      return;
    }
...
function renderCampaignPills(item, idx) {
  ...
  // Aynı ürün içi mevcut seçim tipleri
  const itemHasKilitli  = Object.entries(item._selectedCamps).some(
    ([ci, sel]) => sel && item._campaigns[parseInt(ci)]?.tip === 'kilitli'
  );
  const itemHasBirlesen = Object.entries(item._selectedCamps).some(
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    // Devre dışı: aynı üründe tür çakışması (🔒 seçiliyken ⎇ göster, tersi)
    ...
    if (isBilgi) {
      bg = pal.bg; border = pal.bd; color = pal.tx; icon = '✦';
    } else if (sel && !pending) {
    ...
    // Eşik rozeti — sadece eşik≥2 ve seçilmemiş
    const esikRozet = (camp.esik > 1 && !isBilgi && !sel)
    ...
    // Harf rozeti — rol seçilmişse göster (A/B/C)
    const harfRozet = (camp.rol && camp.rol !== 'ANY' && !sel)
      ? '<span style="font-size:.50rem;font-weight:800;margin-left:2px;opacity:.7">' + camp.rol + '</span>'
      : '';
    ...
    const cursor  = (disabled || isBilgi) ? 'default' : 'pointer';
    const esikTip = camp.esik > 1 ? ' · ' + camp.esik + ' ürün gerekli' : '';
    const tipStr  = isBilgi ? 'Bilgi' : locked ? 'Kilitli — diğerleriyle birleşmez' : 'Birleşebilir — diğerleriyle toplanır';

    return '<button type="button"'
      + (disabled || isBilgi ? '' : ' onclick="toggleCampaign(' + idx + ',' + ci + ')"')
      ...
      + 'opacity:' + (disabled ? 0.35 : 1) + ';transition:all .18s;' + ring + '">'
      + '<span style="font-size:.70rem;line-height:1">' + icon + '</span>'
      + '<span style="margin-left:2px">' + (camp.metin.length > 28 ? camp.metin.slice(0, 26) + '…' : camp.metin) + '</span>'
      + tutarStr + harfRozet + esikRozet
      + '</button>';
  }).join('');
    ...
    ? '<div style="display:flex;align-items:center;gap:6px;margin-top:4px;flex-wrap:wrap">'
      + (campDisc > 0
        ...
        : '<span style="font-size:.57rem;color:#92400e;background:#fffbeb;border:1px solid #fde68a;'
          + 'border-radius:20px;padding:2px 8px;font-weight:600">⏳ Eşik bekleniyor…</span>')
      + '<button type="button" onclick="clearAllCampaigns(' + idx + ')"'
        ...
        + 'background:#fee2e2;border:1px solid #fca5a5;color:#dc2626;font-family:inherit">✕ Temizle</button>'
      + '</div>'
    : '';

  return '<div style="margin-top:5px">' + pills + summaryRow + '</div>';
}
