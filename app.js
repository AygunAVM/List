...
function parseCampaigns(aciklama) {
  ...
  // Segmentleri ⎇ ve 🔒︎ ayraçlarıyla böl
  const regex = /([^⎇🔒]+?)(?:(⎇)|(🔒︎))/g;
  let match;
  while ((match = regex.exec(aciklama)) !== null) {
    const metin = match[1].trim();
    if (!metin) continue;
    const tip = match[2] ? 'birlesen' : 'kilitli';
    result.push(_buildCampObj(metin, tip));
  }
  // Son ayraçsız segment
  const sonIndex = aciklama.lastIndexOf('🔒︎') > aciklama.lastIndexOf('⎇')
    ? aciklama.lastIndexOf('🔒︎') + '🔒︎'.length
    : aciklama.lastIndexOf('⎇') + '⎇'.length;
  if (sonIndex < aciklama.length) {
    const sonMetin = aciklama.slice(sonIndex).trim();
    if (sonMetin) result.push(_buildCampObj(sonMetin, 'birlesen'));
  }
  return result;
}

// Her kampanya segmenti için grup + eşik bilgisi çıkar
function _buildCampObj(metin, tip) {
  ...
  if (/KEA/i.test(metin)) {
    grup = 'KEA';
    esik = 2;  // 2 adet KEA ürünü seçilmeli
  } else if (/KM/i.test(metin)) {
    grup = 'KM';
    esik = 2;  // 2 adet KM ürünü seçilmeli — kilitli
    tip  = 'kilitli'; // KM her zaman kilitli
  } else if (/PAP/i.test(metin)) {
    grup = 'PAP';
    esik = 1;  // Tekil, hemen aktif
  } else {
    grup = 'DIGER';
    esik = 1;  // Bilinmeyen → tek ürün yeterli
  }

  return { metin, tutar, tip, grup, esik };
}

function _extractTutar(metin) {
  // "5.500", "-3k", "14,5k", "500", "1,5k", "-1,5k", "3,5k", "7k" gibi formatları çıkar
  // Negatif işareti de al (KEA -3k → 3000)
  const m = metin.match(/[-–]?\s*([\d]{1,3}(?:[.,]\d{1,3})*)\s*[kK]?/g);
  if (!m) return 0;
  // En büyük mutlak değeri al (birden fazla sayı varsa)
  let max = 0;
  m.forEach(s => {
    let clean = s.replace(/[–\s]/g, '').replace(',', '.');
    const isK = /k$/i.test(clean);
    clean = clean.replace(/k$/i, '').replace('-', '');
...
function toggleCampaign(idx, campIdx) {
  ...
  // Başka üründe KM kilitli seçilmişse, bu ürüne de yeni seçim yasak
  const globalKMActive = basket.some((b, bi) => {
    if (!b._campaigns || !b._selectedCamps) return false;
    return Object.entries(b._selectedCamps).some(([ci, sel]) =>
      sel && b._campaigns[parseInt(ci)]?.grup === 'KM'
    );
  });
  ...
  if (isSelected) {
    ...
    // Kilitli KM seçiliyse başka şey seçme (aynı ürün veya diğer ürün)
    if (globalKMActive) return;
    // Bu pill KM ise → tüm sepetteki diğer seçimleri temizle
    if (camp.grup === 'KM') {
      basket.forEach(b => { if (b._selectedCamps) b._selectedCamps = {}; });
    }
...
function recalculateAllGroupCampaigns() {
  // Önce tüm kampanya kaynaklı itemDisc'leri sıfırla
  // (kampanya olmayan manuel girişler korunmaz — kullanıcı pilleri kullanıyorsa
  //  manuel giriş yerine pill sistemi geçerlidir)
  basket.forEach(item => {
    if (item._campaigns && item._campaigns.length > 0) {
      item.itemDisc = 0;
      item._campDisc = 0; // kampanyadan gelen indirim (debug için)
    }
  });
  ...
  // Her grup için eşik kontrolü yap ve indirimi dağıt
  Object.entries(grupSecimler).forEach(([grup, secimler]) => {
    const esik = secimler[0]?.camp?.esik || 1;
    const urunSayisi = new Set(secimler.map(s => s.basketIdx)).size; // kaç farklı ürün
    const esikSaglandi = urunSayisi >= esik;

    // Pending (sarı) durumunu işaretle
    secimler.forEach(s => {
    ...
    if (!esikSaglandi) {
      // Eşik sağlanmadı — indirim 0, pill sarı gösterilecek (renderCampaignPills halleder)
      return;
    }

    // Eşik sağlandı — indirim toplamını hesapla
    // Her grubun toplam indirimi: seçilen pill'lerin tutar toplamı
    // NOT: Aynı üründe birden fazla pill seçilmişse toplanır (⎇ mantığı)
    const toplamGrupIndirim = secimler.reduce((s, e) => s + (e.camp.tutar || 0), 0);

    if (toplamGrupIndirim <= 0) return;

    // Ağırlıklı oranlama: ürün nakit fiyatı / gruptaki toplam nakit
    ...
    if (grupToplamNakit <= 0) {
      // Fiyat sıfırsa eşit dağıt
      const esitPay = Math.round(toplamGrupIndirim / grupUrunler.length);
      grupUrunler.forEach(bi => { basket[bi].itemDisc = (basket[bi].itemDisc || 0) + esitPay; });
    } else {
      // Ağırlıklı oranlama
      let dagitilan = 0;
      grupUrunler.forEach((bi, i) => {
        ...
        const pay = i === grupUrunler.length - 1
          ? toplamGrupIndirim - dagitilan  // son ürün: yuvarlama farkını al
          : Math.round(agirlik * toplamGrupIndirim);
...
function renderCampaignPills(item, idx) {
  ...
  // Sepette herhangi bir üründe KM kilitli seçili mi?
  const globalKMActive = basket.some((b, bi) => {
    if (!b._campaigns || !b._selectedCamps) return false;
    return Object.entries(b._selectedCamps).some(([ci, sel]) =>
      sel && b._campaigns[parseInt(ci)]?.grup === 'KM'
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    const pending = sel && !!item._pendingGroups[camp.grup]; // seçili ama eşik bekliyor
    // Devre dışı: KM başka üründe seçiliyken bu KM değilse
    const disabled = globalKMActive && !sel;
    ...
    return '<span'
      + ' onclick="' + (disabled ? '' : 'toggleCampaign('+idx+','+ci+')') + '"'
    ...
    rows += `<tr class="${hasDisc ? 'row-has-disc' : ''}">
      ...
      <td style="padding:4px 6px">
        <div style="display:flex;align-items:center;gap:3px">
          <input type="number" class="item-disc-input" data-idx="${idx}" min="0" value="${itemDisc || ''}" placeholder="ind."
