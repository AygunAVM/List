...
// Firestore'un kendi bağlantı durumunu izleyerek ve periyodik heartbeat
// ile kopukluğu tespit edip otomatik recover ediyoruz.
(function _initFirebaseWatchdog() {
  ...
  function _scheduleReconnect(delayMs) {
    ...
    _reconnectTimer = setTimeout(_reconnectFirestore, delayMs);
  }

  // 1) Tarayıcı online/offline
  window.addEventListener('online',  () => _scheduleReconnect(1500));
  window.addEventListener('offline', () => { clearTimeout(_heartbeatTimer); });

  // 2) Tab görünür olduğunda
  ...
  // 3) Firestore iç bağlantı durumu — SDK'nın kendi "connected" flag'i
  //    404 WebChannel hatası sonrası SDK false → true geçişini tetikler;
  //    biz false'ta hemen reconnect yaparak süreci hızlandırıyoruz.
  import('https://www.gstatic.com/firebasejs/12.10.0/firebase-firestore.js')
    .then(({ doc: _doc, onSnapshot: _onSnapshot }) => {
      const connectedRef = _doc(_db, '.info/connected'); // Firestore özel meta
      _onSnapshot(connectedRef, snap => {
        if (snap.data()?.value === false && navigator.onLine) {
          _scheduleReconnect(3000);
        }
      }, () => { /* izin hatası — sessiz */ });
    }).catch(() => {});

  // 4) Periyodik heartbeat — 8 dakikada bir "sessiz" reconnect
  //    (Spark planı ~10 dk'da session timeout yapar; biz 8'de yenileriz)
...
function buildPremiumPDF(docType, data) {
  ...
  const urunFaturaFiyatlari = urunler.map((u, idx) => {
    if (isNakit || data.odemeTipi === 'tek_cekim') return urunNakitler[idx];
  ...
  urunler.forEach((u, i) => {
    ...
    const secilenCamp = Array.isArray(campPills)
      ...
      : [];
    if (u._projeNakit !== undefined) secilenCamp.unshift('❖ Proje Fiyatı');
  ...
  if (isNakit) {
    ...
    odemeHTML = `
      <tr><td class="rl">Liste Toplamı</td><td class="rr">${fmt(indirimliToplam)}</td></tr>
...
<div class="page">
  ...
  <div class="logo-band">
    <div>${logoHTML}</div>
    <div class="tagline">Aygün AVM &nbsp;·&nbsp; Güvenilir Alışveriş</div>
  </div>
