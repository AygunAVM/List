...
function optimizeCampaigns() {
  ...
  function atanabilir(k) {
    ...
    if (!d) return true;
    // ⎇ ve 🔒 aynı üründe HİÇBİR KOŞULDA birlikte olamaz (esik fark etmez)
    if (k.camp.tip === 'kilitli' && (d.kilitliAtandi || d.birlesenAtandi)) return false;
    if (k.camp.tip === 'birlesen' && d.kilitliAtandi) return false;
    return true;
  }

  function ata(k) {
    if (!urunDurum[k.bi]) urunDurum[k.bi] = { kilitliAtandi: false, birlesenAtandi: false };
    // Her ⎇ (PAP dahil esik=1 olanlar da) birlesenAtandi'yi true yapar
    if (k.camp.tip === 'kilitli') urunDurum[k.bi].kilitliAtandi  = true;
    if (k.camp.tip === 'birlesen') urunDurum[k.bi].birlesenAtandi = true;
  }
