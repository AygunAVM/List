...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', function(e) {
    ...
    if (!btn) return;
    e.stopPropagation();
    const kelime = btn.dataset.kelime || '';
    if (!kelime) return;
    ...
    // 2. Tab geçişi — switchMainTab varsa kullan, yoksa manuel
    if (typeof switchMainTab === 'function') {
      // switchMainTab urunler sekmesine geçerken aramayı silmemeli
      // Bu yüzden önce arama değerini set et, sonra tab geç
      const s = document.getElementById('search');
      if (s) s.value = kelime;
      switchMainTab('urunler');
      // switchMainTab filterData çağırmıyor, manuel tetikle
      setTimeout(function() {
      ...
      document.querySelectorAll('.tab-btn[data-tab]').forEach(function(b) {
        b.classList.toggle('active', b.dataset.tab === 'urunler');
      });
      setTimeout(function() {
        const s = document.getElementById('search');
        if (s) s.value = kelime;
...
function _campSearchBtn(item, camp, ci) {
  ...
  // data-kelime attribute üzerinden — onclick'te tırnak çakışması yok
