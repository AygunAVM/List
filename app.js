...
function finalizeAksiyon() {
  ...
  if(_aksiyonMode === 'wa') {
    ...
    let odemeBlok='';
    if(abakusSelection) {
      const taksit=abakusSelection.label||'';
      const zincir=abakusSelection.zincir||'';
      const aylik=fmt(abakusSelection.aylik||0);
      const toplam=fmt(abakusSelection.tahsilat||tahsilat);
      odemeBlok='`'+taksit+(zincir?' / '+zincir:'')+'`\n*Aylık Taksit* x '+Math.round((abakusSelection.tahsilat||tahsilat)/(abakusSelection.aylik||1))+' Taksit\n*Ödenecek Tutar* '+toplam+' ₺';
    } else {
      odemeBlok='`Nakit`\n*Ödenecek Tutar* '+fmt(nakit)+' ₺';
    }
...
function resendProposalWa(id) {
  ...
  // Ödeme bloğu — odeme string'ini parse et
  const odemeBlok='`'+p.odeme+'`\n*Ödenecek Tutar* '+fmt(p.tahsilat||p.nakit||0)+' ₺';
  const msg='Aygün AVM Teklif'
    +'\n*Sn* '+p.custName
    +'\n*Telefon* '+p.phone
    +'\n*Geçerlilik* '+expDate
...
function showPendingChanges(logKey) {
  ...
  // Popup zaten açıksa yeniden render etme — kullanıcı işaretleme yapıyor olabilir
  ...
  showChangePopup(allChanges, logKey);
}
...
function showChangePopup(changes, logKey) {
  ...
  // Değişim satırlarını oluştur (versiyon başlıkları hariç)
  let itemIdx = 0;
  list.innerHTML = changes.map(c => {
    ...
    return `<div class="change-item ${doneCls}" data-idx="${idx}">${chk}${inner}</div>`;
  }).join('');
...
function markAllChanges() {
  document.querySelectorAll('#change-list .change-item:not(.change-item-done)').forEach(item => {
