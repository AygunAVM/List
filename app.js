...
function calcAbakus() {
  ...
  // Nakit satırı
  const nakitSafeJson = JSON.stringify({type:'nakit', nakit}).replace(/\\/g,'\\\\').replace(/'/g,"\\'");
  html+=`<tr class="ab-row-nakit ab-row-sel" id="ab-row-nakit-tr" onclick="selectAbakusRow(this,'${nakitSafeJson}')">
  ...
  liste.forEach(s => {
    ...
    html+=`<tr class="${rowCls}" onclick="selectAbakusRow(this,'${safeJson}')">
...
function loadMsgRecipients() {
  ...
  // Tüm proposal'lardan kullanıcı listesini çıkar + analytics
  ...
  sel.innerHTML='<option value="all">Herkese</option>'+
    [...userSet].filter(u=>u!==currentUser?.Email).map(u=>`<option value="${u}">${u}</option>`).join('');
}
