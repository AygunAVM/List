...
function checkUpsellOpportunities() {
  ...
  // ── Para format — sadece rakam+k, ₺ yok ─────────────────────────
  function _fmt(tutar) {
    if (tutar >= 1000) {
      const k = tutar / 1000;
  ...
  // ── Açıklama metninden tüm kampanya segmentlerini parse et ───────
  function _parseCamps(ac) {
    ...
    while ((m = re.exec(ac)) !== null) {
      ...
      const tutar = m[3] ? raw * 1000 : raw;
      out.push({ grup: m[1].toUpperCase(), tutar, esik: parseInt(m[4]), rol: m[5].toUpperCase() });
    }
    return out;
  }
  ...
  Object.entries(grupBilgi).forEach(([grup, info]) => {
    ...
    if (seciliSayi === 0 && sahipSayi >= info.esik) {
      ...
      if (farkliRol >= info.esik || sepetRoller.has('ANY')) {
        firsatlar.push({ grup, kazanc: info.tutar, tip: 'hazir', eksikUrun: 0, oncelik: 1 });
      }
      return;
    }
    ...
    if (seciliSayi > 0) {
      ...
      if (acikPos > 0 && (info.esik - acikPos) === 1 && sahipSayi >= info.esik) {
        firsatlar.push({ grup, kazanc: info.tutar, tip: 'secim_eksik', eksikUrun: 1, oncelik: 2 });
      }
      return;
    }
    ...
    if (sahipSayi > 0 && sahipSayi < info.esik) {
      const eksikUrun    = info.esik - sahipSayi;
      const mevcutRoller = new Set(Object.values(info.roller));

      // Sepet fiyat ortalaması
      const sf = info.sepetNakitler.filter(x => x > 0);
      const sepetOrt = sf.length ? sf.reduce((a, b) => a + b, 0) / sf.length : 0;

      // allProducts'ta tamamlayıcıları bul
      ...
      for (const p of allProducts) {
        ...
        for (const pc of _parseCamps(p[acikKey] || '')) {
          ...
          tamamlayicilar.push({
            ...
            kod: pKod, nakit: pNakit, stok, rol: pc.rol
          });
          break;
        }
      }

      if (!tamamlayicilar.length) return;

      // ── YÖN FİLTRESİ ────────────────────────────────────────────
      // Tamamlayıcı sepetten 2 kattan fazla pahalıysa "ters yön" demektir.
      // Örn: sepette küçük ev aleti (3750₺), tamamlayıcı buzdolabı (14000₺+)
      // Bu yönde öneri gösterme — müşteri o ürünü zaten biliyorsa almış olurdu.
      // Eşik: sepet ortalamasının 2 katı (makul benzer fiyat bandı)
      const YON_CARPANI = 2.5;
      const stokluMakul = tamamlayicilar.filter(t =>
        t.stok > 0 && (sepetOrt === 0 || t.nakit <= sepetOrt * YON_CARPANI)
      );

      // Stoklu+makul fiyatlı tamamlayıcı yoksa bu grubu atla
      if (stokluMakul.length === 0) return;

      // Sıralama: ucuz önce (aynı bandı içinde en erişimlisi)
      stokluMakul.sort((a, b) => a.nakit - b.nakit);
      const enIyi = stokluMakul[0];

      // ── ÖNCELIK SKORU ────────────────────────────────────────────
      // Az ürün ekle = kolay = düşük sayı = önce göster
      // oncelik = 3 + eksikUrun
      //   1 ürün eksik → oncelik 4 (en iyi)
      //   2 ürün eksik → oncelik 5
      //   3 ürün eksik → oncelik 6
      const oncelik = 3 + eksikUrun;

      firsatlar.push({
        grup, kazanc: info.tutar, tip: 'ekle',
        eksikUrun, tamamlayici: enIyi,
        tamamlayiciSayi: stokluMakul.length,
        oncelik,
        kazancPerUrun: info.tutar / eksikUrun
      });
    }
  });
  ...
  // ── 4. Sıralama ──────────────────────────────────────────────────
  // Birincil : az eksik ürün (kolay)
  // İkincil  : kazanç/ürün (verimli)
  firsatlar.sort((a, b) =>
    a.oncelik - b.oncelik ||
    (b.kazancPerUrun || 0) - (a.kazancPerUrun || 0)
  );

  const f    = firsatlar[0];
  const kFmt = _fmt(f.kazanc);
  ...
  if (f.tip === 'hazir') {
    ...
    const eksikStr = f.eksikUrun === 1 ? '1 ürün ekle' : `${f.eksikUrun} ürün ekle`;

    mesaj = `<b>${f.grup}</b>: ${eksikStr} — <b>${t.ad}</b>`
          + (detay ? ` <span style="opacity:.65;font-weight:400">(${detay})</span>` : '')
          + ` → <b>+${kFmt}</b>`;
