...
const APP_BUILD_VERSION = 'V8.8-20260729-1927';
...
async function adminForceLogoutAllStale() {
  if(!isAdmin()) return;
  try {
    const stale = Object.values(window._fbSessions || {})
      .filter(s => s.appVersion !== APP_BUILD_VERSION);
    if(stale.length === 0) {
      await ayAlert('✅ Şu an çevrimiçi kimse eski sürüm çalıştırmıyor.');
      return;
    }
    const names = stale.map(s => (s.email||'').split('@')[0]).join(', ');
    if(!(await ayConfirm(stale.length + ' kişi eski sürümde görünüyor: ' + names + '\nHepsi zorla çıkış yapıp sayfaları otomatik yenilenecek. Devam edilsin mi?'))) return;
    for(const s of stale) {
      await updateDoc(doc(_db,'sessions', s.email), { forceLogout: true, forceLogoutAt: serverTimestamp() });
      const _email = s.email;
      setTimeout(async () => { try { await deleteDoc(doc(_db,'sessions', _email)); } catch(e){} }, 5000);
    }
    haptic(22);
    showToast('🔄 ' + stale.length + ' cihaz güncellemeye zorlandı, birazdan yeni sürümle geri dönecekler.', 'success');
    renderAdminUsers();
