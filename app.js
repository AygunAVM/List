...
async function showApp() {
  ...
  // Tab bar stock-filter-btn (stok) — id çakışmasını önlemek için
  const tabStokBtn = document.getElementById('tab-btn-stok');
  if (tabStokBtn) tabStokBtn.id = 'stock-filter-btn';
  updateProposalBadge();
...
window.deleteSelectedItems = function() {
  ...
  // Admin: neden sormadan direkt sil
  if (isAdmin && isAdmin()) {
...
async function teklifRevizeSepet(propId) {
  ...
  if (degisimler.length > 0) {
    ...
    await ayAlert(
      `<div style="margin-bottom:8px;font-weight:700">⚠️ Fiyat Değişikliği Tespit Edildi</div>
       <div style="font-size:.76rem;color:var(--text-3);margin-bottom:10px">Güncel fiyatlar tekliften farklı. Sepete güncel fiyatla, teklifteki manuel indirimlerle eklenecek.</div>
       ${satirlar}`,
      'Güncellendi'
    );
  }
...
function switchMainTab(tab) {
  ...
  ['urunler','trend','kategori'].forEach(t => {
  ...
  // Trend sekmesine geçince listeyi doldur
  if (tab === 'trend') renderTrendFullList();
  if (tab === 'kategori') renderKategoriGrid();
  // Arama kutusunu temizle
  if (tab !== 'urunler') {
    const s = document.getElementById('search');
    if (s && s.value) { s.value = ''; renderTable(); }
  }
  // Trend paneli: urunler sekmesinde göster
...
// ── Trend Skoru hesapla ─────────────────────────────────────
// Skor = sepete ekleme sayısı × 3 + prim değeri × 0.01
function _trendSkoru() {
  // Tüm analytics'ten ürün sayımlarını topla
  const skorlar = {};
  if (window._fbAnalytics) {
    Object.values(window._fbAnalytics).forEach(rec => {
      if (!rec.products) return;
      Object.entries(rec.products).forEach(([urun, cnt]) => {
        skorlar[urun] = (skorlar[urun] || 0) + cnt;
      });
    });
  }
  // localStorage analytics de ekle
  try {
    const local = JSON.parse(localStorage.getItem('analytics_local') || '{}');
    Object.values(local).forEach(gunRec => {
  ...
  // allProducts ile eşleştir ve prim skorunu ekle
  const sonuclar = allProducts.map(u => {
    const keys = Object.keys(u);
    const urunKey = keys.find(k => k.toLowerCase() === 'urun' || k.toLowerCase().includes('urun')) || '';
    const urunAdi = u[urunKey] || '';
    const primKey = keys.find(k => k.toLowerCase() === 'prim') || '';
    const prim = primKey ? (parseFloat(u[primKey]) || 0) : 0;
    const addCnt = skorlar[urunAdi] || 0;
    const skor = addCnt * 3 + prim * 0.01;
    return { u, urunAdi, skor, addCnt, prim };
  });

  // Skor > 0 olanları sırala, yoksa prim'e göre
  const aktif = sonuclar.filter(x => x.skor > 0).sort((a,b) => b.skor - a.skor);
  if (aktif.length >= 5) return aktif;
  // Yeterli veri yoksa prim'e göre sırala
  return sonuclar.filter(x => x.prim > 0).sort((a,b) => b.prim - a.prim);
}

function _urunFmt(u) {
  const keys = Object.keys(u);
  const nakitKey = keys.find(k => k.toLowerCase() === 'nakit') || '';
  const nakit = nakitKey ? parseFloat(u[nakitKey]) : 0;
  return nakit > 0 ? fmt(nakit) + '₺' : '—';
}

function _urunIdx(u) {
  return allProducts.indexOf(u);
}

// ── Trend Kartlar (yatay scroll, ana ekranda) ───────────────
function renderTrendCards() {
  const container = document.getElementById('trend-cards');
  if (!container || !allProducts.length) return;

  const trend = _trendSkoru().slice(0, 12);
  if (!trend.length) {
  ...
  if (subEl) {
    const now = new Date();
    subEl.textContent = 'son 30 gün';
  }

  container.innerHTML = trend.map((item, i) => {
    const rank = i + 1;
    const rankCls = rank <= 3 ? 'rank-' + rank : '';
    const hasPrim = item.prim > 0;
    const oi = _urunIdx(item.u);
    const scoreLabel = item.addCnt > 0 ? item.addCnt + 'x' : (item.prim > 0 ? '★' : '');

    return `<div class="trend-card" onclick="switchMainTab('urunler');setTimeout(()=>{const s=document.getElementById('search');if(s){s.value=${JSON.stringify(item.urunAdi.split(' ').slice(0,2).join(' '))};filterData();}},100)">
      <div class="trend-rank ${rankCls}">#${rank}</div>
      <div class="trend-card-name">${item.urunAdi}</div>
      <div class="trend-card-meta">
        <span class="trend-card-price">${_urunFmt(item.u)}</span>
        ${scoreLabel ? `<span class="trend-card-score">${scoreLabel}</span>` : ''}
      </div>
      <button class="trend-card-add haptic-btn" onclick="event.stopPropagation();${oi >= 0 ? 'addToBasket(' + oi + ')' : ''}" title="Sepete ekle">+ Sepete</button>
      ${hasPrim ? '<div class="trend-card-prim"></div>' : ''}
    </div>`;
  }).join('');
}

// ── Trend Tam Liste (Sekme 2) ───────────────────────────────
function renderTrendFullList() {
  const container = document.getElementById('trend-full-list');
  if (!container || !allProducts.length) return;

  const trend = _trendSkoru().slice(0, 30);
  if (!trend.length) {
    container.innerHTML = '<div style="grid-column:1/-1;padding:24px;text-align:center;color:var(--text-3);font-size:.78rem">Henüz yeterli veri yok. Ürünler sepete eklendikçe bu liste dolacak.</div>';
    return;
  }
  ...
  container.innerHTML = trend.map((item, i) => {
    ...
    const markaKey = keys.find(k => k.toLowerCase() === 'marka') || '';
    const marka = markaKey ? item.u[markaKey] : '';
    const scoreLabel = item.addCnt > 0
      ? item.addCnt + 'x sepete eklendi'
      : (item.prim > 0 ? item.prim + ' prim' : '');

    return `<div class="trend-full-card">
      <div class="trend-full-rank ${rankCls}">#${rank}</div>
      <div class="trend-full-name">${marka ? `<span style="color:var(--text-3);font-weight:600">${marka} </span>` : ''}${item.urunAdi}</div>
      <div class="trend-full-price">${_urunFmt(item.u)}</div>
      ${scoreLabel ? `<div class="trend-full-meta">${scoreLabel}</div>` : ''}
      <button class="trend-full-add haptic-btn" onclick="${oi >= 0 ? 'addToBasket(' + oi + ')' : ''}" title="Sepete ekle">+ Sepete Ekle</button>
      ${hasPrim ? '<div class="trend-full-bar"></div>' : ''}
    </div>`;
  }).join('');
}

// ── Kategori Grid (Sekme 3) ─────────────────────────────────
function renderKategoriGrid() {
  const container = document.getElementById('kategori-grid');
  if (!container || !allProducts.length) return;

  // Gam sütunundan kategorileri çek
  const keys0 = allProducts.length ? Object.keys(allProducts[0]) : [];
  const gamKey = keys0.find(k => k.toLowerCase().includes('gam') || k.toLowerCase() === 'gam') || '';
  const markaKey = keys0.find(k => k.toLowerCase() === 'marka') || '';

  if (!gamKey && !markaKey) {
    container.innerHTML = '<div style="grid-column:1/-1;padding:24px;text-align:center;color:var(--text-3);font-size:.78rem">Kategori verisi bulunamadı.</div>';
    return;
  }
  ...
  allProducts.forEach(u => {
    const cat = (gamKey ? u[gamKey] : '') || (markaKey ? u[markaKey] : '') || 'Diğer';
    if (!cat || cat === '-') return;
    catMap[cat] = (catMap[cat] || 0) + 1;
  });

  const cats = Object.entries(catMap).sort((a,b) => b[1] - a[1]).slice(0, 20);

  // Kategori ikonları — basit anahtar kelime eşleşmesi
  const ikonMap = [
    ['tv|televizyon|ekran',     '📺'],
    ['buzdolabı|buzdolabi|soğutucu|sogutucu|derin dondurucu', '🧊'],
    ['çamaşır|camasir|yıkama',  '🫧'],
    ['bulaşık|bulasik',         '🍽️'],
    ['fırın|firin|ocak|pişirici','🔥'],
    ['klima|soğutma|sogutma',   '❄️'],
    ['telefon|gsm|mobil',       '📱'],
    ['bilgisayar|laptop|pc',    '💻'],
    ['küçük|kucuk|mikser|tost', '🍞'],
    ['süpürge|supurge|temizlik','🧹'],
    ['ses|hoparlör|hoparlor|kulaklık', '🎧'],
    ['akıllı|akilli|saat',      '⌚'],
  ];
  function getIcon(cat) {
    const c = cat.toLowerCase();
    for (const [kw, icon] of ikonMap) {
      if (new RegExp(kw).test(c)) return icon;
    }
    return '📦';
  }

  container.innerHTML = cats.map(([cat, cnt]) => `
    <div class="kategori-card" onclick="switchMainTab('urunler');setTimeout(()=>{const s=document.getElementById('search');if(s){s.value=${JSON.stringify(cat)};filterData();}},100)">
      <span class="kategori-icon">${getIcon(cat)}</span>
      <span class="kategori-name">${cat}</span>
      <span class="kategori-count">${cnt} ürün</span>
    </div>`).join('');
}
...
window.renderKategoriGrid = renderKategoriGrid;
...
async function saveVitrinUrun() {
  ...
  if (!urunAdi) { _campToast('Ürün adı boş bırakılamaz.', 'warn'); return; }
  if (!fiyat)   { _campToast('Fiyat girilmedi.', 'warn'); return; }

  try {
    await setDoc(doc(_db, 'vitrin', 'vitrin_' + Date.now()), {
      urunAdi, fiyat, kampanya,
      aktif: true,
      ts: serverTimestamp(),
      ekleyen: currentUser?.Email || '-'
    });
    document.getElementById('vitrin-urun-input').value  = '';
    document.getElementById('vitrin-fiyat-input').value = '';
    document.getElementById('vitrin-kampanya-input').value = '';
    _campToast('Vitrine eklendi.', 'ok');
  } catch(e) {
    ...
    <div style="display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid var(--border)">
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:.65rem;color:var(--text-3)">
          ${fmt(v.fiyat)}₺${v.kampanya ? ' · ' + v.kampanya : ''}
        </div>
      </div>
...
function renderSiparisBildirimBar() {
  ...
  const bekleyen = (window._siparisData || []).filter(s => s.durum === 'bekliyor');

  // Bar container: search-container'ın hemen altına
  if (!bar) {
  ...
  // Son 3 bekleyen sipariş
  const son3 = bekleyen.slice(0, 3);
  const metin = son3.map(s => `<b>${s.urun || s.urunAdi || '?'}</b>`).join(', ');
  const fazla = bekleyen.length > 3 ? ` +${bekleyen.length - 3} daha` : '';

  bar.innerHTML = `
    <div onclick="${isAdmin() ? "openAdmin();setTimeout(()=>switchAdminTab('siparis'),300)" : ''}"
      style="display:flex;align-items:center;gap:8px;
        background:#0f172a;color:#f8fafc;
        padding:7px 12px;font-size:.66rem;cursor:${isAdmin() ? 'pointer' : 'default'};
        border-bottom:2px solid #e11d48;">
      <span style="flex-shrink:0">📦</span>
      <span style="flex:1;line-height:1.4">
        <b style="color:#f87171">${bekleyen.length} sipariş bekliyor:</b> ${metin}${fazla}
      </span>
      ${isAdmin() ? '<span style="color:#94a3b8;font-size:.60rem">→ Yönet</span>' : ''}
    </div>`;
}
...
function _primFmt(prim) {
  if (!prim || prim <= 0) return '';
  // Prim: ondalık varsa 1 hane, yoksa tam sayı
  if (prim >= 1000) return (prim / 1000).toFixed(prim % 1000 === 0 ? 0 : 1) + 'K prim';
  return Math.round(prim) + ' prim';
}

function _urunIdx(u) { return allProducts.indexOf(u); }

// ── Trend Kartlar — ana ekran scroll row ────────────────────
function renderTrendCards() {
  const container = document.getElementById('trend-cards');
  if (!container) return;

  // Vitrin ürünleri var mı? Varsa önce onları göster
  const vitrinAktif = _vitrinUrunler.filter(v => v.aktif !== false);

  // Vitrin ürünleri → blur trend karışımı
  const vitrinHtml = vitrinAktif.map((v, i) => {
    // allProducts'ta eşleşen ürünü bul (sepete ekle için)
    const oi = allProducts.findIndex(u => {
      const keys = Object.keys(u);
      const uk = keys.find(k => k.toLowerCase() === 'urun') || '';
      return (u[uk] || '').toLowerCase().includes(v.urunAdi.toLowerCase().split(' ')[0]);
    });
    return `<div class="trend-card vitrin-card">
      <div class="trend-rank" style="color:#e11d48;font-size:.58rem">🛍️</div>
      <div class="trend-card-name">${v.urunAdi}</div>
      <div class="trend-card-meta">
        <span class="trend-card-price">${fmt(v.fiyat)}₺</span>
        ${v.kampanya ? `<span class="trend-card-score" style="background:#fef3c7;color:#92400e">${v.kampanya}</span>` : ''}
      </div>
      <button class="trend-card-add haptic-btn"
        onclick="event.stopPropagation();${oi >= 0 ? 'addToBasket(' + oi + ')' : ''}"
        title="Sepete ekle">+ Sepete</button>
      <div class="trend-card-prim"></div>
    </div>`;
  }).join('');

  // Blur trend ürünleri (vitrin doluysa sadece 5, değilse 12)
  ...
  const trendHtml = trendData.map((item, i) => {
    ...
    const rankCls = rank <= 3 ? 'rank-' + rank : '';
    const oi = _urunIdx(item.u);
    const primLabel = _primFmt(item.prim);
    ...
    return `<div class="trend-card"
      ...
      ${primLabel ? `<div style="font-size:.55rem;color:#b45309;margin:2px 0 4px;font-weight:700">${primLabel}</div>` : ''}
      <button class="trend-card-add haptic-btn"
        onclick="event.stopPropagation();${oi >= 0 ? 'addToBasket(' + oi + ')' : ''}"
        title="Sepete ekle">+ Sepete</button>
      ${item.prim > 0 ? '<div class="trend-card-prim"></div>' : ''}
    </div>`;
  }).join('');
  ...
  if (!vitrinAktif.length && !trendData.length) {
    container.innerHTML = `<div style="grid-column:1/-1;padding:32px 16px;text-align:center">
      ...
      <div style="font-size:.78rem;color:var(--text-3);line-height:1.6">
        Kampanya vitrin ürünleri admin panelinden eklenebilir.<br>
        Popüler ürünler sepete eklendikçe burada görünecek.
      </div>
    </div>`;
    return;
  }

  // Vitrin bölümü başlığı
  let html = '';
  if (vitrinAktif.length) {
    html += `<div style="grid-column:1/-1;display:flex;align-items:center;gap:8px;margin-bottom:2px">
      <span style="font-size:.70rem;font-weight:800;color:var(--text-1)">🛍️ Kampanya Ürünleri</span>
      <span style="font-size:.58rem;color:var(--text-3)">admin seçimi</span>
    </div>`;
    html += vitrinAktif.map(v => {
      ...
      return `<div class="trend-full-card" style="border-color:#fca5a5">
        <div class="trend-full-rank" style="color:#e11d48">🛍️</div>
        <div class="trend-full-name" style="padding-right:24px">${v.urunAdi}</div>
        <div class="trend-full-price">${fmt(v.fiyat)}₺</div>
        ${v.kampanya ? `<div class="trend-full-meta" style="color:#92400e;font-weight:700">${v.kampanya}</div>` : ''}
        <button class="trend-full-add haptic-btn"
          onclick="${oi >= 0 ? 'addToBasket(' + oi + ')' : ''}">+ Sepete Ekle</button>
        <div class="trend-full-bar"></div>
      </div>`;
    }).join('');
  }

  if (trendData.length) {
    html += `<div style="grid-column:1/-1;display:flex;align-items:center;gap:8px;margin-top:${vitrinAktif.length ? '10px' : '0'};margin-bottom:2px">
      <span style="font-size:.70rem;font-weight:800;color:var(--text-1)">🔥 Popüler</span>
      <span style="font-size:.58rem;color:var(--text-3)">son 14 gün</span>
    </div>`;
    html += trendData.map((item, i) => {
      ...
      return `<div class="trend-full-card">
        <div class="trend-full-rank ${rankCls}">#${rank}</div>
        <div class="trend-full-name">
          ${marka ? `<span style="color:var(--text-3)">${marka} </span>` : ''}${item.urunAdi}
        </div>
        <div class="trend-full-price">${_urunFmt(item.u)}₺</div>
        <div class="trend-full-meta">
          ${item.addCnt > 0 ? item.addCnt + '× eklendi' : ''}
          ${primLabel ? (item.addCnt > 0 ? ' · ' : '') + primLabel : ''}
        </div>
        <button class="trend-full-add haptic-btn"
          onclick="${oi >= 0 ? 'addToBasket(' + oi + ')' : ''}">+ Sepete Ekle</button>
        ${item.prim > 0 ? '<div class="trend-full-bar"></div>' : ''}
      </div>`;
    }).join('');
  }
...
// switchMainTab güncelle: 'kategori' kaldırıldı
window.switchMainTab = function switchMainTab(tab) {
  _aktifMainTab = tab;
