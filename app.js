...
function _startMotdTicker() {
  ...
  const ad = currentUser?.Ad || currentUser?.Email?.split('@')[0] || '';
  const staticPlaceholder = ad ? 'Ouuuv, ' + ad + ' — Ürün arama' : 'Ürün arama';
...
function toggleCampaign(idx, campIdx) {
  ...
  //   UI disabled olduğu için buraya normalde gelinmez; güvenlik için sessiz engel.
  // ═══════════════════════════════════════════════════════════
  if (camp.tip === 'kilitli' && itemHasKilitli) {
    return; // sessizce engelle, toast gösterme
  }
...
function renderCampaignPills(item, idx) {
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    const disabled = !sel && !isBilgi && (
      ...
      (itemHasKilitli  && camp.tip === 'kilitli')   // aynı üründe 2. 🔒 engelle
    );
    ...
    // Label: kısa grup adı + tutar
    const label = isBilgi
      ? camp.metin.slice(0,22)
      : (camp.grup + (tutarStr ? ' −' + tutarStr : '') + harfStr);
    ...
    if (isBilgi) {
      ...
      // Normal seçilebilir
      style = `background:${pal.bg};color:${pal.unsel};border-bottom:1.5px solid ${pal.line};font-weight:600;cursor:pointer;`;
    }
    ...
    const clickAttr = (!disabled && !isBilgi)
