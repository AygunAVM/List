...
function toggleCampaign(idx, campIdx) {
  ...
  // ═══════════════════════════════════════════════════════════
  // KURAL 2: Aynı grupta aynı harf (rol) iki kez seçilemez
  //   ANCAK: Mevcut çift zaten doluysa (esik sağlandıysa) yeni
  //   çift başlatılabilir — aynı harf yeniden kullanılabilir.
  //   Örn: KEA(2/A)+KEA(2/B) = 1. çift tam → KEA(2/A)+KEA(2/B)
  //   için 2. çift başlatılabilir.
  // ═══════════════════════════════════════════════════════════
  if (camp.rol && camp.rol !== 'ANY' && camp.tip === 'birlesen') {
    const grupAdi = camp.grup;
    const esikDeger = camp.esik || 2;

    // Mevcut grupta seçili tüm harfleri topla
    const seciliHarfler = [];
    basket.forEach((b, bi) => {
      ...
      Object.entries(b._selectedCamps).forEach(([ci, sel]) => {
        ...
        if (bc && bc.grup === grupAdi && bc.rol && bc.rol !== 'ANY') {
          seciliHarfler.push(bc.rol);
        }
      });
    });

    // Kaç tamamlanmış çift var? Her çiftte esikDeger farklı harf olmalı.
    // Tamamlanmış çift sayısı = Math.floor(benzersizHarfler / esikDeger)
    // Mevcut açık çiftteki harfler = kalanlar
    const benzersizHarfSayisi = new Set(seciliHarfler).size;
    const tamamlananCift = Math.floor(benzersizHarfSayisi / esikDeger);
    // Açık çiftteki mevcut harfler (mod)
    const acikCiftHarfler = seciliHarfler.slice(tamamlananCift * esikDeger);

    // Açık çiftte aynı harf var mı?
    if (acikCiftHarfler.includes(camp.rol)) {
      _campToast(
        camp.grup + ' grubunda "' + camp.rol + '" harfi zaten seçili. '
  ...
  // ═══════════════════════════════════════════════════════════
  // KURAL 3: Açık çiftte eşik doluysa yeni çift başlatılabilir
  //   Tamamlanmış çiftler (esik×n ürün) serbest — yeni çift açılır.
  //   Örn: IKILI esik=2 → 2 ürün = 1. çift tam → 3. ürün yeni çift başlatır.
  // ═══════════════════════════════════════════════════════════
  if (camp.esik > 1) {
    const grupAdi   = camp.grup;
    const esikDeger = camp.esik;
    // Bu grupta toplam kaç farklı ürün seçilmiş?
    ...
    if (!buUrunGrupta) {
      // Açık çiftte kaç ürün var? (toplam mod esik)
      const acikCiftUrunSayisi = grupSeciliUrunler.size % esikDeger;
      // Açık çift doluysa yeni çift başlatılabilir (mod=0 → yeni çift)
      // Açık çiftte yer varsa eklenebilir
      // Sadece açık çift tam VE bu yeni ürün eklenirse mod=1 olur → serbest
      // Engel: açık çiftte yer yok (acikCiftUrunSayisi >= esikDeger — mümkün değil mod ile)
      // Yani KURAL 3 artık sadece ⎇ birlesen için özel durumlarda geçerli,
      // 🔒 kilitli için birden fazla çift serbest — hiç engelleme yok
      if (camp.tip !== 'kilitli' && acikCiftUrunSayisi >= esikDeger) {
        _campToast(
          camp.grup + ' kampanyası en fazla ' + esikDeger + ' ürün ile kullanılabilir.',
          'warn'
        );
        return;
      }
...
function recalculateAllGroupCampaigns() {
  ...
  Object.entries(grupSecimler).forEach(([grup, secimler]) => {
    ...
    // ⎇ birleşen gruplar birden fazla tamamlanmış çift oluşturabilir.
    // Örn: KEA(2/A)+KEA(2/B) = 1. çift, sonra KEA(2/A)+KEA(2/B) = 2. çift
    // Her tamamlanan çift için ayrı ayrı indirim uygulanır.
    // Seçimler ürün bazında tekilleştirilir
    ...
    // Tamamlanan çiftleri greedy bul
    const tamamlananCiftler = [];
    if (hepsiANY) {
      for (let i = 0; i < tekliSecimler.length; i += esik) {
        const cift = tekliSecimler.slice(i, i + esik);
        if (cift.length === esik) tamamlananCiftler.push(cift);
      }
    } else {
      let kalan = [...tekliSecimler];
      while (kalan.length >= esik) {
        const cift = [];
        const kullanilanHarfler = new Set();
        const sonraki = [];
        for (const s of kalan) {
          const r = s.camp.rol || 'ANY';
          if (!kullanilanHarfler.has(r) && cift.length < esik) {
            cift.push(s);
            kullanilanHarfler.add(r);
          } else {
            sonraki.push(s);
          }
        }
        if (cift.length === esik) { tamamlananCiftler.push(cift); kalan = sonraki; }
        else break;
      }
    }
...
// ═══════════════════════════════════════════════════════════════
// ✨ EN İYİ FİYAT OPTİMİZASYON MOTORU
// Sadece ⎇ (birlesen) ve 🔒 (kilitli) kampanyaları değerlendirir.
// ❖ ✦ ⌗ kampanyaları kapsam dışıdır.
// ═══════════════════════════════════════════════════════════════
...
function optimizeCampaigns() {
  ...
  // ── 1. Mevcut kampanya seçimlerini sıfırla ──────────────────
  basket.forEach(item => {
    item._selectedCamps  = {};
    item._pendingGroups  = {};
  ...
  // ── 2. Optimize edilecek kampanyaları topla ──────────────────
  // Her ürün+kampanyaIndex çifti bir "seçenek"
  const secenekler = []; // { bi, ci, camp }
  basket.forEach((item, bi) => {
    if (!item._campaigns) item._campaigns = parseCampaigns(item.aciklama || '');
    item._campaigns.forEach((camp, ci) => {
      if (camp.tip !== 'birlesen' && camp.tip !== 'kilitli') return;
      if (camp.tip === 'bilgi' || camp.grup === 'BILGI') return;
      if (camp.sonTarih && new Date() > camp.sonTarih) return; // tarihi dolmuş
      if (camp.tutar <= 0) return;
      secenekler.push({ bi, ci, camp });
    });
  });
  ...
  // ── 3. Brute-force kombinasyon taraması ─────────────────────
  // Kombinasyon sayısı yönetilebilir tutmak için her ürün için
  // en yüksek tutarlı kampanyaları ön filtreden geçir (max 3/ürün)
  const urunSecenekler = {};
  secenekler.forEach(s => {
    if (!urunSecenekler[s.bi]) urunSecenekler[s.bi] = [];
    urunSecenekler[s.bi].push(s);
  });
  Object.keys(urunSecenekler).forEach(bi => {
    urunSecenekler[bi].sort((a,b) => b.camp.tutar - a.camp.tutar);
    urunSecenekler[bi] = urunSecenekler[bi].slice(0, 4); // en fazla 4/ürün
  });
  const filtreliSecenekler = Object.values(urunSecenekler).flat();

  // Her ürün en fazla 1 kampanya seçebilir (kısıt 1)
  // Aynı üründe ⎇ ve 🔒 birlikte olamaz (kısıt 2)
  // 🔒 aynı gruptan aynı üründe 2 kez seçilemez (kısıt 3)
  // Kombinasyon: ürün başına seçenek seç veya seçme
  const urunler = [...new Set(filtreliSecenekler.map(s => s.bi))];

  // Her ürün için olası seçimler: null (seçme) veya bir seçenek
  const urunOlasılıkları = urunler.map(bi => {
    const opts = filtreliSecenekler.filter(s => s.bi === bi);
    return [null, ...opts]; // null = bu üründen seçme
  });

  let enIyiDisc   = -1;
  let enIyiSecim  = null;
  const limit     = 3000; // kombinasyon üst sınırı — performans için
  let saydim      = 0;

  function kartezyenDene(urunIdx, mevcutSecim) {
    if (saydim > limit) return;
    if (urunIdx === urunler.length) {
      saydim++;
      const disc = _hesaplaKombDiscount(mevcutSecim);
      if (disc > enIyiDisc) { enIyiDisc = disc; enIyiSecim = [...mevcutSecim]; }
      return;
    }
    for (const opt of urunOlasılıkları[urunIdx]) {
      if (opt !== null && !_kombinasyonGecerliMi(mevcutSecim, opt)) continue;
      kartezyenDene(urunIdx + 1, opt ? [...mevcutSecim, opt] : mevcutSecim);
    }
  }

  kartezyenDene(0, []);

  // ── 4. En iyi kombinasyonu uygula ───────────────────────────
  if (!enIyiSecim || enIyiDisc <= 0) {
  ...
  const fmtD = enIyiDisc >= 1000
    ? (enIyiDisc/1000).toFixed(enIyiDisc%1000===0?0:1)+'k'
    : enIyiDisc;
  _campToast('✨ En iyi kombinasyon seçildi — ' + fmtD + '₺ indirim', 'ok');
  haptic(30);
}

// Geçici simülasyon: verilen seçim listesinin sağlayacağı toplam indirimi hesapla
function _hesaplaKombDiscount(secimler) {
  if (!secimler.length) return 0;

  // Kilitli (🔒): aynı gruptan esik kadar farklı üründen seçim → 1 çift indirimi
  // Birleşen (⎇): esik kadar farklı ürün + farklı rol → toplam indirim
  const grupSecimler = {};
  secimler.forEach(s => {
    const g = s.camp.grup;
    if (!grupSecimler[g]) grupSecimler[g] = [];
    grupSecimler[g].push(s);
  });

  let toplam = 0;
  Object.values(grupSecimler).forEach(list => {
    const esik   = list[0].camp.esik || 1;
    const tutar  = Math.max(...list.map(s => s.camp.tutar || 0));
    if (esik === 1) {
      toplam += list.reduce((s, x) => s + (x.camp.tutar || 0), 0);
      return;
    }
    // Birden fazla çift mümkün (greedy)
    const urunler = [...new Set(list.map(s => s.bi))];
    const roller  = list.map(s => s.camp.rol || 'ANY');
    const hepsiANY = roller.every(r => r === 'ANY');
    if (hepsiANY) {
      const ciftSayisi = Math.floor(urunler.length / esik);
      toplam += ciftSayisi * tutar;
    } else {
      // greedy farklı harfli çift
      let kalan = [...list];
      while (kalan.length >= esik) {
        const kullanilanH = new Set();
        const kullanilanU = new Set();
        const cift = [];
        const sonraki = [];
        for (const s of kalan) {
          const r = s.camp.rol || 'ANY';
          if (!kullanilanH.has(r) && !kullanilanU.has(s.bi) && cift.length < esik) {
            cift.push(s); kullanilanH.add(r); kullanilanU.add(s.bi);
          } else sonraki.push(s);
        }
        if (cift.length === esik) { toplam += tutar; kalan = sonraki; }
        else break;
      }
    }
  });
...
// Yeni seçenek mevcut seçim listesiyle çelişiyor mu?
function _kombinasyonGecerliMi(mevcutSecim, yeniOpt) {
  const bi   = yeniOpt.bi;
  const camp = yeniOpt.camp;

  // Aynı ürün zaten seçili mi?
  const urunSecimi = mevcutSecim.find(s => s.bi === bi);
  if (urunSecimi) {
    // Aynı üründe ⎇ ve 🔒 birlikte olamaz
    if (urunSecimi.camp.tip !== camp.tip) return false;
    // Aynı gruptan 2. 🔒 olamaz
    if (camp.tip === 'kilitli' && urunSecimi.camp.grup === camp.grup) return false;
    return false; // aynı üründen zaten seçim var (birden fazla izin yok basit model)
  }
  return true;
}

// ═══════════════════════════════════════════════════════════════
// 🔔 AKILLI ÇAPRAZ SATIŞ (SMART UPSELL) MOTORU
// Eşiğe 1-2 ürün kalan kampanyaları tespit eder ve bar gösterir.
// ═══════════════════════════════════════════════════════════════

function checkUpsellOpportunities() {
  const barId = 'upsell-opportunity-bar';
  let bar = document.getElementById(barId);

  if (!basket.length || !isNakitSepet()) {
    if (bar) bar.remove();
    return;
  }

  // ── Tüm ürünlerin kampanyalarını tara ───────────────────────
  // Bir grupta kaç farklı ürün seçili? Eşiğe kaç eksik?
  const grupBilgi = {}; // { grup: { esik, tutar, secili:Set, tipik_tip } }
  ...
  basket.forEach((item, bi) => {
    ...
    item._campaigns.forEach((camp, ci) => {
      ...
      const g = camp.grup;
      if (!grupBilgi[g]) grupBilgi[g] = { esik: camp.esik, tutar: camp.tutar, tip: camp.tip, sahipUrunler: new Set() };
      grupBilgi[g].sahipUrunler.add(bi); // bu kampanyaya sahip ürünler
    });
  });

  // Seçili durumu: halihazırda kaç ürün bu grubu seçmiş?
  const grupSecili = {};
  basket.forEach((item, bi) => {
    Object.entries(item._selectedCamps || {}).forEach(([ci, sel]) => {
      if (!sel) return;
  ...
  // En iyi fırsat: eşiğe en yakın, en yüksek kazanç
  let enIyiFirsat = null;

  Object.entries(grupBilgi).forEach(([grup, info]) => {
    const seciliSayi  = (grupSecili[grup] || new Set()).size;
    const sahipSayi   = info.sahipUrunler.size;
    // Tamamlanan çift sonrası açık çiftteki eksik
    const acikEksik   = info.esik - (seciliSayi % info.esik || info.esik);
    const tamamlanan  = Math.floor(seciliSayi / info.esik);

    // Fırsat: açık çiftte 1 ürün eksikse VE sepette bu gruptan yeterli ürün yok ama eklenebilecek ürünler var
    if (acikEksik === 1 && seciliSayi > 0) {
      const kazanc = info.tutar;
      if (!enIyiFirsat || kazanc > enIyiFirsat.kazanc) {
        enIyiFirsat = { grup, kazanc, tip: info.tip, esik: info.esik, tamamlanan };
      }
    }
    // Fırsat: hiç seçim yok ama grupta esik kadar ürün var (hızlı kazanç)
    if (seciliSayi === 0 && sahipSayi >= info.esik) {
      const kazanc = info.tutar;
      if (!enIyiFirsat || kazanc > enIyiFirsat.kazanc) {
        enIyiFirsat = { grup, kazanc, tip: info.tip, esik: info.esik, tamamlanan: 0, hazir: true };
      }
    }
  });
  ...
  const kazancFmt = enIyiFirsat.kazanc >= 1000
    ? (enIyiFirsat.kazanc/1000).toFixed(enIyiFirsat.kazanc%1000===0?0:1)+'k'
    : enIyiFirsat.kazanc;

  const mesaj = enIyiFirsat.hazir
    ? `<b>${enIyiFirsat.grup}</b> kampanyası için gereken ${enIyiFirsat.esik} ürün sepette — ✨ Optimize Et'e bas, <b>${kazancFmt}₺</b> kazan!`
    : `<b>${enIyiFirsat.grup}</b> kampanyasına <b>1 ürün eksik</b> — tamamlarsan <b>${kazancFmt}₺</b> indirim kazanırsın!`;
  ...
  if (!bar) {
    ...
    bar.style.cssText = [
      ...
      'display:flex','align-items:center','gap:10px',
      'font-size:.72rem','z-index:50',
      'border-top:2px solid #f59e0b',
      'animation:slideInUp .25s ease',
      'cursor:pointer'
    ].join(';');
    bar.onclick = () => optimizeCampaigns();

    const cartArea = document.getElementById('cart-table-area');
    if (cartArea) cartArea.appendChild(bar);
  }

  bar.innerHTML = `
    <span style="font-size:1.1rem">💡</span>
    <span style="flex:1">${mesaj}</span>
    <span style="background:#f59e0b;color:#0f172a;padding:3px 10px;border-radius:20px;
      font-weight:800;font-size:.68rem;white-space:nowrap">✨ Optimize Et</span>
    <button onclick="event.stopPropagation();this.closest('#upsell-opportunity-bar').remove()"
      style="background:none;border:none;color:#94a3b8;font-size:1rem;cursor:pointer;
      padding:0 4px;line-height:1">✕</button>
  `;
}

// ── updateCartUI'a upsell hook ekle (mevcut fonksiyon değiştirilmiyor) ──
const _origUpdateCartUI = updateCartUI;
window.updateCartUI = function() {
  _origUpdateCartUI.call(this);
  try { checkUpsellOpportunities(); } catch(e) { /* sessiz */ }
};
