...
function fmt(val) {
  ...
  // ₺ sembolü bazı iOS/Android fontlarında hatalı render edilir ("köy" sorunu)
  // Çözüm: HTML entity &#8378; kullan — tüm tarayıcılarda güvenli
...
function addToBasket(idx) {
  ...
  // ── HİYERARŞİ ZORUNLULUĞU: Blur → Sepet ──────────────────────────
  // Fiyatına bakılmadan sepete eklenen ürünü blur'lanmış say
  // Bu sayede funnel_logs'ta funnelBlur >= funnelSepet garantilenir
  const _addUrunAdi = (() => {
    const k = Object.keys(p).find(kk => (kk||'').toLowerCase() === 'urun') || '';
    return p[k] || '';
  })();
  if (_addUrunAdi && _sessionData && !_sessionData.blurUrunler[_addUrunAdi]) {
    _sessionData.blurUrunler[_addUrunAdi] = true;
    if (!_sessionData.revealedPrices.includes(_addUrunAdi))
      _sessionData.revealedPrices.push(_addUrunAdi);
    // _blurredThisSession'a da ekle (Abaküs eşleme için)
    const _addIdx = allProducts.findIndex(pr => {
      const kk = Object.keys(pr).find(k2 => (k2||'').toLowerCase() === 'urun') || '';
      return pr[kk] === _addUrunAdi;
    });
    if (_addIdx >= 0) _blurredThisSession[_addUrunAdi] = _addIdx;
    localStorage.setItem('_sd', JSON.stringify(_sessionData));
  }
...
if (!enKarliMap[td.n] || oran < enKarliMap[td.n].oran) {
  ...
  // ── Ek İndirim Alanı (Pazarlık) ──────────────────────────────
  // Yuvarlama SONRASI uygulanır → yukarı yuvarlanma sorunu olmaz
  // Formül: Final = yuvarlaKademe(brüt) − ekIndirim
  html += `<div style="display:flex;align-items:center;gap:8px;margin:6px 0 4px;flex-wrap:wrap">
    <label style="font-size:.68rem;color:var(--text-2);font-weight:700;white-space:nowrap">💬 Pazarlık İndirimi (₺)</label>
    <input id="ab-ek-indirim" type="number" min="0" placeholder="0"
      oninput="calcAbakus()"
      style="width:90px;padding:5px 8px;border:1.5px solid var(--border);border-radius:7px;
             font-size:.80rem;font-family:inherit;text-align:right;background:var(--surface);color:var(--text-1)">
  ...
  <div style="margin-bottom:6px">
    <input id="ab-pazarlik-notu" type="text" placeholder="Pazarlık notu (opsiyonel)…"
      style="width:100%;box-sizing:border-box;padding:5px 8px;border:1px solid var(--border);
             border-radius:7px;font-size:.72rem;font-family:inherit;background:var(--surface);color:var(--text-1)">
  </div>`;
...
async function finalizeAksiyon() {
  ...
  // Pazarlık indirimi ve notu — abaküsten geliyor
  ...
  if (abakusSelection) {
    ...
    // Ek pazarlık indirimi: yuvarlama SONRASI — yukarı yuvarlanma yok
    tahsilat = Math.max(0, yuvarlanmis - _ekIndirimF);
...
function buildPremiumPDF(docType, data) {
  ...
  if (isNakit) {
    ...
    // Pazarlık (ek) indirimi — ayrı satır, şeffaf gösterim
    const _pdfEkIndirim = Number(data.ekIndirim || 0);
    if (_pdfEkIndirim > 0) {
      odemeRows += `
        <tr><td class="ol" style="color:#b45309;">💬 Pazarlık İndirimi<\/td><td class="or" style="color:#b45309;font-weight:700;">-${fmt(_pdfEkIndirim)}<\/td><\/tr>
      `;
...
logs.forEach(l => {
  ...
  if (h >= 0 && h <= 23) { 
    ...
    // Kaçırılan ciro: unique session bazlı — mükerrer sayımı önle
    // Aynı personel+tarih+saat kombinasyonu 1 oturum sayılır
    const _ciroSessionSeen = new Set();
    const l3KayipCiro = logs
      .filter(l => (l.intentLevel||0) >= 3 && l.sonuc === 'kacti' && l.neden !== HAREKETSIZLIK_NEDEN)
      .reduce((s, l) => {
        // Tekil oturum anahtarı
        const _cKey = (l.personelId||'?') + '_' + (l.tarih||'') + '_' + (l.saat??'');
        if (_ciroSessionSeen.has(_cKey)) return s; // mükerrer oturumu atla
        _ciroSessionSeen.add(_cKey);
        // Sepetteki son net tutarı kullan (en güvenilir)
        const tutar = l.toplamTutar || 0;
        if (tutar > 0) return s + tutar;
        // Sepet yoksa bakilanFiyatlar'dan tahmin — birim fiyat * 1 adet
    ...
    // Tekil oturum: ts (ms cinsinden) varsa en güvenilir anahtar
    // Yoksa: personelId + tarih + dakika (saat değil) kombinasyonu
    const _uniqueSessionKey = l => {
      if (l.ts && l.ts.seconds) return l.personelId + '_' + l.ts.seconds; // Firestore Timestamp
      if (l.ts && typeof l.ts === 'number') return l.personelId + '_' + l.ts;
      // Fallback: dakika hassasiyetinde — aynı personelin aynı dakikadaki 2. kaydını atla
      return (l.personelId||'?') + '_' + (l.tarih||'') + '_' + (l.saat??'') + '_' + (l.gun??'');
    };
    ...
    // Tutarsızlık: eski kayıtlarda benzersizBlurSayisi eksik olabilir — UI Math.min ile kırpıyor
    // console.warn kaldırıldı (kasıtlı) — yeni kayıtlarda hiyerarşi koduyla önleniyor
