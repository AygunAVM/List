...
async function showApp() {
  ...
  if (searchEl) {
    ...
    searchEl.placeholder = ad ? 'Yanlış Listesesin, ' + ad + ' — Ürün arama' : 'Ürün arama';
  }
...
function toggleCampaign(idx, campIdx) {
  ...
  if (camp.tip !== 'proje') {
    ...
    if (camp.tip === 'kilitli' && itemHasKilitli) {
      const eskiEntry = Object.entries(item._selectedCamps).find(([ci, s]) => {
        if (!s) return false;
        const c = item._campaigns[parseInt(ci)];
        return c && c.tip === 'kilitli' && c.grup === camp.grup;
      });
      if (eskiEntry) delete item._selectedCamps[parseInt(eskiEntry[0])];
    }
  }
...
function _campSearchDot(item, camp, ci) {
  ...
  // Pill içine gömülü küçük arama noktası — onclick propagation durdurulur
  const isProje = camp.tip === 'proje';
  const dotBg    = pending ? 'rgba(217,119,6,.35)' : isProje ? 'rgba(109,40,217,.3)' : 'rgba(30,64,175,.3)';
  const dotColor = pending ? '#fcd34d' : isProje ? '#c4b5fd' : '#93c5fd';

  return `<span
    data-grup='${grup}' data-esik='${esik}' data-exclude-rol='${mevcutRol}'
    title='${grup} eşik tamamlayıcı ürünleri ara'
    onclick="event.stopPropagation();event.preventDefault();
      document.querySelectorAll('.camp-ara-btn[data-grup]').forEach(function(el){
        if(el.dataset.grup==='${grup}'&&el.dataset.esik==='${esik}')el.click();
      });
      var s=document.getElementById('search');
      if(s){s.value='${grup}';s.dispatchEvent(new Event('input'));}
      var modal=document.getElementById('basket-modal');if(modal)modal.style.display='none';"
    style="display:inline-flex;align-items:center;justify-content:center;
      width:13px;height:13px;border-radius:50%;
      background:${dotBg};color:${dotColor};
      cursor:pointer;font-size:.45rem;flex-shrink:0;
      margin-left:4px;line-height:1;
      border:1px solid ${dotColor};opacity:.85;
      transition:opacity .15s">🔍</span>`;}

function _campSearchBtn(item, camp, ci) {
  // Artık kullanılmıyor — _campSearchDot pill içine entegre edildi
  return '';
}
...
function renderCampaignPills(item, idx) {
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    // Eşik rozeti: seçilmemişken xN, pending'de +N
    let pillRozet = '';
