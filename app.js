...
logs.forEach(l => {
  ...
  if (h>=0) { 
        ...
        return `<div style="background:var(--surface);border:1.5px solid var(--border);border-radius:16px;padding:14px;position:relative">
          ...
          <div style="display:grid;grid-template-columns:repeat(6,1fr);gap:4px;font-size:.62rem">
            ...
            <div style="background:#f8fafc;border-radius:7px;padding:5px 2px;text-align:center">
              <div style="font-size:.60rem;font-weight:700;color:#92400e" title="Altın sepet: 3+ farklı ürün kategorisi">🥇 ${s.altin}</div>
              <div style="font-size:.60rem;font-weight:700;color:#64748b" title="Gümüş sepet: 2 farklı ürün kategorisi">🥈 ${s.gumus}</div>
    ...
    // ── SAATLİK YOĞUNLUK BARI — garantili render ──────────────────
    ...
    const saatBar = [...Array(24).keys()].map(h => {
      ...
      const top = s + k;
      // Genişlik: 100px referans üzerinden hesapla, flex ile büyüsün
      const sW = top === 0 ? 0 : Math.round(s / saatMax * 100);
      const kW = top === 0 ? 0 : Math.round(k / saatMax * 100);
