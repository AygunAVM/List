...
function parseCampaigns(aciklama) {
  ...
  // Format: ⤚KOŞUL|SİMGE  örn: ⤚A17|⌗  ⤚CEP|✦  ⤚*|⌗
...
function _bundleMatches(condition, basket) {
  ...
  if (cond === '*') return basket.length > 0;
  const cLow = cond.toLowerCase();
  return basket.some(item =>
    (item.kod  || '').toLowerCase() === cLow ||
    (item.gam  || '').toLowerCase().includes(cLow) ||
    (item.urun || '').toLowerCase().includes(cLow)
  );
}

function _getBundleSuggestions() {
  if (!basket.length || !allProducts?.length) return [];
  const basketKodlar = new Set(basket.map(i => (i.kod || '').toLowerCase()));
  const results = [];
  ...
  allProducts.forEach((p, idx) => {
    ...
    if (basketKodlar.has((p.Kod || '').toLowerCase())) return;
    ...
    results.push({
      _idx: idx, urun: p[urunKey] || '', kod: p.Kod || '',
  ...
  results.sort((a, b) => {
    const score = r => {
      ...
      if (basket.some(i => (i.kod||'').toLowerCase() === r.condition.toLowerCase())) return 2;
      return 1;
    };
