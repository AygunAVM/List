...
function checkChanges(json) {
  ...
  // Bu versiyon daha önce diff'lendi mi? (snapshot karşılaştırması yapıldı mı)
  if(!seen.includes(vKey)) {
    // Önceki snapshot ile fark hesapla
    ...
    if(lastJson) {
      ...
      if(changes.length > 0) {
        ...
        log.push({
          ...
          ts:    new Date().toISOString(),
          changes,
          shown: false   // popup'ta gösterilmedi
        });
  ...
  // Her açılışta gösterilmemiş log girişlerini kontrol et
...
function showPendingChanges(logKey) {
  ...
  // Gösterilmemiş (shown:false) kayıt var mı?
  const hasNew = log.some(e => !e.shown);
  if(!hasNew) return;

  // Tüm girişleri listele: shown:true → çizili/soluk, shown:false → aktif
  const allChanges = [];
  log.forEach(entry => {
    allChanges.push({
      type: 'versiyon',
  ...
  // shown:true işaretini KAPATINCA yap (logKey'i popup'a aktar)
...
function closeChangePopup() {
  const p = document.getElementById('change-popup');
  // Popup kapatılınca gösterilmemiş log girişlerini shown=true yap
