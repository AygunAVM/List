...
function _qfRenderTaksit() {
  ...
  // Pahalı taksit eleme: taksit N'in toplam fiyatı, N-1'den büyükse gösterme
  // (örn: 5 taksit toplamı > 4 taksit toplamı ise 5 taksit anlamsız)
  const goster = {};
  let oncekiTahsilat = null;
  TAK_LIST.forEach(n => {
    if (!hesap[n]) return;
    if (oncekiTahsilat !== null && hesap[n].tahsilat >= oncekiTahsilat) return; // elendi
    goster[n] = hesap[n];
    oncekiTahsilat = hesap[n].tahsilat;
  });

  let rows = '';
  Object.entries(goster).forEach(([n, v]) => {
    n = parseInt(n);
    const aylik = Math.ceil(v.tahsilat / n);
...
function _qfRenderKredi() {
  ...
  // oranlar.json'dan Tip='kredi' satırları — öncelik buradan
  const krediSatirlari = allRates.filter(r => r.Tip === 'kredi');

  // Fallback: JSON'da kredi satırı yoksa hardcode değerler
  ...
  if (krediSatirlari.length > 0) {
    // JSON'dan oku — her kredi kurumu için tüm vadeleri topla
    const kurumlar = [...new Set(krediSatirlari.map(r => r.Kart).filter(Boolean))];
    kurumlar.forEach(kurum => {
      const satirlar = krediSatirlari.filter(r => r.Kart === kurum);
      html += '<div class="qf-kredi-blok"><div class="qf-kredi-baslik">' + kurum + '</div>'
            + '<table class="qf-taksit-table"><thead><tr><th>Vade</th><th>Aylık Faiz</th><th>Aylık</th><th>Toplam</th></tr></thead><tbody>';
      // 3-48 ay arası vadeleri tara
      [3,6,9,12,18,24,36,48].forEach(ay => {
        satirlar.forEach(r => {
          ...
          const selected = _qfPlan?.kart===kurum && _qfPlan?.taksit===ay;
          html += '<tr class="qf-taksit-row' + (selected?' selected':'') + '" onclick="_qfSelectPlan(this,\'' + kurum + '\',' + ay + ',' + toplam + ',\'' + kurum + '\')">'
                + '<td>' + ay + ' Ay</td>'
                + '<td>%' + faiz + '</td>'
                + '<td>' + fmt(aylik) + '/ay</td>'
                + '<td>' + fmt(toplam) + '</td>'
    ...
    // Fallback
    KREDI_FALLBACK.forEach(k => {
      html += '<div class="qf-kredi-blok"><div class="qf-kredi-baslik">' + k.ad + '</div>'
            + '<table class="qf-taksit-table"><thead><tr><th>Vade</th><th>Aylık Faiz</th><th>Aylık</th><th>Toplam</th></tr></thead><tbody>';
      k.vadeler.forEach(v => {
        const toplam = Math.round(nakit * Math.pow(1 + v.faiz/100, v.ay) / 50) * 50;
        const aylik  = Math.ceil(toplam / v.ay);
        const selected = _qfPlan?.kart===k.ad && _qfPlan?.taksit===v.ay;
        html += '<tr class="qf-taksit-row' + (selected?' selected':'') + '" onclick="_qfSelectPlan(this,\'' + k.ad + '\',' + v.ay + ',' + toplam + ',\'' + k.ad + '\')">'
              + '<td>' + v.ay + ' Ay</td>'
              + '<td>%' + v.faiz + '</td>'
              + '<td>' + fmt(aylik) + '/ay</td>'
              + '<td>' + fmt(toplam) + '</td>'
  ...
  return html || '<div class="qf-empty">Kredi verisi yüklenemedi.</div>';
}
