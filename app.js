...
function openQuickFinance(urunIdx, nakitFiyat) {
  ...
  // ── ÇÖZÜM 2: ❖ Proje fiyatı olan ürün → QF modal yok ────────────
  // Proje fiyatı taksitli hesaplamayı geçersiz kılar; mekanizma sepet+abaküs.
  if (aciklama.includes('❖')) {
    // Sepete ekle ve abaküse yönlendir
    addToBasket(urunIdx);
    incrementDailyStat('blur_sayisi', 1).catch(() => {});
    Swal.fire({
      icon: 'info',
      title: '❖ Proje Fiyatı',
      html: '<b>' + urunAd + '</b><br><br>Bu ürün proje fiyatı mekanizmasıyla çalışmaktadır.<br>Ödeme seçenekleri için sepete eklendi — Ödeme Seçeneklerine yönlendiriliyorsunuz.',
      timer: 2200,
