...
async function fetchWeeklyStar() {
  ...
  try {
    ...
    const haftaOnceTar = haftaOnce.toISOString().slice(0, 10);

    // ── 1. Tekliften satışa dönen sayım (proposals koleksiyonu) ──
    const snap = await getDocs(
      query(collection(_db, 'proposals'),
        where('durum', '==', 'satisDondu')
      )
    );
    const counts = {};
    snap.docs.forEach(d => {
      const data = d.data();
      let tsDate;
      const raw = data.archivedAt || data.ts;
      if (raw?.toDate) tsDate = raw.toDate();
      else if (typeof raw === 'string') tsDate = new Date(raw);
      else return;
      if (tsDate < haftaOnce) return;
      const u = data.user || '—';
      counts[u] = (counts[u] || 0) + 1;
    });

    // ── 2. Doğrudan satış belgesi sayımı (daily_stats) ──────────
    // Her personelin son 7 günlük daily_stats dokümanlarını tara
    // Not: Bu sorgu tüm kullanıcıların verilerini çeker — Firestore kurallarını kontrol edin
    try {
      ...
      dsSnap.docs.forEach(d => {
        const sd = d.data();
        // Yalnızca "doğrudan satış" sayısını ekle — proposals'tan gelmeyenleri temsil eder
        // Ancak proposals'tan gelen satisDondu ile çakışmayı önlemek için
        // daily_stats'taki satis_sayisi'nde teklif->satis dönüşümleri de var.
        // Bu nedenle daily_stats'ı sadece proposals sayısının 0 olduğu personeller için kullan:
        const email = sd.personelId;
        if (!email) return;
        const dsCount = sd.satis_sayisi || 0;
        if (dsCount > 0 && !counts[email]) {
          // Proposals'ta satışı olmayan ama daily_stats'ta olan personeli ekle
          counts[email] = dsCount;
        }
      });
    } catch(e) {
      // collectionGroup erişimi yoksa sessizce geç (Firestore rules)
      console.warn('fetchWeeklyStar daily_stats:', e);
    }
    ...
    ${(() => {
      ...
      if (l3.giris === 0) {
        return \`<div style="background:#f8fafc;border-radius:12px;padding:13px 16px;margin-top:8px;border:1px solid #e2e8f0;display:flex;align-items:center;gap:10px">
          ...
          <div>
            <div style="font-size:.68rem;font-weight:700;color:#64748b">L3 Kapanış Oranı</div>
            <div style="font-size:.60rem;color:#94a3b8;margin-top:2px">Abaküs açıldığında burada görünür — fiyat müzakeresi başlat!</div>
          </div>
        </div>\`;
      }
      ...
      const l3Hint = l3Oran >= 40
        ? '✅ Fiyat müzakeresinde güçlüsün!'
        : l3Oran >= 20
          ? '👍 Gelişiyor — her 5 müzakereden 2'si satışa dönmeli'
          : '⚠️ Abaküs açılan müşteride kapama oranın düşük';
      const hareketsizNote = l3.hareketsizKacti > 0
      ...
      return \`<div style="background:\${l3Bg};border-radius:12px;padding:13px 16px;margin-top:8px;border:1px solid \${l3Renk}33">
        <div style="display:flex;justify-content:space-between;align-items:flex-start">
          <div>
            <div style="font-size:.60rem;font-weight:800;letter-spacing:.06em;color:\${l3Renk};text-transform:uppercase">L3 Kapanış Oranı</div>
            <div style="font-size:.60rem;color:#64748b;margin-top:1px">Abaküs Satış ÷ Abaküs Giriş</div>
          </div>
