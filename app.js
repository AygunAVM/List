...
function renderSiparisBildirimBar() {
  ...
  // Admin için gizle — admin paneli yeterli
  if (isAdmin()) {
    bar.style.display = 'none';
    return;
  }

  // Personel motivasyon analizi — proposals array'inden hafif hesaplama (network yok)
  try {
    const me         = currentUser?.Email || '';
    const bugun      = new Date().toISOString().slice(0, 10);
    const haftaOnce  = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
    const benim      = (proposals || []).filter(p => p.user === me);
    const bugunTeklif = benim.filter(p => (p.ts || '').slice(0, 10) === bugun).length;
    const bugunSatis  = benim.filter(p => p.durum === 'satisDondu' && (p.ts || '').slice(0, 10) === bugun).length;
    const haftaTeklif = benim.filter(p => (p.ts || '').slice(0, 10) >= haftaOnce).length;
    const haftaSatis  = benim.filter(p => p.durum === 'satisDondu' && (p.ts || '').slice(0, 10) >= haftaOnce).length;
    const donusum     = haftaTeklif > 0 ? Math.round((haftaSatis / haftaTeklif) * 100) : 0;

    let emoji, mesaj, alt = '', renk;
    if      (bugunSatis >= 3)  { emoji='🔥'; renk='#15803d'; mesaj=`Bugün ${bugunSatis} satış kapattın!`; alt='Süper gidiyorsun!'; }
    else if (bugunSatis === 2) { emoji='💪'; renk='#1d4ed8'; mesaj=`Bugün 2 satış — bir tane daha!`; alt=haftaTeklif>0?`Bu hafta %${donusum} dönüşüm`:''; }
    else if (bugunSatis === 1) { emoji='✅'; renk='#0369a1'; mesaj=`Bugün 1 satış yaptın.`; alt='Bir teklif daha oluştur!'; }
    else if (bugunTeklif >= 2) { emoji='⚡'; renk='#b45309'; mesaj=`Bugün ${bugunTeklif} aktif teklif var.`; alt='Birini satışa dönüştür!'; }
    else if (haftaSatis > 0)   { emoji='📈'; renk='#0369a1'; mesaj=`Bu hafta ${haftaSatis} satış · %${donusum} dönüşüm`; alt='Bugün yeni teklif oluştur!'; }
    else if (haftaTeklif > 0)  { emoji='🎯'; renk='#7c3aed'; mesaj=`${haftaTeklif} teklifin var — takip et!`; alt='Satışa dönüştürmek için ara!'; }
