...
function _renderSingleProp(p) {
  ...
  const canAct = isAdmin() || p.user===me;
    ...
    // Sepete Ekle — canEdit'ten bağımsız; kendi teklifi olan herkes (satis, destek, admin) ekleyebilir
    const canAddToCart = isAdmin() || p.user === (currentUser?.Email||'');
    if(canAddToCart) btns.push(`<button class="pact-btn haptic-btn" onclick="teklifSepeteEkle('${p.id}')" title="Sepete Ekle" style="color:#16a34a;border-color:#bbf7d0;background:#f0fdf4">🛒</button>`);
    if(canEdit) {
...
window.setMagazaFiltre = function(filtre) {
  console.log('🏬 Mağaza filtresi değiştirildi:', filtre);
  const cont = document.getElementById('funnel-analiz-konteynir');
...
async function loadFunnelAnaliz(gunAralik = 90, force = false) {
  ...
  try {
    ...
    // Mağaza tipi filtresi (AVM / CARSI / hepsi)
    const aktifMagaza = cont.dataset.magazaFiltre || 'hepsi';
    const logs = aktifMagaza === 'hepsi'
      ? rolFiltreli
      : rolFiltreli.filter(l => {
          const mt = (l.magazaTipi || 'BELIRSIZ').toUpperCase();
          return mt === aktifMagaza.toUpperCase();
        });
...
logs.forEach(l => {
  ...
  if (h >= 0 && h <= 23) { 
    ...
    // Tutarsızlık: eski kayıtlarda benzersizBlurSayisi eksik olabilir — UI Math.min ile kırpıyor, uyarı bastırıldı
    // if (_rawSepet > funnelBlur) console.warn(...)  // kasıtlı olarak devre dışı
...
<div style="display:flex;gap:6px;margin-bottom:12px;flex-wrap:wrap;align-items:center">
  <span style="font-size:.65rem;color:var(--text-3);font-weight:600;margin-right:2px">🏬 Mağaza:</span>
