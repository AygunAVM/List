...
function renderTable(searchVal) {
  ...
  data.forEach(u => {
    ...
    if (hasPrim) {
      ...
      // Prim seviyesine göre sınıf (gerçek dağılım: 775 ürün)
      if      (primVal >= 400) primCls = 'prim-platin'; // Platin  400+  (~20 ürün)
      else if (primVal >= 200) primCls = 'prim-altin';  // Altın   200-399 (~60 ürün)
      else if (primVal >= 100) primCls = 'prim-gumus';  // Gümüş   100-199 (~130 ürün)
      else if (primVal >= 31)  primCls = 'prim-bronz';  // Bronz   31-99  (~250 ürün)
      else                     primCls = 'prim-low';    // Düşük   1-30   (~350 ürün)
    }
...
// Seçilmemiş birlesen/kilitli kampanya pill'lerinin sağına 🔍 ekler.
// allProducts'ta stoklu+makul fiyatlı tamamlayıcı yoksa buton çıkmaz.
function _campSearchBtn(item, camp, ci) {
  ...
  // Seçilmiş veya eşik bekleniyor → gizle
  const sc = item._selectedCamps || {};
  const pg = item._pendingGroups || {};
  if (sc[ci] || sc[String(ci)] || sc[Number(ci)]) return '';
  if (pg[camp.grup]) return '';
  ...
  // Sepetteki ürün kodlarını dışla
  const sepetKodlar = new Set(
    (typeof basket !== 'undefined' ? basket : [])
      .map(b => String(b['Kod'] || ''))
  );

  // allProducts'ta stoklu + makul fiyatlı + farklı rollü tamamlayıcı ara
  // Sadece varlık kontrolü — ilk eşleşme yeterli, listelemiyoruz
  let bulunan = false;
  const urunler = typeof allProducts !== 'undefined' ? allProducts : [];

  for (const p of urunler) {
    const pKod = String(p['Kod'] || '');
    if (sepetKodlar.has(pKod)) continue;

    const pNakit = parseFloat(String(p['Nakit'] || 0).replace(',', '.')) || 0;
    if (iNakit > 0 && pNakit > iNakit * YON) continue;

    const pStok = parseFloat(p['Stok'] || 0) || 0;
    if (pStok <= 0) continue;

    const pAciklama = p['Açıklama'] || '';
    if (!pAciklama || pAciklama === '-') continue;

    // _buildCampObj regex'ini doğrudan kullan (parseCampaigns çağrısı yerine)
    // Bu hem daha hızlı hem de ayraç bağımlılığı yok
    const re = /([A-ZÇŞĞÜÖİa-zçşğüöı0-9_]+)\s+[-–]?\s*[\d.,]+[kK]?\s*\((\d+)\/([A-Z])\)/gi;
    let m;
    while ((m = re.exec(pAciklama)) !== null) {
      const pGrup = m[1].toUpperCase();
      const pEsik = parseInt(m[2]);
      const pRol  = m[3].toUpperCase();
      if (pGrup === grup && pEsik === esik && pRol !== mevcutRol) {
        bulunan = true;
        break;
      }
    }
  ...
  return `<button type="button"
    title="Eşik tamamlayıcı ürünleri ara"
    onclick="window._campSearch(${JSON.stringify(grup)})"
    style="display:inline-flex;align-items:center;justify-content:center;
      width:15px;height:15px;border-radius:3px;
      background:#172554;color:#93c5fd;
      border:1px solid #1e40af;cursor:pointer;
      font-size:.55rem;flex-shrink:0;margin-left:2px;
      vertical-align:middle;font-family:inherit;padding:0;
      line-height:1;position:relative;top:-1px">🔍</button>`;
}

// Global: sepeti kapat → Ürünler sekmesi → arama
window._campSearch = function(kelime) {
  if (!kelime) return;
