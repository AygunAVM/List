...
// ─── SATIŞ HUNİSİ ANALİZ ──────────────────────────────────────
// ✅ DÜZELTME: Firestore okuma limiti KALDIRILMADI — her sayfa hâlâ
// limit(FUNNEL_PAGE_SIZE) ile sınırlı (maliyet/performans için gerekli).
// Ama önceden sorgu sadece TEK sayfa (en güncel 500 kayıt) çekiyordu; bu
// yüzden 30/90 gün veya geniş bir Özel aralık, kayıt sayısı 500'ü geçtiği an
// hep AYNI en-güncel-500 kaydı görüyordu (tarih aralığı büyütülse de eskiye
// inilemiyordu). Şimdi 'startAfter' ile sayfa sayfa ilerleyip seçilen tarih
// aralığının TAMAMINI tarıyoruz; FUNNEL_MAX_PAGES sadece çok büyük
// koleksiyonlarda sınırsız okumayı (maliyet patlamasını) önleyen bir
// güvenlik tavanı.
const FUNNEL_PAGE_SIZE = 500;
const FUNNEL_MAX_PAGES = 6; // güvenlik tavanı: en fazla 6 × 500 = 3000 kayıt
async function _fetchFunnelLogsPaged(baseConstraints) {
  const results = [];
  let lastDoc = null;
  let tavanaTakildi = false;
  for (let sayfa = 0; sayfa < FUNNEL_MAX_PAGES; sayfa++) {
    const pageConstraints = lastDoc
      ? [...baseConstraints, startAfter(lastDoc), limit(FUNNEL_PAGE_SIZE)]
      : [...baseConstraints, limit(FUNNEL_PAGE_SIZE)];
    const q = query(collection(_db, 'funnel_logs'), ...pageConstraints);
    const snap = await getDocs(q);
    if (snap.empty) break;
    snap.forEach(d => results.push(d.data()));
    lastDoc = snap.docs[snap.docs.length - 1];
    if (snap.size < FUNNEL_PAGE_SIZE) break; // bu sayfa tam dolmadı → daha fazla kayıt yok
    if (sayfa === FUNNEL_MAX_PAGES - 1) tavanaTakildi = true;
  }
  if (tavanaTakildi) {
    console.warn(`⚠️ Funnel analiz: güvenlik tavanına (${FUNNEL_MAX_PAGES * FUNNEL_PAGE_SIZE} kayıt) ulaşıldı, seçilen aralıkta daha eski kayıtlar olabilir.`);
  }
  results._tavanaTakildi = tavanaTakildi;
  return results;
}
...
async function loadFunnelAnaliz(gunAralik = null, force = false) {
  ...
  try {
    ...
    // ✅ DÜZELTME: limit(500) artık tek seferlik değil — _fetchFunnelLogsPaged
    // 'startAfter' ile sayfa sayfa ilerleyip seçilen tüm aralığı tarıyor.
    const baseConstraints = limitDateEnd
      ? [where('ts', '>=', limitDate), where('ts', '<=', limitDateEnd), orderBy('ts', 'desc')]
      : [where('ts', '>=', limitDate), orderBy('ts', 'desc')];
    const allLogs = await _fetchFunnelLogsPaged(baseConstraints);
...
function _renderFunnelHTML(cont, aktifFiltre, s) {
  ...
  cont.innerHTML = _renderFunnelFilterPanelHTML(cont, aktifFiltre, totN, gosterimLabel) + `
      ${tavanaTakildi ? `<div style="background:#78350f;color:#fde68a;border-radius:10px;padding:7px 12px;margin-bottom:12px;font-size:.66rem;text-align:center">⚠️ Seçilen aralıkta çok fazla kayıt var, güvenlik sınırı nedeniyle yalnızca en güncel ${FUNNEL_MAX_PAGES * FUNNEL_PAGE_SIZE} kayıt taranıyor — daha eski bazı kayıtlar bu sonuca dahil olmayabilir.</div>` : ''}
      <!-- Genel Özet -->
