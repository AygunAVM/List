...
function toggleCampaign(idx, campIdx) {
  ...
  if (camp.tip !== 'proje') {
    ...
    // Aynı üründe aynı GRUPTAN ikinci 🔒 seçilemez — ama farklı eşik miktarındaki 🔒'lar
    // radio-button gibi davranmalı: yeni seçilen eskisini otomatik bıraksın
    if (camp.tip === 'kilitli' && itemHasKilitli) {
      const ayniGrupKilitliCi = Object.entries(item._selectedCamps).find(([ci, sel]) => {
        if (!sel) return false;
        const c = item._campaigns[parseInt(ci)];
        return c && c.tip === 'kilitli' && c.grup === camp.grup;
      });
      if (ayniGrupKilitliCi) {
        // Önceki 🔒'yı sil, yeni seçilecek (aşağıda eklenir)
        delete item._selectedCamps[parseInt(ayniGrupKilitliCi[0])];
      }
    }
  }
...
function renderCampaignPills(item, idx) {
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    // Label: tarih ve eşik/rol parantezi temizlenmiş kısa metin
    ...
    if (esikGerekli >= 2) {
      ...
      esikSeçilen = sepBiSet.size;
    }
    const kalanEsik = Math.max(0, esikGerekli - esikSeçilen);
    ...
    if (isBilgi) {
      ...
      // Pending: saydam turuncu-amber arka plan, okunabilir
      style = `background:rgba(251,191,36,.18);color:#b45309;border-bottom:2px solid #f59e0b;font-weight:700;cursor:pointer;backdrop-filter:blur(2px);`;
    } else if (sel) {
    ...
    // Pill içi label & rozet
    // • Seçili+pending → "⎇ KEA -3k" + turuncu "+N" rozet
    // • Seçilmemiş, eşik≥2 → label + soluk "xN" rozet
    let pillLabel, pillRozet;
    if (sel && pending && kalanEsik > 0) {
      pillLabel = label; // orijinal kampanya adını göster
      pillRozet = `<span style="font-size:.52rem;background:#d97706;color:#fff;`
        + `padding:1px 5px;border-radius:3px;font-weight:800;margin-left:3px;letter-spacing:.3px">`
        + `+${kalanEsik}</span>`;
    } else if (!sel && !isBilgi && esikGerekli >= 2) {
      pillLabel = label;
      pillRozet = `<span style="font-size:.50rem;background:rgba(0,0,0,.10);color:inherit;`
        + `padding:1px 4px;border-radius:3px;font-weight:700;margin-left:2px;opacity:.8">`
        + `x${esikGerekli}</span>`;
    } else {
      pillLabel = label;
      pillRozet = '';
    ...
    return `<button type="button"${clickAttr} title="${camp.metin.replace(/"/g,'&quot;')}" `
      ...
      + `<span style="margin-left:1px">${pillLabel}</span>`
      + pillRozet
      + `</button>`
      + _campSearchBtn(item, camp, ci);
  }).join('');
  ...
  // Pending olan grupları bul — kaç eşik eksik ve hangi kampanya
  let pendingSummaryText = '⏳ eşik bekleniyor';
  if (secilenSayi > 0 && campDisc === 0 && !projeAktifPill) {
    // Seçili ama pending olan ilk kampanyayı bul
    const pendingEntries = Object.entries(item._selectedCamps)
      .filter(([ci2, s2]) => s2 && item._pendingGroups && item._pendingGroups[item._campaigns?.[parseInt(ci2)]?.grup]);
    if (pendingEntries.length > 0) {
      const firstPendingCamp = item._campaigns?.[parseInt(pendingEntries[0][0])];
      if (firstPendingCamp) {
        // Sepette bu grubun kaç üyesi var
        const _b = (typeof basket !== 'undefined' ? basket : []);
        const sepBiSet2 = new Set();
        _b.forEach((bi3, biIdx3) => {
          ...
          Object.entries(bi3._selectedCamps).forEach(([ci3, s3]) => {
            ...
            if (c3 && c3.grup === firstPendingCamp.grup) sepBiSet2.add(biIdx3);
          });
        });
        const kalan2 = Math.max(0, (firstPendingCamp.esik || 2) - sepBiSet2.size);
        const campAdKisa = (firstPendingCamp.metin || '')
          .replace(/\s*\[\d{2}\.\d{2}\.\d{2,4}\]\s*/g, '')
          .replace(/\s*\(\d+\/[A-Z]\)\s*/g, '')
          .trim().slice(0, 18);
        pendingSummaryText = kalan2 > 0
          ? `⏳ ${kalan2} eşik eklenmelidir`
          : '⏳ eşik bekleniyor';
      }
    }
  }
...
function _parseCampSegments(oldAc, newAc) {
  const re = /([A-ZÇŞĞÜÖİa-zçşğüöı0-9_+]+)\s+[-–]?\s*([\d.,]+)[kK]?\s*(\(\d+\/[A-Z]\))?/gi;
  function toMap(ac) {
    const m = new Map(); let hit; re.lastIndex=0;
    while ((hit=re.exec(ac))!==null) {
      const key = hit[1].toUpperCase()+(hit[3]?hit[3].toUpperCase():'');
      m.set(key, hit[0].trim().replace(/\s+/g,' '));
    }
    return m;
  }
  const oldMap=toMap(oldAc), newMap=toMap(newAc);
  const eklendi=[], kaldirildi=[], degisti=[];
  newMap.forEach((tok,key)=>{
    if(!oldMap.has(key)) eklendi.push(tok);
    else if(oldMap.get(key)!==tok) degisti.push({old:oldMap.get(key),new:tok});
  });
  oldMap.forEach((tok,key)=>{ if(!newMap.has(key)) kaldirildi.push(tok); });
...
function _diffJson(oldJson, newJson) {
  ...
  newJson.data.forEach(p => {
    ...
    if(descKey && p[descKey]!==old[descKey]) {
      const segs=_parseCampSegments(old[descKey]||'',p[descKey]||'');
...
function _renderMergedItem(m, idx, isRequired=true) {
  ...
  let emoji2 = hasPrice?(m.price.diff>0?'📈':'📉'):hasKamp?'📋':'📦';
  let badges2='';
  if(hasPrice){const up=m.price.diff>0,sign=up?'+':'';badges2+=`<span class="change-badge ${up?'badge-price-up':'badge-price-down'}">${sign}${m.price.pct}%</span>`;}
  if(kampEk.length)  badges2+=`<span class="change-badge badge-kamp-eklendi">+${kampEk.length} Kampanya</span>`;
  if(kampKal.length) badges2+=`<span class="change-badge badge-kamp-kaldirildi">-${kampKal.length} Kampanya</span>`;
  if(kampDeg.length) badges2+=`<span class="change-badge badge-kamp-degisti">±${kampDeg.length} Kampanya</span>`;
  if(hasStok){const up=m.stok.diff>0;badges2+=`<span class="change-badge ${up?'badge-stok-up':'badge-stok-down'}">Stok ${up?'+':''}${m.stok.diff}</span>`;}
  let details = '';
  if(hasPrice) details+=`<span class="ci-row">Nakit: ${fmt(m.price.old)} → <strong>${fmt(m.price.new)}</strong></span>`;
  kampKal.forEach(c=>{details+=`<span class="ci-row ci-kamp-kaldirildi">➖ ${c.seg}</span>`;});
...
function showChangePopup(changes, logKey) {
  ...
  function _markaGizli(c) {
    ...
    if(!mk) return false;
    return istisnalar.some(ist=>mk===ist||mk.startsWith(ist));
  }
...
function checkUpsellOpportunities(){
  const c=document.getElementById('upsell-bar-container');
  if(c) c.innerHTML='';
}
