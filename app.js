...
async function logSessionResult(sonuc, neden) {
  ...
  try {
    ...
    const _now = new Date();
    const _ist = new Date(_now.toLocaleString('en-US', { timeZone: 'Europe/Istanbul' }));
    const _tarih = _ist.getFullYear() + '-' +
      String(_ist.getMonth()+1).padStart(2,'0') + '-' +
      String(_ist.getDate()).padStart(2,'0');
    const _gun  = _ist.getDay();   // 0=Pazar
    const _saat = _ist.getHours(); // 0-23
...
logs.forEach(l => {
  ...
  if (h>=0) { 
    ...
    // Ciro kaybı: sadece gerçek müşteri kaçışlarından hesapla (hareketsizlik dahil değil)
    ...
    // ── SAATLİK YOĞUNLUK BARI (Satış/Kaçan) — çakışmayı önle ──────
    ...
    const saatBar = [...Array(24).keys()].map(h => {
      ...
      // Sayıyı sadece değer > 0 ise ve çubuk yeterince genişse göster
      const showNum = top > 0;
      return `<div style="display:flex;align-items:center;gap:4px;margin-bottom:2px">
        <span style="min-width:22px;color:var(--text-3);text-align:right;font-size:.52rem">${h<10?'0'+h:h}</span>
        <div style="flex:1;height:9px;border-radius:3px;background:#f1f5f9;display:flex;overflow:hidden">
          <div style="width:${sW}%;background:#16a34a"></div>
          <div style="width:${kW}%;background:#dc2626"></div>
        </div>
        <span style="min-width:14px;color:var(--text-2);font-size:.52rem;text-align:left">${showNum?top:''}</span>
      </div>`;
    }).join('');
    ...
    // ── FUNNEL AŞAMA TOTALLERİ (Blur→Sepet→L3→Teklif→Satış) ────────
    const funnelBlur   = logs.filter(l => (l.benzersizBlurSayisi || 0) > 0 || (l.bakilanFiyatlar||[]).length > 0).length;
    const funnelSepet  = logs.filter(l => (l.derinlik || 0) > 0 || (l.urunler||[]).length > 0).length;
    const funnelL3     = l3Toplam;
    const funnelTeklif = proposals.length;
    const funnelSatis  = totS;
    ...
    function _dinamikOneri(u) {
      ...
      if (yuksekBlur && dusukSepet) {
        // Yüksek Blur + Düşük Sepet → Fiyat/Teşhir sorunu
        return { oneri: 'Fiyat/Teşhir Revizyonu — Çok bakılıyor ama sepete eklenmiyor.', icon: '💸', renk: '#fef2f2', kenar: '#fecaca' };
      } else if (yuksekSepet && sifirSatis) {
        // Yüksek Sepet + Sıfır Satış → Taksit/vade sorunu
        return { oneri: 'Taksit/Vade Farkı Kontrolü — Sepete alınıyor ama satışa dönmüyor.', icon: '📋', renk: '#fff7ed', kenar: '#fed7aa' };
      } else if (dusukBlur && yuksekSatis) {
        // Düşük Blur + Yüksek Satış → Fiyat avantajlı, öne çıkar
        return { oneri: 'Fiyat Avantajlı — Az bakılan ama çok satan ürün. Öne çıkarın.', icon: '📢', renk: '#eff6ff', kenar: '#bfdbfe' };
      }
      return null;
    }

    // Ürün bazında tavsiye üret
    const _tavsiyeListesi = [];
    direktenDonenler.concat(sessizDegerler).concat(olduStok).forEach(u => {
      // Önce dinamik if-else mantığını dene
      const dinamik = _dinamikOneri(u);
      if (dinamik) {
        _tavsiyeListesi.push({ urun: u.ad, kural: { ...dinamik, durum: dinamik.oneri }, blur: u.blur, satis: u.satis, sepet: u.sepet || 0, stok: u.stok || 0 });
        return;
      }
      // Sonra özelleştirilebilir kurallara bak
      _tavsiyeKurallari.filter(k => k.aktif).forEach(kural => {
        try {
          if (kural.test(u)) {
            _tavsiyeListesi.push({ urun: u.ad, kural: { ...kural, renk: '#f8fafc', kenar: '#e2e8f0' }, blur: u.blur, satis: u.satis, sepet: u.sepet || 0, stok: u.stok || 0 });
          }
        } catch(e) {}
      });
    });
    // En fazla 15 öneri, blur'a göre sırala
    _tavsiyeListesi.sort((a,b) => b.blur - a.blur);
    // Tekrar önlemek için ürün başına 1 öneri
    const _tavsiyeUrunSet = new Set();
    const _tavsiyeListesiFinal = _tavsiyeListesi.filter(t => {
      if (_tavsiyeUrunSet.has(t.urun)) return false;
      _tavsiyeUrunSet.add(t.urun);
      return true;
    });
        ...
        <div style="display:flex;flex-direction:column;gap:6px;max-height:260px;overflow-y:auto;padding-right:2px">
          ...
          <div style="display:flex;align-items:flex-start;gap:8px;background:${t.kural.renk||'#f8fafc'};border-radius:8px;padding:8px 10px;border:1px solid ${t.kural.kenar||'#e2e8f0'}">
            ...
            <div style="flex:1;min-width:0">
              <div style="font-size:.68rem;font-weight:700;color:#1e293b;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${t.urun}</div>
              <div style="font-size:.62rem;color:#475569;margin-top:2px">${t.kural.oneri}</div>
              <div style="font-size:.56rem;color:#94a3b8;margin-top:2px">${t.blur} blur · ${t.sepet} sepet · ${t.satis} satış · Stok: ${t.stok}</div>
            </div>
          </div>`).join('')}
...
window.adminHizliDuzenle = async function(urunAdi) {
  ...
  // Fix 5: EventBus reaktif tetikleme
