...
async function _ensureSummariesExist(sonKacGun = 365) {
  if (!currentUser || !_db) return;
  const isler = [];
  const bugunStr = _bugunIstanbul();
  ...
  Promise.allSettled(isler).then(results => {
...
function switchAdminTab(tab) {
  ...
  if(tab==='analiz')   {
    ...
    _ensureSummariesExist(365).catch(() => {});
    loadFunnelAnaliz();
    renderAdminProducts();
