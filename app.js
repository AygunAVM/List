...
async function showApp() {
  ...
  if (searchEl) {
    ...
    searchEl.placeholder = ad ? 'Liste Geçersizdir!, ' + ad + ' — Ürün arama' : 'Ürün arama';
  }
...
function renderCampaignPills(item, idx) {
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    // Kaç eşik seçildi (bu grup için sepette kaç ürün seçili)
    ...
    // Eşik rozeti — seçilmemişse (x3) göster, seçili+pending ise kalan sayı
    let esikRozet = '';
    if (!isBilgi && esikGerekli >= 2) {
      if (sel && pending && kalanEsik > 0) {
        esikRozet = `<span style="font-size:.52rem;opacity:.9;margin-left:2px;background:#d97706;color:#fff;padding:1px 4px;border-radius:3px;font-weight:700">+${kalanEsik}</span>`;
      } else if (!sel) {
        esikRozet = `<span style="font-size:.52rem;opacity:.75;margin-left:2px;background:rgba(0,0,0,.13);color:inherit;padding:1px 4px;border-radius:3px;font-weight:700">x${esikGerekli}</span>`;
      }
    }
    ...
    // Title: pending iken kalan eşik sayısını belirt
    const titleStr = (sel && pending && kalanEsik > 0)
      ? `⏳ ${kalanEsik} eşik daha eklenmeli — ${camp.metin.replace(/"/g,'&quot;')}`
      : camp.metin.replace(/"/g,'&quot;');

    return `<button type="button"${clickAttr} title="${titleStr}" `
      + `style="${baseStyle}${style}">`
      + `<span style="font-size:.68rem;line-height:1">${icon}</span>`
      + `<span style="margin-left:1px">${sel && pending && kalanEsik > 0 ? '⏳ ' + kalanEsik + ' eşik bekleniyor' : label}</span>`
      + esikRozet
      + `</button>`
...
function _renderMergedItem(m, idx, isRequired=true) {
  ...
  // Başlık ikonu ve Badge'lar — önce tanımla, sonra kullan
