...
// Vitrin ürün arama — fiyat listesinden
function vitrinUrunAra(query) {
  const dropdown = document.getElementById('vitrin-urun-dropdown');
  const fiyatEl  = document.getElementById('vitrin-fiyat-input');
  const notEl    = document.getElementById('vitrin-not-input');
  if (!dropdown) return;
  ...
  const urunKey  = keys0.find(k => k.toLowerCase() === 'urun')  || keys0.find(k => k.toLowerCase().includes('urun')) || '';
  const nakitKey = keys0.find(k => k.toLowerCase() === 'nakit') || '';
  const notKey   = keys0.find(k => k.toLowerCase() === 'not' || k.toLowerCase() === 'sipariş notu' || k.toLowerCase() === 'aciklama') || '';

  const sonuclar = allProducts.filter(u => {
    const ad = (u[urunKey] || '').toLowerCase();
    return ad.includes(q);
  }).slice(0, 12);

  if (!sonuclar.length) { dropdown.style.display = 'none'; return; }

  dropdown.style.display = 'block';
  dropdown.innerHTML = sonuclar.map(u => {
    const ad    = u[urunKey] || '';
    const nakit = nakitKey ? parseFloat(u[nakitKey]) || 0 : 0;
    const not   = notKey ? (u[notKey] || '') : '';
    return `<div style="padding:8px 10px;cursor:pointer;border-bottom:1px solid var(--border);
        display:flex;justify-content:space-between;align-items:center"
      onmousedown="event.preventDefault();vitrinUrunSec(${JSON.stringify(ad)},${nakit},${JSON.stringify(not)})"
      onmouseover="this.style.background='var(--surface-2)'"
      onmouseout="this.style.background=''">
      <span style="font-size:.76rem;color:var(--text-1)">${ad}</span>
      ${nakit > 0 ? `<span style="font-size:.70rem;color:var(--text-3);font-weight:700;flex-shrink:0;margin-left:8px">${fmt(nakit)}₺</span>` : ''}
    </div>`;
  }).join('');
}

function vitrinUrunSec(urunAdi, nakit, not) {
  const inputEl  = document.getElementById('vitrin-urun-input');
  const fiyatEl  = document.getElementById('vitrin-fiyat-input');
  const notEl    = document.getElementById('vitrin-not-input');
  const dropdown = document.getElementById('vitrin-urun-dropdown');

  if (inputEl)  inputEl.value  = urunAdi;
  if (fiyatEl && nakit > 0) fiyatEl.value = nakit;
  if (notEl)    notEl.value    = not || '';
...
async function saveVitrinUrun() {
  ...
  const not      = document.getElementById('vitrin-not-input')?.value?.trim() || '';
  ...
  try {
    ...
    if (document.getElementById('vitrin-not-input')) document.getElementById('vitrin-not-input').value = '';
...
function renderSiparisBildirimBar() {
  // Admin kullanıcılar için gösterme — admin panelinde zaten görür
  const bar = document.getElementById('user-siparis-bar');
  if (!bar) return;
  ...
  // Admin ise bandı gizle (admin panelinden yönetir)
  if (isAdmin()) {
    bar.style.display = 'none';
    return;
  }

  // Non-admin kullanıcı: kendi siparişlerini göster
  const benim = bekleyen.filter(s => !s.ekleyen || s.ekleyen === (currentUser?.Email || ''));
  const goster = benim.length ? benim : bekleyen; // kendi yoksa tümünü göster

  bar.style.display = 'block';
  const son3 = goster.slice(0, 3);
  const metin = son3.map(s => `<b>${s.urun || s.urunAdi || '?'}</b>`).join(', ');
  const fazla = goster.length > 3 ? ` +${goster.length - 3} daha` : '';
  ...
  bar.innerHTML = `
    <div style="display:flex;align-items:center;gap:8px;
      ...
      <span style="flex:1;line-height:1.4">
        <b style="color:#f87171">${goster.length} siparişiniz bekliyor:</b> ${metin}${fazla}
      </span>
    </div>`;
}
...
function _trendSkoru() {
  // Son 14 günlük analytics verisini filtrele — sadece blur/açılma sayısı
  ...
  // Sadece blur/açılma sayısına göre sırala (prim etkisi yok)
  const sonuclar = allProducts.map(u => {
    ...
    return { u, urunAdi, addCnt, prim: 0 };
  }).filter(x => x.addCnt > 0);

  return sonuclar.sort((a, b) => b.addCnt - a.addCnt);
}
...
function _urunFmt(u) {
  ...
  // Sadece sayı döner, ₺ kart tarafında eklenir (çift ₺ olmaması için)
  return nakit > 0 ? fmt(nakit) : '—';
}

// Ürün adından marka adını çıkar (trend kartlarda marka yazmasın)
function _urunAdiTemiz(item) {
  const keys = Object.keys(item.u);
  const markaKey = keys.find(k => k.toLowerCase() === 'marka') || '';
  const marka = markaKey ? (item.u[markaKey] || '').trim() : '';
  let ad = item.urunAdi || '';
...
function renderTrendCards() {
  ...
  const trendHtml = trendData.map((item, i) => {
    ...
    const gorAd = _urunAdiTemiz(item);
    const fiyatStr = _urunFmt(item.u);
    ...
    return `<div class="trend-card"
      ...
      <div class="trend-card-meta">
        ${fiyatStr !== '—' ? `<span class="trend-card-price">${fiyatStr}₺</span>` : ''}
