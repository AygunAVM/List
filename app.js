...
// ═══════════════════════════════════════════════════════════════
// ✨ EN İYİ FİYAT OPTİMİZASYON MOTORU  v3
// KURAL: Her üründe en fazla 1 tane 🔒 (hangi gruptan olursa olsun)
//        Her üründe 🔒 varsa ⎇ olamaz, tersi de geçerli
//        Farklı ürünler kendi kampanyalarını bağımsız seçer
// ═══════════════════════════════════════════════════════════════
...
function optimizeCampaigns() {
  ...
  // 1. Sıfırla (proje/bagimsiz/bilgi korunur)
  basket.forEach(item => {
    item._selectedCamps = {};
    item._pendingGroups = {};
  ...
  // 2. Kampanyaları parse et ve kandidatları topla
  basket.forEach((item, bi) => {
    if (!item._campaigns) item._campaigns = parseCampaigns(item.aciklama || '');
  });

  // 3. Grup bazında organize et — sadece ⎇ ve 🔒
  ...
  // 4. Ürün başına atama durumu
  // Her ürün için: kilitliAtandi (bool) — 1 kez 🔒 atandıysa true
  //               birlesenAtandi (bool) — ⎇ atandıysa true
  const urunDurum = {}; // bi → { kilitliAtandi, birlesenAtandi }

  function atanabilir(k) {
    const d = urunDurum[k.bi];
    if (!d) return true;
    if (k.camp.tip === 'kilitli') {
      if (d.kilitliAtandi) return false;  // zaten 1 🔒 var
      if (d.birlesenAtandi) return false; // ⎇ var, 🔒 olamaz
    }
    if (k.camp.tip === 'birlesen') {
      if (k.camp.esik <= 1) {
        // PAP: birden fazla ⎇ olabilir (farklı gruplar)
        if (d.kilitliAtandi) return false; // 🔒 var, ⎇ olamaz
      } else {
        if (d.kilitliAtandi) return false;
        // Aynı gruptan 2. ⎇ — KURAL 2 kontrol edilmeyecek burada (recalc halleder)
      }
    }
    return true;
  }
  ...
  function ata(k) {
    if (!urunDurum[k.bi]) urunDurum[k.bi] = { kilitliAtandi: false, birlesenAtandi: false };
    if (k.camp.tip === 'kilitli') urunDurum[k.bi].kilitliAtandi = true;
    if (k.camp.tip === 'birlesen' && k.camp.esik > 1) urunDurum[k.bi].birlesenAtandi = true;
  }

  const sonSecimler = [];

  // 5. Grupları tutara göre büyükten küçüğe sırala
  const siraliGruplar = Object.entries(gruplar).sort((a, b) => {
    const tA = Math.max(...a[1].map(k => k.camp.tutar));
    const tB = Math.max(...b[1].map(k => k.camp.tutar));
    return tB - tA;
  });

  siraliGruplar.forEach(([grup, liste]) => {
    const esik = liste[0].camp.esik || 1;

    // PAP/tek ürün grupları
    if (esik === 1) {
      liste.forEach(k => {
        if (atanabilir(k)) {
          ata(k);
    ...
    // Eşikli gruplar — greedy çift döngüsü
    const hepsiANY = liste.every(k => !k.camp.rol || k.camp.rol === 'ANY');

    if (hepsiANY) {
      // ANY: sırayla esik'li ürün grupla
      const uygunlar = liste.filter(k => atanabilir(k));
      // Aynı üründen en yüksek tutarlı tek aday
      const urunEn = {};
      uygunlar.forEach(k => {
        if (!urunEn[k.bi] || k.camp.tutar > urunEn[k.bi].camp.tutar) urunEn[k.bi] = k;
      });
      const tekli = Object.values(urunEn);
      for (let i = 0; i + esik <= tekli.length; i += esik) {
        tekli.slice(i, i + esik).forEach(k => { ata(k); sonSecimler.push(k); });
      }
    } else {
      // Rol bazlı: greedy farklı harfli çiftler
      let kalan = liste.filter(k => atanabilir(k));
      while (kalan.length >= esik) {
        ...
        if (cift.length === esik) {
          cift.forEach(k => { ata(k); sonSecimler.push(k); });
          kalan = sonraki.filter(k => atanabilir(k));
        } else break;
      }
    }
  });
...
// ═══════════════════════════════════════════════════════════════
// 💡 AKILLI ÇAPRAZ SATIŞ (UPSELL) MOTORU  v3
// updateCartUI içinden çağrılır, cart-modal içinde sabit container
// ═══════════════════════════════════════════════════════════════

function checkUpsellOpportunities() {
  const CONTAINER_ID = 'upsell-bar-container';

  // Container'ı cart-modal-content içinde cart-footer'ın önünde tut
  ...
  Object.entries(grupBilgi).forEach(([grup, info]) => {
    ...
    } else if (sahipSayi > 0 && sahipSayi < info.esik && (info.esik - sahipSayi) <= 2) {
      firsatlar.push({ grup, kazanc: info.tutar, eksik: info.esik - sahipSayi, tip: 'ekle' });
    }
  });
  ...
  if (f.tip === 'hazir') {
    ...
    buton = `<button onclick="optimizeCampaigns()" style="background:#f59e0b;color:#0f172a;
      border:none;border-radius:6px;padding:4px 10px;font-weight:800;font-size:.64rem;
      cursor:pointer;white-space:nowrap;font-family:inherit;flex-shrink:0">✨ +${kFmt}</button>`;
  } else if (f.tip === 'eksik1') {
    mesaj = `<b>${f.grup}</b> kampanyasına <b>1 ürün</b> ekle → <b>+${kFmt}</b>`;
  } else {
    mesaj = `<b>${f.grup}</b> için <b>${f.eksik} ürün</b> daha ekle → <b>+${kFmt}</b>`;
  }

  container.innerHTML = `<div style="display:flex;align-items:center;gap:8px;
    background:#0f172a;color:#f8fafc;padding:7px 12px;font-size:.67rem;
    border-top:2px solid #f59e0b;flex-wrap:wrap;">
