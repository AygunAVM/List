...
// Sessions listener — admin için tüm oturumları izle + personel için kendi oturumunu izle
window._fbSessions = {};
function _startSessionListener() {
  if(!_db) return;

  // Personel: kendi session'ını dinle — forceLogout kontrolü
  const _mySessionId = localStorage.getItem('_aygun_session_id');
  if(_mySessionId && !isAdmin()) {
    if(window._mySessionUnsub) window._mySessionUnsub();
    window._mySessionUnsub = onSnapshot(
      doc(_db, 'sessions', _mySessionId),
      (docSnap) => {
        if(!docSnap.exists()) {
          // Session silindi — otomatik çıkış
          if(currentUser) {
            ayAlert('⚠️ Oturumunuz sonlandırıldı. Lütfen tekrar giriş yapın.')
              .then(() => logoutUser());
          }
          return;
        }
        const sessData = docSnap.data();
        if(sessData.forceLogout === true) {
          // Admin tarafından zorla çıkarıldı
          if(currentUser) {
            ayAlert('⚠️ Yönetici tarafından oturumunuz sonlandırıldı. Lütfen tekrar giriş yapın.')
              .then(() => logoutUser());
          }
        }
      },
...
// Pazarlık Raporu (Sadece Ek İndirim) — admin özet panelinde
function _renderIndirimOzet() {
  const el = document.getElementById('admin-indirim-ozet');
  if (!el) return;

  // Sadece personelin ek pazarlık indirimi (ekIndirim alanı)
  ...
  el.innerHTML = '<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:10px">'
    ...
    + '</div>'
    + '</div>'
    + '<div style="font-size:.65rem;color:var(--text-3);margin-bottom:8px;text-align:center">'
    + '💰 Sadece personelin ek pazarlık indirimi (kampanya/satır indirimleri hariç)'
    + '</div>'
    + (sorted.length
      ? sorted.map(([email, s]) =>
...
async function adminForceLogout(targetEmail) {
  ...
  try {
    ...
    // 1. forceLogout flag'ini true yap — personelin cihazına anlık bildirim gider
    for(const docSnap of sesSnap.docs) {
      await updateDoc(doc(_db, 'sessions', docSnap.id), {
        forceLogout: true,
        forceLogoutAt: serverTimestamp()
      });
      // 2. 5 saniye sonra session belgesini sil (temizlik)
      const _docId = docSnap.id;
      setTimeout(async () => {
        try { await deleteDoc(doc(_db, 'sessions', _docId)); } catch(e) {}
      }, 5000);
    }
    haptic(22);
    const _ct = document.getElementById('change-toast');
    if(_ct) {
      _ct.textContent = '✅ ' + targetEmail.split('@')[0] + ' oturumu kapatılıyor…';
      _ct.classList.add('show');
      setTimeout(() => _ct.classList.remove('show'), 2800);
    }
    renderAdminUsers();
