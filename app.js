...
function renderProposals(container, forceAll) {
  ...
  target.innerHTML = myProps.map(p => {
    ...
    const canNote = isAdmin() || p.user === (currentUser?.Email||'');
    const noteBtn = canNote ? `<button class="btn-add-note haptic-btn" onclick="openPropNote('${p.id}')" title="Not ekle">📝 Not</button>` : '';
    // Süre kontrolü
    const now = new Date();
    const expDate = p.sureBitis ? new Date(p.sureBitis) : null;
    const isSureDoldu = expDate && now > expDate && p.durum === 'bekliyor';
    if(isSureDoldu && p.durum === 'bekliyor') { p.durum = 'sureDoldu'; }

    const editBtn = isAdmin() ? `<button class="btn-edit-prop haptic-btn" onclick="openEditProp('${p.id}')" title="Teklifi düzenle">✏️ Düzenle</button><button class="btn-del-prop haptic-btn" onclick="deleteProp('${p.id}')" title="Sil">🗑</button>` : '';
    const satisDondurBtn = (canAct && (p.durum==='bekliyor'||p.durum==='sureDoldu')) ? `<button class="btn-satis-dondu haptic-btn" onclick="updatePropStatus('${p.id}','satisDondu')">✅ Satışa Döndü</button>` : '';
    const actBtns = canAct && (p.durum==='bekliyor'||p.durum==='sureDoldu')
      ? `<div class="proposal-actions">
          ${satisDondurBtn}
          <button class="btn-cancel"  onclick="updatePropStatus('${p.id}','iptal')">✕ İptal</button>
          ${waBtn}${noteBtn}${editBtn}
         </div>`
      : canAct
        ? `<div class="proposal-actions"><span class="prop-closed-label">${statusMap[p.durum]||p.durum}</span>${waBtn}${noteBtn}${editBtn}</div>`
        : '';
...
function renderStokUyari() {
  ...
  if(!el) return;
  // Ürün listesini localStorage cache'den oku
  let urunler = [];
  try { urunler = JSON.parse(localStorage.getItem('aygun_urunler_cache')||'[]'); } catch(e){}
  if(!urunler.length) {
    // Dinamik fetch
    fetch(dataUrl('urunler.json')).then(r=>r.json()).then(rows=>{
      localStorage.setItem('aygun_urunler_cache', JSON.stringify(rows));
      _doStokUyari(el, rows);
    }).catch(()=>{ el.innerHTML='<div class="admin-empty">Ürün verisi yüklenemedi</div>'; });
    return;
  }
  _doStokUyari(el, urunler);
}

function _doStokUyari(el, rows) {
  const stokSifir   = rows.filter(r => Number(r.Stok||r.stok)===0);
  const stokKritik  = rows.filter(r => { const s=Number(r.Stok||r.stok); return s>=1&&s<=3; });
  if(!stokSifir.length && !stokKritik.length) {
    el.innerHTML='<div class="stok-ok">✅ Tüm ürünlerde stok normal</div>'; return;
  }
  const html = [
    ...stokSifir.map(r=>`<div class="stok-alert stok-0"><span class="stok-dot red"></span><span class="stok-urun">${r[Object.keys(r).find(k=>k.toLowerCase().includes('urun')||k==='Ürün')||'Ürün']||r.urun||'?'}</span><span class="stok-badge s0">Stok Yok</span></div>`),
    ...stokKritik.map(r=>`<div class="stok-alert stok-kritik"><span class="stok-dot orange"></span><span class="stok-urun">${r[Object.keys(r).find(k=>k.toLowerCase().includes('urun')||k==='Ürün')||'Ürün']||r.urun||'?'}</span><span class="stok-badge sk">${Number(r.Stok||r.stok)} adet</span></div>`)
  ].join('');
  el.innerHTML = html;
}
...
function renderSepetDurum() {
  ...
  const bekleyenProps = proposals.filter(p=>p.durum==='bekliyor');
  const byUser = {};
  ...
  if(!entries.length){ el.innerHTML='<div class="admin-empty">Bekleyen teklif yok</div>'; return; }
...
function renderUyuyanStok(urunler) {
  ...
  if(!el) return;
  // Analytics'ten sepete eklenen ürünler
  const data=JSON.parse(localStorage.getItem('analytics_local')||'{}');
  ...
  // proposals'tan da eklenen ürünleri topla
  proposals.forEach(p=>(p.urunler||[]).forEach(u=>{ if(u.urun) eklenen.add(u.urun); }));

  const uyuyan = urunler.filter(r=>{
    const stok = Number(r.Stok||r.stok||0);
    const ad = r[Object.keys(r).find(k=>k.toLowerCase().includes('urun'))||'Ürün']||r.urun||'';
