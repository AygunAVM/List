...
// ── Kampanya Segment Diff ───────────────────────────────────
// İki açıklama metni arasındaki kampanya segmentlerini karşılaştırır
// "KEA -3k(2/A)⎇" gibi token'ları set olarak ele alır
function _parseCampSegments(oldAc, newAc) {
  const re = /([A-ZÇŞĞÜÖİa-zçşğüöı0-9_+]+\s+[-–]?\s*[\d.,]+[kK]?\s*(?:\(\d+\/[A-Z]\))?(?:\s*\[[\d.,\s\w.]+\])?)/gi;

  function parseSet(ac) {
    const map = new Map();
    let m;
    re.lastIndex = 0;
    while ((m = re.exec(ac)) !== null) {
      const raw = m[1].trim();
      // Anahtar: grup adı + (esik/rol) — tarih ve tutar farkı görmezden gel
      const keyM = raw.match(/^([A-ZÇŞĞÜÖİa-z0-9_+]+)/i);
      if (!keyM) continue;
      const grupM = raw.match(/([A-ZÇŞĞÜÖİa-z0-9_+]+)\s+[-–]?\s*([\d.,]+[kK]?)\s*(\(\d+\/[A-Z]\))?/i);
      if (!grupM) continue;
      const key = grupM[1].toUpperCase() + (grupM[3] || '');
      map.set(key, raw.replace(/\s+/g, ' '));
    }
    return map;
  }

  const oldMap = parseSet(oldAc);
  const newMap = parseSet(newAc);
  const eklendi    = [];
  const kaldirildi = [];
  const degisti    = [];

  // Eklenenler: yeni'de var, eski'de yok
  newMap.forEach((seg, key) => {
    if (!oldMap.has(key)) {
      eklendi.push(seg);
    } else if (oldMap.get(key) !== seg) {
      degisti.push({ old: oldMap.get(key), new: seg });
    }
  });

  // Kaldırılanlar: eski'de var, yeni'de yok
  oldMap.forEach((seg, key) => {
    if (!newMap.has(key)) kaldirildi.push(seg);
  });
...
function _diffJson(oldJson, newJson) {
  ...
  newJson.data.forEach(p => {
    ...
    if(!isNaN(nv)&&!isNaN(ov)&&nv!==ov) {
      ...
      changes.push({ type:'price', urun:urunAdi, field:'Nakit', old:ov, new:nv, diff, pct, marka:p['Marka']||'' });
    }
    // Stok
    const ns=Number(p.Stok), os=Number(old.Stok);
    if(!isNaN(ns)&&!isNaN(os)&&ns!==os)
      changes.push({ type:'stok', urun:urunAdi, old:os, new:ns, diff:ns-os, marka:p['Marka']||'' });
    // Açıklama — kampanya segmentlerini parse ederek diff al
    if(descKey && p[descKey] !== old[descKey]) {
      const campSegs = _parseCampSegments(old[descKey]||'', p[descKey]||'');
      if(campSegs.eklendi.length || campSegs.kaldirildi.length || campSegs.degisti.length) {
        // Kampanya değişimi — her segment ayrı satır
        campSegs.kaldirildi.forEach(seg => {
          changes.push({ type:'kamp_kaldirildi', urun:urunAdi, seg, marka:p['Marka']||'' });
        });
        campSegs.eklendi.forEach(seg => {
          changes.push({ type:'kamp_eklendi', urun:urunAdi, seg, marka:p['Marka']||'' });
        });
        campSegs.degisti.forEach(d => {
          changes.push({ type:'kamp_degisti', urun:urunAdi, seg:d.new, segOld:d.old, marka:p['Marka']||'' });
        });
      } else {
        // Kampanya dışı açıklama değişimi (PAP, tarih vs.)
        changes.push({ type:'aciklama', urun:urunAdi, old:old[descKey]||'', new:p[descKey]||'', marka:p['Marka']||'' });
      }
    }
    // Ürün adı
...
function _isMandatory(type) {
  return ['price','aciklama','urunadi','kamp_eklendi','kamp_kaldirildi','kamp_degisti'].includes(type);
}
...
function _renderMergedItem(m, idx, isRequired=true) {
  ...
  const hasKamp = !!(m.kamp_eklendi?.length || m.kamp_kaldirildi?.length || m.kamp_degisti?.length);
  const hasStok     = !!m.stok;

  // Başlık ikonu
  let emoji = hasPrice ? (m.price.diff > 0 ? '📈' : '📉') : hasKamp ? '📋' : hasAciklama ? '📝' : '📦';
  ...
  const _ke = (m.kamp_eklendi   ||[]).length;
  const _kk = (m.kamp_kaldirildi||[]).length;
  const _kd = (m.kamp_degisti   ||[]).length;
  if(_ke)  badges += `<span class="change-badge badge-kamp-eklendi">+${_ke} Kampanya</span>`;
  if(_kk)  badges += `<span class="change-badge badge-kamp-kaldirildi">-${_kk} Kampanya</span>`;
  if(_kd)  badges += `<span class="change-badge badge-kamp-degisti">±${_kd} Kampanya</span>`;
  if(hasStok) {
  ...
  if(hasAciklama) {
    details += `<span class="ci-row ci-aciklama-block">📝 Açıklama güncellendi</span>`;
  }
  // Kampanya segmentleri
  const kampEk  = m.kamp_eklendi   || [];
  const kampKal = m.kamp_kaldirildi || [];
  const kampDeg = m.kamp_degisti    || [];
  if(kampKal.length || kampEk.length || kampDeg.length) {
    kampKal.forEach(c => { details += `<span class="ci-row"><span class="ci-kamp-kaldirildi">➖ ${c.seg}</span></span>`; });
    kampEk.forEach(c  => { details += `<span class="ci-row"><span class="ci-kamp-eklendi">➕ ${c.seg}</span></span>`; });
    kampDeg.forEach(c => { details += `<span class="ci-row">🔄 <s style="opacity:.5">${c.segOld}</s> → <strong>${c.seg}</strong></span>`; });
  }
...
function _renderChangeItem(c, idx, infoMode=false) {
  ...
  } else if(c.type === 'kamp_eklendi') {
    inner = `<span class="ci-emoji">➕</span>
      <div class="ci-body">
        <span class="ci-urun">${c.urun}</span>
        <span class="ci-detail ci-kamp-eklendi">${c.seg}</span>
      </div>
      <span class="change-badge badge-kamp-eklendi">Kampanya +</span>`;
  } else if(c.type === 'kamp_kaldirildi') {
    inner = `<span class="ci-emoji">➖</span>
      <div class="ci-body">
        <span class="ci-urun">${c.urun}</span>
        <span class="ci-detail ci-kamp-kaldirildi">${c.seg}</span>
      </div>
      <span class="change-badge badge-kamp-kaldirildi">Kampanya −</span>`;
  } else if(c.type === 'kamp_degisti') {
    inner = `<span class="ci-emoji">🔄</span>
      <div class="ci-body">
        <span class="ci-urun">${c.urun}</span>
        <span class="ci-detail"><s style="opacity:.5">${c.segOld}</s> → <strong>${c.seg}</strong></span>
      </div>
      <span class="change-badge badge-kamp-degisti">Kampanya ±</span>`;
  } else if(c.type === 'aciklama') {
    inner = `<span class="ci-emoji">📝</span>
      <div class="ci-body">
        <span class="ci-urun">${c.urun}</span>
        <span class="ci-detail">Açıklama güncellendi</span>
      </div>
...
function showChangePopup(changes, logKey) {
  ...
  // ── Istisna filtresi ─────────────────────────────────────────
  // currentUser.Istisna = "Samsung,Philips" → o markaların değişimleri görünmez
  // Boşsa tüm değişimler gösterilir
  const istisnalar = (currentUser?.Istisna || currentUser?.istisna || '')
    .split(',')
    .map(m => m.trim().toLowerCase())
    .filter(Boolean);

  function _markaFiltre(c) {
    if (!istisnalar.length) return true;
    const marka = (c.marka || '').toLowerCase();
    if (!marka) return true; // markası bilinmeyen → göster
    return !istisnalar.some(ist => marka.includes(ist) || ist.includes(marka));
  }

  // Değişimleri grupla + marka filtrele
  const verItems  = changes.filter(c => c.type === 'versiyon' && !c.isOld);
  const optItems  = changes.filter(c => c.type !== 'versiyon' && !c.isOld && !_isMandatory(c.type) && _markaFiltre(c));
  const mandItems = changes.filter(c => c.type !== 'versiyon' && !c.isOld &&  _isMandatory(c.type) && _markaFiltre(c));
  ...
  if(mandItems.length) {
    ...
    mandItems.forEach(c => {
      ...
      if(!mergedMap.has(key)) mergedMap.set(key, {
        urun:key, price:null, aciklama:null, stok:null,
        kamp_eklendi:[], kamp_kaldirildi:[], kamp_degisti:[]
      });
      const m = mergedMap.get(key);
      if(c.type === 'price')          m.price = c;
      if(c.type === 'aciklama')       m.aciklama = c;
