...
function _qfRenderTaksit() {
  if (!allRates.length) return '<div class="qf-empty">Oran verisi yüklenemedi.</div>';
  const nakit = _qfNakit;
  const TAK_LIST = [2,3,6,9,12];

  // Kart listesi
  const kartlar = [...new Set(allRates.map(r => r.Kart).filter(Boolean))];

  // Aktif kart seçimi (state)
  if (!_qfTaksitKart) _qfTaksitKart = kartlar[0] || '';

  const kartBtns = kartlar.map(k =>
    '<button class="qf-mini-tab' + (k===_qfTaksitKart?' active':'') + '" onclick="_qfTaksitKartSec(\'' + k + '\');">' + k + '</button>'
  ).join('');

  const zRows = allRates.filter(r => r.Kart === _qfTaksitKart);
  let rows = '';
  TAK_LIST.forEach(n => {
    let bestOran = null, bestTahsilat = null, bestZincir = null;
    zRows.forEach(r => {
      const oran = parseFloat(r[n + 'Taksit']);
      if (isNaN(oran) || oran <= 0) return;
      if (bestOran === null || oran < bestOran) {
        bestOran = oran;
        bestTahsilat = yuvarlaKademe(nakit / (1 - oran / 100), n);
        bestZincir = r.Zincir;
      }
    });
    if (bestTahsilat === null) return;
    const aylik = Math.ceil(bestTahsilat / n);
    const selected = _qfPlan?.kart===_qfTaksitKart && _qfPlan?.taksit===n;
    rows += '<tr class="qf-taksit-row' + (selected?' selected':'') + '" onclick="_qfSelectPlan(this,\'' + _qfTaksitKart + '\',' + n + ',' + bestTahsilat + ',\'' + (bestZincir||_qfTaksitKart) + '\')">'
          + '<td>' + n + ' Taksit</td>'
          + '<td>' + fmt(aylik) + '/ay</td>'
          + '<td>' + fmt(bestTahsilat) + '</td>'
          + '<td style="font-size:.60rem;color:#94a3b8">' + (bestZincir||'—') + '</td>'
          + '</tr>';
  });

  return '<div class="qf-kart-scroll">' + kartBtns + '</div>'
       + '<table class="qf-taksit-table"><thead><tr><th>Taksit</th><th>Aylık</th><th>Toplam</th><th>POS</th></tr></thead><tbody>' + (rows||'<tr><td colspan="4" class="qf-empty">Bu kart için taksit seçeneği yok.</td></tr>') + '</tbody></table>';
}
...
function _qfRenderKredi() {
  const nakit = _qfNakit;
  // TOMBank ve Zip sabit kredi oranları (oranlar.json yoksa static fallback)
  const KREDI = [
  ...
  let html = '';
  KREDI.forEach(k => {
    html += '<div class="qf-kredi-blok"><div class="qf-kredi-baslik">' + k.ad + '</div><table class="qf-taksit-table"><thead><tr><th>Vade</th><th>Aylık Faiz</th><th>Aylık Taksit</th><th>Toplam</th></tr></thead><tbody>';
    k.vadeler.forEach(v => {
      const toplam   = nakit * Math.pow(1 + v.faiz/100, v.ay);
      const toplamR  = Math.round(toplam / 50) * 50;
      const aylik    = Math.ceil(toplamR / v.ay);
      const selected = _qfPlan?.kart===k.ad && _qfPlan?.taksit===v.ay;
      html += '<tr class="qf-taksit-row' + (selected?' selected':'') + '" onclick="_qfSelectPlan(this,\'' + k.ad + '\',' + v.ay + ',' + toplamR + ',\'' + k.ad + '\')">'
            + '<td>' + v.ay + ' Ay</td>'
            + '<td>%' + v.faiz + '</td>'
            + '<td>' + fmt(aylik) + '/ay</td>'
            + '<td>' + fmt(toplamR) + '</td>'
