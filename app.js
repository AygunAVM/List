...
function _startMotdTicker() {
  ...
  const ad = currentUser?.Ad || currentUser?.Email?.split('@')[0] || '';
  const staticPlaceholder = ad ? 'En İyisisin, ' + ad + ' — Ürün arama' : 'Ürün arama';
...
function toggleCampaign(idx, campIdx) {
  ...
  // Herhangi bir kilitli (🔒) kampanya aktif mi?
  const globalKMActive = basket.some((b) => {
    if (!b._campaigns || !b._selectedCamps) return false;
    return Object.entries(b._selectedCamps).some(([ci, sel]) =>
      sel && b._campaigns[parseInt(ci)]?.tip === 'kilitli'
    );
  });
  ...
  if (isSelected) {
    ...
    // Kilitli bir grup aktifken başka kilitli grup seçme
    if (globalKMActive && camp.tip === 'kilitli') return;
    // Bu pill kilitli ise → sepetteki diğer KILITLı seçimleri temizle (PAP/tekil kalır)
    if (camp.tip === 'kilitli') {
      basket.forEach(b => {
        if (!b._campaigns || !b._selectedCamps) return;
        Object.keys(b._selectedCamps).forEach(ci => {
          if (b._selectedCamps[ci] && b._campaigns[parseInt(ci)]?.tip === 'kilitli') {
            delete b._selectedCamps[ci];
          }
        });
      });
    }
...
function renderCampaignPills(item, idx) {
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    // Devre dışı: kilitli kampanya aktifken bu kilitli değilse
    const disabled = globalKMActive && camp.tip === 'kilitli' && !sel;
