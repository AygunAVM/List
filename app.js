...
function renderTable(searchVal) {
  ...
  data.forEach(u => {
    ...
    // Prim badge — para ikonu + tutar, tiklaninca sepete ekler
    const primLbl = hasPrim
      ? (primVal >= 1000 ? (primVal/1000).toFixed(primVal%1000===0?0:1)+'K' : String(primVal))
      : '';
    const primBadge = hasPrim
      ? '<button class="prim-btn haptic-btn" onclick="addToBasketPrim(' + oi + ')" title="Sepete ekle — Prim: ' + fmt(primVal) + '"><span class="prim-icon">₺</span><span class="prim-amt">' + primLbl + '</span></button>'
      : '';

    // Siparis notu butonu — urun adindaki ozel karakterleri guvende encodelan
    const safeAdi = JSON.stringify(u[urunKey]||'');

    const tr = document.createElement('tr');
    tr.innerHTML =
      '<td class="td-add-cell">' +
        '<button class="' + addBtnCls + ' haptic-btn" onclick="addToBasket(' + oi + ')">+</button>' +
        primBadge +
      '</td>'+
...
function _showPrimAnimation(primVal) {
  ...
  el.textContent = '+' + (primVal>=1000?(primVal/1000).toFixed(primVal%1000===0?0:1)+'K ₺':primVal+' ₺') + ' PRİM';
  el.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);' +
    'background:linear-gradient(135deg,#22c55e,#16a34a);color:#fff;font-weight:800;' +
    'font-size:1.4rem;padding:14px 28px;border-radius:16px;z-index:99999;' +
    'box-shadow:0 8px 32px rgba(34,197,94,.45);pointer-events:none;' +
    'animation:primFlyUp 1.1s ease forwards;';
