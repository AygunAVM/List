...
if (!enKarliMap[td.n] || oran < enKarliMap[td.n].oran) {
  ...
  const _indirimBadge = ekIndirim > 0 ? \` · <span style="color:#16a34a">-\${fmt(ekIndirim)}</span>\` : '';
  html += \`<div style="margin:4px 0 6px">
    <button onclick="(function(){var p=document.getElementById('ab-pazarlik-panel');var b=document.getElementById('ab-pazarlik-togbtn');var open=p.style.display!=='none';p.style.display=open?'none':'block';if(open){document.getElementById('ab-ek-indirim').value='';calcAbakus();}b.style.borderColor=open?'var(--border)':'#f59e0b';b.style.color=open?'var(--text-2)':'#b45309';})()"
      id="ab-pazarlik-togbtn"
      style="background:none;border:1px dashed \${_pazarlikPanelAcik ? '#f59e0b' : 'var(--border)'};border-radius:7px;
             padding:4px 10px;font-size:.68rem;font-weight:700;cursor:pointer;
             color:\${_pazarlikPanelAcik ? '#b45309' : 'var(--text-2)'};font-family:inherit;transition:all .2s">
      💬 Pazarlık İndirimi\${_indirimBadge}
    </button>
  </div>
  <div id="ab-pazarlik-panel" style="display:\${_pazarlikPanelAcik ? 'block' : 'none'};background:#fffbeb;border:1px solid #fde68a;border-radius:9px;padding:10px 12px;margin-bottom:8px">
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:6px">
      ...
      <span id="ab-ek-indirim-uyari" style="font-size:.62rem;color:#dc2626;display:none;font-weight:700">
        ⚠️ %7 sınırını aşıyor!
      </span>
    </div>
