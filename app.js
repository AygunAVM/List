...
// İki açıklama metninden kampanya token setlerini çıkarır
// "KEA -3k(2/A)⎇" → key="KEA(2/A)", value=token string
function _parseCampSegments(oldAc, newAc) {
  ...
  function toMap(ac) {
    ...
    while ((hit = re.exec(ac)) !== null) {
      const grupRaw = hit[1].toUpperCase();
      const rol     = hit[3] ? hit[3].toUpperCase() : '';
      const key     = grupRaw + rol;  // "KEA(2/A)"
      const token   = hit[0].trim().replace(/\s+/g,' ');
      m.set(key, token);
    }
    return m;
  }

  const oldMap = toMap(oldAc);
  const newMap = toMap(newAc);
  const eklendi    = [];
  const kaldirildi = [];
  const degisti    = [];
  ...
  oldMap.forEach((tok, key) => {
    if (!newMap.has(key)) kaldirildi.push(tok);
  });
...
function _diffJson(oldJson, newJson) {
  ...
  newJson.data.forEach(p => {
    ...
    // Açıklama → kampanya segment diff
    if(descKey && p[descKey] !== old[descKey]) {
      const segs = _parseCampSegments(old[descKey]||'', p[descKey]||'');
      segs.kaldirildi.forEach(seg =>
        changes.push({ type:'kamp_kaldirildi', urun:urunAdi, seg, marka })
      );
      segs.eklendi.forEach(seg =>
        changes.push({ type:'kamp_eklendi', urun:urunAdi, seg, marka })
      );
      segs.degisti.forEach(d =>
        changes.push({ type:'kamp_degisti', urun:urunAdi, segOld:d.old, seg:d.new, marka })
      );
      // Kampanya dışı açıklama değişimleri gösterilmez (gürültü)
    }
    // Ürün adı
...
function _isMandatory(type) {
  // Zorunlu onay: fiyat değişimi + kampanya değişimleri
  // Stok → bilgi amaçlı bölümde kalır (zorunlu değil)
  // Açıklama tipi artık üretilmiyor (kampanya diff'e çevrildi)
  return type === 'price'
      || type === 'kamp_eklendi'
      || type === 'kamp_kaldirildi'
      || type === 'kamp_degisti';
}
...
function _renderMergedItem(m, idx, isRequired=true) {
  ...
  // Başlık ikonu
  // emoji hasKamp içinde set edildi

  // Badge'lar
  // badges hasKamp içinde set edildi
  ...
  // Emoji
  let emoji = hasPrice ? (m.price.diff > 0 ? '📈' : '📉') : hasKamp ? '📋' : '📦';
  ...
  if(kampEk.length)  badges += `<span class="change-badge badge-kamp-eklendi">+${kampEk.length} Kampanya</span>`;
  if(kampKal.length) badges += `<span class="change-badge badge-kamp-kaldirildi">-${kampKal.length} Kampanya</span>`;
  if(kampDeg.length) badges += `<span class="change-badge badge-kamp-degisti">±${kampDeg.length} Kampanya</span>`;
  if(hasStok) {
  ...
  kampKal.forEach(c => {
    details += `<span class="ci-row ci-kamp-kaldirildi">➖ ${c.seg}</span>`;
  });
  kampEk.forEach(c => {
    details += `<span class="ci-row ci-kamp-eklendi">➕ ${c.seg}</span>`;
  });
  kampDeg.forEach(c => {
    details += `<span class="ci-row ci-kamp-degisti">🔄 <s style="opacity:.5">${c.segOld}</s> → <strong>${c.seg}</strong></span>`;
  });
...
function showChangePopup(changes, logKey) {
  ...
  // Admin → filtre yok
  // Satış kullanıcısı → Istisna alanındaki markalar hariç tutulur
  // Stok değişimleri → Istisna'dan bağımsız, tüm kullanıcılara çıkar
  ...
  function _markaGizli(c) {
    ...
    const marka = (c.marka || '').trim().toLowerCase();
    if (!marka) return false;
    return istisnalar.some(ist => marka === ist || marka.startsWith(ist));
  }

  // Değişimleri grupla + marka filtrele
  const verItems  = changes.filter(c => c.type === 'versiyon' && !c.isOld);
  // Stok: Istisna'dan bağımsız — tüm markalar
  const stokTumu  = changes.filter(c => c.type === 'stok' && !c.isOld);
  // Kampanya + fiyat: Istisna'ya göre filtrele
  ...
  // stokTumu: Istisna filtresinden bağımsız tüm stok değişimleri
  ...
  if(stokTumu.length || verItems.length) {
    ...
    stokTumu.forEach(c => {
      ...
      stokListHtml += `<div class="info-stok-row">
        <span class="info-stok-icon" style="color:${color}">${icon}</span>
        <span class="info-stok-urun">${c.urun}</span>
        <span class="info-stok-val" style="color:${color}">${sign}${c.diff} <span style="opacity:.55;font-size:.62rem">(${c.old}→${c.new})</span></span>
      </div>`;
    });
    ...
    html += `<div class="section-block section-low">
      ...
      <div class="info-stok-list">${stokListHtml}</div>
