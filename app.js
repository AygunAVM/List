...
function buildPremiumPDF(docType, data) {
  ...
  // ❖ Proje gerekçe etiketi (varsa PDF'de "💵 Nakit" yerine gösterilir)
  const projeLabel = data.projeLabel || '';
  const hasProje   = projeLabel.length > 0;
  ...
  if (isNakit) {
    ...
    const _odemeEtiket = hasProje
      ? `<span class="badge-nakit">❖ ${projeLabel}</span>`
      : `<span class="badge-nakit">💵 Nakit</span>`;
    odemeHTML = `
      <tr><td class="rl">Liste Toplamı</td><td class="rr">${fmt(listeToplam)}</td></tr>
      ${_satirIndStr}
      <tr><td class="rl">Ödeme Şekli</td><td class="rr">${_odemeEtiket}</td></tr>
