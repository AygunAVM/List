...
function renderTable(searchVal) {
  ...
  data.forEach(u => {
    ...
    if (hasPrim) {
      ...
      // Prim seviyesine göre sınıf (gerçek veri dağılımına göre — 775 ürün)
      // Dağılım: 1-30 → ~350 ürün | 31-99 → ~250 | 100-199 → ~130 | 200-399 → ~60 | 400+ → ~20
      if      (primVal >= 400) primCls = 'prim-platin'; // Platin  (~20 ürün)
      else if (primVal >= 200) primCls = 'prim-altin';  // Altın   (~60 ürün)
      else if (primVal >= 100) primCls = 'prim-gumus';  // Gümüş   (~130 ürün)
      else if (primVal >= 31)  primCls = 'prim-bronz';  // Bronz   (~250 ürün)
      else                     primCls = 'prim-low';    // Düşük   (~350 ürün) — stok rengi
    }
...
// Her kampanya pill'inin sağına küçük 🔍 ekler.
// Koşullar: birlesen/kilitli tip, seçilmemiş, eşik beklenmiyor,
//           allProducts'ta stoklu tamamlayıcı mevcut,
//           tamamlayıcı ters yön filtreden geçiyor (max 3x sepet fiyatı)
function _campSearchBtn(item, camp, ci) {
  ...
  // Seçili veya eşik bekleniyor → gizle
  const _sc = item._selectedCamps || {};
  const _pg = item._pendingGroups || {};
  if (_sc[ci] || _sc[String(ci)] || _sc[Number(ci)]) return '';
  if (_pg[camp.grup]) return '';

  const grup      = camp.grup;
  const esik      = camp.esik;
  const mevcutRol = (camp.rol || 'ANY').toUpperCase();
  const iNakit    = parseFloat(String(item['Nakit'] || item['nakit'] || 0).replace(',', '.')) || 0;
  const YON       = 3.0;

  // allProducts'ta tamamlayıcı var mı kontrol (ilk stoklu eşleşme yeterli)
  const sepetKodlar = new Set(basket.map(b => String(b['Kod'] || b['kod'] || '')));
  let bulunan = false;
  for (const p of allProducts) {
    if (sepetKodlar.has(String(p['Kod'] || p['kod'] || ''))) continue;
    ...
    for (const pc of parseCampaigns(p['Açıklama'] || '')) {
      if (pc.grup !== grup || pc.esik !== esik) continue;
      if (pc.rol === mevcutRol) continue;
      bulunan = true;
      break;
    }
    if (bulunan) break;
  ...
  // Arama kelimesi: grup adı (Açıklama sütununda geçer)
  const araKelime = grup;

  return `<button type="button"
    title="Eşik tamamlayıcı ürünleri ara"
    onclick="window._campSearch(${JSON.stringify(araKelime)})"
    style="display:inline-flex;align-items:center;justify-content:center;
      ...
      position:relative;top:-1px">🔍</button>`;
}

// Global: sepeti kapat → urunler sekmesi → arama
