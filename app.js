...
function parseCampaigns(aciklama) {
  ...
  const result = [];
  // ⎇ ile biten segmentler = birleşebilir, 🔒︎ ile biten = kilitli
  // Regex: metin + (⎇ veya 🔒︎) — son segmentte ayraç olmayabilir
  const regex = /([^⎇🔒]+?)(?:(⎇)|(🔒︎))/g;
  let match;
  while ((match = regex.exec(aciklama)) !== null) {
    ...
    result.push({ metin, tutar, tip });
  }
  // Son segmenti de al (ayraçsız kalan)
  ...
  if (sonIndex < aciklama.length) {
    ...
    if (sonMetin) {
      result.push({ metin: sonMetin, tutar: _extractTutar(sonMetin), tip: 'birlesen' });
    }
  }
  return result;
}
...
// Kampanya durumunu yönetir — basket item üzerinde saklanır
function toggleCampaign(idx, campIdx) {
  ...
  const hasKilitli = Object.entries(item._selectedCamps).some(
  ...
  if (isSelected) {
    ...
    // Kilitli kampanya seçiliyken başka şey seçilemez
    if (hasKilitli) return;
    // Yeni kilitli seçilecekse diğerlerini temizle
    if (camp.tip === 'kilitli') {
      item._selectedCamps = {};
    }
  ...
  // Satır indirimini seçili kampanyaların toplamından hesapla
  const toplamInd = Object.entries(item._selectedCamps)
    .filter(([,v]) => v)
    .reduce((s, [ci]) => s + (item._campaigns[parseInt(ci)]?.tutar || 0), 0);

  Basket.setItemDisc(idx, toplamInd);

  // Input'u güncelle (DOM zaten yenilenecek ama anlık sync için)
  const inp = document.querySelector('.item-disc-input[data-idx="' + idx + '"]');
  if (inp) inp.value = toplamInd || '';
...
function renderCampaignPills(item, idx) {
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    // Devre dışı: kilitli seçiliyken ve bu pill o kilitli değilse
    const disabled = hasKilitli && !sel;

    let bg, border, color, textDeco = '';
    if (sel) {
    ...
    return '<span'
      ...
      + ' title="' + camp.metin + ' — ' + (locked?'Kilitli: diğerleriyle birleşmez':'Birleşebilir: diğerleriyle toplanır') + '"'
      ...
      + icon + (camp.metin.length > 28 ? camp.metin.slice(0,26)+'…' : camp.metin) + tutarStr
      + '</span>';
  }).join('');
  ...
  return '<div style="margin-top:4px;line-height:1.6">' + pills + clearBtn + '</div>';
}
...
Object.assign(window, {
  ...
  toggleCampaign, clearAllCampaigns,
