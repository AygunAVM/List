...
//   Tv+CepTelefonu         → "Tv" VEYA "Cep Telefonu" gamı varsa (OR, + ile)
//   *~Adaptör~Epilasyon    → herkese ama bu gamlar sepette varsa gösterme (~ ile)
//   Tv+Cep~Adaptör         → (Tv OR Cep) AND NOT Adaptör
//   SM-A175F               → tire içeren ürün kodu güvenle eşleşir
// ─────────────────────────────────────────────────────────────────────────────
function _bundleMatches(condition, basket) {
  ...
  // Boşluksuz normalize: "Cep Telefonu" → "ceptelefonu" ile "CepTelefonu" eşleşebilsin
  const ns = s => (s || '').toLowerCase().replace(/\s+/g, '');

  // ── Hariç tutma: '~' ayracından sonraki terimler
  ...
  const itemMatches = (term, item) => {
    const t = ns(term);
    // Kod: TAM eşleşme (kısmi kabul yok — 'sm-s94' → 'SM-S948D' eşleşmemeli)
    // Gam / Ürün adı: kısmi eşleşme OK ('Buzdolabı' → 'Büro Tipi Buzdolabı')
  ...
  if (excludePart) {
    ...
    if (excTerms.some(term => basket.some(item => itemMatches(term, item)))) return false;
  }
  ...
  // OR koşulları
  const incTerms = includePart.split('+').map(s => s.trim()).filter(Boolean);
  return incTerms.some(term => basket.some(item => itemMatches(term, item)));
}
