...
function checkUpsellOpportunities() {
  ...
  // ── 1. Sepetteki ürünlerin kampanya bilgilerini topla ─────────────
  // Her grup için: eşik, en yüksek tutar, sepetteki ürünlerin rolleri
  const grupBilgi = {};
  basket.forEach((item, bi) => {
    (item._campaigns || []).forEach(camp => {
      ...
      if (camp.sonTarih && new Date() > camp.sonTarih) return;
      const g = camp.grup;
      if (!grupBilgi[g]) grupBilgi[g] = { esik: camp.esik, tutar: camp.tutar, roller: {}, sahipBi: new Set() };
      grupBilgi[g].sahipBi.add(bi);
      if (camp.tutar > grupBilgi[g].tutar) grupBilgi[g].tutar = camp.tutar;
      // Sepetteki bu ürünün rolünü kaydet (eşik tamamlama kontrolü için)
      const rol = (camp.rol || 'ANY').toUpperCase();
      if (!grupBilgi[g].roller[bi]) grupBilgi[g].roller[bi] = rol;
    });
  });

  // ── 2. Sepette zaten seçili olan kampanyaları tespit et ───────────
  const grupSecili = {};
  basket.forEach((item, bi) => {
    Object.entries(item._selectedCamps || {}).forEach(([ci, sel]) => {
      if (!sel) return;
  ...
  // ── 3. allProducts'tan kampanya parse yardımcısı ──────────────────
  // Açıklama metninde verilen grup+rol'ü hızlı çeker
  function _parseGrupRollerFromAciklama(ac) {
    // "KEA -3k(2/A)⎇", "KM -14,5k(2/B)🔒" gibi formatlar
    const sonuclar = [];
    const re = /([A-ZÇŞĞÜÖİ0-9_+]+)\s+[-–]?\s*[\d.,]+[kK]?\s*\((\d+)\/([A-Z])\)/gi;
    let m;
    while ((m = re.exec(ac)) !== null) {
      sonuclar.push({ grup: m[1].toUpperCase(), esik: parseInt(m[2]), rol: m[3].toUpperCase() });
    }
    return sonuclar;
  }

  // ── 4. Her grup için "eşiği tamamlayacak ürün var mı?" kontrolü ───
  // Sepette mevcut rolleri bul, eksik rolleri allProducts'ta ara
  const urunKey = (() => {
  ...
  const stokKey   = 'Stok';
  const kodKey    = 'Kod';
  const sepetKodlar = new Set(basket.map(i => String(i[kodKey] || i.Kod || '')));
  ...
  Object.entries(grupBilgi).forEach(([grup, info]) => {
    ...
    // Sepetteki bu grup için mevcut rolleri topla
    const sepetRoller = new Set(Object.values(info.roller));

    // ── Tip A: Eşik TAMAMEN SAĞLANMIŞ ama kampanya seçilmemiş ──────
    // sahipSayi >= esik VE secili=0 → "Hazır, sadece seç"
    if (seciliSayi === 0 && sahipSayi >= info.esik) {
      // Gerçekten farklı roller yeterli mi kontrol et
      const farkliRolSayi = sepetRoller.has('ANY') ? info.esik : sepetRoller.size;
      if (farkliRolSayi >= info.esik || sepetRoller.has('ANY')) {
        firsatlar.push({
          grup, kazanc: info.tutar, eksikUrun: 0,
          tip: 'hazir',
          oncelik: 1  // En kolay — zaten sepette, sadece seçim gerekiyor
        });
      }
      return;
    }

    // ── Tip B: Seçim başlamış, 1 ürün eksik ────────────────────────
    if (seciliSayi > 0) {
      const acikPos = seciliSayi % info.esik;
      if (acikPos > 0 && (info.esik - acikPos) === 1) {
        // Tamamlamak için sepette zaten ürün var mı?
        const kalan = info.esik - acikPos;
        if (sahipSayi >= info.esik) {
          firsatlar.push({
            grup, kazanc: info.tutar, eksikUrun: 0,
            tip: 'secim_eksik',
            mesajEk: `${kalan} ürün daha seç`,
            oncelik: 2
          });
        }
      }
      return; // Seçim devam ediyorsa ekle önerisi gösterme
    }

    // ── Tip C: Eşik TAMAMLANAMAZ — sepette tek rol var, eksik rol ───
    // allProducts'ta tamamlayıcı ürün ara (stoklu önce)
    if (sahipSayi > 0 && sahipSayi < info.esik) {
      // Sepetteki rolleri bul
      const mevcutRoller = new Set(Object.values(info.roller).map(r => r.toUpperCase()));

      // allProducts'ta aynı gruba ait, farklı rolde, sepette olmayan ürünleri bul
      const tamamlayicilar = [];
      for (const p of allProducts) {
        const pKod = String(p[kodKey] || '');
        if (sepetKodlar.has(pKod)) continue; // zaten sepette
        const ac = p[acikKey] || '';
        const parsedCamps = _parseGrupRollerFromAciklama(ac);
        for (const pc of parsedCamps) {
          if (pc.grup !== grup) continue;
          if (pc.esik !== info.esik) continue;
          const pRol = pc.rol;
          // Bu ürünün rolü, sepetteki rollerden farklı olmalı (eşiği tamamlar)
          if (mevcutRoller.has(pRol)) continue;
          const stok = parseFloat(p[stokKey]);
          tamamlayicilar.push({
            ad: (p[urunKey] || '').replace(/^[➥♲]\s*/, '').trim(),
            kod: pKod,
            stok: isNaN(stok) ? 0 : stok,
            rol: pRol
          });
          break; // bu ürün için bir eşleşme yeterli
        }
      }

      if (tamamlayicilar.length === 0) return; // Tamamlayacak ürün yok → öneri gösterme

      // Stokluyu önce getir, stok sıfırları sona at
      tamamlayicilar.sort((a, b) => (b.stok > 0 ? 1 : 0) - (a.stok > 0 ? 1 : 0));
      const en_iyi = tamamlayicilar[0];
      const stokluSayi = tamamlayicilar.filter(t => t.stok > 0).length;

      firsatlar.push({
        grup, kazanc: info.tutar, eksikUrun: info.esik - sahipSayi,
        tip: 'ekle',
        tamamlayici: en_iyi,
        tamamlayiciSayi: tamamlayicilar.length,
        stokluSayi,
        oncelik: 3  // Ürün ekleme gerekiyor — en son öncelik
      });
    }
  });
  ...
  // ── 6. Öncelik sıralaması: en kolay fırsat önce ─────────────────
  // oncelik 1 (hazır) > 2 (seçim_eksik) > 3 (ekle)
  // Aynı oncelikte kazanç büyük olan öne geç
  firsatlar.sort((a, b) => a.oncelik - b.oncelik || b.kazanc - a.kazanc);

  const f   = firsatlar[0];
  const kFmt = f.kazanc >= 1000
    ? (f.kazanc / 1000).toFixed(f.kazanc % 1000 === 0 ? 0 : 1) + 'k₺'
    : f.kazanc + '₺';

  // ── 7. Mesaj üret — ✨ En İyi butonundan bağımsız ────────────────
  let mesaj = '';
  let araBtn = '';

  if (f.tip === 'hazir') {
    mesaj = `Sepette <b>${f.grup}</b> kampanyası tamamlanabilir → <b>+${kFmt}</b>`;

  } else if (f.tip === 'secim_eksik') {
    mesaj = `<b>${f.grup}</b> kampanyası için ${f.mesajEk} → <b>+${kFmt}</b>`;

  } else if (f.tip === 'ekle') {
    const t = f.tamamlayici;
    const stokDurumu = t.stok > 0 ? `(${t.stok} stok)` : '<span style="color:#fca5a5">stok yok</span>';
    // Ara butonu — ürünü tabloda aratır
    const araKelime = t.ad.split(' ').slice(0, 3).join(' ');
    araBtn = `<button onclick="(function(){
      document.getElementById('cart-modal').classList.remove('open');
      document.getElementById('cart-modal').style.display='none';
      const s=document.getElementById('search');
      if(s){s.value=${JSON.stringify(araKelime)};filterData();}
    })()" style="background:#1e40af;color:#fff;border:none;border-radius:6px;padding:4px 9px;
      font-weight:800;font-size:.62rem;cursor:pointer;white-space:nowrap;
      font-family:inherit;flex-shrink:0">🔍 Ara</button>`;

    if (f.stokluSayi > 0) {
      mesaj = `<b>${f.grup}</b> tamamlanır: <b>${t.ad}</b> ekle ${stokDurumu} → <b>+${kFmt}</b>`;
    } else {
      // Stoklu ürün yok — ama tamamlayıcılar var, stok bilgisi ver
      mesaj = `<b>${f.grup}</b> tamamlanabilir ancak uygun stok yok (${f.tamamlayiciSayi} ürün)`;
      araBtn = ''; // Stok yok — arama butonu gösterme
    }
  }

  if (!mesaj) { container.innerHTML = ''; return; }

  container.innerHTML = `<div style="display:flex;align-items:center;gap:8px;
      background:#0f172a;color:#f8fafc;padding:7px 12px;
      font-size:.67rem;border-top:2px solid #f59e0b;flex-wrap:wrap;">
    <span style="flex-shrink:0">💡</span>
    <span style="flex:1;min-width:0;line-height:1.4">${mesaj}</span>
    ${araBtn}
    <button onclick="document.getElementById('${CONTAINER_ID}').innerHTML=''"
      style="background:none;border:none;color:#64748b;font-size:.85rem;
