    ...
    function start() {
      try {
        ...
        unsub = onSnapshot(queryFn(), onNext, err => {
          ...
          const isRetryable = !err?.code || ['unavailable','resource-exhausted','internal','deadline-exceeded'].includes(err.code)
            || String(err).includes('404') || String(err).includes('WebChannel');
...
function buildPremiumPDF(docType, data) {
  ...
  if (isNakit) {
    odemeHTML = `
      ${listeToplam !== toplamOdeme ? `<tr><td class="rl">Liste Toplamı</td><td class="rr">${fmt(listeToplam)}</td></tr>` : ''}
    ...
    odemeHTML = `
      ${listeToplam > 0 ? `<tr><td class="rl">Liste Toplamı</td><td class="rr">${fmt(listeToplam)}</td></tr>` : ''}
