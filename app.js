...
function renderTable(searchVal) {
  ...
  data.forEach(u => {
    ...
    let btnHtml = '<button class="' + addBtnCls + ' cart-icon-btn haptic-btn" onclick="' + btnClick + '" title="' + btnTitle + '">' +
        '<span class="cart-icon">🛒</span>';
    if (hasPrim) {
      btnHtml += '<span class="cart-prim-amount">' + primLbl + '</span>';
    }
    btnHtml += '</button>';
...
function _showPrimAnimation(primVal) {
  ...
  el.style.cssText = 'position:fixed;top:55%;left:50%;transform:translate(-50%,-50%);' +
    ...
    'box-shadow:0 10px 40px rgba(208,31,46,.50),0 0 0 4px rgba(208,31,46,.15);' +
    'pointer-events:none;letter-spacing:-.01em;' +
    'animation:primFlyUp 1.2s cubic-bezier(.22,1,.36,1) forwards;';
