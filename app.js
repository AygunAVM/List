...
function toggleCampaign(idx, campIdx) {
  ...
  if (isSelected) {
    ...
    // ── AYNI ÜRÜN İÇİ ÇAKIŞMA KURALLARI ────────────────────────
    // Kural: aynı üründe 🔒 ve ⎇ bir arada olamaz.
    // 🔒 seçince → aynı üründeki ⎇ seçimleri temizle
    // ⎇ seçince  → aynı üründeki 🔒 seçimleri temizle
    ...
    if (camp.tip === 'kilitli') {
      // Yeni kilitli seçildi → aynı üründeki birleşebilirleri kaldır
      ...
      // Aynı ürünün diğer kilitli seçimlerini de kaldır (zaten ayrı ayrı seçilebilirler)
      // NOT: farklı ürünlerdeki kilitliler birbirine dokunmaz — ikili kampanya mantığı bu!
    } else if (camp.tip === 'birlesen') {
      // Yeni birleşebilir seçildi → aynı üründeki kilitlileri kaldır
      if (mevcutKilitli) {
        Object.keys(item._selectedCamps).forEach(ci => {
...
// Durumlar: seçili(yeşil), pending(sarı), normal(mavi/turuncu), devre dışı(soluk)
function renderCampaignPills(item, idx) {
  ...
  // Aynı ürün içinde çakışma kontrolü
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    const pending = sel && !!item._pendingGroups[camp.grup];
    // Devre dışı: sadece AYNI ürün içinde çakışma varsa
    // 🔒 seçiliyse → bu üründeki ⎇'ler devre dışı (ve tersi)
    const disabled = !sel && (
    ...
    let bg, border, color, textDeco = '', icon;
    if (isBilgi) {
      // Bilgi/Hediye — tıklanamaz, sadece gösterim
      bg = '#f5f3ff'; border = '#c4b5fd'; color = '#6d28d9';
      icon = 'ℹ️ ';
    } else if (sel && !pending) {
      // Aktif — eşik sağlandı, indirim düştü
      bg = '#dcfce7'; border = '#16a34a'; color = '#15803d';
      textDeco = 'line-through'; icon = '✓ ';
    } else if (sel && pending) {
      // Pending — seçildi ama eşik bekleniyor
      bg = '#fffbeb'; border = '#f59e0b'; color = '#b45309';
      textDeco = ''; icon = '⏳ ';
    } else if (locked) {
      bg = disabled ? '#f9f9f9' : '#fff7ed';
      border = disabled ? '#e5e7eb' : '#f97316';
      color  = disabled ? '#d1d5db' : '#c2410c';
      icon = '🔒 ';
    } else {
      bg = disabled ? '#f9f9f9' : '#eff6ff';
      border = disabled ? '#e5e7eb' : '#3b82f6';
      color  = disabled ? '#d1d5db' : '#1d4ed8';
      icon = '⎇ ';
    }

    // Eşik bilgisi tooltip'e yaz
    const esikBilgi = camp.esik > 1
      ? ' | ' + camp.grup + ': ' + camp.esik + ' ürün seçilmeli'
      : '';
    const tipBilgi  = locked ? 'Kilitli: diğerleriyle birleşmez' : 'Birleşebilir: diğerleriyle toplanır';

    const tutarStr = (camp.tutar > 0 && !isBilgi)
      ? ' <span style="font-weight:800;margin-left:2px">-' + (camp.tutar >= 1000 ? (camp.tutar/1000).toFixed(camp.tutar%1000===0?0:1)+'k' : camp.tutar) + '₺</span>'
      : '';
    const cursor = disabled ? 'not-allowed' : 'pointer';

    return '<span'
      + ' onclick="' + (disabled || isBilgi ? '' : 'toggleCampaign('+idx+','+ci+')') + '"'
      + ' title="' + camp.metin + ' — ' + tipBilgi + esikBilgi + '"'
      + ' style="display:inline-flex;align-items:center;gap:2px;padding:2px 7px;border-radius:20px;font-size:.60rem;font-weight:600;white-space:nowrap;cursor:'+cursor+';user-select:none;'
      + 'background:'+bg+';border:1px solid '+border+';color:'+color+';margin:1px 2px 1px 0;'
      + 'text-decoration:'+textDeco+';opacity:'+(disabled?0.4:1)+';transition:all .15s">'
      + icon + (camp.metin.length > 30 ? camp.metin.slice(0,28)+'…' : camp.metin) + tutarStr
      + '</span>';
  }).join('');

  const secilenSayi = Object.values(item._selectedCamps).filter(Boolean).length;
  const clearBtn = secilenSayi > 0
    ? '<span onclick="clearAllCampaigns('+idx+')" title="Tüm kampanya seçimlerini sıfırla"'
      + ' style="display:inline-flex;align-items:center;padding:2px 5px;border-radius:20px;font-size:.56rem;cursor:pointer;background:#fee2e2;border:1px solid #fca5a5;color:#dc2626;margin:1px 0;vertical-align:middle">✕</span>'
    : '';

  // Aktif indirim özeti
  const campDisc = item._campDisc || 0;
  const summaryBadge = (secilenSayi > 0 && campDisc > 0)
    ? ' <span style="font-size:.58rem;background:#dcfce7;border:1px solid #86efac;color:#15803d;border-radius:20px;padding:1px 6px;font-weight:700">-' + (campDisc >= 1000 ? (campDisc/1000).toFixed(campDisc%1000===0?0:1)+'k' : campDisc) + '₺ dağıtıldı</span>'
    : '';

  return '<div style="margin-top:4px;line-height:1.6">' + pills + clearBtn + summaryBadge + '</div>';
}
...
async function finalizeAksiyon() {
  ...
  // Pazarlık indirimi — abakusSelection'dan al (tablo hesapladı, modal kapalı olabilir)
  const _ekIndirimF    = abakusSelection?.ekIndirim || 0;
...
function buildPremiumPDF(docType, data) {
  ...
  // Birleşik indirim: satır + alt + pazarlık (WA/PDF'te tek satır)
  const _tumIndirimBirleskik = _pdfToplamIndirim + _pdfEkInd;

  let gosterilecekToplam  = Number(data.toplamOdeme || 0);
  // ÖNEMLİ: data.toplamOdeme nakit için zaten TÜM indirimler (satır+alt+pazarlık) düşülmüş değerdir.
  // targetBrutTotal = liste fiyatı = gosterilecekToplam + tüm indirimler
  // Kart işlemlerinde: gosterilecekToplam = ab.tahsilat (vade farkı eklenmiş, indirimler DÜŞÜLMÜŞ)
  const targetBrutTotal = isNakit
    ? (gosterilecekToplam + _tumIndirimBirleskik)   // nakit: net + tüm indirimler = liste
    : gosterilecekToplam;                            // kart: ab.tahsilat doğrudan kullan
  ...
  // Toplam indirim = satır + alt + pazarlık (tek satırda birleştirilmiş)
  const _toplamTumIndirim = _tumIndirimBirleskik;
