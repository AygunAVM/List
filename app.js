...
const Basket = {
  ...
  // Ürün ekle (async — logSepet fire-and-forget ama hata kaybını önlemek için await)
  async add(item, productIdx) {
    basket.push(item);
    if (basket.length === 1) {
      logSepet('session_basla', 0, null).catch(e => console.warn('logSepet session_basla:', e));
      resetSessionTimer();
    }
    logSepet('ekle', item.nakit || 0, item.urun || '').catch(e => console.warn('logSepet ekle:', e));
...
async function showApp() {
  ...
  // Süresi dolmuş teklifleri Firebase'e kalıcı yaz (giriş sonrası + her 10 dk)
  setTimeout(checkExpiredProposals, 5000);
  setInterval(checkExpiredProposals, 10 * 60 * 1000); // 10 dakikada bir
...
function _renderSingleProp(p) {
  ...
  const canAct = isAdmin() || p.user===me;
    ...
    // Süre dolmuşsa görsel olarak 'sureDoldu' gibi davran — state mutasyonu YOK
    // Kalıcı güncelleme checkExpiredProposals() ile periyodik yapılır
    const isExpired = expDate && now > expDate && p.durum === 'bekliyor';
    if (isExpired) p = { ...p, durum: 'sureDoldu' }; // shallow copy — orijinal korunur
...
// ─── SÜRESI DOLAN TEKLİFLERİ KALICI GÜNCELLE ───────────────────
// renderProposals sırasında state değiştirmek yerine bu fonksiyon
// periyodik çalışır ve Firebase + localStorage'a yazar.
// showApp içinde çağrılır (giriş sonrası) + 10 dakikada bir tekrar eder.
async function checkExpiredProposals() {
  if (!currentUser || !_db) return;
  const now = new Date();
  const expired = proposals.filter(
    p => p.durum === 'bekliyor' && p.sureBitis && new Date(p.sureBitis) < now
  );
  if (!expired.length) return;
  console.log(`⏰ ${expired.length} süresi dolmuş teklif güncelleniyor…`);
  for (const p of expired) {
    await updatePropStatus(p.id, 'sureDoldu');
  }
}
...
async function loadFunnelAnaliz(gunAralik = 90, force = false) {
  ...
  try {
    ...
    // NOT: Bu sorgu Firestore'da "ts" alanı üzerinde composite index gerektirir.
    // Firebase Console → Firestore → Indexes → Composite: ts (ASC + DESC)
    // İlk çalıştırmada console'daki link ile otomatik index oluşturabilirsiniz.
    const q = query(
