...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', function(e) {
    ...
    if (typeof switchMainTab==='function') {
      ...
      setTimeout(function(){
        const s=document.getElementById('search'); if(s) s.value=grup;
...
// ── Haftanın Yıldızı: Tüm kullanıcıların haftalık satışlarını karşılaştır ──
async function fetchWeeklyStar() {
  if (!_db) return null;
  try {
    const haftaOnce = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
    // proposals koleksiyonundan son 7 günün satışlarını say
    ...
    // Her personel için satış sayısını topla
    const counts = {};
    snap.docs.forEach(d => {
      const u = d.data().user || '—';
    ...
    // En yüksek satışçı
    const star = Object.entries(counts).sort((a, b) => b[1] - a[1])[0];
    return { email: star[0], count: star[1] };
...
async function renderMyStats() {
  ...
  if (!container) return;
  container.innerHTML = '<div class="mystats-loading">⏳ Veriler yükleniyor…</div>';

  const me = currentUser?.Email || '';
  const ad = currentUser?.Ad || me.split('@')[0];
  const now = new Date();
  const bugun       = now.toISOString().slice(0, 10);
  const haftaOnce   = new Date(now - 7  * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
  const ayOnce      = new Date(now - 30 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);

  // ── Proposals'tan kendi verim ──────────────────────────────
  const benimler    = (proposals || []).filter(p => p.user === me);
  const blurData    = await _getMyBlurStats(me);   // fiyat_bakislari koleksiyonu

  // Bugün
  const bugTeklif   = benimler.filter(p => (p.ts||'').slice(0,10) === bugun).length;
  const bugSatis    = benimler.filter(p => p.durum==='satisDondu' && (p.archivedAt||p.ts||'').slice(0,10) === bugun).length;

  // Bu hafta
  const hftTeklif   = benimler.filter(p => (p.ts||'').slice(0,10) >= haftaOnce).length;
  const hftSatis    = benimler.filter(p => p.durum==='satisDondu' && (p.archivedAt||p.ts||'').slice(0,10) >= haftaOnce).length;
  const hftBlur     = blurData.hafta || 0;
  const hftSepet    = benimler.filter(p => (p.ts||'').slice(0,10) >= haftaOnce).length; // her teklif bir sepet olur

  // Bu ay
  const ayTeklif    = benimler.filter(p => (p.ts||'').slice(0,10) >= ayOnce).length;
  const aySatis     = benimler.filter(p => p.durum==='satisDondu' && (p.archivedAt||p.ts||'').slice(0,10) >= ayOnce).length;

  // ── BTB = (SepeteEklenen / BlurAçılan) × 100 ──────────────
  const btbHafta   = hftBlur > 0 ? ((hftSepet / hftBlur) * 100) : null;
  // ── BTS = (Satış / ToplamTeklif) × 100 ───────────────────
  const btsHafta   = hftTeklif > 0 ? ((hftSatis  / hftTeklif) * 100) : null;
  const btsAy      = ayTeklif  > 0 ? ((aySatis   / ayTeklif)  * 100) : null;

  // ── Toplam ciro (hafta) ────────────────────────────────────
  const hftCiro    = benimler
    .filter(p => p.durum==='satisDondu' && (p.archivedAt||p.ts||'').slice(0,10) >= haftaOnce)
  ...
  // ── Motivasyon seviyesi ────────────────────────────────────
  const { emoji: motEmoji, mesaj: motMesaj, renk: motRenk } = _getMotivation(bugSatis, bugTeklif, hftSatis, btsHafta);
  ...
  container.innerHTML = `
    ...
    <div class="ms-motivasyon" style="background:${motRenk}15;border-left:3px solid ${motRenk}">
      ...
      <span style="color:${motRenk};font-weight:700;font-size:.82rem">${motMesaj}</span>
    </div>

    <!-- Bugün Özet -->
    <div class="ms-section-title">📅 Bugün</div>
    <div class="ms-stat-row">
      <div class="ms-stat-card">
        <div class="ms-stat-val">${bugTeklif}</div>
        <div class="ms-stat-lbl">Teklif</div>
      </div>
      <div class="ms-stat-card ms-stat-green">
        <div class="ms-stat-val">${bugSatis}</div>
        <div class="ms-stat-lbl">Satış ✅</div>
      </div>
    </div>

    <!-- Haftalık Metrikler -->
    <div class="ms-section-title">📈 Bu Hafta (7 gün)</div>
    <div class="ms-stat-row">
      <div class="ms-stat-card">
        <div class="ms-stat-val">${hftTeklif}</div>
        <div class="ms-stat-lbl">Teklif</div>
      </div>
      <div class="ms-stat-card ms-stat-green">
        <div class="ms-stat-val">${hftSatis}</div>
        <div class="ms-stat-lbl">Satış</div>
      </div>
      <div class="ms-stat-card ms-stat-blue">
        <div class="ms-stat-val">${hftCiro > 0 ? _fmtK(hftCiro) : '—'}</div>
        <div class="ms-stat-lbl">Ciro ₺</div>
      </div>
    </div>

    <!-- BTB Metriği -->
    <div class="ms-metric-card ${btbHafta !== null && btbHafta < 10 ? 'ms-metric-warn' : 'ms-metric-ok'}">
      <div class="ms-metric-header">
        <span class="ms-metric-name">İlgi Dönüşüm Oranı (BTB)</span>
        <span class="ms-metric-formula">Sepete / Blur × 100</span>
      </div>
      <div class="ms-metric-val">${btbHafta !== null ? btbHafta.toFixed(1) + '%' : '—'}</div>
      <div class="ms-metric-bar-bg"><div class="ms-metric-bar-fill" style="width:${Math.min(btbHafta||0,100)}%"></div></div>
      <div class="ms-metric-hint">
        ${btbHafta === null ? 'Henüz yeterli veri yok' :
          btbHafta < 10 ? '⚠️ %10 altı — fiyat sorulup sepet kurulmayan müşteri var' :
          btbHafta < 30 ? '👍 Makul — %30\'a ulaşmayı hedefle' :
          '🔥 Mükemmel dönüşüm!'}
      </div>
    </div>

    <!-- BTS Metriği -->
    <div class="ms-metric-card ${btsHafta !== null && btsHafta < 20 ? 'ms-metric-warn' : 'ms-metric-ok'}">
      <div class="ms-metric-header">
        <span class="ms-metric-name">Kapanış Oranı (BTS)</span>
        <span class="ms-metric-formula">Satış / Toplam Teklif × 100</span>
      </div>
      <div class="ms-metric-val">${btsHafta !== null ? btsHafta.toFixed(1) + '%' : '—'}</div>
      <div class="ms-metric-bar-bg"><div class="ms-metric-bar-fill" style="width:${Math.min(btsHafta||0,100)}%"></div></div>
      <div class="ms-metric-hint">
        ${btsHafta === null ? 'Henüz yeterli veri yok' :
          btsHafta < 20 ? '⚠️ Her 5 tekliften en az 1 satışa dönüşmeli' :
          btsHafta < 50 ? '👍 İyi gidiyorsun' :
          '🔥 Kapanış ustası!'}
      </div>
      <div style="font-size:.64rem;color:var(--text-3);margin-top:4px">Bu ay: <strong>${btsAy !== null ? btsAy.toFixed(1) + '%' : '—'}</strong></div>
    </div>

    <!-- Haftanın Yıldızı Sıralaması -->
    ${star ? `
    <div class="ms-section-title">⭐ Haftanın Yıldızı</div>
    <div class="ms-yildiz-card ${benimYildiz ? 'ms-yildiz-ben' : ''}">
      <div class="ms-yildiz-avatar">${(star.email.split('@')[0]).charAt(0).toUpperCase()}</div>
      <div style="flex:1">
        <div style="font-weight:800;font-size:.82rem;color:var(--text-1)">${star.email.split('@')[0]}</div>
        <div style="font-size:.66rem;color:var(--text-3)">Bu hafta ${star.count} satış</div>
      </div>
      <div class="ms-yildiz-crown">👑</div>
      ${benimYildiz ? '' : `<div style="font-size:.68rem;color:var(--text-3);text-align:right">Senin: <strong>${hftSatis}</strong></div>`}
    </div>
    ` : ''}

    <!-- Alt bilgi -->
    <div style="text-align:center;font-size:.60rem;color:var(--text-3);padding:14px 0 4px">
      Veriler proposals & fiyat_bakislari koleksiyonundan anlık hesaplanmaktadır.
    </div>
  `;
}
...
async function _getMyBlurStats(email) {
  if (!_db || !email) return { bugun: 0, hafta: 0 };
  try {
    const snap = await getDoc(doc(_db, 'fiyat_bakislari', email));
    if (!snap.exists()) return { bugun: 0, hafta: 0 };
    const data = snap.data();
    // revealedPrices dizisinin uzunluğunu kullan (toplam unique ürün)
    const toplam = (data.revealedPrices || []).length;
    // Yaklaşık hafta payı (ham veri, geçmişi yok — ileride daily ile genişletilebilir)
    return { bugun: 0, hafta: toplam };
  } catch (e) { return { bugun: 0, hafta: 0 }; }
}
...
function _getMotivation(bugSatis, bugTeklif, hftSatis, bts) {
  ...
  if (bugTeklif >= 2) return { emoji:'⚡', mesaj:`${bugTeklif} teklifin var — birini kapat!`, renk:'#b45309' };
  if (hftSatis > 0)   return { emoji:'📈', mesaj:`Bu hafta ${hftSatis} satış — devam et!`, renk:'#0369a1' };
  if (bts !== null && bts < 10) return { emoji:'🎯', mesaj:'Teklifleri satışa dönüştürmek için takip et!', renk:'#7c3aed' };
  return { emoji:'🚀', mesaj:'İlk teklifini oluştur ve yarışa katıl!', renk:'#7c3aed' };
}
