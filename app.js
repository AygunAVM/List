...
logs.forEach(l => {
  ...
  if (h>=0) { 
    ...
    // ── OTOMATİK TAVSİYE KURALLARI ──────────────────────────────────
    // localStorage'dan özelleştirilebilir kuralları al
    ...
    direktenDonenler.concat(sessizDegerler).concat(olduStok).forEach(u => {
      _tavsiyeKurallari.filter(k => k.aktif).forEach(kural => {
        try {
          if (kural.test(u)) {
            _tavsiyeListesi.push({ urun: u.ad, kural, blur: u.blur, satis: u.satis, sepet: u.sepet || 0, stok: u.stok || 0 });
          }
        } catch(e) {}
      });
    });
    // En fazla 15 öneri göster, blur'a göre sırala
    ...
    const _funnelStats = {
      ...
      _tavsiyeListesi, _tavsiyeKurallari
    };
    ...
    <button class="funnel-filter-btn" data-filter="${f}"
      ...
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:14px;margin-bottom:12px">
        ...
        ${(() => {
          ...
          return asamalar.map((a, i) => {
            const pct = Math.round(a.sayi/maxSayi*100);
            const oncekiSayi = i===0 ? a.sayi : asamalar[i-1].sayi;
            const kayip = oncekiSayi - a.sayi;
            const kayipPct = oncekiSayi===0 ? 0 : Math.round(kayip/oncekiSayi*100);
            return `<div style="margin-bottom:8px">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px">
                <span style="font-size:.68rem;font-weight:600;color:#1e293b">${a.ad}</span>
                <div style="display:flex;align-items:center;gap:8px">
                  ${i>0 && kayip>0 ? `<span style="font-size:.58rem;color:#dc2626;font-weight:700">▼ ${kayip} kaybedildi (${kayipPct}%)</span>` : ''}
              ...
              <div style="height:20px;border-radius:6px;background:#f1f5f9;overflow:hidden">
                <div style="width:${pct}%;height:100%;background:${a.renk};border-radius:6px;transition:width .3s;display:flex;align-items:center;padding-left:6px">
                  ${pct>10?`<span style="font-size:.56rem;color:#fff;font-weight:700">${pct}%</span>`:''}
                </div>
              </div>
            </div>`;
          }).join('');
        })()}
        ${funnelBlur > 0 ? `<div style="background:#fef2f2;border-radius:8px;padding:8px;margin-top:6px;font-size:.64rem;color:#dc2626">
          ⚠️ Abaküs açanların <b>${l3Donusum}%</b>'i satışa dönüyor — ${parseFloat(l3Donusum)<20?'Acil müdahale önerilir!':parseFloat(l3Donusum)<40?'İyileştirme alanı var.':'İyi seviye.'}
        </div>` : ''}
      </div>
      ...
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:14px;margin-bottom:12px">
        ...
        <div style="display:flex;flex-direction:column;gap:6px;max-height:260px;overflow-y:auto;padding-right:2px">
          ...
          <div style="display:flex;align-items:flex-start;gap:8px;background:${t.kural.icon==='💸'?'#fef2f2':t.kural.icon==='📍'?'#fffbeb':t.kural.icon==='📢'?'#eff6ff':'#f0fdf4'};border-radius:8px;padding:8px 10px;border:1px solid ${t.kural.icon==='💸'?'#fecaca':t.kural.icon==='📍'?'#fde68a':t.kural.icon==='📢'?'#bfdbfe':'#bbf7d0'}">
