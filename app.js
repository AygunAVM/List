...
// kullanicilar.json'daki "magazaTipi": "AVM" | "CARSI" alanını okur.
// Tanımsız ise "BELIRSIZ" döner — loglarda boş bırakılmaz.
function getMagazaTipi() {
  if (!currentUser) return 'BELIRSIZ';
  return (currentUser.magazaTipi || 'BELIRSIZ').toUpperCase();
}
// Kısa etiket (UI için)
...
async function logSepet(islem, tutar, urunAdi) {
  ...
  try {
    await addDoc(collection(_db, 'sepet_loglari'), {
      ...
      magazaTipi:   getMagazaTipi()   // 'AVM' | 'CARSI' | 'BELIRSIZ'
    });
...
async function logSessionResult(sonuc, neden) {
  ...
  try {
    ...
    await addDoc(collection(_db, 'funnel_logs'), {
      personelId:      currentUser.Email,
      personelAd:      currentUser.Ad || currentUser.Email.split('@')[0],
      funnelRol:       funnelRol,   // 'saha' | 'destek' | 'admin'
      magazaTipi:      getMagazaTipi(),  // 'AVM' | 'CARSI' | 'BELIRSIZ'
      ts:              serverTimestamp(),
      tarih:           _tarih,
      gun:             _gun,
...
async function _fbWriteAnalytics(email, today, rec) {
  ...
  try {
    const docId = email.replace(/[^a-zA-Z0-9]/g,'_') + '_' + today;
    await setDoc(doc(_db, 'analytics', docId), { email, date: today, magazaTipi: getMagazaTipi(), ...rec }, { merge: true });
  } catch(e) { /* sessiz */ }
}
...
// ─── DETAYLI VERİ AKTAR — funnel_logs + sepet_loglari → CSV ──────
// Ham Firestore verisini doğrudan CSV'e döker.
// Aktif filtreler (Rol + Mağaza Tipi) uygulanır.
// Yeni analiz ihtiyaçlarında sadece alan listesini genişletmek yeterli.
window.exportFunnelCSV = async function() {
  if (!isAdmin()) { ayAlert('Bu özellik sadece admin için geçerlidir.'); return; }

  const cont = document.getElementById('funnel-analiz-konteynir');
  const aktifFiltre  = cont?.dataset.funnelFiltre  || 'hepsi';
  const magazaFiltre = cont?.dataset.magazaFiltre   || 'hepsi';

  // Yükleniyor göstergesi
  const btn = document.querySelector('[onclick="exportFunnelCSV()"]');
  ...
  try {
    ...
    let fLogs = [];
    fSnap.forEach(d => fLogs.push({ _col: 'funnel_logs', ...d.data() }));

    // ── 2. Sepet Logları ────────────────────────────────────────
    const sSnap = await getDocs(query(
      collection(_db, 'sepet_loglari'),
      where('ts', '>=', limitDate),
      orderBy('ts', 'desc')
    ));
    let sLogs = [];
    sSnap.forEach(d => sLogs.push({ _col: 'sepet_loglari', ...d.data() }));

    // ── 3. Rol Filtresi ─────────────────────────────────────────
    if (aktifFiltre !== 'hepsi') {
      fLogs = fLogs.filter(l => (l.funnelRol || 'saha') === aktifFiltre);
      // sepet_loglari'nda funnelRol yok — geç
    }

    // ── 4. Mağaza Tipi Filtresi ─────────────────────────────────
    if (magazaFiltre !== 'hepsi') {
      fLogs = fLogs.filter(l => (l.magazaTipi || 'BELIRSIZ') === magazaFiltre);
      sLogs = sLogs.filter(l => (l.magazaTipi || 'BELIRSIZ') === magazaFiltre);
    }

    // ── 5. Funnel CSV ────────────────────────────────────────────
    const fHeaders = [
      'Koleksiyon','Personel','Personel Ad','Mağaza Tipi','Rol',
      'Tarih','Saat','Gün','Sonuç','Neden',
      'Toplam Tutar (₺)','Derinlik','Benzersiz Ürün',
      'Sepet Kategorisi','Süre (sn)','Intent Level',
      'Blur Sayısı','Bakılan Fiyatlar','Aramalar',
      'Zincir','Kart','Taksit','İndirim Var mı',
      'Sepete Eklenenler','Alınmayanlar','Bundle Var mı','Bundle Yapıldı mı'
    ];
    const tsStr = r => {
      if (!r.ts) return '';
      try {
        const d = r.ts.toDate ? r.ts.toDate() : new Date(r.ts);
        return d.toLocaleString('tr-TR', { timeZone: 'Europe/Istanbul' });
      } catch(e) { return String(r.ts); }
    };
    const arr2str = v => Array.isArray(v) ? v.join(' | ') : (v || '');
    const fRows = fLogs.map(r => [
      'funnel_logs',
      r.personelId || '',
      r.personelAd || '',
      r.magazaTipi || 'BELIRSIZ',
      r.funnelRol  || '',
      r.tarih || '',
      r.saat != null ? r.saat : '',
      r.gun  != null ? r.gun  : '',
      ...
      r.sure || 0,
      r.intentLevel || 0,
      r.benzersizBlurSayisi || 0,
      arr2str(r.bakilanFiyatlar),
      arr2str(r.aramalar),
      r.zincir  || '',
      r.kart    || '',
      r.taksit  || '',
      r.indirimVarMi ? 'Evet' : 'Hayır',
      arr2str(r.sepeteEklenenUrunler || r.urunler?.map(u => u.urun) || []),
      arr2str(r.alinmayanUrunler),
      r.bundleVarMi   ? 'Evet' : 'Hayır',
      r.bundleYapildi ? 'Evet' : 'Hayır'
    ]);

    // ── 6. Sepet Logları CSV ────────────────────────────────────
    const sHeaders = [
      'Koleksiyon','Personel','Personel Ad','Mağaza Tipi',
      'Tarih','İşlem','Tutar (₺)','Ürün','Sepet Adedi','Timestamp'
    ];
    const sRows = sLogs.map(r => [
      'sepet_loglari',
      r.personelId || '',
      r.personelAd || '',
      r.magazaTipi || 'BELIRSIZ',
    ...
    const magsLabel = magazaFiltre === 'AVM' ? 'AVM' : magazaFiltre === 'CARSI' ? 'Carsi' : 'Tumu';
    const rolLabel  = aktifFiltre  === 'hepsi' ? 'TumRol' : aktifFiltre;
    const tarih     = new Date().toLocaleDateString('tr-TR').replace(/\./g,'-');

    // Funnel CSV
    const fCsv  = BOM + toCSV(fHeaders, fRows);
    const fBlob = new Blob([fCsv], { type: 'text/csv;charset=utf-8;' });
    const fUrl  = URL.createObjectURL(fBlob);
    const fA    = document.createElement('a');
    fA.href = fUrl;
    fA.download = `aygun-funnel-${magsLabel}-${rolLabel}-${tarih}.csv`;
    document.body.appendChild(fA); fA.click(); document.body.removeChild(fA);
    URL.revokeObjectURL(fUrl);

    // Sepet CSV (küçük gecikmeyle ayrı dosya)
    await new Promise(r => setTimeout(r, 400));
    const sCsv  = BOM + toCSV(sHeaders, sRows);
    const sBlob = new Blob([sCsv], { type: 'text/csv;charset=utf-8;' });
    const sUrl  = URL.createObjectURL(sBlob);
    const sA    = document.createElement('a');
    sA.href = sUrl;
    sA.download = `aygun-sepet-loglari-${magsLabel}-${rolLabel}-${tarih}.csv`;
    document.body.appendChild(sA); sA.click(); document.body.removeChild(sA);
    URL.revokeObjectURL(sUrl);

    showToast(`✅ ${fLogs.length} funnel + ${sLogs.length} sepet kaydı indirildi`);
    haptic && haptic(20);

  } catch(e) {
    console.error('exportFunnelCSV:', e);
    ayAlert('Export hatası: ' + e.message);
  } finally {
    if (btn) { btn.innerHTML = orgText; btn.disabled = false; }
  }
};
window.setFunnelFilter = function(filter) {
  console.log("🎯 Rol filtresi değiştirildi:", filter);
  const cont = document.getElementById('funnel-analiz-konteynir');
...
// Mağaza tipi filtresi — "AVM | Çarşı | Tümü" seçici
window.setMagazaFiltre = function(tip) {
  console.log("🏬 Mağaza filtresi değiştirildi:", tip);
  const cont = document.getElementById('funnel-analiz-konteynir');
  if (cont) cont.dataset.magazaFiltre = tip;
  // Mağaza butonlarını güncelle
  document.querySelectorAll('.magaza-filter-btn').forEach(btn => {
    const isActive = btn.dataset.magaza === tip;
...
async function loadFunnelAnaliz(gunAralik = 90, force = false) {
  ...
  try {
    ...
    // ── FİLTRE SEÇİMİ (Rol + Mağaza Tipi) ──────────────────────────
    let aktifFiltre  = cont.dataset.funnelFiltre  || 'saha';
    let magazaFiltre = cont.dataset.magazaFiltre  || 'hepsi'; // 'AVM' | 'CARSI' | 'hepsi'

    // Rol filtre butonlarını güncelle
    document.querySelectorAll('.funnel-filter-btn').forEach(btn => {
    ...
    // Mağaza filtre butonlarını güncelle
    document.querySelectorAll('.magaza-filter-btn').forEach(btn => {
      const isActive = btn.dataset.magaza === magazaFiltre;
      btn.style.borderColor = isActive ? '#2563eb' : 'var(--border)';
      btn.style.background  = isActive ? '#2563eb' : 'var(--surface)';
      btn.style.color       = isActive ? '#fff'    : 'var(--text-2)';
    });
    ...
    // Adım 1: Rol filtresi
    const logsRolFiltered = aktifFiltre === 'hepsi'
      ? allLogs
      : allLogs.filter(l => {
    ...
    // Adım 2: Mağaza Tipi filtresi (AVM / Çarşı / Tümü)
    const logs = magazaFiltre === 'hepsi'
      ? logsRolFiltered
      : logsRolFiltered.filter(l => (l.magazaTipi || 'BELIRSIZ') === magazaFiltre);

    // Log sayısı sıfırsa uyarı göster
    if (logs.length === 0) {
      const rolLabel = aktifFiltre === 'saha' ? '👷 Saha' : aktifFiltre === 'destek' ? '🖥 Destek' : aktifFiltre === 'admin' ? '👑 Admin' : '🌐 Tümü';
      const magsLabel = magazaFiltre === 'AVM' ? '🏬 AVM' : magazaFiltre === 'CARSI' ? '🏪 Çarşı' : '';
      cont.innerHTML = `<div class="admin-empty">📭 "${rolLabel}${magsLabel ? ' · ' + magsLabel : ''}" filtresinde veri yok.<br><span style="font-size:.72rem;color:var(--text-3)">Farklı bir filtre deneyin.</span></div>`;
      _isFunnelLoading = false;
      return;
    }
...
logs.forEach(l => {
  ...
  const h = l.saat ?? -1;
  // serverTimestamp null iken gelen kayıtları saatlik grafikten dışla (00:00 yığılma önlenir)
  if (h >= 0 && h <= 23) { 
    ...
    // ── FUNNEL AŞAMA TOTALLERİ — Oturum Bazlı (Session-Based) ───────
    // Her log kaydı = 1 benzersiz müşteri oturumu. personelId+tarih+saat ile
    // nadir duplicate'ler temizlenir (session-based, event-based değil).
    ...
    // ── ÜRÜ BAZLI TAVSİYE MOTORU — Map<productId, recommendation> ──
    // Her ürün için yalnızca 1 öneri tutulur. Duplicate oluşmaz.
    // Öncelik: Dinamik kural (1) > Özelleştirilebilir kural (2)
    ...
    direktenDonenler.concat(sessizDegerler).concat(olduStok).forEach(u => {
      ...
      if (dinamik) {
        if (!_tavsiyeMap.has(u.ad)) {
          _tavsiyeMap.set(u.ad, { ...entry, kural: { ...dinamik, durum: dinamik.oneri }, _oncelik: 1 });
        }
        return;
      }
      _tavsiyeKurallari.filter(k => k.aktif).forEach(kural => {
        try {
          if (kural.test(u) && !_tavsiyeMap.has(u.ad)) {
            _tavsiyeMap.set(u.ad, { ...entry, kural: { ...kural, renk: '#f8fafc', kenar: '#e2e8f0' }, _oncelik: 2 });
          }
        } catch(e) {}
      });
    });
    const _tavsiyeListesiFinal = [..._tavsiyeMap.values()]
...
<div style="display:flex;gap:6px;margin-bottom:6px;flex-wrap:wrap;align-items:center">
  <span style="font-size:.60rem;color:var(--text-3);font-weight:700;white-space:nowrap">👥 Rol:</span>
  ${['saha','destek','admin','hepsi'].map(f=>`
    <button class="funnel-filter-btn" data-filter="${f}"
      onclick="setFunnelFilter('${f}')"
      style="padding:5px 12px;border-radius:20px;font-size:.70rem;font-weight:700;cursor:pointer;font-family:inherit;border:1.5px solid ${aktifFiltre===f?'var(--red)':'var(--border)'};background:${aktifFiltre===f?'var(--red)':'var(--surface)'};color:${aktifFiltre===f?'#fff':'var(--text-2)'}">
      ${f==='saha'?'👷 Saha':f==='destek'?'🖥 Destek':f==='admin'?'👑 Admin':'🌐 Tümü'}
    </button>`).join('')}
</div>
<!-- Mağaza Tipi Filtresi -->
<div style="display:flex;gap:6px;margin-bottom:12px;flex-wrap:wrap;align-items:center">
  <span style="font-size:.60rem;color:var(--text-3);font-weight:700;white-space:nowrap">🏬 Mağaza:</span>
  ${[
    { val:'hepsi', label:'🌐 Tümü' },
    { val:'AVM',   label:'🏬 AVM (10:00-22:00)' },
    { val:'CARSI', label:'🏪 Çarşı (09:00-19:00)' }
  ].map(m=>`
    <button class="magaza-filter-btn" data-magaza="${m.val}"
      onclick="setMagazaFiltre('${m.val}')"
      style="padding:5px 12px;border-radius:20px;font-size:.70rem;font-weight:700;cursor:pointer;font-family:inherit;border:1.5px solid ${magazaFiltre===m.val?'#2563eb':'var(--border)'};background:${magazaFiltre===m.val?'#2563eb':'var(--surface)'};color:${magazaFiltre===m.val?'#fff':'var(--text-2)'}">
      ${m.label}
    </button>`).join('')}
</div>

      <!-- Bilgi: Hangi tarih aralığı gösteriliyor + Export -->
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;flex-wrap:wrap;gap:6px">
        <div style="font-size:.6rem;color:var(--text-3)">
          📅 Son ${gunAralik} gün · ${totN} oturum
          ${magazaFiltre !== 'hepsi' ? `· <b style="color:#2563eb">${magazaFiltre === 'AVM' ? '🏬 AVM' : '🏪 Çarşı'}</b>` : ''}
        </div>
        <button onclick="exportFunnelCSV()"
          style="padding:5px 12px;border-radius:8px;font-size:.65rem;font-weight:700;cursor:pointer;font-family:inherit;border:1.5px solid #16a34a;background:#f0fdf4;color:#15803d;display:flex;align-items:center;gap:4px;white-space:nowrap">
          📊 Detaylı Veri Aktar
        </button>
      </div>
...
Object.assign(window, {
  ...
  setMagazaFiltre, exportFunnelCSV,
