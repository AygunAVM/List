...
document.addEventListener('click', e => {
  if (e.target.closest('.haptic-btn,.add-btn,.remove-btn,.btn-login,.cart-trigger'))
    haptic();
  // Global aktivite: sepet doluysa hareketsizlik sayacını sıfırla
  if (basket.length > 0 && typeof resetSessionTimer === 'function') resetSessionTimer();
}, { passive:true });

// ─── DOM HAZIR ──────────────────────────────────────────────────

// ─── KAÇIŞ KORUMASI (sayfa kapanma / yenileme) ─────────────────
// Personel sayfayı kapansa/yenilese bile sessionData Firebase'e gider
function _flushSessionOnEscape() {
  if (!currentUser || !_db || basket.length === 0) return;
  // En güncel _sd'yi al
  try { Object.assign(_sessionData, JSON.parse(localStorage.getItem('_sd')||'{}')); } catch(e){}
  // navigator.sendBeacon ile (sync olmayan, sayfa kapanırken çalışır)
  // Firebase için fetch/beacon kullanamayız; en iyi çözüm: zaten saveBasket sessionData'yı yazdı
  // Ek: 'abandoned' flag'i live_baskets'e yaz (kaçış işareti)
  ...
  // sendBeacon — Firebase REST API (ücretsiz, 1 istek)
  const url = `https://firestore.googleapis.com/v1/projects/aygun-teklif/databases/(default)/documents/abandoned_sessions`;
  try { navigator.sendBeacon(url, new Blob([data], {type:'application/json'})); } catch(e){}
}

window.addEventListener('beforeunload', _flushSessionOnEscape);
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden' && basket.length > 0) {
    _flushSessionOnEscape();
    // saveBasket çağır — en güvenilir yol
    saveBasket();
  }
});
...
async function updateLiveBasket() {
  ...
  try {
    await setDoc(doc(_db, "live_baskets", user), {
      basket: basket,
...
async function fetchLiveBasket() {
  ...
  if (!user) return;
  try {
    const snap = await getDoc(doc(_db, "live_baskets", user));
    if (snap.exists()) {
...
// ─── FİYAT GÖSTER (data-attr + anlık sessionData log) ───────────
function _fyGos(el) {
  if (!el) return;
...
function fiyatGoster(el, urunAdi, fiyat) {
  ...
  if (_sessionData && urunAdi && !_sessionData.revealedPrices.includes(urunAdi)) {
    ...
    // Firebase'e hafif anlık log (sadece fiyat bakışı — sepet kapatılmasa bile kayıt var)
    if (currentUser && _db) {
...
function saveBasket() {
  ...
  // Session datasını anlık localStorage'a yaz (sayfa yenileme/kaçış koruması)
  localStorage.setItem('_sd', JSON.stringify({
    searches:       _sessionData.searches       || [],
    revealedPrices: _sessionData.revealedPrices || [],
    startTime:      _sessionData.startTime      || Date.now()
  ...
  if (currentUser && _db) {
    ...
    setDoc(doc(_db, 'basket_snapshots', email.replace(/[^a-zA-Z0-9]/g,'_')+'_'+today), {
      email, date: today,
      basketSnapshot: basket.map(i=>({urun:i.urun,nakit:i.nakit,stok:i.stok})),
      basketTs: new Date().toISOString()
    }, {merge: true}).catch(() => {});
    // Canlı sepet — her değişimde yaz (cihazlar arası + veri kaybı önleme)
    if (basket.length > 0) {
...
async function clearBasket(bypass = false) {
  ...
  // 1. Sepet boşsa bildir
  if (basket.length === 0) {
    if (!bypass) await ayAlert('Sepet zaten boş.');
    return;
  }

  // 2. bypass=true → log zaten yazıldı (finalizeAksiyon/aySonucSecimi'den)
  if (bypass) {
    _doClearBasket();
    return;
  }
  ...
  // 4. Satış personeli → session-result-modal aç
  const modal = document.getElementById('session-result-modal');
  if (!modal) { _doClearBasket(); return; }

  // Kaçtı nedenleri panelini gizle, butonları sıfırla
  ...
  const handleSonuc = async (sonuc, neden = '') => {
    ...
    if (kpanel) kpanel.style.display = 'none';
    await aySonucSecimi(sonuc, neden);
  };

  document.getElementById('session-result-satis') ?.addEventListener('click',
    () => handleSonuc('Satis', ''), { once: true });
  document.getElementById('session-result-teklif')?.addEventListener('click',
    () => handleSonuc('Teklif', ''), { once: true });
  // Kaçtı → neden paneli aç
...
// Gerçek sepet temizleme (log yazıldıktan SONRA çağrılır)
function _doClearBasket() {
  ...
  // Session sıfırla + localStorage temizle
...
// Funnel log yaz + sepeti bypass ile temizle
async function aySonucSecimi(sonuc, detay = "") {
  ...
  // localStorage'dan en güncel session datasını al
  try { Object.assign(_sessionData, JSON.parse(localStorage.getItem('_sd') || '{}')); } catch(e){}
