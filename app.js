...
function renderCampaignPills(item, idx) {
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    // disabled durumda onclick yok ama kullanıcıya hover'da sebep göster
    const disabledTitle = disabled
      ? (itemHasProje && camp.tip !== 'proje'
          ? 'Bu üründe ❎ proje kampanyası seçili — önce kaldırın.'
          : !itemHasProje && camp.tip === 'proje' && (itemHasKilitli || itemHasBirlesen)
            ? 'Bu üründe ⇹/🔒 kampanyası seçili — önce kaldırın.'
