...
const _colLiveBaskets = () => collection(_db, 'live_baskets');
...
function startFirebaseListeners() {
  ...
  window._siparisUnsub = onSnapshot(
    ...
    snap => {
      const prevCount = (window._siparisData||[]).filter(s=>s.durum==='bekliyor').length;
      window._siparisData = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      const newCount  = window._siparisData.filter(s=>s.durum==='bekliyor').length;
      ...
      // Yeni sipariş geldi → üstte bildirim göster (admin ve satış user)
      if(newCount > prevCount && window._siparisData.length > 0) {
        const newest = window._siparisData[0];
        _showSiparisUyari(newest);
      }
    },
  ...
  window._analyticsUnsub = onSnapshot(
    ...
    snap => {
      ...
      // Admin paneli açıksa performans güncelle
      const adminOpen = document.getElementById('admin-modal')?.classList.contains('open');
      if(adminOpen) {
        const activeTab = document.querySelector('.admin-tab.active')?.dataset?.tab;
  ...
  // Live baskets listener — anlık sepet takibi
  if(window._liveBasketUnsub) window._liveBasketUnsub();
  window._liveBasketUnsub = onSnapshot(
    _colLiveBaskets(),
    snap => {
      window._liveBaskets = {};
      ...
      if(adminOpen) {
        const activeTab = document.querySelector('.admin-tab.active')?.dataset?.tab;
        if(activeTab === 'sepetler') renderSepetDetay();
        if(activeTab === 'overview' || !activeTab) renderSepetDurum();
      }
    },
...
function saveBasket() {
  ...
  updateCartUI();
  // live_baskets koleksiyonuna anlık yaz
  if(currentUser && _db) {
    const email   = currentUser.Email;
    const docId   = email.replace(/[^a-zA-Z0-9]/g,'_');
    const items   = basket.map(i => ({urun:i.urun, nakit:i.nakit, stok:i.stok, aciklama:i.aciklama||''}));
    const total   = basket.reduce((s,i) => s + (i.nakit||0), 0);
...
async function renderAdminPanel() {
  ...
  // Son 7 gün: giriş + teklif + satış birleşik çubuk
  const dc = dates.map(date => {
    let logins=0, props=0, sals=0;
    Object.values(data[date]||{}).forEach(r => {
      logins += r.logins||0;
      props  += r.proposals||0;
      sals   += r.sales||0;
    });
    // proposals/sales'dan o güne ait gerçek sayıları da ekle
    const dayProps = proposals.filter(p => (p.ts||'').startsWith(date)).length;
    const daySals  = sales.filter(s => (s.ts||'').startsWith(date)).length;
    return { date, logins, props: Math.max(props, dayProps), sals: Math.max(sals, daySals) };
  });
  const md = Math.max(1, ...dc.map(d => d.logins + d.props + d.sals));
  const dcEl = document.getElementById('admin-daily-chart');
  if(dcEl) dcEl.innerHTML = dc.map(d => {
    const total = d.logins + d.props + d.sals;
    const h = Math.max(6, Math.round(total/md*100));
    const isToday = d.date === today;
    const label = d.date.slice(5); // MM-DD
    return `<div class="chart-bar-wrap" title="${d.date}: ${d.logins} giriş, ${d.props} teklif, ${d.sals} satış">
      <div class="chart-bar-stack" style="height:${h}%">
        ${d.sals  ? `<div class="cbs-sales"  style="flex:${d.sals}"></div>`  : ''}
        ${d.props ? `<div class="cbs-props"  style="flex:${d.props}"></div>` : ''}
        ${d.logins? `<div class="cbs-logins" style="flex:${d.logins}"></div>`:''}
      </div>
      <span class="chart-val-tip">${total||''}</span>
      <span class="chart-label ${isToday?'today':''}">${label}</span>
    </div>`;
  }).join('');
...
function renderPersonelBugun(data, today) {
  ...
  // Tüm kullanıcıları proposals+sales+analytics'ten topla
  const users = {};
  proposals.forEach(p => {
    if(!p.user||p.user==='-') return;
    if(!users[p.user]) users[p.user] = {logins:0, proposals:0, sales:0, bekliyor:0};
    users[p.user].proposals++;
    if(p.durum==='bekliyor') users[p.user].bekliyor++;
  });
  sales.forEach(s => {
    if(!s.user||s.user==='-') return;
    if(!users[s.user]) users[s.user] = {logins:0, proposals:0, sales:0, bekliyor:0};
    users[s.user].sales++;
  });
  // Bugünkü login + teklif (analytics'ten)
  const todayData = data[today]||{};
  Object.entries(todayData).forEach(([email, rec]) => {
    if(!users[email]) users[email] = {logins:0, proposals:0, sales:0, bekliyor:0};
    users[email].logins = rec.logins||0;
    // analytics proposals'ı gerçek sayıyla güncelle
    const realProps = proposals.filter(p=>p.user===email&&(p.ts||'').startsWith(today)).length;
    if(realProps > users[email].proposals) {} // proposals zaten sayıldı
  });
  // Firebase analytics'ten de ekle
  if(window._fbAnalytics) {
    Object.values(window._fbAnalytics).filter(d=>d.date===today).forEach(d => {
      if(!d.email) return;
      if(!users[d.email]) users[d.email] = {logins:0, proposals:0, sales:0, bekliyor:0};
      users[d.email].logins = Math.max(users[d.email].logins, d.logins||0);
    });
  }

  const entries = Object.entries(users).sort((a,b) => (b[1].proposals+b[1].sales) - (a[1].proposals+a[1].sales));
  if(!entries.length) { el.innerHTML='<div class="admin-empty">Henüz aktivite yok</div>'; return; }

  // Bugün teklif/satış sayıları (proposals'dan gerçek zaman)
  const todayProps = {};
  const todaySales = {};
  proposals.forEach(p => { if((p.ts||'').startsWith(today) && p.user) todayProps[p.user]=(todayProps[p.user]||0)+1; });
  sales.forEach(s => { if((s.ts||'').startsWith(today) && s.user) todaySales[s.user]=(todaySales[s.user]||0)+1; });

  const maxProps = Math.max(1, ...entries.map(([e,s])=>s.proposals));
  const maxSales = Math.max(1, ...entries.map(([e,s])=>s.sales));

  el.innerHTML = '<div class="perf-table-header"><span>Personel</span><span>Bugün</span><span>Toplam</span><span>Bekleyen</span></div>' +
  entries.map(([email, s]) => {
    const ini  = email.split('@')[0].slice(0,2).toUpperCase();
    const name = email.split('@')[0];
    const isOnline = (todayData[email]?.logins||0) > 0 ||
      (window._fbAnalytics && Object.values(window._fbAnalytics).some(d=>d.email===email&&d.date===today&&(d.logins||0)>0));
    const tP = todayProps[email]||0;
    const tS = todaySales[email]||0;
    const propW = Math.round(s.proposals/maxProps*60);
    const saleW = Math.round(s.sales/maxSales*60);
    return `<div class="perf-row">
      <div style="display:flex;align-items:center;gap:8px;min-width:0;flex:1.2">
        <div class="user-avatar" style="width:30px;height:30px;font-size:.68rem;flex-shrink:0;position:relative">
          ${ini}${isOnline?'<span style="position:absolute;bottom:-1px;right:-1px;width:8px;height:8px;background:#22c55e;border-radius:50%;border:1.5px solid #fff"></span>':''}
        </div>
        <div style="min-width:0">
          <div style="font-weight:600;font-size:.8rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${name}</div>
          ${s.logins?`<div style="font-size:.65rem;color:var(--text-3)">${s.logins} giriş</div>`:''}
        </div>
      </div>
      <div style="font-size:.78rem;text-align:center;flex:.8">
        ${tP?`<span style="color:#3b82f6;font-weight:700">${tP}T</span>`:'<span style="color:#cbd5e1">—</span>'}
        ${tS?` <span style="color:#f97316;font-weight:700">${tS}S</span>`:''}
      </div>
      <div style="flex:1;display:flex;flex-direction:column;gap:2px">
        ${s.proposals?`<div style="display:flex;align-items:center;gap:4px"><div style="height:5px;background:#3b82f6;border-radius:3px;width:${propW}px;min-width:4px"></div><span style="font-size:.65rem;color:#64748b">${s.proposals}T</span></div>`:''}
        ${s.sales?`<div style="display:flex;align-items:center;gap:4px"><div style="height:5px;background:#f97316;border-radius:3px;width:${saleW}px;min-width:4px"></div><span style="font-size:.65rem;color:#64748b">${s.sales}S</span></div>`:''}
      </div>
      <div style="flex:.6;text-align:center">
        ${s.bekliyor?`<span class="stok-badge sk" style="background:#fef3c7;color:#92400e;font-size:.65rem">${s.bekliyor}</span>`:'<span style="color:#e2e8f0;font-size:.75rem">—</span>'}
      </div>
    </div>`;
  }).join('');
}
...
function renderAdminUsers() {
  ...
  el.innerHTML = su.map(([email,s])=>{
    ...
    // Versiyon badge kaldırıldı — SW ile otomatik güncelleme var

    // Toplam ve bu hafta stats
    const weekAgo = new Date(); weekAgo.setDate(weekAgo.getDate()-7);
    const weekStr = weekAgo.toISOString().split('T')[0];
    const weekProps = proposals.filter(p=>p.user===email&&(p.ts||'')>=weekStr).length;
    const weekSales = sales.filter(s=>s.user===email&&(s.ts||'')>=weekStr).length;
    // Bugün aktif mi?
    const today2 = new Date().toISOString().split('T')[0];
    const isActiveToday = window._fbAnalytics && Object.values(window._fbAnalytics)
      .some(d => d.email===email && d.date===today2 && (d.logins||0)>0);
    const onlineDot = isActiveToday ? '<span class="online-dot" style="position:absolute;bottom:0;right:0;width:9px;height:9px;background:#22c55e;border-radius:50%;border:2px solid #fff"></span>' : '';
    return `<div class="user-row">
      <div class="user-avatar" style="position:relative">${ini}${onlineDot}</div>
      <div class="user-info">
        <div class="user-email">${email.split('@')[0]}</div>
        <div class="user-meta">Bu hafta: ${weekProps} teklif · ${weekSales} satış &nbsp;·&nbsp; Son: ${s.lastSeen||'-'}</div>
      </div>
      <div class="user-badges">
        <span class="badge badge-blue" title="Toplam Teklif">${s.proposals}T</span>
        <span class="badge badge-orange" title="Toplam Satış">${s.sales}S</span>
        ${pending?`<span class="badge" style="background:#fef3c7;color:#92400e" title="Bekleyen teklif">${pending}⏳</span>`:''}
      </div>
    </div>`;
  }).join('');
}
...
async function clearUserBasket(email) {
  ...
  try {
    ...
    const docId = email.replace(/[^a-zA-Z0-9]/g,'_');
    await deleteDoc(doc(_db, 'live_baskets', docId));
    if(window._liveBaskets) delete window._liveBaskets[docId];
    renderSepetDetay();
...
function renderSepetDetay() {
  ...
  const liveBaskets = window._liveBaskets || {};
  const html_parts  = [];

  // Tüm live_baskets dökümanları (email bazlı)
  Object.values(liveBaskets).forEach(b => {
    if(!b.email || !b.items || !b.items.length) return;
    const isMine = b.email === myEmail;
    const ini    = b.email.split('@')[0].slice(0,2).toUpperCase();
    const name   = b.email.split('@')[0];
    const tsStr  = b.ts ? new Date(b.ts).toLocaleTimeString('tr-TR',{hour:'2-digit',minute:'2-digit'}) : '';
    const total  = b.items.reduce((s,i)=>s+(i.nakit||0),0);
    ...
    const clearFn  = isMine ? 'clearBasket()' : 'clearUserBasket(\'' + b.email + '\')';
    const bgColor  = isMine ? 'var(--red)' : '#6366f1';

    html_parts.push(
      '<div class="sepet-user-block">' +
      '<div class="sepet-user-header">' +
      '<div class="user-avatar" style="width:32px;height:32px;font-size:.75rem;background:' + bgColor + '">' + ini + '</div>' +
      '<div style="flex:1;min-width:0">' +
      '<div style="font-weight:700;font-size:.82rem">' + name + '</div>' +
      '<div style="font-size:.7rem;color:var(--text-3)">' + b.items.length + ' ürün' + (tsStr ? ' · ' + tsStr : '') + '</div>' +
      '</div>' +
      '<span style="font-weight:700;font-size:.82rem;color:#1e293b;margin-right:8px">' + fmt(total) + '</span>' +
      '<button onclick="' + clearFn + '" style="background:#fee2e2;border:none;border-radius:6px;padding:4px 12px;font-size:.68rem;cursor:pointer;color:#dc2626;font-weight:600">🗑 Boşalt</button>' +
      '</div>' +
      itemRows +
      '</div>'
    );
  ...
  if(!html_parts.length) {
    el.innerHTML = '<div class="admin-empty">Aktif sepet yok — personeller ürün eklediğinde burada görünür</div>';
    return;
  }
...
function _showSiparisUyari(siparis) {
  // Sayfa üstünde kırmızı bant uyarısı
  let bar = document.getElementById('siparis-uyari-bar');
  if(!bar) {
    bar = document.createElement('div');
    bar.id = 'siparis-uyari-bar';
    bar.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:99999;background:#dc2626;color:#fff;padding:10px 16px;display:flex;align-items:center;gap:10px;font-size:.85rem;font-weight:600;box-shadow:0 2px 8px rgba(0,0,0,.25);animation:slideDown .3s ease';
    document.body.prepend(bar);
  }
  const urunAdi = siparis.urun || '—';
  const kim     = (siparis.user||'').split('@')[0];
  bar.innerHTML = `<span style="font-size:1.1rem">📦</span>
    <span style="font-size:.72rem;background:rgba(255,255,255,.2);padding:2px 7px;border-radius:4px;font-weight:700;letter-spacing:.03em">BİLDİRİM</span>
    <span style="flex:1">Sipariş Notu: <strong>${urunAdi}</strong> — ${kim}</span>
    <button onclick="switchAdminTab('siparis');closeBarIfAdmin(this)" style="background:rgba(255,255,255,.2);border:none;color:#fff;border-radius:6px;padding:4px 10px;cursor:pointer;font-size:.75rem;font-weight:600">Görüntüle</button>
    <button onclick="this.parentElement.remove()" style="background:none;border:none;color:rgba(255,255,255,.7);cursor:pointer;font-size:1rem;padding:2px 6px">✕</button>`;
  // 12 saniye sonra otomatik kapat
  clearTimeout(bar._timer);
  bar._timer = setTimeout(() => bar.remove(), 12000);
  // Admin paneli kapalıysa sepet badge'ini de güncelle
  updateSiparisBadge();
}
function closeBarIfAdmin(btn) {
  btn.closest('#siparis-uyari-bar')?.remove();
