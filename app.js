...
//  GÜNCELLENMİŞ: Taksit, Teklif, Admin Mesajları, Profesyonel Paneli
// ═══════════════════════════════════════════════════════════════
...
function fmt(n)  { return (n||0).toLocaleString('tr-TR', {style:'currency',currency:'TRY', minimumFractionDigits:0, maximumFractionDigits:2}); }
...
function yuvarlaCeyrek(n) { return Math.round(n/0.25)*0.25; }
...
function calcAbakus() {
  ...
  // ⭐ GÜNCELLENMIŞ: "Oran" sütunu KALDIRILIYOR - 4 sütunlu tablo
  html+=`<div class="ab-table-wrap"><table class="ab-table"><thead><tr><th>Taksit</th><th>Zincir POS</th><th>Aylık Taksit</th><th>Toplam Tahsilat</th><th></th></tr></thead><tbody>`;
  liste.forEach(s => {
    ...
    // ⭐ ORAN BİLGİSİ TOOLTIP'E TAŞINDI (title attribute)
    html+=`<tr class="${rowCls}" onclick="selectAbakusRow(this,'${safeJson}')" title="Oran: %${s.oran.toFixed(2)}"><td><strong>${s.label}</strong>${vurgu}</td><td class="ab-zincir-cell">${s.zincir}</td><td class="ab-mono">${fmt(s.aylik)}</td><td class="ab-mono ab-tahsilat-cell">${fmt(s.tahsilat)}</td><td class="ab-badge-cell">${badge}</td></tr>`;
  });
...
function selectAbakusRow(rowEl, jsonStr) {
  ...
  // ⭐ YENİ: Seçili taksiti göster
  updateAbakusSelectionUI();
}

// ⭐ YENİ FONKSİYON: Seçili taksiti UI'da göster
function updateAbakusSelectionUI() {
  if(!abakusSelection) return;
  const taksitBadge = document.getElementById('selected-taksit-badge');
  if(taksitBadge) {
    taksitBadge.innerHTML = `✓ ${abakusSelection.label}`;
    taksitBadge.style.display = 'flex';
  }
}

// ⭐ YENİ FONKSİYON: Nakit seçeneği
function selectNakitOption() {
  haptic(14);
  const t = basketTotals();
  abakusSelection = {
    label: 'Nakit',
    ...
    zincir: 'Doğrudan',
    oran: 0,
    tahsilat: t.nakit - getDisc(t.nakit),
    aylik: t.nakit - getDisc(t.nakit),
    karli: true
  };
  ...
  if(waBtn) {
    waBtn.style.display = 'flex';
    waBtn.innerHTML = `<span>💰</span><span>Nakit</span><strong>${fmt(abakusSelection.tahsilat)}</strong>`;
  }
...
function finalizeProposal() {
  ...
  let od='', odText='';
  let selectedOption='nakit'; // ⭐ YENİ: Seçim türü
  
  if(abakusSelection) {
    if(abakusSelection.label==='Nakit') {
      od=`Nakit: ${fmt(nakit)}`;
      odText=`Nakit — ${fmt(nakit)}`;
      selectedOption='nakit';
    } else {
  ...
  // ⭐ TEKLIF KAYDEDILIRKEN SEÇIM TÜRÜ SAVLENİYOR
  const prop = {
    ...
    selectedOption: selectedOption, // ⭐ SEÇIM TÜRÜ (nakit/taksit)
    type: 'proposal' // ⭐ TEKLIF TÜRÜ İŞARETİ
  };
    ...
    return `<div class="proposal-card">
      ...
      <div class="proposal-body">
        <div class="proposal-row">
          ...
          ${p.selectedOption === 'taksit' ? '<span class="proposal-tag" style="background:var(--surface-3);color:var(--accent)">⭐ Taksit</span>' : ''}
          ${p.not?`<span class="proposal-tag">📝 ${p.not}</span>`:''}</div>
...
function generateSalePDF() {
  ...
  // ⭐ SATIŞ BELGESI KAYDEDILIRKEN FARKLI TÜR
  const saleRecord = {
    ...
    type: 'sale' // ⭐ SATIŞ BELGESI TÜRÜ İŞARETİ
  };
...
function openMessages() {
  ...
  m.style.display='flex'; m.classList.add('open');
  renderMessages();
  // ⭐ ADMİN MESAJ GÖNDERME ALANINI AÇ
  if(isAdmin()) { 
    const ca=document.getElementById('msg-compose-area'); 
    if(ca){ ca.style.display='block'; loadMsgRecipients(); } 
  }
}
...
function renderMessages() {
  ...
  body.innerHTML=myMsgs.map(msg=>{
    ...
    const isSent = msg.from === 'Admin' && isAdmin();
    return `<div class="msg-item ${unread?'unread':''} ${isSent ? 'msg-admin-sent' : ''}">
...
function loadMsgRecipients() {
  ...
  try {
    ...
    proposals.forEach(p => {
      if(p.user) allUsers.add(p.user);
    });
    sales.forEach(s => {
      if(s.user) allUsers.add(s.user);
    });
...
function sendAdminMessage() {
  ...
  checkUnreadMessages();
  haptic(20);
  showToast(`✓ Mesaj gönderildi (${to==='all'?'Herkese':to})`);
}

// ⭐ YENİ FONKSİYON: Toast göster
function showToast(message) {
  const ct=document.getElementById('change-toast');
  if(!ct) return;
  const el=document.createElement('div');
  el.className='toast-item toast-success';
  el.innerHTML=`<span>✓</span><span style="flex:1">${message}</span><button class="toast-close" onclick="this.parentElement.remove()">×</button>`;
  ct.appendChild(el);
...
function switchAdminTab(tab) {
  ...
  if(tab==='messages') { renderMessages(); loadMsgRecipients(); }
}
async function renderAdminPanel() {
  ...
  proposals.slice(0, 5).forEach(p => {
    recentActions.push({
      type: 'teklif',
      desc: `${p.custName} için teklif (${p.user})`,
      time: p.ts
    });
  });
  sales.slice(0, 5).forEach(s => {
    recentActions.push({
      type: 'satış',
      desc: `${s.custName} satışı (${s.user})`,
      time: s.ts
    });
  });
  recentActions.sort((a, b) => new Date(b.time) - new Date(a.time));
  
  const recentEl = document.getElementById('admin-recent-activity');
  if(recentEl) {
    recentEl.innerHTML = recentActions.slice(0, 10).map(a =>
      `<div class="activity-item">
        <span class="activity-type ${a.type === 'teklif' ? 'type-proposal' : 'type-sale'}">${a.type === 'teklif' ? '📋' : '💳'}</span>
