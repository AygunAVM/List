...
const Basket = {
  ...
  totals() {
    return basket.reduce((t, i) => {
      ...
      const efektif = i._projeNakit   !== undefined ? i._projeNakit     // ❖ Proje fiyatı (NET'e yansır)
                    : i._nakitOverride !== undefined ? i._nakitOverride  // Manuel override (❖ yoksa NET'e yansır)
                    : (i.nakit || 0);                                     // Ham nakit
      t.nakit += efektif;
      return t;
...
function _buildAdminCartHTML() {
  ...
  basket.forEach((item, idx) => {
    ...
    const efektifFiyat = projeAktif  ? item._projeNakit     // ❖ Proje NET'e yansır
                       : hasOverride ? item._nakitOverride  // Override (❖ yoksa)
                       : item.nakit;                         // Ham nakit
...
Object.assign(window, {
  // Temel fonksiyonlar
  checkAuth, toggleCart, toggleZeroStock, filterData, clearMainSearch, iosLoadMore,
