...
function markAllChanges() {
  ...
  // Bilgi bölümü "Okudum" butonunu da işaretle
  const lowBtn = document.querySelector('#change-list ~ .change-footer .section-confirm-low, .section-confirm-low:not(.scb-confirmed)');
  if(!lowBtn) {
    // Doğrudan section-low içindeki butonu bul
    const lowSection = document.querySelector('#change-list .section-low');
    if(lowSection) {
      const btn = lowSection.querySelector('.section-confirm-btn:not(.scb-confirmed)');
      if(btn) confirmSection('low', btn);
    }
  }
  _updateChangeBtn();
  haptic(18);
