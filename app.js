...
function renderProposals(container, forceAll) {
  ...
  target.innerHTML = myProps.map(p => {
    ...
    if(canEdit) {
      const canNote2 = isAdmin() || p.user===me;
      if(canNote2) btns.push(`<button class="pact-btn pact-note haptic-btn" onclick="openPropNote('${p.id}')" title="Not Ekle"><svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M2 12V14h2l7.5-7.5-2-2L2 12zm12.7-7.3a1 1 0 000-1.4l-1-1a1 1 0 00-1.4 0L11 3.6l2.4 2.4 1.3-1.3z" fill="currentColor"/></svg>${noteDot}</button>`);
    ...
    const userTag = isAdmin() ? `<span class="proposal-tag prop-user-tag">${p.user.split('@')[0]}</span>` : '';
...
function renderStokUyari() {
  ...
  if(!el) return;
  // Önce bellekteki ürün listesini dene (loadProducts zaten çektiyse)
...
function renderUyuyanStok(urunler) {
  if(!urunler && window._cachedUrunler) urunler = window._cachedUrunler;
  if(!urunler) return;
