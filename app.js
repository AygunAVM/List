...
window.setTarihAralik = function(tip) {
  ...
  if (tip === 'bugun') {
    ...
    bas = new Date(bugun); bas.setDate(bugun.getDate() - 1);
    ...
    bas = new Date(bugun); bas.setDate(bugun.getDate() - 364);
    bas = new Date(bas.getFullYear(), bas.getMonth(), bas.getDate());
    bit = new Date(bugun.getFullYear(), bugun.getMonth(), bugun.getDate() + 1);
  } else if (tip === 'tumveri') {
    // Tüm veri: uygulama başlangıcından bugüne (5 yıl geriye kadar)
    bas = new Date(bugun); bas.setFullYear(bas.getFullYear() - 5);
    bas = new Date(bas.getFullYear(), bas.getMonth(), bas.getDate());
    bit = new Date(bugun.getFullYear(), bugun.getMonth(), bugun.getDate() + 1);
  }
  if (bas && bit) {
...
async function _fetchFunnelHybrid(baslangicTarih, bitisTarih) {
  const bugun = _istanbulTarih(new Date());
  const dun   = _istanbulTarih(new Date(Date.now() - 86400000));
  ...
  const ozetBitis = _istanbulTarih(new Date(Date.now() - 2 * 86400000));
  const ozetLogs  = [];
