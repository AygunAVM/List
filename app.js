...
function showPendingChanges(logKey) {
  ...
  newItems.sort((a,b) => {
    ...
    // Aynı öncelikteyse: 1) Gam 2) Marka 3) Ürün adı
...
function showChangePopup(changes, logKey) {
  ...
  list.innerHTML = withDividers.map(c => {
    ...
    // Opsiyonellerde checkbox yok, zorunlularda tıklanınca satır highlight'lanır
    const chk = isOld
    ...
    return `<div class="change-item ${doneCls} ${mandatoryCls} ${optionalCls}" data-idx="${idx}" ${clickAttr}>${chk}${inner}</div>`;
  }).join('');
...
function _updateChangeBtn() {
  requestAnimationFrame(() => {
    ...
    btn.textContent   = canClose ? '✓ Anladım, Kapat' : '🔴 ' + mandatoryLeft + ' zorunlu değişim kaldı';
  });
}
...
Object.assign(window, {
  ...
  clearAllPendingProps, logoutUser, toggleChangeItem, toggleChangeItemRow, markAllChanges,
