...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', function(e) {
    ...
    // Filtre bilgisini global'a yaz — renderTable okuyacak
    ...
    if (typeof switchMainTab === 'function') {
      // Önce arama kutusunu doldur (switchMainTab silmesin diye)
...
function renderTable(searchVal) {
  ...
  // camp-ara-btn tıklandığında _campAraFilter set edilir.
  // Aynı grup+esik ama excludeRol ile eşleşen ürünler (sepetteki ürün) filtrelenir.
  // excludeRol'ü TAŞIYAN ürünler hariç, diğer roller gösterilir.
  if (_campAraFilter) {
    ...
    data = data.filter(u => {
      ...
      while ((m = campRe.exec(ac)) !== null) {
        if (m[1].toUpperCase() === cfGrup && parseInt(m[2]) === cfEsik) {
          // Bu ürün gruba ait — excludeRol ise gösterme, diğer roller göster
          if (m[3].toUpperCase() === excludeRol) return false;
          return true;
        }
      }
      return false; // Bu grubu hiç taşımıyor
    });
    // Filtre tek seferlik — sonraki normal aramalarda temizle
    _campAraFilter = null;
  }
