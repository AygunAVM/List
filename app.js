...
async function fetchWeeklyStar() {
  ...
  try {
    ...
    snap.docs.forEach(d => {
      const data = d.data();
      // Satış kapanma tarihi archivedAt; yoksa ts fallback
      let tsDate;
...
async function renderMyStats() {
  ...
  // Hesaplamalar — 3 dönem (blur döneme göre ayrı alınır)
  const _calc = (tarihFiltre, satisTarih, blurN) => {
    const teklif = benimler.filter(p => (p.ts||'').slice(0,10) >= tarihFiltre).length;
    ...
    // Satışa dönüşme: Satış ÷ Teklif (anlamlı, net)
    const bts = teklif > 0 ? Math.min(100, (satis / teklif) * 100) : null;
    // Fiyat → Teklif: Teklif ÷ Fiyat Sorgulama
    // blurN aynı dönemden geliyor; teklif blur'dan fazlaysa gösterme (veri tutarsız)
  ...
  const d = {
    ...
    hafta: _calc(haftaOnce, haftaOnce, blurData.hafta || 0),
    ay:    _calc(ayOnce,    ayOnce,    blurData.ay    || 0),
  };
...
function _renderMyStatsUI() {
  ...
  // Fiyat → Teklif dönüşüm oranı
  const btbVal  = v.btb !== null ? v.btb : 0;
  const btbWarn = v.btb !== null && btbVal < 20;
  const btbHint = v.btb === null
    ? 'Fiyat sorgulama ile teklif aynı dönemde eşleşmedi — veri birikince görünür'
  ...
  container.innerHTML = `
    ...
    <div class="ms-metric-card ${btbWarn ? 'ms-metric-warn' : 'ms-metric-ok'}" style="margin-top:10px">
      <div class="ms-metric-header">
        <span class="ms-metric-name">Fiyat → Teklif Dönüşümü</span>
...
async function _getMyBlurStats(email) {
  // Blur verisi sessions koleksiyonundan dönemsel olarak okunur.
  // fiyat_bakislari tüm zamanın toplamını tutar — dönem filtresi uygulanamaz.
  // Bu yüzden blur sayısını doğrudan sessions'tan hesaplıyoruz.
