...
function renderCampaignPills(item, idx) {
  ...
  const pills = item._campaigns.map((camp, ci) => {
    ...
    // Label: proje/bilgi/bagimsiz için tam metin (tarih parantezi temizlenmiş), diğerleri grup+tutar
    const isProje2    = camp.tip === 'proje';
    const isBagimsiz2 = camp.tip === 'bagimsiz';
    ...
    const label = (isBilgi || isProje2)
      ...
      : isBagimsiz2
        ? (metinTemiz.length > 24 ? metinTemiz.slice(0,22) + '…' : metinTemiz)
