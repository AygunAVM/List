...
async function finalizeAksiyon() {
  ...
  if (abakusSelection) {
    ...
    od     = 'Nakit: ' + fmt(nakitTahsilat) + (toplamIndirim + _ekIndirimF > 0 ? ' (İndirim: -' + fmt(toplamIndirim + _ekIndirimF) + ')' : '');
...
function _startSessionListener() {
  ...
  onSnapshot(collection(_db,'sessions'), snap => {
    ...
    // Personel sekmesi açıksa otomatik güncelle
    const adminOpen = document.getElementById('admin-modal')?.classList.contains('open');
...
function _renderIndirimOzet() {
  ...
  // funnel_logs'tan indirim verisi yok — proposals üzerinden hesapla
  const indirimliTeklifler = proposals.filter(p => {
    const altInd = Number(p.indirim || 0);
    const itemInd = (p.urunler||[]).reduce((s,u)=>s+Number(u.itemDisc||0), 0);
    const pazarlik = Number(p.ekIndirim || 0);
    return altInd > 0 || itemInd > 0 || pazarlik > 0;
  });

  // Personel bazlı indirim kullanımı
  const perUser = {};
  indirimliTeklifler.forEach(p => {
    const u = p.user || '-';
    if (!perUser[u]) perUser[u] = { teklifSayisi: 0, toplamIndirim: 0, pazarlikSayisi: 0 };
    perUser[u].teklifSayisi++;
    const altInd  = Number(p.indirim || 0);
    const itemInd = (p.urunler||[]).reduce((s,u2)=>s+Number(u2.itemDisc||0), 0);
    const pazarlik = Number(p.ekIndirim || 0);
    perUser[u].toplamIndirim += altInd + itemInd + pazarlik;
    if (pazarlik > 0) perUser[u].pazarlikSayisi++;
  });

  const toplam = proposals.length;
  const indirimliSayisi = indirimliTeklifler.length;
  const indirimOrani = toplam > 0 ? ((indirimliSayisi / toplam) * 100).toFixed(0) : 0;

  const sorted = Object.entries(perUser).sort((a,b) => b[1].toplamIndirim - a[1].toplamIndirim);

  el.innerHTML = `
    <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:10px">
      <div style="flex:1;min-width:100px;background:#fef3c7;border-radius:8px;padding:8px 10px;text-align:center">
        <div style="font-size:1.2rem;font-weight:800;color:#92400e">${indirimOrani}%</div>
        <div style="font-size:.62rem;color:#92400e">teklifte indirim</div>
      </div>
      <div style="flex:1;min-width:100px;background:#fef2f2;border-radius:8px;padding:8px 10px;text-align:center">
        <div style="font-size:1.2rem;font-weight:800;color:#dc2626">${indirimliSayisi}</div>
        <div style="font-size:.62rem;color:#dc2626">indirimli teklif</div>
      </div>
      <div style="flex:1;min-width:100px;background:#f0fdf4;border-radius:8px;padding:8px 10px;text-align:center">
        <div style="font-size:1.2rem;font-weight:800;color:#16a34a">${toplam - indirimliSayisi}</div>
        <div style="font-size:.62rem;color:#16a34a">indirimsiz teklif</div>
      </div>
    </div>
    ${sorted.length ? sorted.map(([email, s]) => `
      <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--border);font-size:.74rem">
        ...
        <span style="color:#92400e;font-size:.68rem">${s.teklifSayisi} teklif · ${fmt(s.toplamIndirim)} indirim</span>
        ${s.pazarlikSayisi > 0 ? `<span style="background:#fef3c7;color:#92400e;border-radius:4px;padding:1px 5px;font-size:.60rem;font-weight:700">${s.pazarlikSayisi}x pazarlık</span>` : ''}
      </div>`).join('') : '<div style="font-size:.72rem;color:var(--text-3);text-align:center;padding:8px">Henüz veri yok</div>'}
  `;
}
...
async function adminForceLogout(targetEmail) {
  ...
  try {
    // Firestore sessions koleksiyonundan bu kullanıcının tüm session'larını sil
    const sesSnap = await getDocs(
      query(collection(_db, 'sessions'), where('email', '==', targetEmail))
    );
    const delPromises = sesSnap.docs.map(d => deleteDoc(doc(_db, 'sessions', d.id)));
    await Promise.all(delPromises);
    haptic(22);
    const _ct = document.getElementById('change-toast');
    if(_ct) { _ct.textContent = targetEmail.split('@')[0] + ' oturumu kapatıldı'; _ct.classList.add('show'); setTimeout(()=>_ct.classList.remove('show'), 2500); }
    renderAdminUsers();
