...
function parseCampaigns(aciklama) {
  ...
  // Tüm ⎇ ve 🔒︎ ayraç konumlarını bul
  const ayraclar = [];
  const KILITLI = '🔒︎';
  ...
  if (ayraclar.length === 0) {
    const metin = str.trim();
    if (metin) result.push(_buildCampObj(metin, 'birlesen'));
    return result;
  }

  // Her ayraçtan önceki metin → o ayracın tipiyle kaydet
  let onceki = 0;
  ayraclar.forEach(a => {
...
// Eşik parantezden okuma: "3k(2)+" → esik=2, "3k(3)+" → esik=3
function _buildCampObj(metin, tip) {
  const tutar = _extractTutar(metin);

  // Parantez içindeki eşik: (2), (3), (4) gibi
  const esikMatch = metin.match(/\((\d+)\)/);
  let esikOverride = esikMatch ? parseInt(esikMatch[1]) : null;
  ...
  if (/KEA/i.test(metin)) {
    ...
    esik = esikOverride || 2;
    tip  = 'kilitli';
  } else if (/ANK|[Üü][çc]l[üu]|Ankastre/i.test(metin)) {
    grup = 'ANK';
    esik = esikOverride || 3;
    tip  = 'birlesen';
    ...
    // Tutar yoksa Bilgi/Hediye — göster ama indirim yapma
    grup = 'BILGI';
    esik = 1;
    tip  = 'bilgi';
  } else {
...
function recalculateAllGroupCampaigns() {
  ...
  Object.entries(grupSecimler).forEach(([grup, secimler]) => {
    // BILGI grubu: indirim yok, pill sadece görsel
