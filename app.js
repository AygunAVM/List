...
async function renderMyStats() {
  ...
  const _calc = (tarihFiltre, satisTarih, blurN) => {
    ...
    const satis  = benimler.filter(p => p.durum==='satisDondu' && (_tarih(p.archivedAt) || _tarih(p.ts)) >= satisTarih).length;
  ...
  const d = {
    bugun: (() => {
      ...
      const s = benimler.filter(p => p.durum==='satisDondu' && (_tarih(p.archivedAt) || _tarih(p.ts)) === bugun).length;
